...
- static documentation checks continue to list RETF, stack-frame creation, LOCAL, PROC USES runtime save/restore, PROTO, INVOKE, ADDR, calling-convention behavior, and Irvine32 callable routine dispatch as deferred unless later phases implement them, while verifying that Phase 73 `LEAVE`, Phase 74 `RET imm16`, and Phase 76 `PROC USES` parsing metadata are documented as implemented.
