...
Phase 93 adds direct virtual Irvine32 `WriteBin` fixed-width 32-bit binary Program Console output and advances runtime/source-run metadata to the Phase 93 WriteBin contract.

## Current regression coverage

Phase 93 regression coverage verifies virtual Irvine32 `WriteBin` fixed-width 32-bit binary Program Console output for representative `EAX` bit patterns including `00000000h`, `FFFFFFFFh`, `00000001h`, `80000000h`, `AAAAAAAAh`, and `55555555h`; exactly 32 output bytes with leading zeroes and no newline; complete state preservation; exact-fit 32-byte and one-byte-short atomic output-limit behavior; exact missing-include, bare-form, future-owned `INVOKE WriteBin`, and output-limit diagnostics; continued deferral of `DumpRegs` and `DumpMem`; and the current output-contract token `phase-93-irvine32-writebin-contract-v1`. Phase 92 `WriteHex`, Phase 91 `WriteInt`, and Phase 90 `WriteDec` regression coverage remain active.

Earlier regression coverage remains active. Phase 87 regression coverage verifies virtual Irvine32 `Crlf` source-run Program Console newline output, zero-argument `INVOKE Crlf`, register/flag/memory preservation, no-partial Program Console limit failures through `Crlf`, precise Crlf invalid-source diagnostics, rendered Simulator Messages for those diagnostics, and continued deferral of non-target Irvine32 routines except for direct `CALL WriteChar`, direct `CALL WriteString`, direct `CALL WriteDec`, direct `CALL WriteInt`, direct `CALL WriteHex`, and direct `CALL WriteBin`. Phase 86 regression coverage verifies deterministic Program Console byte/line limits, no-partial-append output-limit failure behavior, source-run limit-status serialization, rendered `console-output-limit-exceeded` Simulator Messages, and separate Program Console and Simulator Messages streams.

Earlier implemented parser/runtime families remain covered by focused test groups. Phase 84 covers limited same-file user-procedure `INVOKE` DWORD argument lowering, Phase 83 covers helper-level `ADDR` preparation, Phase 82 covers zero-argument same-file user-procedure `INVOKE`, Phase 81 covers `PROTO` metadata, Phase 80 covers LOCAL operands, and Phase 79 adds tests for automatic LOCAL frame setup and release, including `local-frame-entry-unsupported`, `invalid-frame-state`, `stack-overflow`, rendered Simulator Messages, output-contract metadata, and future-owned deferral of executable source-level `ADDR` outside accepted INVOKE argument forms.
...
For Phase 93 - Irvine32 WriteBin, this smoke-test rule is triggered if the implementation changes any of the following:

- `web/index.html` current milestone text;
- the default editor program;
- source-run syntax support;
- source-run behavior metadata;
- source-run output-contract token;
- virtual Irvine32 include behavior;
- `exit` handling;
- Program Console or Simulator Messages stream contracts.

When triggered, the smoke test must prove that the exact default editor source shown in the browser still parses and runs successfully, that the output-contract token matches the current implemented milestone, and that no stale unsupported-feature diagnostic appears for already-implemented Irvine32 routine forms.

## Next Irvine32 output milestone testing guardrails

### Irvine32 Program Console golden-output tests

Every future Irvine32 routine that writes more than one byte of deterministic Program Console output must include exact golden-output tests. The already-implemented `WriteString`, `WriteDec`, `WriteInt`, `WriteHex`, and `WriteBin` routines retain their exact golden-output regressions; the rule applies next to `DumpRegs` and `DumpMem` when their owning phases are implemented.

The tests must verify:

1. exact Program Console text;
2. exact source-run output-contract token for the implemented phase;
3. exact byte count where the source-run JSON exposes it;
4. exact line count where line counting is affected;
5. absence of unintended CR bytes when the routine specifies internal line-feed bytes only;
6. exact spacing, casing, prefixes, suffixes, and trailing newline behavior;
7. no Program Console mutation on no-partial-output fatal failures;
8. preservation of previously committed Program Console output from earlier successful operations;
9. separation between Program Console program output and Simulator Messages diagnostics/status text.

For `DumpRegs` and `DumpMem`, substring checks are not sufficient. Their milestone tests must compare the full Program Console output against the exact format documented by the guide. Any accepted change to their output format must update the full spec, implementation guide, supported syntax reference when relevant, rendered-output tests, and milestone report together.

### No-partial-output warning/fatal interaction tests

For Irvine32 routines that preflight candidate output before committing it, tests must distinguish successful warning cases from fatal failure cases.

Required coverage:

- A successful call that reads warning-eligible data emits the expected warning and commits the expected Program Console output.
- A failing no-partial-output call that collects warning-eligible candidate bytes but later fails fatally emits the fatal diagnostic, appends no candidate Program Console output from the failing call, suppresses candidate-byte warnings from that failed call when the routine-specific guide says to do so, and omits `execution-complete`.
- Warnings emitted by earlier completed operations remain visible even if a later no-partial-output call fails fatally.
- Rendered Simulator Messages tests must prove the exact visible message set, not only diagnostic codes.
...
| `CALL DumpRegs` or another still-deferred Irvine32 routine | `unsupported-feature` | `unsupported-irvine32-routine` | Proves that implemented Irvine32 output routines did not execute future Irvine32 routines. |
...
### A native test executable exits with `3221225725`

On Windows this code is commonly `0xC00000FD` (`STATUS_STACK_OVERFLOW`). Several native regression fixtures intentionally exercise a full modeled `Vm`, and some test functions require more stack than the ordinary Windows executable default. The repository test runner therefore links Windows native test executables with a 16 MiB stack reserve. Run native tests through `py scripts\run_tests.py ...` with the recommended Visual Studio Clang environment so that policy is applied consistently.

If reproducing a native test with a manual `clang` command outside the runner, add `-Xlinker /STACK:16777216` to match the repository test policy. Do not treat increasing the Windows native-test process stack as a simulator runtime or modeled-stack behavior change; it affects only the host process used by the native regression harness.
...
- The Phase 89A-originated Irvine32 active-status guardrail fixtures must use explicit file-role classification and the following current Phase 93 outcome matrix. A fixture passes or fails because of its assigned role and content condition, not because a stale sentence was individually allowlisted.
  ...
  | Active current-status file | Omits direct `WriteInt` from a current implemented-routine inventory while naming `Crlf`, `WriteChar`, `WriteString`, and `WriteDec` | Fail |
  | Active current-status file | Omits direct `WriteHex` from a current implemented-routine inventory while naming `Crlf`, `WriteChar`, `WriteString`, `WriteDec`, and `WriteInt` | Fail |
  | Active current-status file | Omits direct `WriteBin` from a current implemented-routine inventory while naming `Crlf`, `WriteChar`, `WriteString`, `WriteDec`, `WriteInt`, and `WriteHex` | Fail |
  ...
  | Active current-status file | Lists direct `CALL WriteString`, direct `CALL WriteDec`, direct `CALL WriteInt`, direct `CALL WriteHex`, and direct `CALL WriteBin` as implemented and separately lists their corresponding `INVOKE` forms as deferred | Pass |
  | Active current-status file | Implies that direct `CALL WriteString`, direct `CALL WriteDec`, direct `CALL WriteInt`, direct `CALL WriteHex`, or direct `CALL WriteBin` also implements the corresponding `INVOKE` form | Fail |
  | Active current-status file | Lists direct `CALL WriteString`, direct `CALL WriteDec`, direct `CALL WriteInt`, direct `CALL WriteHex`, or direct `CALL WriteBin` in a generic deferred direct-call table after the owning phase is implemented | Fail |
  ...
  | Historical Phase 89 file | Accurately lists `Crlf`, `WriteChar`, and `WriteString` for that historical state | Pass |
  | Active metadata/protocol fixture | Implemented phase is 93 and output-contract token is exactly `phase-93-irvine32-writebin-contract-v1` | Pass |
...
Phase 89A originally implemented this guardrail through `scripts/phase89a_status_guardrail.py`, the explicit manifest `tests/static/phase89a/manifest.json`, and the complete fixture set in `tests/static/phase89a/`. The current regression set also covers status supplied by a lead paragraph to a following list or table, implicit inventory wording such as `consists of` and `provides`, `Yes` status tables, same-sentence direct-CALL/INVOKE distinctions, stale named `Irvine32 routines beyond ...` boundaries that omit implemented direct routines, ambiguous `other Irvine32 output routines` deferral wording that does not identify the current implemented boundary or still-deferred siblings, Windows API and PE-loader wording variants, `.EXIT` directive text versus virtual zero-operand `exit`, phase-number/name/suffix drift, premature Phase 94-95 token activation, and the accepted Phase 93 default-editor source SHA-256. Run these commands from the repository root:
...
The standalone guardrail and focused static command must pass after every completed guardrail change. The aggregate command must pass before the current target milestone is reported complete.
...
The original Phase 89A milestone added no runtime diagnostic or Program Console behavior. Later runtime milestones that evolve this guardrail, including Phase 90, Phase 91, Phase 92, and Phase 93, must also run their owning structured-diagnostic and rendered-message suites. Historical Phase 89A documentation remains unchanged; active fixtures and current-status checks advance to the latest accepted runtime phase.
