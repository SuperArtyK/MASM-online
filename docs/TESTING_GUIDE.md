...
Phase 90 adds direct virtual Irvine32 `WriteDec` Program Console output and advances runtime/source-run metadata to the Phase 90 WriteDec contract.

## Current regression coverage

Phase 90 regression coverage verifies virtual Irvine32 `WriteDec` unsigned decimal Program Console output for `EAX`, exact values including `0`, `1`, `10`, and `4294967295`, no-newline formatting, complete state preservation, exact-fit and one-byte-short atomic output-limit behavior, exact missing-include and bare-form diagnostics, continued deferral of `INVOKE WriteDec` and sibling numeric routines, and the current output-contract token `phase-90-irvine32-writedec-contract-v1`. Phase 89 `WriteString` regression coverage remains active.

Earlier regression coverage remains active. Phase 87 regression coverage verifies virtual Irvine32 `Crlf` source-run Program Console newline output, zero-argument `INVOKE Crlf`, register/flag/memory preservation, no-partial Program Console limit failures through `Crlf`, precise Crlf invalid-source diagnostics, rendered Simulator Messages for those diagnostics, and continued deferral of non-target Irvine32 routines except for direct `CALL WriteChar`, direct `CALL WriteString`, and direct `CALL WriteDec`. Phase 86 regression coverage verifies deterministic Program Console byte/line limits, no-partial-append output-limit failure behavior, source-run limit-status serialization, rendered `console-output-limit-exceeded` Simulator Messages, and separate Program Console and Simulator Messages streams.
...
For Phase 90 - Irvine32 WriteDec, this smoke-test rule is triggered if the implementation changes any of the following:
...
Every future Irvine32 routine that writes more than one byte of deterministic Program Console output must include exact golden-output tests. This applies at minimum to `WriteString`, `WriteDec`, `WriteInt`, `WriteHex`, `WriteBin`, `DumpRegs`, and `DumpMem` when their owning phases are implemented.
...
| `CALL WriteInt` or another still-deferred Irvine32 routine | `unsupported-feature` | `unsupported-irvine32-routine` | Proves that implemented Irvine32 output routines did not execute future Irvine32 routines. |
...
- The Phase 89A-originated Irvine32 active-status guardrail fixtures must use explicit file-role classification and the following current Phase 90 outcome matrix. A fixture passes or fails because of its assigned role and content condition, not because a stale sentence was individually allowlisted.
  ...
  | Active current-status file | Omits direct `WriteDec` from a current implemented-routine inventory while naming `Crlf`, `WriteChar`, and `WriteString` | Fail |
  ...
  | Active current-status file | Lists direct `CALL WriteString` and direct `CALL WriteDec` as implemented and separately lists `INVOKE WriteString` and `INVOKE WriteDec` as deferred | Pass |
  | Active current-status file | Implies that direct `CALL WriteString` or direct `CALL WriteDec` also implements the corresponding `INVOKE` form | Fail |
  | Active current-status file | Lists direct `CALL WriteString` or direct `CALL WriteDec` in a generic deferred direct-call table after the owning phase is implemented | Fail |
  ...
  | Active metadata/protocol fixture | Implemented phase is 90 and output-contract token is exactly `phase-90-irvine32-writedec-contract-v1` | Pass |
