...
Phase 80 regression coverage also preserves accepted limited `OPTION NOKEYWORD` behavior, Phase 79 automatic LOCAL frame setup/release, Phase 78 parser LOCAL metadata, Phase 77 `PROC USES` runtime behavior, Phase 74 `RET imm16`, Phase 73 `LEAVE`, Phase 72A `PUSH`/`POP`, helper `CALL`/plain `RET`, root `RET`, call-depth limits, procedure fallthrough, entry-end compatibility, Irvine32 `exit`, and future-owned deferral of `OFFSET local`, `ADDR`, `PROTO`, `INVOKE`, scaled-index LOCAL addressing, parameters, calling conventions, and QWORD/SQWORD executable LOCAL memory operands.
...
- static documentation checks continue to list RETF, `OFFSET local`, scaled-index LOCAL addressing, QWORD/SQWORD executable LOCAL memory operands, general procedure-frame features beyond Phase 80 LOCAL operand access, PROTO, INVOKE, ADDR, calling-convention behavior, and Irvine32 callable routine dispatch as deferred unless later phases implement them, while verifying that Phase 73 `LEAVE`, Phase 74 `RET imm16`, Phase 76 `PROC USES` parsing metadata, Phase 77 `PROC USES` runtime save/restore, Phase 78 `LOCAL` declaration metadata, Phase 79 automatic LOCAL frame allocation/lifetime, and Phase 80 supported LOCAL operand resolution/addressing are documented as implemented. The same checks should reject stale active fixed-layout examples that cite stack top `0x00800000` instead of the current fixed-layout stack top `0x00900000`, except inside historical files, explicit stale-value allowlists, or audit-only draft text.
