...
Phase 75 adds tests for bare `PROC` metadata preservation and targeted `PROC` declaration diagnostics. Tests must cover unsupported attributes and parameters, malformed declarations, mismatched `ENDP` names, duplicate procedures, rendered Simulator Messages, and regressions for Phase 74 `RET imm16`, Phase 73 `LEAVE`, Phase 72A `PUSH`/`POP`, helper `CALL`/plain `RET`, root `RET`, call-depth limits, procedure fallthrough, entry-end compatibility, and Irvine32 `exit`.
