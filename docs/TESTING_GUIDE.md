...
Phase 92 adds direct virtual Irvine32 `WriteHex` fixed-width uppercase hexadecimal Program Console output and advances runtime/source-run metadata to the Phase 92 WriteHex contract.

## Current regression coverage

Phase 92 regression coverage verifies virtual Irvine32 `WriteHex` fixed-width uppercase hexadecimal Program Console output for representative `EAX` bit patterns including `00000000h`, `00000001h`, `0000000Ah`, `7FFFFFFFh`, `80000000h`, and `FFFFFFFFh`; exactly eight output bytes with leading zeroes and no newline; complete state preservation; exact-fit eight-byte and one-byte-short atomic output-limit behavior; exact missing-include, bare-form, future-owned `INVOKE WriteHex`, and output-limit diagnostics; continued deferral of `WriteBin`, `DumpRegs`, and `DumpMem`; and the current output-contract token `phase-92-irvine32-writehex-contract-v1`. Phase 91 `WriteInt` and Phase 90 `WriteDec` regression coverage remain active.

Earlier regression coverage remains active. Phase 87 regression coverage verifies virtual Irvine32 `Crlf` source-run Program Console newline output, zero-argument `INVOKE Crlf`, register/flag/memory preservation, no-partial Program Console limit failures through `Crlf`, precise Crlf invalid-source diagnostics, rendered Simulator Messages for those diagnostics, and continued deferral of non-target Irvine32 routines except for direct `CALL WriteChar`, direct `CALL WriteString`, direct `CALL WriteDec`, direct `CALL WriteInt`, and direct `CALL WriteHex`. Phase 86 regression coverage verifies deterministic Program Console byte/line limits, no-partial-append output-limit failure behavior, source-run limit-status serialization, rendered `console-output-limit-exceeded` Simulator Messages, and separate Program Console and Simulator Messages streams.
...
For Phase 92 - Irvine32 WriteHex, this smoke-test rule is triggered if the implementation changes any of the following:
...
Every future Irvine32 routine that writes more than one byte of deterministic Program Console output must include exact golden-output tests. The already-implemented `WriteString`, `WriteDec`, `WriteInt`, and `WriteHex` routines retain their exact golden-output regressions; the rule applies next to `WriteBin`, `DumpRegs`, and `DumpMem` when their owning phases are implemented.
...
| `CALL WriteBin` or another still-deferred Irvine32 routine | `unsupported-feature` | `unsupported-irvine32-routine` | Proves that implemented Irvine32 output routines did not execute future Irvine32 routines. |
...
- The Phase 89A-originated Irvine32 active-status guardrail fixtures must use explicit file-role classification and the following current Phase 92 outcome matrix. A fixture passes or fails because of its assigned role and content condition, not because a stale sentence was individually allowlisted.
  ...
  | Active current-status file | Lists direct `CALL WriteString`, direct `CALL WriteDec`, direct `CALL WriteInt`, and direct `CALL WriteHex` as implemented and separately lists their corresponding `INVOKE` forms as deferred | Pass |
  | Active current-status file | Implies that direct `CALL WriteString`, direct `CALL WriteDec`, direct `CALL WriteInt`, or direct `CALL WriteHex` also implements the corresponding `INVOKE` form | Fail |
  | Active current-status file | Lists direct `CALL WriteString`, direct `CALL WriteDec`, direct `CALL WriteInt`, or direct `CALL WriteHex` in a generic deferred direct-call table after the owning phase is implemented | Fail |
  ...
  | Active metadata/protocol fixture | Implemented phase is 92 and output-contract token is exactly `phase-92-irvine32-writehex-contract-v1` | Pass |
