...
Phase 91 adds direct virtual Irvine32 `WriteInt` signed-decimal Program Console output and advances runtime/source-run metadata to the Phase 91 WriteInt contract.

## Current regression coverage

Phase 91 regression coverage verifies virtual Irvine32 `WriteInt` signed decimal Program Console output for `EAX`, including `0`, `1`, `-1`, `2147483647`, and `-2147483648`, no-newline formatting, complete state preservation, exact-fit and one-byte-short atomic output-limit behavior including the minus sign, exact missing-include and bare-form diagnostics, continued deferral of `INVOKE WriteInt` and sibling numeric routines, and the current output-contract token `phase-91-irvine32-writeint-contract-v1`. Phase 90 `WriteDec` regression coverage remains active.

Earlier regression coverage remains active. Phase 87 regression coverage verifies virtual Irvine32 `Crlf` source-run Program Console newline output, zero-argument `INVOKE Crlf`, register/flag/memory preservation, no-partial Program Console limit failures through `Crlf`, precise Crlf invalid-source diagnostics, rendered Simulator Messages for those diagnostics, and continued deferral of non-target Irvine32 routines except for direct `CALL WriteChar`, direct `CALL WriteString`, direct `CALL WriteDec`, and direct `CALL WriteInt`. Phase 86 regression coverage verifies deterministic Program Console byte/line limits, no-partial-append output-limit failure behavior, source-run limit-status serialization, rendered `console-output-limit-exceeded` Simulator Messages, and separate Program Console and Simulator Messages streams.
...
For Phase 91 - Irvine32 WriteInt, this smoke-test rule is triggered if the implementation changes any of the following:
...
Every future Irvine32 routine that writes more than one byte of deterministic Program Console output must include exact golden-output tests. The already-implemented `WriteString`, `WriteDec`, and `WriteInt` routines retain their exact golden-output regressions; the rule applies next to `WriteHex`, `WriteBin`, `DumpRegs`, and `DumpMem` when their owning phases are implemented.
...
| `CALL WriteHex` or another still-deferred Irvine32 routine | `unsupported-feature` | `unsupported-irvine32-routine` | Proves that implemented Irvine32 output routines did not execute future Irvine32 routines. |
...
- The Phase 89A-originated Irvine32 active-status guardrail fixtures must use explicit file-role classification and the following current Phase 91 outcome matrix. A fixture passes or fails because of its assigned role and content condition, not because a stale sentence was individually allowlisted.
  ...
  | Active current-status file | Lists direct `CALL WriteString`, direct `CALL WriteDec`, and direct `CALL WriteInt` as implemented and separately lists their corresponding `INVOKE` forms as deferred | Pass |
  | Active current-status file | Implies that direct `CALL WriteString`, direct `CALL WriteDec`, or direct `CALL WriteInt` also implements the corresponding `INVOKE` form | Fail |
  | Active current-status file | Lists direct `CALL WriteString`, direct `CALL WriteDec`, or direct `CALL WriteInt` in a generic deferred direct-call table after the owning phase is implemented | Fail |
  ...
  | Active metadata/protocol fixture | Implemented phase is 91 and output-contract token is exactly `phase-91-irvine32-writeint-contract-v1` | Pass |
