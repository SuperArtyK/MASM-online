...
Phase 78 adds tests for parser-only `LOCAL` declaration metadata, deterministic local frame layout, valid scalar/array/comma-separated declarations, scoped local shadowing, targeted invalid `LOCAL` diagnostics, rendered Simulator Messages for `unsupported-local-type` and `invalid-local-count`, and source-run regressions proving no runtime local stack allocation or local operand resolution has been introduced. Tests must also preserve Phase 77 `PROC USES` runtime save/restore, including `stack-overflow` and `stack-underflow` diagnostics, Phase 76 parsing metadata, Phase 75 non-USES `PROC` diagnostics, and regressions for Phase 74 `RET imm16`, Phase 73 `LEAVE`, Phase 72A `PUSH`/`POP`, helper `CALL`/plain `RET`, root `RET`, call-depth limits, procedure fallthrough, entry-end compatibility, and Irvine32 `exit`.
...
- static documentation checks continue to list RETF, runtime local stack allocation, local operand resolution, stack-frame creation, PROTO, INVOKE, ADDR, calling-convention behavior, and Irvine32 callable routine dispatch as deferred unless later phases implement them, while verifying that Phase 73 `LEAVE`, Phase 74 `RET imm16`, Phase 76 `PROC USES` parsing metadata, Phase 77 `PROC USES` runtime save/restore, and Phase 78 parser-only `LOCAL` declaration metadata are documented as implemented.
