...
Phase 78A adds tests for accepted limited `OPTION NOKEYWORD:<LOOP>`, `OPTION NOKEYWORD:<OFFSET>`, and whitespace-list forms; malformed forms; unknown listed words; protected words; atomic failure for mixed valid/invalid lists; source-order behavior; case-insensitive list matching; disabled `LOOP` and disabled `OFFSET` as later symbols; disabled `LOOP` rejected in its old instruction-family role; disabled `OFFSET` rejected in its old operator role; rendered Simulator Messages; output-contract metadata; and regressions proving `OPTION CASEMAP`, ordinary reserved-word rejection, registers, Irvine32 registry names, Phase 78 parser-only LOCAL metadata, and Phase 77 `PROC USES` runtime behavior remain unchanged, including `stack-overflow`, `stack-underflow`, and regressions for Phase 74 `RET imm16`, Phase 73 `LEAVE`, Phase 72A `PUSH`/`POP`, helper `CALL`/plain `RET`, root `RET`, call-depth limits, procedure fallthrough, entry-end compatibility, and Irvine32 `exit`.

Future LOCAL runtime phases must add tests before implementation that prove selected-entry locals, direct-CALL helper locals, `PROC USES` plus LOCAL ordering, root `RET`, helper `RET`, helper `RET imm16`, virtual Irvine32 `exit`, source-level `LEAVE`, branch/fallthrough rejection, deterministic visible byte initialization, uninitialized-origin metadata, local-object bounds, rendered Simulator Messages, output-contract tokens, and no-partial-mutation behavior. A future LOCAL frame implementation must not rely only on a single helper-call happy-path fixture.
...
- static documentation checks continue to list RETF, runtime local stack allocation, local operand resolution, general procedure-frame creation, PROTO, INVOKE, ADDR, calling-convention behavior, and Irvine32 callable routine dispatch as deferred unless later phases implement them, while verifying that Phase 73 `LEAVE`, Phase 74 `RET imm16`, Phase 76 `PROC USES` parsing metadata, Phase 77 `PROC USES` runtime save/restore, and Phase 78 parser-only `LOCAL` declaration metadata are documented as implemented. The same checks should reject stale active fixed-layout examples that cite stack top `0x00800000` instead of the current fixed-layout stack top `0x00900000`, except inside historical files, explicit stale-value allowlists, or audit-only draft text.
