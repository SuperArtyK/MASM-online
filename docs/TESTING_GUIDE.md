...
Phase 77 adds tests for runtime `PROC USES` register save/restore on supported direct `CALL` entry, listed-register preservation, unlisted `EAX` return behavior, modeled flag preservation, stack balance, `stack-overflow` save failure, `stack-underflow` restore failure, and rendered Simulator Messages for automatic USES save/restore diagnostics. Tests must also preserve Phase 76 parsing metadata, ordered canonical register storage, invalid register/list diagnostics, Phase 75 non-USES `PROC` diagnostics, and regressions for Phase 74 `RET imm16`, Phase 73 `LEAVE`, Phase 72A `PUSH`/`POP`, helper `CALL`/plain `RET`, root `RET`, call-depth limits, procedure fallthrough, entry-end compatibility, and Irvine32 `exit`.
...
- static documentation checks continue to list RETF, stack-frame creation, LOCAL, PROTO, INVOKE, ADDR, calling-convention behavior, and Irvine32 callable routine dispatch as deferred unless later phases implement them, while verifying that Phase 73 `LEAVE`, Phase 74 `RET imm16`, Phase 76 `PROC USES` parsing metadata, and Phase 77 `PROC USES` runtime save/restore are documented as implemented.
