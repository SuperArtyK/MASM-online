...
Phase 82 advances runtime/source-run behavior metadata and the source-run output-contract identifier because zero-argument `INVOKE Helper` / `invoke Helper` to same-file user procedures is now executable, INVOKE-specific diagnostics changed, rendered Simulator Messages changed, and browser protocol expectations changed. Phase 81 limited parser-owned `PROTO` metadata, Phase 80 LOCAL operand resolution/addressing, and Phase 79 automatic LOCAL frame behavior remain part of the public source-run behavior.
...
2. **Checked-in artifact-content verification**
   ...
   A binary-content scan of checked-in `web/dist/masm32_sim_core.wasm` may confirm that the artifact contains an expected output-contract string such as `phase-82-invoke-zero-argument-output-contract-v1`. This is useful stale-artifact evidence, but it is not a rebuild.
