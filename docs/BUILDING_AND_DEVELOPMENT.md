...
Phase 71D advances runtime/source-run MASM behavior. Ordinary procedure-boundary fallthrough is now governed by `procedureFallthroughPolicy`, where default `warn` emits `procedure-fell-through` and continues, `off` suppresses only that diagnostic, and `error` emits a runtime error before the destination procedure instruction executes. `code-fell-off-end`, root `RET`, invalid return-token, parser, and memory diagnostics remain independent.
...
2. **Checked-in artifact-content verification**
   ...
   A binary-content scan of checked-in `web/dist/masm32_sim_core.wasm` may confirm that the artifact contains an expected output-contract string such as `phase-71d-procedure-fallthrough-policy-output-contract-v1`. This is useful stale-artifact evidence, but it is not a rebuild.
