...
Phase 80 advances runtime/source-run behavior metadata and the source-run output-contract identifier because supported source-level LOCAL operands and LOCAL addressing now execute against active automatic LOCAL frames. Phase 79 automatic LOCAL frame setup/release, frame-entry validation, stack-overflow diagnostics, invalid-frame-state diagnostics, and rendered Simulator Messages remain part of the public source-run behavior.
...
2. **Checked-in artifact-content verification**
   ...
   A binary-content scan of checked-in `web/dist/masm32_sim_core.wasm` may confirm that the artifact contains an expected output-contract string such as `phase-80-local-operand-output-contract-v1`. This is useful stale-artifact evidence, but it is not a rebuild.
