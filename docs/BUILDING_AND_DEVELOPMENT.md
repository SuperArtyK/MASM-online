...
Phase 75 advances runtime/source-run behavior metadata and the source-run output-contract identifier because targeted `PROC` declaration diagnostics are now part of the public source-run and browser-rendered diagnostic contract. Bare `PROC`/`ENDP` behavior remains metadata-only and runtime behavior is unchanged from Phase 74.
...
2. **Checked-in artifact-content verification**
   ...
   A binary-content scan of checked-in `web/dist/masm32_sim_core.wasm` may confirm that the artifact contains an expected output-contract string such as `phase-75-proc-metadata-output-contract-v1`. This is useful stale-artifact evidence, but it is not a rebuild.
...
```text
phase-75-proc-metadata-output-contract-v1
```
