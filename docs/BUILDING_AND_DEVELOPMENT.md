...
Phase 64C changes final-state display formatting by showing modeled flag child rows under EFLAGS. It does not add MASM syntax, parser behavior, VM instruction behavior, executor behavior, new modeled flags, new flag semantics, new diagnostics, new diagnostic-policy families, or new source-run status fields. Phase 64B changes rendered Simulator Messages ordering and group separators for existing source-less notices, runtime diagnostics, and final execution-status messages. Accepted MASM syntax remains the Phase 64 equality-jump subset; runtime/source-run MASM behavior phase remains Phase 64A; existing read-modify-write memory destinations continue to route through planned-read validation before bytes are consumed or write-back occurs. Phase 64 - Equality Conditional Jumps remains implemented for `je`, `jz`, `jne`, and `jnz`. Phase 63 - CMP Memory Operand Forms remains implemented; CMP memory reads use checked helpers and planned-read validation. Phase 61E reserved-word diagnostics remain active through `reserved-word-symbol`: `OPTION CASEMAP:NONE` does not make reserved words usable as symbols, and `OPTION NOKEYWORD` remains unsupported until a later explicit keyword-control phase. Phase 61C - Branch Debugger Dependency Cleanup remains the debugger/editor boundary cleanup. Phase 61D - Source-Run Capacity Documentation and Diagnostic Hardening remains the documentation/static-check and regression-test hardening phase for parser/source-run capacity behavior. It distinguishes capacity diagnostics such as `token-capacity-exceeded`, `source-text-capacity-exceeded`, `code-label-capacity-exceeded`, and `data-capacity-exceeded` from the runtime `instructionLimit` watchdog. Phase 64C does not add signed relational jumps, unsigned relational jumps, `loop`, debugger stepping, breakpoint behavior, editor source navigation, current-instruction highlighting, CodeMirror gutter behavior, or branch-target editor highlighting. Active-time watchdog behavior remains future work owned by Phase 200 - Active Time Watchdog and Worker Responsiveness.
