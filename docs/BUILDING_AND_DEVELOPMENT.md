...
Phase 64 changes parser/source-run MASM behavior by adding executable equality conditional jumps: `je label`, `jz label`, `jne label`, and `jnz label`. These direct branch forms consume `ZF` through the shared undefined-flag-use policy, branch to executable code-label or procedure-entry targets when true, fall through when false, count as one executed instruction, preserve registers, memory, and modeled flags, and respect the Phase 59 `instructionLimit` watchdog. Phase 63 - CMP Memory Operand Forms remains implemented; CMP memory reads use checked helpers and planned-read validation. Phase 61E reserved-word diagnostics remain active through `reserved-word-symbol`: `OPTION CASEMAP:NONE` does not make reserved words usable as symbols, and `OPTION NOKEYWORD` remains unsupported until a later explicit keyword-control phase. Phase 61D - Source-Run Capacity Documentation and Diagnostic Hardening remains the documentation/static-check and regression-test hardening phase for parser/source-run capacity behavior. It distinguishes capacity diagnostics such as `token-capacity-exceeded`, `source-text-capacity-exceeded`, `code-label-capacity-exceeded`, and `data-capacity-exceeded` from the runtime `instructionLimit` watchdog. Phase 61C - Branch Debugger Dependency Cleanup remains the debugger/editor boundary cleanup. Phase 64 does not add signed relational jumps, unsigned relational jumps, `loop`, debugger stepping, breakpoint behavior, editor source navigation, current-instruction highlighting, CodeMirror gutter behavior, or branch-target editor highlighting. Active-time watchdog behavior remains future work owned by Phase 200 - Active Time Watchdog and Worker Responsiveness.
