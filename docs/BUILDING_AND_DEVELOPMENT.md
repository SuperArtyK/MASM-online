...
Phase 64A corrects source-run planned-read coverage for existing memory-reading instructions. Accepted MASM syntax remains the Phase 64 equality-jump subset; existing read-modify-write memory destinations now route through planned-read validation before bytes are consumed or write-back occurs. Phase 64 - Equality Conditional Jumps remains implemented for `je`, `jz`, `jne`, and `jnz`. Phase 63 - CMP Memory Operand Forms remains implemented; CMP memory reads use checked helpers and planned-read validation. Phase 61E reserved-word diagnostics remain active through `reserved-word-symbol`: `OPTION CASEMAP:NONE` does not make reserved words usable as symbols, and `OPTION NOKEYWORD` remains unsupported until a later explicit keyword-control phase. Phase 61C - Branch Debugger Dependency Cleanup remains the debugger/editor boundary cleanup. Phase 61D - Source-Run Capacity Documentation and Diagnostic Hardening remains the documentation/static-check and regression-test hardening phase for parser/source-run capacity behavior. It distinguishes capacity diagnostics such as `token-capacity-exceeded`, `source-text-capacity-exceeded`, `code-label-capacity-exceeded`, and `data-capacity-exceeded` from the runtime `instructionLimit` watchdog. Phase 64A does not add signed relational jumps, unsigned relational jumps, `loop`, debugger stepping, breakpoint behavior, editor source navigation, current-instruction highlighting, CodeMirror gutter behavior, or branch-target editor highlighting. Active-time watchdog behavior remains future work owned by Phase 200 - Active Time Watchdog and Worker Responsiveness.
