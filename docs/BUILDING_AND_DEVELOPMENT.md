...
Phase 65 adds executable direct-label signed relational conditional jumps and advances both repository/archive status and runtime/source-run MASM behavior status. The current direct-branch subset includes direct `jmp label`, equality conditional jumps `je`, `jz`, `jne`, and `jnz`, and signed relational conditional jumps `jl`, `jnge`, `jle`, `jng`, `jg`, `jnle`, `jge`, and `jnl`. Signed relational jumps consume the required modeled flags through `undefined-flag-use` policy, preserve registers, memory, modeled flags, Program Console output, and memory-change rows, and respect the Phase 59 instruction-count watchdog.

Phase 64D - Memory Change Source Attribution Display remains implemented and adds source-run memory-change source attribution display. Phase 64C changes final-state display formatting by showing modeled flag child rows under EFLAGS. Phase 64B changes rendered Simulator Messages ordering and group separators for existing source-less notices, runtime diagnostics, and final execution-status messages. Phase 64A keeps existing read-modify-write memory destinations on planned-read validation before bytes are consumed or write-back occurs. Phase 63 - CMP Memory Operand Forms remains implemented; CMP memory reads use checked helpers and planned-read validation. Phase 61E reserved-word diagnostics remain active through `reserved-word-symbol`: `OPTION CASEMAP:NONE` does not make reserved words usable as symbols, and `OPTION NOKEYWORD` remains unsupported until a later explicit keyword-control phase. Phase 61C - Branch Debugger Dependency Cleanup remains the debugger/editor boundary cleanup. Phase 61D - Source-Run Capacity Documentation and Diagnostic Hardening remains the documentation/static-check and regression-test hardening phase for parser/source-run capacity behavior. It distinguishes capacity diagnostics such as `token-capacity-exceeded`, `source-text-capacity-exceeded`, `code-label-capacity-exceeded`, and `data-capacity-exceeded` from the runtime `instructionLimit` watchdog. Phase 65 does not add unsigned relational jumps, `loop`, indirect jumps, stack/procedure behavior, debugger stepping, breakpoint behavior, editor source navigation, current-instruction highlighting, CodeMirror gutter behavior, or branch-target editor highlighting. Active-time watchdog behavior remains future work owned by Phase 200 - Active Time Watchdog and Worker Responsiveness.
