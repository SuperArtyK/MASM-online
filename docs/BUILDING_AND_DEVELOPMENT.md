...
Phase 73 advances runtime/source-run behavior metadata and the source-run output-contract identifier because source-level `leave`/`LEAVE` frame teardown is now executable. Source-run output keeps `LEAVE` stack reads internal with no public `memoryChanges` rows, while checked-memory diagnostics still stop fatal `[EBP]` reads before `ESP` or `EBP` mutation.
...
2. **Checked-in artifact-content verification**
   ...
   A binary-content scan of checked-in `web/dist/masm32_sim_core.wasm` may confirm that the artifact contains an expected output-contract string such as `phase-73-leave-output-contract-v1`. This is useful stale-artifact evidence, but it is not a rebuild.
...
```text
phase-73-leave-output-contract-v1
```
