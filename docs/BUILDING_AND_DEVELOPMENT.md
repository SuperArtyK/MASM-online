...
Phase 78A advances runtime/source-run behavior metadata and the source-run output-contract identifier because the limited educational `OPTION NOKEYWORD` forms for `LOOP` and `OFFSET` are now accepted, invalid NOKEYWORD lists use targeted structured diagnostics, and rendered Simulator Messages cover the new keyword-control diagnostic paths. Phase 78 parser-only `LOCAL` metadata remains implemented; runtime local stack allocation and local operand resolution remain deferred.
...
2. **Checked-in artifact-content verification**
   ...
   A binary-content scan of checked-in `web/dist/masm32_sim_core.wasm` may confirm that the artifact contains an expected output-contract string such as `phase-78a-nokeyword-output-contract-v1`. This is useful stale-artifact evidence, but it is not a rebuild.
