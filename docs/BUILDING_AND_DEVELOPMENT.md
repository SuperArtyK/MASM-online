...
Phase 81 advances runtime/source-run behavior metadata and the source-run output-contract identifier because limited parser-owned `PROTO` metadata, structured `PROTO` diagnostics, rendered Simulator Messages, and browser protocol expectations changed. Phase 80 supported LOCAL operand resolution/addressing and Phase 79 automatic LOCAL frame behavior remain part of the public source-run behavior.
...
2. **Checked-in artifact-content verification**
   ...
   A binary-content scan of checked-in `web/dist/masm32_sim_core.wasm` may confirm that the artifact contains an expected output-contract string such as `phase-81-proto-metadata-output-contract-v1`. This is useful stale-artifact evidence, but it is not a rebuild.
