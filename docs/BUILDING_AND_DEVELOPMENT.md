...
2. **Checked-in artifact-content verification**
   ...
   A binary-content scan of checked-in `web/dist/masm32_sim_core.wasm` may confirm that the artifact contains an expected output-contract string such as `phase-89-irvine32-writestring-contract-v1`. This is useful stale-artifact evidence, but it is not a rebuild.
