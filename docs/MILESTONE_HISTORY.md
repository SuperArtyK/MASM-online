...
Forward-looking phase navigation is guide-owned. At the time this history entry was updated, Phase 75 had been implemented as PROC metadata baseline and attribute diagnostics. That statement is historical navigation for this history entry, not an implementation permission by itself and not a substitute for reading the current implementation guide.
...
Phase 75 advances runtime/source-run MASM behavior metadata and source-run output-contract metadata to `phase-75-proc-metadata-output-contract-v1` because structured diagnostics and rendered Simulator Messages changed. Accepted `PROC USES`, `LOCAL`, `PROTO`, `INVOKE`, `ADDR`, parameters, calling conventions, automatic stack frames, and `USES` save/restore remain future-owned.
