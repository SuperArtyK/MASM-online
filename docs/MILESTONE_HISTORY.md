...
Forward-looking phase navigation is guide-owned. At the time this history entry was updated, Phase 77 had been implemented as PROC USES runtime save/restore. That statement is historical navigation for this history entry, not an implementation permission by itself and not a substitute for reading the current implementation guide.
