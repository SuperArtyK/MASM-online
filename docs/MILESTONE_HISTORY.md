...
Phase 93 implements focused virtual Irvine32 fixed-width binary output. After `INCLUDE Irvine32.inc`, direct `call WriteBin` reads the current 32-bit bit pattern of `EAX` as an unsigned value and appends exactly 32 binary digits to Program Console, most-significant bit first. Leading zeroes are retained to reach 32 digits; output contains only `0` and `1` and has no prefix, suffix, separators, whitespace, carriage return, or line feed. Irvine32 routine-name matching remains case-insensitive even under `OPTION CASEMAP:NONE`.

`WriteBin` preserves all modeled registers including `EAX`, modeled flags and flag-validity metadata, simulated memory, and memory-change rows. It performs no simulated-memory reads or writes. The complete 32-byte candidate is preflighted through the shared Program Console output-limit path; exactly 32 remaining bytes succeed, while an over-limit call appends no partial digits, preserves earlier Program Console content, reports `console-output-limit-exceeded` against the complete direct-call instruction span, and does not emit `execution-complete`.

Phase 93 advances runtime/source-run metadata to `Phase 93 - Irvine32 WriteBin` with output-contract token `phase-93-irvine32-writebin-contract-v1`, updates the browser default program to demonstrate direct `WriteBin`, and advances active status guardrails. `INVOKE WriteBin`, `DumpRegs`, `DumpMem`, later Irvine32 routine families, and permanent WinAPI/PE/linker/host/native/full-x86 non-goals remain outside Phase 93 scope. Existing `WriteHex`, `WriteInt`, and `WriteDec` behavior remains unchanged.
...
Phase 91 implements focused virtual Irvine32 signed decimal output. After `INCLUDE Irvine32.inc`, direct `call WriteInt` reads the current 32-bit bit pattern of `EAX` as a signed two's-complement integer and appends its shortest base-10 representation to Program Console. Negative values use exactly one leading `-`; nonnegative values have no `+`; output has no leading zeroes except `0`, separators, whitespace, prefix, suffix, carriage return, or line feed. The `80000000h` bit pattern is formatted as `-2147483648` without relying on signed-negation overflow. Irvine32 routine-name matching remains case-insensitive even under `OPTION CASEMAP:NONE`.

`WriteInt` preserves all modeled registers including `EAX`, modeled flags and flag-validity metadata, simulated memory, and memory-change rows. It performs no simulated-memory reads or writes. The complete signed decimal candidate, including the minus sign, is preflighted through the shared Program Console output-limit path; exact remaining capacity succeeds, while an over-limit call appends no partial sign or digits, preserves earlier Program Console content, reports `console-output-limit-exceeded` against the complete direct-call instruction span, and does not emit `execution-complete`.

Phase 91 advances runtime/source-run metadata to `Phase 91 - Irvine32 WriteInt` with output-contract token `phase-91-irvine32-writeint-contract-v1`, updates the browser default program to demonstrate the signed `INT32_MIN` case, and advances active status guardrails. `INVOKE WriteInt`, `WriteHex`, `WriteBin`, `DumpRegs`, `DumpMem`, later Irvine32 routine families, and permanent WinAPI/PE/linker/host/native/full-x86 non-goals remain outside Phase 91 scope. Existing `WriteDec` unsigned-decimal behavior remains unchanged.

This milestone-history entry is implementation history. If a future assistant finds a conflict between this entry and the active canonical `FULL_IMPLEMENTATION_SPEC.md` or `INCREMENTAL_IMPLEMENTATION_GUIDE.md`, the active spec and guide control.

## Phase 90 - Irvine32 WriteDec

Phase 90 implements focused virtual Irvine32 unsigned decimal output. After `INCLUDE Irvine32.inc`, direct `call WriteDec` reads `EAX` as an unsigned 32-bit value and appends its shortest base-10 representation to Program Console without a sign, leading zeroes, separators, whitespace, prefix, suffix, carriage return, or line feed. Irvine32 routine-name matching remains case-insensitive even under `OPTION CASEMAP:NONE`.

`WriteDec` preserves all modeled registers including `EAX`, modeled flags and flag-validity metadata, simulated memory, and memory-change rows. It performs no simulated-memory reads or writes. The complete decimal candidate is preflighted through the shared Program Console output-limit path; exact remaining capacity succeeds, while an over-limit call appends no partial digits, preserves earlier Program Console content, reports `console-output-limit-exceeded` against the complete direct-call instruction span, and does not emit `execution-complete`.

Phase 90 advances runtime/source-run metadata to `Phase 90 - Irvine32 WriteDec` with output-contract token `phase-90-irvine32-writedec-contract-v1`, updates the browser default program to demonstrate direct `WriteDec`, and advances active status guardrails. `INVOKE WriteDec`, `WriteInt`, `WriteHex`, `WriteBin`, later Irvine32 routine families, and permanent WinAPI/PE/linker/host/native/full-x86 non-goals remain outside Phase 90 scope.
...
Phase 88 implements the focused virtual Irvine32 `WriteChar` routine. With `INCLUDE Irvine32.inc`, direct `call WriteChar` appends exactly one Program Console byte copied from the low byte of `AL`. The high bytes of `EAX` do not affect output. The routine preserves all registers, modeled flags, flag-validity metadata, and memory, and it keeps Program Console output separate from Simulator Messages.

`WriteChar` uses the Phase 86 Program Console append path, so byte-limit and line-limit failures emit `console-output-limit-exceeded` through Simulator Messages and append no partial byte. Phase 88 does not implement `INVOKE WriteChar`, `WriteString`, numeric output routines, input routines, macros, WinAPI behavior, PE loading, linking, host console I/O, or filesystem behavior. Those remain future-owned or permanent non-goals according to the canonical specification and guide.
...
Phase 84 implements the limited same-file user-procedure `INVOKE` DWORD argument subset defined by the guide. Accepted arguments include 32-bit immediates, full 32-bit registers, `DWORD`/`SDWORD` data-symbol values, `OFFSET` data/`.DATA?`/`.CONST` symbols, and `ADDR` data/`.DATA?`/`.CONST`/active `LOCAL` symbols. Accepted arguments lower to checked stack slots from right to left before the call transfer.

Phase 84 requires exact cleanup metadata for nonzero-argument INVOKE: the callee must return with consistent `ret imm16` cleanup equal to four bytes per argument. Cleanup mismatches report `invoke-cleanup-mismatch`, prototype argument-count mismatches report `invoke-argument-count-mismatch`, 8-bit and 16-bit register arguments report `invoke-argument-width-unsupported`, unsupported argument forms report `unsupported-invoke-argument`, invalid ADDR targets report `invalid-addr-target`, and external/API targets report `unsupported-external-invoke`. Irvine32 routine dispatch through `INVOKE`, named runtime PROC parameter access, pointer or unnamed prototype parameters, VARARG, calling conventions, computed ADDR expressions, and general source-level ADDR remain future-owned or non-goal behavior.

## Phase 83 - ADDR Preparation for Future INVOKE Arguments

Phase 83 prepares limited helper-level `ADDR symbol` argument records for a future INVOKE argument-lowering milestone. The helper record carries a resolved 32-bit address value plus source spans for both the ADDR token and the operand token. It can resolve data, `.DATA?`, `.CONST`, and explicitly active-frame LOCAL metadata in parser/helper tests. It does not read pointed-to memory, write pointed-to memory, push arguments, clean arguments, access runtime parameters, implement calling conventions, or bypass `.CONST` protection.

Phase 83 adds targeted ADDR diagnostics for helper and source-level rejection paths: `invalid-addr-target`, `addr-outside-invoke`, `unknown-addr-symbol`, and `unsupported-addr-expression`. Complete source-level forms such as `INVOKE Helper, ADDR msg` and `INVOKE Helper, ADDR localVar` remain non-executable and still report `invoke-arguments-not-supported-yet`. Phase 82 zero-argument `INVOKE Helper` / `invoke Helper` behavior remains implemented.
...
2. Phase 83 - helper-level `ADDR` preparation for INVOKE arguments;
...
Phase 80 implements supported source-level LOCAL operand resolution and addressing on top of Phase 79 automatic runtime stack allocation and lifetime for parser-owned `LOCAL` metadata. Supported instructions can read and write scalar LOCAL variables and byte-addressed LOCAL array elements through active EBP-relative frame storage, and `LEA` can compute supported LOCAL addresses without reading memory. Selected-entry procedures and direct-CALL helper procedures with LOCAL declarations continue to receive checked EBP/ESP-based frames, deterministic zero-initialized visible local bytes, active local-object descriptors, and release on `RET`, `RET imm16`, `LEAVE` followed by `RET`, and Irvine32 `exit`. Branch or fallthrough entry into a LOCAL procedure without an automatic frame remains rejected with `local-frame-entry-unsupported`; inconsistent frame release reports `invalid-frame-state`; failed frame preflight/setup reports `stack-overflow`. `OFFSET local`, `ADDR`, `PROTO`, `INVOKE`, parameters, calling conventions, scaled-index LOCAL addressing, QWORD/SQWORD executable LOCAL memory access, and expanded LOCAL grammar remain future-owned.

## Phase 78A - Limited OPTION NOKEYWORD Reserved-Word Opt-Out

Phase 78A implements the limited educational `OPTION NOKEYWORD` subset for the exact words `LOOP` and `OFFSET`. Valid directives disable those keyword roles only for later source lines, allow the disabled spellings and ASCII case variants as ordinary user-defined symbols where otherwise valid, accumulate across repeated valid directives, and treat duplicate words idempotently. Malformed lists, unknown words, protected reserved words, and disabled keywords used in their old keyword roles now use targeted structured diagnostics. The phase advances source-run metadata to `phase-78a-nokeyword-output-contract-v1` while preserving Phase 78 LOCAL metadata and leaving runtime LOCAL allocation plus local operand behavior future-owned at the time of Phase 78A. Phase 79 later implements runtime LOCAL allocation/lifetime, and Phase 80 later implements supported LOCAL operand resolution/addressing.
...
Phase 70 preserves Phase 67A selected-entry source-run behavior, Phase 68 call-target metadata, Phase 68B pseudo-EIP display tokens, and Phase 69 direct CALL mechanics. Phase 71 adds selected-entry root RET termination and called non-entry procedure fallthrough diagnostics. Phase 72A adds source-level 32-bit PUSH/POP stack transfers. Phase 73 adds source-level LEAVE frame teardown. Phase 74 adds near `RET imm16` caller cleanup. Phase 75 adds targeted `PROC` declaration diagnostics while preserving bare procedure metadata. Phase 76 adds accepted `PROC USES` parsing metadata. Phase 77 adds direct-CALL runtime save/restore for `PROC USES`. Phase 78 adds `LOCAL` declaration metadata and deterministic local frame layout metadata. Phase 79 adds automatic runtime LOCAL stack allocation and lifetime on selected-entry and direct-CALL procedure paths. Phase 80 adds supported source-level LOCAL operand resolution/addressing for active automatic LOCAL frames. Phase 81 adds limited parser-owned `PROTO` metadata with no executable behavior. Phase 82 adds zero-argument same-file user-procedure `INVOKE`. Phase 83 adds helper-level `ADDR symbol` preparation for future INVOKE arguments. Phase 84 adds limited same-file user-procedure INVOKE DWORD argument lowering and exact cleanup validation. Procedure-frame features beyond Phase 84 limited INVOKE DWORD arguments, Phase 83 helper-level ADDR preparation, Phase 82 zero-argument INVOKE, Phase 81 PROTO metadata, and Phase 80 LOCAL operand access, Irvine32 routine dispatch beyond virtual `exit`, far returns, `OFFSET local`, scaled-index LOCAL addressing, QWORD/SQWORD executable LOCAL operands, unsupported `INVOKE` argument forms, executable source-level `ADDR` outside accepted INVOKE arguments, executable `PROTO` behavior beyond Phase 84 validation, pointer/unnamed prototype parameters, and `VARARG` remain future-owned.
...
Phase 78A advances runtime/source-run MASM behavior metadata and source-run output-contract metadata to `phase-78a-nokeyword-output-contract-v1` because accepted syntax, structured diagnostics, rendered Simulator Messages, and browser protocol interpretation changed. Phase 79 then advances runtime/source-run MASM behavior metadata and source-run output-contract metadata to `phase-79-local-frame-output-contract-v1` because automatic LOCAL frame allocation/lifetime, frame-state diagnostics, and rendered Simulator Messages changed. Phase 80 then advances runtime/source-run MASM behavior metadata and source-run output-contract metadata to `phase-80-local-operand-output-contract-v1` because supported source-level LOCAL operand execution and frame-relative LOCAL addressing changed. Phase 81 then advances runtime/source-run MASM behavior metadata and source-run output-contract metadata to `phase-81-proto-metadata-output-contract-v1` because limited parser-owned PROTO metadata, targeted PROTO diagnostics, rendered Simulator Messages, and browser protocol interpretation changed. Phase 82 then advances metadata for zero-argument user-procedure INVOKE. Phase 83 then advances metadata for helper-level ADDR preparation and targeted ADDR diagnostics. Phase 84 then advances metadata for limited same-file user-procedure INVOKE DWORD arguments, stack lowering, cleanup diagnostics, and rendered Simulator Messages. `OFFSET local`, executable source-level `ADDR` outside accepted INVOKE arguments, unsupported executable `INVOKE` argument forms, executable `PROTO` behavior beyond Phase 84 validation, pointer or unnamed prototype parameters, `VARARG`, named runtime parameters, calling conventions, automatic stack-frame features beyond Phase 84 limited INVOKE DWORD arguments, Phase 83 helper-level ADDR preparation, Phase 82 zero-argument INVOKE, Phase 81 PROTO metadata, and Phase 80 LOCAL operand access, plus Irvine32 callable routine dispatch remain future-owned.
