...
Next canonical corrective phase:
...
Phase 70A - Runtime Metadata Exact-Match Compatibility Check is the next canonical corrective phase in the implementation guide. It is protocol/artifact compatibility cleanup only and must not change MASM source syntax, parser behavior, VM execution semantics, CALL behavior, RET behavior, Program Console output, or public `memoryChanges` rows. It exists to make newer-runtime and older-runtime Wasm metadata mismatches visible by default.
...
- Phase 70A is the next canonical protocol/artifact compatibility corrective phase. It tightens runtime metadata mismatch checks by default and does not change MASM source behavior.
