...
Forward-looking phase navigation is guide-owned. At the time this history entry was updated, Phase 83 had been implemented as helper-level `ADDR symbol` preparation for future INVOKE arguments while preserving Phase 82 zero-argument `INVOKE` behavior and rejecting source-level INVOKE-with-arguments. That statement is historical navigation for this history entry, not an implementation permission by itself and not a substitute for reading the current implementation guide.
