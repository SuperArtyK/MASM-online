...
- `INCLUDE Irvine32.inc` and `INCLUDE Macros.inc` as virtual built-ins. The simulator does not load host files. `INCLUDE Irvine32.inc` registers known Irvine32 names as virtual metadata for classification and diagnostics; it does not implement `call`, `exit`, routine bodies, Program Console output, Windows API behavior, linking, or stack behavior. `INCLUDE Macros.inc` remains a virtual no-op and does not populate the Irvine32 registry. Other include files report `unsupported-include`.
