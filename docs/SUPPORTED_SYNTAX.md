...
- Milestone 53A clarifies memory validation as Level 1 region-only validation, future Level 2 section-capacity validation, future Level 3 section-image validation, and existing Level 4 declared-object validation. Default execution remains Level 1 only. Existing allocated-object warning/strict modes remain Level 4 test/configuration policies. Symbol-offset operands such as `DWORD PTR [x+1]` or `mul [x+1]` are no longer assembly errors merely because the inferred access crosses a declared object, section image, section capacity, or fixed-layout slack; final byte ranges are controlled by checked runtime memory helpers and enabled validation modes.
...
- Milestone 53A documents the active memory-validation levels: Level 1 region-only validation is always enforced and is the default; Level 2 section-capacity and Level 3 section-image validation are future Phase 53B policies; Level 4 declared-object validation is the existing allocated-object warning/strict policy. The current runtime stores `.data` and `.DATA?` together in one writable data VM region and `.CONST` in a separate read-only const VM region. The parser does not use object, section-image, or section-capacity bounds to reject otherwise valid symbol-offset memory operands; final byte-range, permission, `.CONST`, Level 4, uninitialized-read, and unaligned diagnostics are runtime concerns.
...
- Milestone 50B adds opt-in consumer-side diagnostics for using architecturally undefined modeled flags. The current implementation checks already-supported `CF` consumers (`adc`, `sbb`, and `cmc`) when the source-run/API policy is set to `warn` or `error`. Warning mode emits `undefined-flag-use` and continues with the deterministic fallback flag bit. Error mode emits `undefined-flag-use` and stops before the consumer uses the invalid flag. Default browser/source-run behavior keeps the policy off and remains compatible with earlier milestones.
...
Deferred systems include control flow, stack initialization, `push`, `pop`, `call`, `ret`, Irvine32 routines beyond the virtual `exit` terminator, debugger stepping, breakpoints, scaled-index addressing, carry rotates, signed multiplication, division, macro expansion, Windows API modeling, and full MASM expression compatibility.
