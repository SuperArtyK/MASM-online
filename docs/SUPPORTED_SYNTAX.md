...
This document describes the currently accepted MASM32 Educational Mode syntax, rejected forms, diagnostics, and future/deferred syntax. Phase 73 adds source-level `leave`/`LEAVE` frame teardown shorthand. `LEAVE` validates a DWORD read from `[EBP]` through checked VM memory before mutation, then commits `ESP = old EBP + 4` and `EBP = saved EBP`; it preserves modeled flags, creates no public `memoryChanges` rows, and preserves no-partial-mutation behavior on fatal checked-memory failures.
...
Consult the current instruction-support table in this document for implemented branch, control-flow, and stack-transfer forms. Direct near `call ProcedureName`, plain near `ret`/`RET` with no operands, the documented 32-bit source-level `push`/`pop` subset, and `leave`/`LEAVE` are implemented. Do not infer `RET imm16`, unsupported CALL forms, procedure-frame creation, Irvine32 routine dispatch, argument handling, or calling-convention behavior from future roadmap examples.
