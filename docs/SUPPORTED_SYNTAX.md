...
Phase 92 implements direct virtual Irvine32 `WriteHex` fixed-width uppercase hexadecimal output while preserving prior Irvine32 behavior and keeping `INVOKE WriteHex` and later routine families deferred.

This document describes the currently accepted MASM32 Educational Mode syntax, rejected forms, diagnostics, and future/deferred syntax. The implemented virtual Irvine32 Program Console output forms are `call Crlf`, zero-argument `INVOKE Crlf`, direct `call WriteChar`, direct `call WriteString`, direct `call WriteDec`, direct `call WriteInt`, and direct `call WriteHex` after `INCLUDE Irvine32.inc`. `INVOKE WriteChar`, `INVOKE WriteString`, `INVOKE WriteDec`, `INVOKE WriteInt`, and `INVOKE WriteHex` remain deferred exact source forms. Future Irvine32 simulator work is documented separately from permanent WinAPI, PE/linker, host, native-process, Windows-process, and full-x86 product non-goals.

Current direct control-transfer support includes direct `jmp label`, equality conditional jumps, signed relational conditional jumps, unsigned relational conditional jumps, direct near user-procedure `call ProcedureName`, virtual Irvine32 `call Crlf`, virtual Irvine32 `call WriteChar`, virtual Irvine32 `call WriteString`, virtual Irvine32 `call WriteDec`, virtual Irvine32 `call WriteInt`, virtual Irvine32 `call WriteHex`, plain near `ret`/`RET` with no operands, and near `ret imm16`/`RET imm16`.

### Current Irvine32 Program Console output

`INCLUDE Irvine32.inc` enables the implemented virtual Irvine32 `Crlf`, `WriteChar`, `WriteString`, `WriteDec`, `WriteInt`, and `WriteHex` routines. Accepted executable forms are `call Crlf`, zero-argument `INVOKE Crlf`, direct `call WriteChar`, direct `call WriteString`, direct `call WriteDec`, direct `call WriteInt`, and direct `call WriteHex`, matched case-insensitively even when `OPTION CASEMAP:NONE` is active. `Crlf` appends exactly one internal line-feed byte (`\n`) to Program Console. It does not append `\r\n`, does not read or modify registers, does not modify modeled flags or flag-validity metadata, does not read memory, and does not write memory. If appending the newline would exceed the Program Console byte or line limit, execution stops with `console-output-limit-exceeded` and appends no partial output.
...
Still-deferred Irvine32 routines continue to use unsupported-feature diagnostics. For example, `CALL WriteBin` remains `unsupported-irvine32-routine`, while `INVOKE WriteHex`, `INVOKE WriteInt`, `INVOKE WriteDec`, and `INVOKE WriteString` remain `unsupported-irvine-invoke` until later accepted phases implement those exact source forms.
...
Current public MASM source supports virtual Irvine32 `Crlf`, direct virtual Irvine32 `WriteChar`, direct virtual Irvine32 `WriteString`, direct virtual Irvine32 `WriteDec`, direct virtual Irvine32 `WriteInt`, and direct virtual Irvine32 `WriteHex` after `INCLUDE Irvine32.inc`.
...
Zero-argument `INVOKE Crlf` is implemented. `INVOKE WriteChar`, `INVOKE WriteString`, `INVOKE WriteDec`, `INVOKE WriteInt`, and `INVOKE WriteHex` remain unimplemented exact source forms and report `unsupported-irvine-invoke` unless a later accepted phase implements those exact forms. Direct Irvine32 output routines beyond the implemented `Crlf`, `WriteChar`, `WriteString`, `WriteDec`, `WriteInt`, and `WriteHex` forms, including `WriteBin`, remain deferred until their owning phases implement the exact routine and exact source form.
...
Deferred simulator features include `loop`; indirect, register-target, memory-target, immediate-target, and branch-distance/type-overridden jumps; unsupported stack-transfer widths; remaining simulator-owned CALL/RET behavior explicitly assigned to later phases; additional stack-frame behavior beyond accepted automatic `LOCAL` frames; Irvine32 routines beyond virtual `exit`, virtual `Crlf`, direct `WriteChar`, direct `WriteString`, direct `WriteDec`, direct `WriteInt`, and direct `WriteHex`; unimplemented Irvine32 `INVOKE` forms other than zero-argument `INVOKE Crlf`; debugger stepping; breakpoints; scaled-index addressing; carry rotates; selected macro-compatibility conveniences explicitly assigned to later accepted milestones; and fuller MASM expression compatibility beyond the currently implemented subset.
