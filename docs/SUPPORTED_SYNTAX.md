...
Phase 90 implements direct virtual Irvine32 `WriteDec` while preserving prior Irvine32 behavior and keeping `INVOKE WriteDec` and later routine families deferred.

This document describes the currently accepted MASM32 Educational Mode syntax, rejected forms, diagnostics, and future/deferred syntax. The implemented virtual Irvine32 Program Console output forms are `call Crlf`, zero-argument `INVOKE Crlf`, direct `call WriteChar`, direct `call WriteString`, and direct `call WriteDec` after `INCLUDE Irvine32.inc`. `INVOKE WriteChar`, `INVOKE WriteString`, and `INVOKE WriteDec` remain deferred exact source forms. Future Irvine32 simulator work is documented separately from permanent WinAPI, PE/linker, host, native-process, Windows-process, and full-x86 product non-goals.

Current direct control-transfer support includes direct `jmp label`, equality conditional jumps, signed relational conditional jumps, unsigned relational conditional jumps, direct near user-procedure `call ProcedureName`, virtual Irvine32 `call Crlf`, virtual Irvine32 `call WriteChar`, virtual Irvine32 `call WriteString`, virtual Irvine32 `call WriteDec`, plain near `ret`/`RET` with no operands, and near `ret imm16`/`RET imm16`.

### Current Irvine32 Program Console output

`INCLUDE Irvine32.inc` enables the implemented virtual Irvine32 `Crlf`, `WriteChar`, `WriteString`, and `WriteDec` routines. Accepted executable forms are `call Crlf`, zero-argument `INVOKE Crlf`, direct `call WriteChar`, direct `call WriteString`, and direct `call WriteDec`, matched case-insensitively even when `OPTION CASEMAP:NONE` is active. `Crlf` appends exactly one internal line-feed byte (`\n`) to Program Console. It does not append `\r\n`, does not read or modify registers, does not modify modeled flags or flag-validity metadata, does not read memory, and does not write memory. If appending the newline would exceed the Program Console byte or line limit, execution stops with `console-output-limit-exceeded` and appends no partial output.
...
Invalid source forms for implemented `Crlf`, `WriteChar`, `WriteString`, and `WriteDec` are `assembly-error` diagnostics, not `unsupported-feature` diagnostics. `CALL Crlf`, `CALL WriteChar`, `CALL WriteString`, and `CALL WriteDec` without `INCLUDE Irvine32.inc` report `missing-irvine32-include`. `INVOKE Crlf` without `INCLUDE Irvine32.inc` reports `missing-irvine32-include`. `INVOKE Crlf` with one or more arguments after `INCLUDE Irvine32.inc` reports `invalid-irvine32-argument-count`. Bare `Crlf` after `INCLUDE Irvine32.inc` reports `invalid-irvine32-call-form`. Bare `WriteChar` after `INCLUDE Irvine32.inc` reports `invalid-irvine32-call-form`. Bare `WriteString` after `INCLUDE Irvine32.inc` reports `invalid-irvine32-call-form`. Bare `WriteDec` after `INCLUDE Irvine32.inc` reports `invalid-irvine32-call-form`. These diagnostics refuse execution, write no Program Console output, and preserve source line, column, byte offset, and span length.

Other Irvine32 numeric output routines, including `WriteInt`, `WriteHex`, and `WriteBin`, remain recognized but deferred. `INVOKE WriteChar`, `INVOKE WriteString`, and `INVOKE WriteDec` remain future-owned Irvine32 INVOKE routine dispatch and report `unsupported-irvine-invoke`. Irvine32 input, debug, random, console-control, and selected virtual macro behavior remain future-owned only where the canonical guide assigns explicit phases. Irvine32 file routines remain unsupported in v1 unless a later reviewed virtual-filesystem phase deliberately changes that boundary. WinAPI execution, PE loading/linking, host I/O, host filesystem access, external libraries, native process behavior, and full x86 emulation are non-goals, not ordinary deferred simulator features.
...
Still-deferred Irvine32 routines continue to use unsupported-feature diagnostics. For example, `CALL WriteInt` remains `unsupported-irvine32-routine`, while `INVOKE WriteDec` and `INVOKE WriteString` remain `unsupported-irvine-invoke` until later accepted phases implement those exact source forms.
...
Current public MASM source supports virtual Irvine32 `Crlf`, direct virtual Irvine32 `WriteChar`, direct virtual Irvine32 `WriteString`, and direct virtual Irvine32 `WriteDec` after `INCLUDE Irvine32.inc`.
...
Zero-argument `INVOKE Crlf` is implemented. `INVOKE WriteChar` and `INVOKE WriteString` remain unimplemented exact source forms and report `unsupported-irvine-invoke` unless a later accepted phase implements those forms. Other Irvine32 output routines remain deferred until their owning phases implement the exact routine and exact source form.
...
Deferred simulator features include `loop`; indirect, register-target, memory-target, immediate-target, and branch-distance/type-overridden jumps; unsupported stack-transfer widths; remaining simulator-owned CALL/RET behavior explicitly assigned to later phases; additional stack-frame behavior beyond accepted automatic `LOCAL` frames; Irvine32 routines beyond virtual `exit`, virtual `Crlf`, direct `WriteChar`, and direct `WriteString`; unimplemented Irvine32 `INVOKE` forms other than zero-argument `INVOKE Crlf`; debugger stepping; breakpoints; scaled-index addressing; carry rotates; selected macro-compatibility conveniences explicitly assigned to later accepted milestones; and fuller MASM expression compatibility beyond the currently implemented subset.
