...
`imul` supports the one-operand signed accumulator form with one register or unambiguous memory source at 8-bit, 16-bit, or 32-bit width. `imul r/m8` multiplies signed `AL` by the signed source and stores the 16-bit product in `AX`; `imul r/m16` multiplies signed `AX` by the signed source and stores the 32-bit product in `DX:AX`; `imul r/m32` multiplies signed `EAX` by the signed source and stores the 64-bit product in `EDX:EAX`. `CF` and `OF` are clear when the full signed product is exactly the sign extension of the lower result half and set otherwise. `ZF` and `SF` are preserved deterministically. Memory sources use the same direct-symbol, symbol-offset, unsigned `PTR`, and signed `PTR` width rules as `mul`; memory reads go through checked VM helpers and do not create memory-change rows. Untyped register-indirect forms such as `imul [eax]` and immediate sources such as `imul 5` are rejected. Two- and three-operand forms such as `imul eax, ebx` and `imul eax, ebx, 5` are deferred to Phase 55 - Two- and Three-Operand IMUL. Executable QWORD/SQWORD memory operations remain deferred.
...
Deferred systems include control flow, stack initialization, `push`, `pop`, `call`, `ret`, Irvine32 routines beyond the virtual `exit` terminator, debugger stepping, breakpoints, scaled-index addressing, carry rotates, two- and three-operand signed multiplication, division, macro expansion, Windows API modeling, and full MASM expression compatibility.
