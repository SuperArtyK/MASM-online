...
Phase 89A updates active documentation and static validation only; accepted and rejected MASM source behavior remains the Phase 89 Irvine32 WriteString subset.

This document describes the currently accepted MASM32 Educational Mode syntax, rejected forms, diagnostics, and future/deferred syntax. The implemented virtual Irvine32 Program Console output forms are `call Crlf`, zero-argument `INVOKE Crlf`, direct `call WriteChar`, and direct `call WriteString` after `INCLUDE Irvine32.inc`. `INVOKE WriteChar` and `INVOKE WriteString` remain deferred exact source forms. Future Irvine32 simulator work is documented separately from permanent WinAPI, PE/linker, host, native-process, Windows-process, and full-x86 product non-goals.

Current direct control-transfer support includes direct `jmp label`, equality conditional jumps, signed relational conditional jumps, unsigned relational conditional jumps, direct near user-procedure `call ProcedureName`, virtual Irvine32 `call Crlf`, virtual Irvine32 `call WriteChar`, virtual Irvine32 `call WriteString`, plain near `ret`/`RET` with no operands, and near `ret imm16`/`RET imm16`.

### Current Irvine32 Program Console output

`INCLUDE Irvine32.inc` enables the implemented virtual Irvine32 `Crlf`, `WriteChar`, and `WriteString` routines. Accepted executable forms are `call Crlf`, zero-argument `INVOKE Crlf`, direct `call WriteChar`, and direct `call WriteString`, matched case-insensitively even when `OPTION CASEMAP:NONE` is active. `Crlf` appends exactly one internal line-feed byte (`\n`) to Program Console. It does not append `\r\n`, does not read or modify registers, does not modify modeled flags or flag-validity metadata, does not read memory, and does not write memory. If appending the newline would exceed the Program Console byte or line limit, execution stops with `console-output-limit-exceeded` and appends no partial output.
...
Invalid source forms for implemented `Crlf`, `WriteChar`, and `WriteString` are `assembly-error` diagnostics, not `unsupported-feature` diagnostics. `CALL Crlf`, `CALL WriteChar`, and `CALL WriteString` without `INCLUDE Irvine32.inc` report `missing-irvine32-include`. `INVOKE Crlf` without `INCLUDE Irvine32.inc` reports `missing-irvine32-include`. `INVOKE Crlf` with one or more arguments after `INCLUDE Irvine32.inc` reports `invalid-irvine32-argument-count`. Bare `Crlf` after `INCLUDE Irvine32.inc` reports `invalid-irvine32-call-form`. Bare `WriteChar` after `INCLUDE Irvine32.inc` reports `invalid-irvine32-call-form`. Bare `WriteString` after `INCLUDE Irvine32.inc` reports `invalid-irvine32-call-form`. These diagnostics refuse execution, write no Program Console output, and preserve source line, column, byte offset, and span length.

Other Irvine32 output routines, including `WriteInt`, `WriteDec`, `WriteHex`, and `WriteBin`, remain recognized but deferred. `INVOKE WriteChar` and `INVOKE WriteString` remain future-owned Irvine32 INVOKE routine dispatch and report `unsupported-irvine-invoke`. Irvine32 input, debug, random, console-control, and selected virtual macro behavior remain future-owned only where the canonical guide assigns explicit phases. Irvine32 file routines remain unsupported in v1 unless a later reviewed virtual-filesystem phase deliberately changes that boundary. WinAPI execution, PE loading/linking, host I/O, host filesystem access, external libraries, native process behavior, and full x86 emulation are non-goals, not ordinary deferred simulator features.
...
Still-deferred Irvine32 routines continue to use unsupported-feature diagnostics. For example, `CALL WriteDec` remains `unsupported-irvine32-routine`, and `INVOKE WriteString` remains `unsupported-irvine-invoke`, until a later accepted phase implements the specific routine and source form.
...
Current public MASM source supports virtual Irvine32 `Crlf`, direct virtual Irvine32 `WriteChar`, and direct virtual Irvine32 `WriteString` after `INCLUDE Irvine32.inc`.
...
- The virtual Irvine32 registry recognizes known routine names for classification and diagnostics, including CALL-target and INVOKE-target classification. The zero-operand virtual `exit` terminator, virtual Irvine32 `Crlf`, direct virtual Irvine32 `WriteChar`, and direct virtual Irvine32 `WriteString` are executable in the current subset after `INCLUDE Irvine32.inc`. Direct user-procedure `CALL` is executable only for user `PROC` entries. Zero-argument same-file user-procedure `INVOKE` and the documented Phase 84 same-file user-procedure DWORD argument subset are executable through their own procedure-call rules. Irvine32 routine calls beyond virtual `Crlf`, direct `WriteChar`, and direct `WriteString` remain deferred according to the future-feature sections in this document. Irvine32 `INVOKE` dispatch beyond zero-argument `INVOKE Crlf` remains deferred. Additional procedure-frame and calling-convention behavior beyond accepted automatic LOCAL frames, supported LOCAL operands, `PROC USES` preservation, direct user-procedure `CALL`/`RET`, Phase 82 zero-argument user-procedure `INVOKE`, Phase 83 helper-level ADDR preparation, and Phase 84 limited user-procedure INVOKE DWORD arguments also remains deferred unless a later accepted phase explicitly owns it. Near `RET imm16` is implemented only as explicit helper return cleanup; it does not imply Irvine32 routine calls or procedure metadata. Windows/API execution remains outside the simulator boundary as a permanent non-goal unless the canonical specification and guide are deliberately revised.
