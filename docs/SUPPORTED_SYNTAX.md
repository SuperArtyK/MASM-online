...
### Phase 57K `.code` and MASM segment/group symbol policy

Phase 57K locks the v1 policy for `.code` memory access without adding runtime/source-run diagnostics. The simulator executes internal IR and source metadata. It does not expose `.code` as user-readable or user-writable simulated program memory, does not emit real x86 opcode bytes, and does not model a PE `.text` image. The current fixed-layout low-level `.code` region starts at `00400000h`, has `00100000h` bytes of capacity, carries read and execute permissions but not write permission, and has zero-initialized backing storage. Low-level reads therefore currently return zero bytes as an implementation artifact, not as a supported source-level code image. Low-level writes are already denied through ordinary region permissions rather than through a dedicated `.code` policy diagnostic. Existing source forms can currently reach `.code` addresses through register-indirect or displacement memory operands after loading a `.code` address into a register. Phase 57L owns the later `unsupported-code-memory-access` runtime diagnostic for final memory ranges that overlap `.code`.

Phase 57K also locks the MASM segment/group symbol policy for later parser diagnostics. `_TEXT`, `_DATA`, `_BSS`, `CONST`, `STACK`, `DGROUP`, and `FLAT` are MASM/object/linker concepts, not aliases for simulator regions. They must not be used to access `.code`, `.data`, `.DATA?`, `.CONST`, stack, heap, or any internal VM region. Users should declare ordinary data labels and use `OFFSET label` for simulator data addresses. Phase 57M - MASM Segment and Group Symbol Diagnostics owns targeted `unsupported-segment-symbol` diagnostics for these names.
