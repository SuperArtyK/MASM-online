...
Phase 57A is documentation and repository-hygiene work only. It keeps runtime/source-run MASM behavior metadata at Phase 57 - Signed IDIV. Phase 57-CORR2 remains the accepted compact negative register-indirect displacement parser correction, and Phase 57-CORR1 remains the accepted diagnostic-context clarification for fatal `region-boundary-crossing` Simulator Messages on cross-region reads and writes whose requested final byte range intersects protected `.CONST` storage.
