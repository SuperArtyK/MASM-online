...
This document describes the currently accepted MASM32 Educational Mode syntax, rejected forms, diagnostics, and future/deferred syntax. Phase 83 prepares limited helper-level `ADDR symbol` records for future `INVOKE` argument lowering while preserving the Phase 82 executable zero-argument `INVOKE Helper` / `invoke Helper` form for same-file user procedures that require zero arguments. Phase 81 limited parser-owned `PROTO` metadata for same-file educational procedure prototypes, Phase 80 source-level LOCAL operand resolution/addressing, Phase 79 automatic LOCAL frames, Phase 78A limited `OPTION NOKEYWORD` forms, Phase 78 LOCAL parser metadata, Phase 77 direct-CALL `PROC USES` runtime save/restore, Phase 76 `PROC USES` parsing metadata, and Phase 75 targeted `PROC` diagnostics remain implemented.
...
In the current repository state, source-level executable `INVOKE` is accepted only in the zero-argument same-file user-procedure form introduced in Phase 82: `INVOKE Helper` or `invoke Helper`, where `Helper` resolves to a user `PROC` that requires zero arguments under current metadata. Accepted zero-argument `INVOKE` lowers to the same checked runtime behavior as direct user-procedure `call Helper`.

All source-level `INVOKE` forms with arguments remain unsupported. Source-level executable `ADDR` remains unsupported as a standalone instruction, as a general operand, and as a source-level INVOKE argument. Phase 83 adds parser/helper preparation only: a helper-level `ADDR symbol` record can resolve data, `.DATA?`, `.CONST`, and explicitly active-frame LOCAL metadata for future INVOKE argument lowering, preserving a 32-bit address plus ADDR-token and operand-token source spans. That helper does not read pointed-to memory, write pointed-to memory, push arguments, clean arguments, access runtime parameters, or bypass `.CONST` write protection. `INVOKE Helper, ADDR symbol` still reports `invoke-arguments-not-supported-yet` and does not execute. `PROTO` metadata remains parser-owned metadata and does not make arguments, runtime parameters, external calls, WinAPI calls, Irvine32 routine calls, C runtime calls, MASM32 runtime calls, import libraries, linking, or calling conventions executable. A later INVOKE-argument phase may accept a limited DWORD argument subset only after the owning guide phase, tests, output contracts, and this supported-syntax file are updated.
...
Consult the current instruction-support table in this document for implemented branch, control-flow, stack-transfer, LOCAL operand, limited `PROTO` metadata, zero-argument `INVOKE` forms, and Phase 83 helper-level `ADDR symbol` record preparation. Direct near `call ProcedureName`, zero-argument `INVOKE ProcedureName` to same-file user procedures, plain near `ret`/`RET` with no operands, near `ret imm16`/`RET imm16`, the documented 32-bit source-level `push`/`pop` subset, `leave`/`LEAVE`, automatic runtime LOCAL frames for procedures with parsed `LOCAL` declarations, supported Phase 80 LOCAL operands, Phase 81 limited same-file `PROTO` metadata, and helper-level ADDR record construction are implemented. Do not infer unsupported CALL forms, executable source-level `INVOKE` arguments, `OFFSET local`, executable source-level `ADDR`, pointer or unnamed prototype parameters, scaled-index LOCAL addressing, far returns, Irvine32 routine dispatch through INVOKE, runtime argument metadata, external calls, import/linker behavior, or calling-convention behavior from future roadmap examples.

Phase 83 has completed limited helper-level `ADDR` argument preparation. The next future sequence is limited DWORD `INVOKE` argument lowering and cleanup. Until that phase is implemented and this file is updated, all source-level `INVOKE` forms with arguments and all source-level `ADDR` operands remain unsupported.

The following stack/procedure features remain future work only if a later accepted milestone explicitly implements them:

- executable `INVOKE` arguments beyond the accepted zero-argument same-file user-procedure form;
- source-level `INVOKE` with arguments;
- executable source-level `ADDR` as an instruction, general operand, or source-level INVOKE argument;
- `OFFSET local` and executable `ADDR local` source-level argument behavior;
- scaled-index LOCAL addressing and QWORD/SQWORD executable LOCAL memory access;
- automatic procedure-frame features beyond Phase 83 helper-level ADDR preparation, Phase 82 zero-argument INVOKE, Phase 81 PROTO metadata, Phase 80 LOCAL operand access, and already implemented `PROC USES` preservation frames;
...
Phase 83 `INVOKE` and ADDR diagnostics: accepted `INVOKE Helper` / `invoke Helper` to same-file zero-argument user procedures lowers to direct-CALL behavior. Source-level `INVOKE` with arguments reports `invoke-arguments-not-supported-yet` unless a more specific target non-goal applies. Wrong-kind, unknown, malformed, data-symbol, label, equate, register, memory, immediate, or other non-procedure INVOKE targets report `invalid-invoke-target`. External/API targets such as `ExitProcess`, C runtime targets such as `crt_printf`, and MASM32 runtime targets such as `StdOut` report `unsupported-external-invoke`. Irvine32 routine targets such as `WriteString` report `unsupported-irvine-invoke`. Standalone/general source-level `ADDR` reports `addr-outside-invoke`. Helper-level wrong-kind ADDR targets such as registers, immediates, numeric equates, and memory expressions report `invalid-addr-target`; helper-level unknown addressable symbols report `unknown-addr-symbol`; helper-level computed ADDR expressions such as `ADDR symbol + 4` report `unsupported-addr-expression`. These diagnostics are emitted through Simulator Messages, refuse execution, and do not write to Program Console. Phase 83 prepares helper-level address argument records only; a later INVOKE-argument phase must update this section before any source-level `INVOKE target, ADDR symbol` form is documented as accepted.
