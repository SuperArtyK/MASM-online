...
This document describes the currently accepted MASM32 Educational Mode syntax, rejected forms, diagnostics, and future/deferred syntax. Phase 76 accepts `PROC USES` parsing metadata for `EAX`, `EBX`, `ECX`, `EDX`, `ESI`, and `EDI`, preserves the declared register order on procedure records, rejects invalid `USES` lists with targeted diagnostics, and stops source-run with `unsupported-proc-uses-runtime` when execution enters or calls a procedure because `USES` save/restore behavior begins in Phase 77. Phase 75 targeted diagnostics for unsupported non-USES `PROC` attributes or parameters, malformed `PROC` declarations, mismatched `ENDP` names, and duplicate procedures remain implemented.
...
Accepted `USES` register names are `EAX`, `EBX`, `ECX`, `EDX`, `ESI`, and `EDI`, case-insensitively. The parser stores canonical register identities in declared order. Runtime save/restore is not implemented until Phase 77; source-run reports `unsupported-proc-uses-runtime` before execution enters a `USES` procedure, including selected-entry startup, direct `CALL`, direct branch transfer, or ordinary fallthrough.
...
- Direct near `CALL` to accepted user procedures is implemented for the supported direct form. Direct `call ProcedureName` is executable only when `ProcedureName` resolves to a user `PROC` entry under the active user-symbol `CASEMAP` policy. A direct `CALL` to a procedure with `PROC USES` metadata reports `unsupported-proc-uses-runtime` before stack mutation because register save/restore begins in Phase 77. Otherwise, a successful direct user-procedure `CALL` writes a pseudo-EIP return token to `ESP - 4`, updates `ESP`, and transfers to the target procedure entry through the checked VM control-flow path. The current public source-run output contract does not expose that implicit write as a user-visible `memoryChanges` row.
...
Consult the current instruction-support table in this document for implemented branch, control-flow, and stack-transfer forms. Direct near `call ProcedureName`, plain near `ret`/`RET` with no operands, near `ret imm16`/`RET imm16`, the documented 32-bit source-level `push`/`pop` subset, and `leave`/`LEAVE` are implemented. Do not infer unsupported CALL forms, far returns, procedure-frame creation, Irvine32 routine dispatch, argument metadata, or calling-convention behavior from future roadmap examples.

The following stack/procedure features remain future work only if a later accepted milestone explicitly implements them:

- `PROC USES` runtime save/restore;
- `LOCAL`;
- `PROTO`;
- `INVOKE`;
- `ADDR`;
...
- The virtual Irvine32 registry recognizes known routine names for classification and diagnostics, including CALL-target classification. Only the zero-operand virtual `exit` terminator is executable as an Irvine32 behavior in the current subset; direct user-procedure `CALL` is executable only for user `PROC` entries. Calls to Irvine32 routines and procedure frames remain deferred according to the future-feature sections in this document. Near `RET imm16` is implemented only as explicit helper return cleanup; it does not imply Irvine32 routine calls or procedure metadata. Windows/API execution remains outside the simulator boundary as a permanent non-goal unless the canonical specification and guide are deliberately revised.
