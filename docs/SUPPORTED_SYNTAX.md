...
Phase 93 implements direct virtual Irvine32 `WriteBin` fixed-width 32-bit binary output while preserving prior Irvine32 behavior and keeping `INVOKE WriteBin` and later routine families deferred.

This document describes the currently accepted MASM32 Educational Mode syntax, rejected forms, diagnostics, and future/deferred syntax. The implemented virtual Irvine32 Program Console output forms are `call Crlf`, zero-argument `INVOKE Crlf`, direct `call WriteChar`, direct `call WriteString`, direct `call WriteDec`, direct `call WriteInt`, direct `call WriteHex`, and direct `call WriteBin` after `INCLUDE Irvine32.inc`. `INVOKE WriteChar`, `INVOKE WriteString`, `INVOKE WriteDec`, `INVOKE WriteInt`, `INVOKE WriteHex`, and `INVOKE WriteBin` remain deferred exact source forms. Future Irvine32 simulator work is documented separately from permanent WinAPI, PE/linker, host, native-process, Windows-process, and full-x86 product non-goals.

Current direct control-transfer support includes direct `jmp label`, equality conditional jumps, signed relational conditional jumps, unsigned relational conditional jumps, direct near user-procedure `call ProcedureName`, virtual Irvine32 `call Crlf`, virtual Irvine32 `call WriteChar`, virtual Irvine32 `call WriteString`, virtual Irvine32 `call WriteDec`, virtual Irvine32 `call WriteInt`, virtual Irvine32 `call WriteHex`, virtual Irvine32 `call WriteBin`, plain near `ret`/`RET` with no operands, and near `ret imm16`/`RET imm16`.

### Current Irvine32 Program Console output

`INCLUDE Irvine32.inc` enables the implemented virtual Irvine32 `Crlf`, `WriteChar`, `WriteString`, `WriteDec`, `WriteInt`, `WriteHex`, and `WriteBin` routines. Accepted executable forms are `call Crlf`, zero-argument `INVOKE Crlf`, direct `call WriteChar`, direct `call WriteString`, direct `call WriteDec`, direct `call WriteInt`, direct `call WriteHex`, and direct `call WriteBin`, matched case-insensitively even when `OPTION CASEMAP:NONE` is active. `Crlf` appends exactly one internal line-feed byte (`\n`) to Program Console. It does not append `\r\n`, does not read or modify registers, does not modify modeled flags or flag-validity metadata, does not read memory, and does not write memory. If appending the newline would exceed the Program Console byte or line limit, execution stops with `console-output-limit-exceeded` and appends no partial output.

`WriteChar` appends exactly one Program Console byte copied from the low byte of `AL`. High bytes of `EAX` do not affect the emitted byte. `WriteChar` reads no simulated memory, writes no simulated memory, preserves all registers, preserves modeled flags and flag-validity metadata, and uses the same Program Console byte/line limit path as `Crlf`. If appending the byte would exceed the Program Console byte or line limit, execution stops with `console-output-limit-exceeded` and appends no partial output.

`WriteString` reads `EDX` as a simulated VM address of a null-terminated byte string. It reads bytes one at a time through checked VM memory helpers, stops at the first `00h` byte, and appends only the nonzero bytes before the terminator to Program Console. The terminator is read but not printed. `WriteString` preserves all registers including `EDX`, preserves modeled flags and flag-validity metadata, writes no simulated memory, creates no memory-change rows, and commits no candidate bytes from a failing call. Fatal checked-memory failures, strict uninitialized-read diagnostics, Program Console byte/line limit failures, and `string-scan-limit-exceeded` failures stop before candidate output from the current `WriteString` call is appended.

`WriteDec` reads `EAX` as an unsigned 32-bit value and appends its shortest base-10 representation to Program Console. The range is `0` through `4294967295`; output has no sign, leading zeroes except the single digit `0`, grouping separators, whitespace, prefix, suffix, carriage return, or line feed. `WriteDec` reads no simulated memory, writes no simulated memory, preserves all registers including `EAX`, preserves modeled flags and flag-validity metadata, and creates no memory-change rows. The complete decimal candidate is preflighted through the shared Program Console output-limit path, so an over-limit call appends no partial digits and preserves previously committed console text.

`WriteInt` reads the current 32-bit bit pattern of `EAX` as a signed two's-complement value and appends its shortest base-10 representation to Program Console. The range is `-2147483648` through `2147483647`; negative values have exactly one leading `-`, nonnegative values have no `+`, and output has no leading zeroes except the single digit `0`, grouping separators, whitespace, prefix, suffix, carriage return, or line feed. `WriteInt` reads no simulated memory, writes no simulated memory, preserves all registers including `EAX`, preserves modeled flags and flag-validity metadata, and creates no memory-change rows. The complete signed candidate, including the minus sign when present, is preflighted through the shared Program Console output-limit path, so an over-limit call appends no partial sign or digits and preserves previously committed console text.

`WriteHex` reads the current 32-bit bit pattern of `EAX` as an unsigned value and appends exactly eight uppercase hexadecimal digits to Program Console. Leading zeroes are retained to reach eight digits; output has no `0x` prefix, `h` suffix, grouping separators, whitespace, carriage return, or line feed. `WriteHex` reads no simulated memory, writes no simulated memory, preserves all registers including `EAX`, preserves modeled flags and flag-validity metadata, and creates no memory-change rows. The complete eight-byte candidate is preflighted through the shared Program Console output-limit path, so an over-limit call appends no partial digits and preserves previously committed console text.

`WriteBin` reads the current 32-bit bit pattern of `EAX` as an unsigned value and appends exactly 32 binary digits to Program Console, most-significant bit first. Leading zeroes are retained to reach 32 digits; output uses only `0` and `1` and has no prefix, `b` suffix, grouping separators, spaces, whitespace, carriage return, or line feed. `WriteBin` reads no simulated memory, writes no simulated memory, preserves all registers including `EAX`, preserves modeled flags and flag-validity metadata, and creates no memory-change rows. The complete 32-byte candidate is preflighted through the shared Program Console output-limit path, so an over-limit call appends no partial digits and preserves previously committed console text.

Invalid source forms for implemented `Crlf`, `WriteChar`, `WriteString`, `WriteDec`, `WriteInt`, `WriteHex`, and `WriteBin` are `assembly-error` diagnostics, not `unsupported-feature` diagnostics. `CALL Crlf`, `CALL WriteChar`, `CALL WriteString`, `CALL WriteDec`, `CALL WriteInt`, `CALL WriteHex`, and `CALL WriteBin` without `INCLUDE Irvine32.inc` report `missing-irvine32-include`. `INVOKE Crlf` without `INCLUDE Irvine32.inc` reports `missing-irvine32-include`. `INVOKE Crlf` with one or more arguments after `INCLUDE Irvine32.inc` reports `invalid-irvine32-argument-count`. Bare `Crlf` after `INCLUDE Irvine32.inc` reports `invalid-irvine32-call-form`. Bare `WriteChar` after `INCLUDE Irvine32.inc` reports `invalid-irvine32-call-form`. Bare `WriteString` after `INCLUDE Irvine32.inc` reports `invalid-irvine32-call-form`. Bare `WriteDec` after `INCLUDE Irvine32.inc` reports `invalid-irvine32-call-form`. Bare `WriteInt` after `INCLUDE Irvine32.inc` reports `invalid-irvine32-call-form`. Bare `WriteHex` after `INCLUDE Irvine32.inc` reports `invalid-irvine32-call-form`. Bare `WriteBin` after `INCLUDE Irvine32.inc` reports `invalid-irvine32-call-form`. These diagnostics refuse execution, write no Program Console output, and preserve source line, column, byte offset, and span length.

Other Irvine32 routines, including `DumpRegs` and `DumpMem`, remain recognized but deferred. `INVOKE WriteChar`, `INVOKE WriteString`, `INVOKE WriteDec`, `INVOKE WriteInt`, `INVOKE WriteHex`, and `INVOKE WriteBin` remain future-owned Irvine32 INVOKE routine dispatch and report `unsupported-irvine-invoke`. Irvine32 input, debug, random, console-control, and selected virtual macro behavior remain future-owned only where the canonical guide assigns explicit phases. Irvine32 file routines remain unsupported in v1 unless a later reviewed virtual-filesystem phase deliberately changes that boundary. WinAPI execution, PE loading/linking, host I/O, host filesystem access, external libraries, native process behavior, and full x86 emulation are non-goals, not ordinary deferred simulator features.

### Current INVOKE and ADDR status

In the current repository state, source-level executable `INVOKE` is accepted for same-file user procedures in the zero-argument form introduced in Phase 82 and in the limited Phase 84 DWORD argument subset. Accepted argument forms are 32-bit immediates or constant expressions, full 32-bit registers, `DWORD`/`SDWORD` data-symbol values, `OFFSET` data/`.DATA?`/`.CONST` symbols, and `ADDR` data/`.DATA?`/`.CONST`/active `LOCAL` symbols. Accepted argument `ADDR` passes the address value, not the pointed-to value, and does not weaken `.CONST` write protection. Accepted argument pushes are lowered to checked DWORD stack writes from right to left before transferring to the target procedure.

Accepted nonzero-argument `INVOKE` requires metadata proving exact callee cleanup with `ret imm16` equal to four bytes per accepted argument. A plain `ret`, inconsistent `ret imm16` metadata, or mismatched cleanup byte count reports `invoke-cleanup-mismatch` before execution. A `PROTO` argument-count mismatch reports `invoke-argument-count-mismatch`. Full 32-bit register arguments are accepted for the documented same-file user-procedure subset; 8-bit and 16-bit register aliases report `invoke-argument-width-unsupported`. Unsupported same-file user-procedure argument shapes report `unsupported-invoke-argument` or a more specific ADDR diagnostic such as `invalid-addr-target`. External/API targets report `unsupported-external-invoke`. Among Irvine32 routines, only zero-argument `INVOKE Crlf` is currently executable through `INVOKE`; direct `call WriteChar`, direct `call WriteString`, direct `call WriteDec`, direct `call WriteInt`, direct `call WriteHex`, and direct `call WriteBin` are executable, but `INVOKE WriteChar`, `INVOKE WriteString`, `INVOKE WriteDec`, `INVOKE WriteInt`, `INVOKE WriteHex`, and `INVOKE WriteBin` remain rejected with `unsupported-irvine-invoke`. `INVOKE Crlf` without `INCLUDE Irvine32.inc` reports `missing-irvine32-include`. `INVOKE Crlf` with one or more arguments reports `invalid-irvine32-argument-count` because `Crlf` is implemented but takes no arguments. Other Irvine32 routine targets remain rejected with `unsupported-irvine-invoke`. Source-level executable `ADDR` remains unsupported as a standalone instruction or general operand outside accepted Phase 84 `INVOKE` argument forms. `PROTO` metadata does not make named runtime parameters, external calls, WinAPI calls, non-Crlf Irvine32 `INVOKE` routine calls, C runtime calls, MASM32 runtime calls, import libraries, linking, or full calling conventions executable.


### Implemented Crlf diagnostic reference

The following diagnostics are specific to implemented virtual Irvine32 `Crlf`. They describe invalid source forms, so their category is `assembly-error`.

| Source form | Category | Code | Rendered message text |
|---|---|---|---|
| `CALL Crlf` without `INCLUDE Irvine32.inc` | `assembly-error` | `missing-irvine32-include` | `CALL Crlf requires INCLUDE Irvine32.inc before Crlf can be used as a virtual Irvine32 routine.` |
| `INVOKE Crlf` without `INCLUDE Irvine32.inc` | `assembly-error` | `missing-irvine32-include` | `INVOKE Crlf requires INCLUDE Irvine32.inc before Crlf can be used as a virtual Irvine32 routine.` |
| `INVOKE Crlf, ...` after `INCLUDE Irvine32.inc` | `assembly-error` | `invalid-irvine32-argument-count` | `INVOKE Crlf takes no arguments. Remove the argument list or use CALL Crlf.` |
| Bare `Crlf` after `INCLUDE Irvine32.inc` | `assembly-error` | `invalid-irvine32-call-form` | `Crlf is a virtual Irvine32 routine and must be called with CALL Crlf or zero-argument INVOKE Crlf.` |

These diagnostics are intentionally not `unsupported-feature`. `Crlf` is implemented; the user's source form is invalid or incomplete.

Expected source-span targets:

- `CALL Crlf` without include points at the `Crlf` target token.
- `INVOKE Crlf` without include points at the `Crlf` target token.
- `INVOKE Crlf, ...` points at the first offending argument when available.
- Bare `Crlf` points at the bare `Crlf` token.

Still-deferred Irvine32 routines continue to use unsupported-feature diagnostics. For example, `CALL DumpRegs` remains `unsupported-irvine32-routine`, while `INVOKE WriteBin`, `INVOKE WriteHex`, `INVOKE WriteInt`, `INVOKE WriteDec`, and `INVOKE WriteString` remain `unsupported-irvine-invoke` until later accepted phases implement those exact source forms.
...
Phase 81 stores parser-owned prototype metadata separately from data symbols, labels, procedures, equates, and LOCAL declarations. Accepted prototypes preserve prototype name spelling, parameter names, parameter type identities, declaration order, parameter count, and source locations/spans for prototype and parameter tokens. Accepted `PROTO` declarations emit no executable IR, do not allocate stack frames, do not make `ADDR` executable, and do not model calling conventions, external linkage, imports, WinAPI behavior, Irvine32 routine dispatch, C runtime calls, MASM32 runtime calls, or runtime parameters. Phase 82 may use compatible zero-argument `PROTO` metadata to permit zero-argument same-file user-procedure `INVOKE`; Phase 84 may use named `DWORD`/`SDWORD` prototype metadata for INVOKE argument-count validation. PROTO metadata still does not implement named runtime parameter access or general calling-convention behavior. The first PROTO implementation accepts only named `DWORD` and `SDWORD` parameters.

A zero-argument `PROTO` may link to a same-name bare `PROC`. Phase 84 also allows a named `DWORD`/`SDWORD` parameterized `PROTO` to link to a same-name bare `PROC` for INVOKE argument-count validation and cleanup validation only. This compatibility does not create named PROC parameter operands, runtime parameter symbols, pointer parameters, unnamed parameters, VARARG behavior, or general calling-convention metadata.
...
Source-level 32-bit `push`, source-level 32-bit `pop`, `LEAVE`, near `RET imm16` caller-cleanup returns, `PROC USES` parsing metadata, direct-CALL `PROC USES` runtime save/restore, parser `LOCAL` declaration metadata, automatic runtime LOCAL stack allocation/lifetime, Phase 80 supported source-level LOCAL operands, Phase 81 limited `PROTO` metadata, Phase 82 zero-argument same-file user-procedure `INVOKE`, Phase 83 helper-level `ADDR symbol` preparation, and Phase 84 limited same-file user-procedure `INVOKE` DWORD argument lowering are implemented. Procedures with parsed `LOCAL` declarations receive checked automatic frames on selected-entry startup, direct user-procedure `CALL`, and accepted zero-argument user-procedure `INVOKE`. Those frames are released on root `RET`, helper `RET`, helper `RET imm16`, source-level `LEAVE` followed by `RET`, and virtual Irvine32 `exit`.

Deferred simulator behavior includes Irvine32 routine `INVOKE` dispatch beyond zero-argument `INVOKE Crlf`; `OFFSET local`; executable source-level `ADDR` outside accepted `INVOKE` argument forms; general or computed `ADDR` operands; executable `PROTO` behavior beyond Phase 84 validation; pointer or unnamed prototype parameters; `VARARG`; scaled-index LOCAL addressing; named runtime parameters; calling-convention behavior; QWORD/SQWORD executable LOCAL memory operands; and Irvine32 routine dispatch beyond implemented virtual `Crlf`, direct `WriteChar`, direct `WriteString`, direct `WriteDec`, direct `WriteInt`, direct `WriteHex`, and direct `WriteBin`. A later accepted phase must explicitly own, specify, implement, and test each additional simulator form before it becomes executable. Simulator-owned rejected CALL target forms remain rejected unless a later accepted phase explicitly changes that exact form. Rejection by itself is not a roadmap promise.

External/API execution, WinAPI execution or modeling, PE loading or linking, object-file or import-library linking, external native-library execution, host callbacks, host filesystem access by simulated programs, native process behavior, Windows process emulation, native x86 execution, and full x86 emulation remain product non-goals rather than deferred simulator features. Detailed accepted, deferred, rejected, and non-goal forms are listed in the sections below.
...
Consult the current instruction-support table in this document for implemented branch, control-flow, stack-transfer, LOCAL operand, limited `PROTO` metadata, zero-argument `INVOKE` forms, and Phase 84 limited INVOKE DWORD arguments and Phase 83 helper-level `ADDR symbol` record preparation. Direct near `call ProcedureName`, zero-argument `INVOKE ProcedureName` to same-file user procedures, plain near `ret`/`RET` with no operands, near `ret imm16`/`RET imm16`, the documented 32-bit source-level `push`/`pop` subset, `leave`/`LEAVE`, automatic runtime LOCAL frames for procedures with parsed `LOCAL` declarations, supported Phase 80 LOCAL operands, Phase 81 limited same-file `PROTO` metadata, and helper-level ADDR record construction are implemented. Do not infer unsupported CALL forms, unsupported source-level `INVOKE` argument forms beyond Phase 84 same-file user-procedure arguments and zero-argument `INVOKE Crlf`, `OFFSET local`, executable source-level `ADDR` outside accepted INVOKE arguments, pointer or unnamed prototype parameters, scaled-index LOCAL addressing, far returns, Irvine32 routine dispatch through INVOKE beyond Crlf, named runtime parameter metadata, external calls, import/linker behavior, or calling-convention behavior from future roadmap examples.

Phase 84 has completed limited DWORD `INVOKE` argument lowering and cleanup for same-file user procedures. Future phases may extend parameter access, Irvine32 routine dispatch beyond Crlf, pointer or VARARG metadata, computed ADDR expressions, or calling-convention behavior only when explicitly assigned. Source-level `ADDR` outside accepted INVOKE argument forms remains unsupported.

The following stack/procedure features remain future work only if a later accepted milestone explicitly implements them:

- executable `INVOKE` arguments beyond accepted zero-argument user-procedure forms, Phase 84 limited DWORD same-file user-procedure forms, and zero-argument `INVOKE Crlf`;
- unsupported source-level `INVOKE` argument forms, including 8-bit/16-bit registers, QWORD/SQWORD values, computed ADDR expressions, memory expressions not accepted by Phase 84, pointer metadata, VARARG, external/API targets, and Irvine32 routine dispatch beyond Crlf;
- executable source-level `ADDR` as an instruction, general operand, or source-level operand outside accepted INVOKE argument forms;
- `OFFSET local`, computed `ADDR local` expressions, and `ADDR local` use outside the accepted Phase 84 `INVOKE` argument form;
- scaled-index LOCAL addressing and QWORD/SQWORD executable LOCAL memory access;
- procedure-frame and calling-convention features beyond accepted automatic LOCAL frames, supported Phase 80 LOCAL operands, `PROC USES` preservation, direct user-procedure `CALL`/`RET` behavior, Phase 82 zero-argument INVOKE, Phase 83 helper-level ADDR preparation, and Phase 84 limited INVOKE DWORD arguments;
...
- selected Irvine32 routine calls beyond already implemented virtual `exit` terminator and virtual `Crlf` behavior;
...
Program Console output has deterministic Phase 86 byte and line-feed limits. The default source-run JSON exposes `maxBytes`, `maxLines`, `limitExceeded`, and `limitKind` beside the committed `text`, `truncated`, `byteCount`, and `lineCount` fields. Line counting is byte-oriented: committed LF bytes (`0Ah`) count as lines, CRLF byte pairs (`0Dh 0Ah`) count as one line, standalone CR bytes (`0Dh`) count as zero lines, and an unterminated final text fragment does not add a line. When the complete append would exceed either configured limit, execution stops with `console-output-limit-exceeded`, the failed append contributes no partial Program Console text, and the diagnostic is emitted through Simulator Messages rather than Program Console.

Current public MASM source supports virtual Irvine32 `Crlf`, direct virtual Irvine32 `WriteChar`, direct virtual Irvine32 `WriteString`, direct virtual Irvine32 `WriteDec`, direct virtual Irvine32 `WriteInt`, direct virtual Irvine32 `WriteHex`, and direct virtual Irvine32 `WriteBin` after `INCLUDE Irvine32.inc`.

`Crlf` appends exactly one LF byte (`0Ah`) to Program Console. It preserves all modeled registers, modeled flags, and flag-validity metadata; reads no simulated memory; writes no simulated memory; and uses the shared no-partial-output limit path.

`WriteChar` appends exactly one byte from `AL`, ignores the high bytes of `EAX`, preserves all modeled registers, modeled flags, and flag-validity metadata, reads no simulated memory, writes no simulated memory, and uses the shared no-partial-output limit path.

`WriteString` treats `EDX` only as a simulated VM address. It reads a null-terminated byte string through checked VM memory helpers, reads but does not print the null terminator, and atomically commits the complete sequence of nonzero bytes. It preserves all modeled registers, modeled flags, flag-validity metadata, simulated memory, and memory-change rows. Candidate-byte uninitialized-origin warnings are emitted only when that `WriteString` call succeeds. A fatal mandatory memory failure, strict uninitialized-read failure, scan-limit failure, or Program Console limit failure appends no candidate bytes from the failing call.

Zero-argument `INVOKE Crlf` is implemented. `INVOKE WriteChar`, `INVOKE WriteString`, `INVOKE WriteDec`, `INVOKE WriteInt`, `INVOKE WriteHex`, and `INVOKE WriteBin` remain unimplemented exact source forms and report `unsupported-irvine-invoke` unless a later accepted phase implements those exact forms. Irvine32 routines beyond the implemented `Crlf`, `WriteChar`, `WriteString`, `WriteDec`, `WriteInt`, `WriteHex`, and `WriteBin` forms, including `DumpRegs` and `DumpMem`, remain deferred until their owning phases implement the exact routine and exact source form.
...
- `.stack` and `.stack size`, where the optional size is stored as parser metadata. In automatic layout selected by tests/configuration, the parsed size controls stack region capacity metadata. Runtime startup initializes `ESP` from the active stack region's exclusive high limit. Direct user-procedure `CALL` performs an internal checked 32-bit return-token write at `ESP - 4`. Helper plain near `RET` performs an internal checked 32-bit return-token read from `[ESP]`, validates the token, and increments `ESP` only after validation succeeds. Selected-entry root `RET` succeeds without reading `[ESP]` when no helper return is pending. Source-level 32-bit `push` and `pop` use the active checked stack region. `LEAVE` reads saved `EBP` from DWORD `[EBP]` through checked VM memory and then commits `ESP = old EBP + 4` and `EBP = saved EBP`. Near `RET imm16` reads and validates the helper return token exactly like plain helper `RET`, then applies the unsigned 16-bit immediate byte cleanup only after validation succeeds. Additional procedure-frame features beyond accepted automatic LOCAL frames, PROC USES save/restore, checked return-token handling, and explicit `RET imm16` cleanup remain deferred. Accepted near return forms include `ret`, `RET`, `ret 0`, `ret 4`, `ret 8`, `ret 16`, and hexadecimal constants such as `ret 0010h`. The immediate must be an unsigned 16-bit constant expression in bytes; `ret -1`, `ret 10000h`, `ret eax`, `ret DWORD PTR [esp]`, `retf`, and `retf 4` remain rejected.
- `INCLUDE Irvine32.inc` and `INCLUDE Macros.inc` as virtual built-ins. The simulator does not load a host file for either directive. `INCLUDE Irvine32.inc` registers known Irvine32 names as virtual metadata for classification and diagnostics. It enables the documented zero-operand virtual `exit` terminator, direct `CALL Crlf`, zero-argument `INVOKE Crlf`, direct `CALL WriteChar`, direct `CALL WriteString`, direct `CALL WriteDec`, direct `CALL WriteInt`, direct `CALL WriteHex`, and direct `CALL WriteBin`. Deferred simulator forms remain disabled, including `INVOKE WriteChar`, `INVOKE WriteString`, `INVOKE WriteDec`, `INVOKE WriteInt`, `INVOKE WriteHex`, `INVOKE WriteBin`, debug routines such as `DumpRegs` and `DumpMem`, input routines, debug routines, random routines, file routines, other Irvine32 routine bodies, and additional procedure-frame or calling-convention behavior. Separately, the virtual include does not change the permanent product boundary: WinAPI behavior, PE loading/linking, object-file or import-library linking, external native-library execution, host callbacks, host filesystem access, native process behavior, Windows process emulation, full x86 emulation, and host include loading remain non-goals. `INCLUDE Macros.inc` remains a virtual no-op and does not populate the Irvine32 routine registry. `INCLUDELIB` directives are not virtual includes and are not accepted as no-ops; they report `unsupported-includelib`, `unsupported-masm32-library`, or `unsupported-windows-api-library` according to the operand. Unsupported basename-only include files report `unsupported-include`. Host/path-like include operands such as `include \masm32\include\masm32.inc`, `include C:\masm32\include\kernel32.inc`, `include ..\include\file.inc`, `include .\local.inc`, and `include /usr/local/include/file.inc` report `unsupported-host-include-path`, `unsupported-masm32-library-include`, or `unsupported-windows-api-include` according to the operand instead of producing repeated lexer path-separator errors.

Phase 84 `INVOKE` and ADDR diagnostics plus Phase 87 `INVOKE Crlf` diagnostics: accepted `INVOKE Helper` / `invoke Helper` to same-file zero-argument user procedures lowers to direct-CALL behavior. Accepted nonzero-argument `INVOKE` forms to same-file user procedures lower DWORD arguments to checked stack slots and require exact `ret imm16` cleanup metadata. Wrong-kind, unknown, malformed, data-symbol, label, equate, register, memory, immediate, or other non-procedure INVOKE targets report `invalid-invoke-target`. External/API targets such as `ExitProcess`, C runtime targets such as `crt_printf`, and MASM32 runtime targets such as `StdOut` report `unsupported-external-invoke`. `INVOKE Crlf` is accepted only with zero arguments after `INCLUDE Irvine32.inc`. `INVOKE Crlf` without `INCLUDE Irvine32.inc` reports `missing-irvine32-include`. `INVOKE Crlf` with one or more arguments reports `invalid-irvine32-argument-count`. Irvine32 routine targets other than zero-argument `Crlf`, including `WriteChar`, `WriteString`, `WriteDec`, `WriteInt`, `WriteHex`, and `WriteBin`, report `unsupported-irvine-invoke` through `INVOKE`. Argument-count mismatches for accepted user-procedure INVOKE forms report `invoke-argument-count-mismatch`. Unsupported user-procedure argument forms report `unsupported-invoke-argument`; 8-bit and 16-bit register aliases report `invoke-argument-width-unsupported`; missing or mismatched callee cleanup reports `invoke-cleanup-mismatch`. Standalone/general source-level `ADDR` reports `addr-outside-invoke`. Wrong-kind ADDR targets such as registers, immediates, numeric equates, memory expressions, and unsupported computed expressions report `invalid-addr-target`, `unknown-addr-symbol`, or `unsupported-addr-expression` as appropriate. These diagnostics are emitted through Simulator Messages, refuse execution, and do not write to Program Console.
...
- The virtual Irvine32 registry recognizes known routine names for classification and diagnostics, including CALL-target and INVOKE-target classification. The zero-operand virtual `exit` terminator, virtual Irvine32 `Crlf`, direct virtual Irvine32 `WriteChar`, direct virtual Irvine32 `WriteString`, direct virtual Irvine32 `WriteDec`, direct virtual Irvine32 `WriteInt`, direct virtual Irvine32 `WriteHex`, and direct virtual Irvine32 `WriteBin` are executable in the current subset after `INCLUDE Irvine32.inc`. Direct user-procedure `CALL` is executable only for user `PROC` entries. Zero-argument same-file user-procedure `INVOKE` and the documented Phase 84 same-file user-procedure DWORD argument subset are executable through their own procedure-call rules. Irvine32 routine calls beyond virtual `Crlf`, direct `WriteChar`, direct `WriteString`, direct `WriteDec`, direct `WriteInt`, direct `WriteHex`, and direct `WriteBin` remain deferred according to the future-feature sections in this document. Irvine32 `INVOKE` dispatch beyond zero-argument `INVOKE Crlf` remains deferred. Additional procedure-frame and calling-convention behavior beyond accepted automatic LOCAL frames, supported LOCAL operands, `PROC USES` preservation, direct user-procedure `CALL`/`RET`, Phase 82 zero-argument user-procedure `INVOKE`, Phase 83 helper-level ADDR preparation, and Phase 84 limited user-procedure INVOKE DWORD arguments also remains deferred unless a later accepted phase explicitly owns it. Near `RET imm16` is implemented only as explicit helper return cleanup; it does not imply Irvine32 routine calls or procedure metadata. Windows/API execution remains outside the simulator boundary as a permanent non-goal unless the canonical specification and guide are deliberately revised.
...
`ret`/`RET` with no operands is implemented as a plain near return. In a called helper procedure, it reads the DWORD return token at `[ESP]` through the central checked-memory path, validates that the token maps to a loaded executable pseudo-EIP, increments `ESP` by 4, and transfers to the target only after the read and token validation both succeed. If `[ESP]` is unreadable, the existing checked-memory diagnostic is emitted and no token validation is attempted. If the read succeeds but the token is not a valid executable pseudo-EIP, execution stops with `invalid-return-address` before changing `ESP` or transferring control. In the selected entry procedure, default `rootRetMode = "masm32-compatible"` lets a no-operand root `RET` with no helper return pending terminate successfully without reading `[ESP]`, validating a pseudo-EIP token, changing `ESP`, or adding public `memoryChanges`. Optional `rootRetMode = "strict-call-frame"` instead rejects that selected-entry root `RET` with `root-ret-disallowed-by-mode` before any stack read, stack mutation, pseudo-EIP validation, public `memoryChanges` row, or successful terminal status. A called non-entry procedure that reaches its `ENDP` boundary without `RET` is governed by `procedureFallthroughPolicy`: default `warn` emits `procedure-fell-through` and continues, `off` suppresses only that diagnostic, and `error` stops with `procedure-fell-through`. Additional source-level stack-frame behavior beyond accepted automatic `LOCAL` frames, named runtime parameters, argument handling beyond the Phase 84 same-file user-procedure `INVOKE` subset, calling-convention behavior, and Irvine32 routine dispatch beyond the implemented virtual `Crlf`, direct `WriteChar`, direct `WriteString`, direct `WriteDec`, direct `WriteInt`, direct `WriteHex`, and direct `WriteBin` forms remains future-owned behavior.

Register, memory, indirect, immediate, `OFFSET`, ordinary-label, data-symbol, equate, unknown, malformed, external/API, and recognized-but-not-yet-executable Irvine32 routine CALL targets are rejected or classified with structured diagnostics rather than dispatched. Implemented virtual `CALL Crlf`, direct `CALL WriteChar`, direct `CALL WriteString`, direct `CALL WriteDec`, direct `CALL WriteInt`, direct `CALL WriteHex`, and direct `CALL WriteBin` are the current Irvine32 direct-call exceptions documented in the Program Console output section. Zero-argument `INVOKE Crlf` is separately implemented. `INVOKE WriteChar`, `INVOKE WriteString`, `INVOKE WriteDec`, `INVOKE WriteInt`, `INVOKE WriteHex`, and `INVOKE WriteBin` remain deferred exact source forms and are not implied by their direct-call implementations. External/API calls and WinAPI behavior are permanent product non-goals, not deferred direct-CALL target forms.
...
- `UNION`
- `RECORD`
- unsupported `INVOKE` forms outside the implemented same-file user-procedure subset and zero-argument `INVOKE Crlf`
- `.IF`
- `.WHILE`
- `.REPEAT`
- `.BREAK`
...
Recovered line-level constructs include unsupported `INVOKE` forms, unsupported or malformed `PROTO` forms, `TEXTEQU`, `INCLUDELIB`, `EXTERN`, `EXTERNDEF`, `EXTRN`, `PUBLIC`, `COMM`, `ASSUME`, `ALIGN`, `EVEN`, `LABEL`, `ORG`, `COMMENT`, and `ECHO`.
...
Deferred simulator features include `loop`; indirect, register-target, memory-target, immediate-target, and branch-distance/type-overridden jumps; unsupported stack-transfer widths; remaining simulator-owned CALL/RET behavior explicitly assigned to later phases; additional stack-frame behavior beyond accepted automatic `LOCAL` frames; Irvine32 routines beyond virtual `exit`, virtual `Crlf`, direct `WriteChar`, direct `WriteString`, direct `WriteDec`, direct `WriteInt`, direct `WriteHex`, and direct `WriteBin`; unimplemented Irvine32 `INVOKE` forms other than zero-argument `INVOKE Crlf`; debugger stepping; breakpoints; scaled-index addressing; carry rotates; selected macro-compatibility conveniences explicitly assigned to later accepted milestones; and fuller MASM expression compatibility beyond the currently implemented subset.

Whenever a later accepted phase implements an additional Irvine32 routine or exact source form, that same phase must update every active Irvine32 capability inventory in this document. Updating only the opening current-status paragraph is insufficient. Historical milestone reports must retain the wording that was accurate for their historical repository state and must not be rewritten merely to satisfy current-status checks.
