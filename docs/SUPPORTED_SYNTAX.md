...
Invalid source forms for implemented `Crlf` and `WriteChar` are `assembly-error` diagnostics, not `unsupported-feature` diagnostics. `CALL Crlf` and `CALL WriteChar` without `INCLUDE Irvine32.inc` report `missing-irvine32-include`. `INVOKE Crlf` without `INCLUDE Irvine32.inc` reports `missing-irvine32-include`. `INVOKE Crlf` with one or more arguments after `INCLUDE Irvine32.inc` reports `invalid-irvine32-argument-count`. Bare `Crlf` after `INCLUDE Irvine32.inc` reports `invalid-irvine32-call-form`. Bare `WriteChar` after `INCLUDE Irvine32.inc` reports `invalid-irvine32-call-form`. These diagnostics refuse execution, write no Program Console output, and preserve source line, column, byte offset, and span length.

Other Irvine32 output routines, including `WriteString`, `WriteInt`, `WriteDec`, `WriteHex`, and `WriteBin`, remain recognized but deferred. `INVOKE WriteChar` remains future-owned Irvine32 INVOKE routine dispatch and reports `unsupported-irvine-invoke`. Irvine32 input, debug, random, console-control, file, macro, WinAPI, PE/linker, and host I/O behavior remain unsupported or future-owned as documented below.
...
Still-deferred Irvine32 routines continue to use unsupported-feature diagnostics. For example, `CALL WriteString` remains `unsupported-irvine32-routine`, and `INVOKE WriteString` remains `unsupported-irvine-invoke`, until a later accepted phase implements the specific routine and source form.
