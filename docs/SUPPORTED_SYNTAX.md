...
### Phase 57E startup-state notice

Repository/archive milestone Phase 57E explains deterministic simulator startup state and activates the `startup-state-notice` diagnostic family.
...
Default successful browser/source-run execution emits a non-fatal Simulator Messages notice explaining that registers and modeled flags start at zero, and uninitialized storage bytes are zero-filled while preserving uninitialized-origin metadata for code-quality diagnostics. The notice does not write to Program Console, does not block execution, and does not change register, flag, memory, or uninitialized-origin initialization. Test/configuration-facing helpers can opt out of the notice through the diagnostic policy path. Phase 57E does not implement startup randomization.
