...
Current public MASM source supports virtual Irvine32 `Crlf` and direct virtual Irvine32 `WriteChar` after `INCLUDE Irvine32.inc`. `Crlf` appends exactly one LF byte (`0Ah`) to Program Console, preserves registers and modeled flags, reads no memory, writes no memory, and uses the same no-partial output-limit failure path. `WriteChar` appends exactly one byte from `AL`, ignores the high bytes of `EAX`, preserves registers, modeled flags, and flag-validity metadata, reads no simulated memory, writes no simulated memory, and uses the same no-partial output-limit failure path. Other Irvine32 output routines remain deferred unless a later accepted phase explicitly implements the specific routine and source form.
...
Deferred simulator features include `loop`, indirect jumps, register-target jumps, memory-target jumps, immediate-target jumps, branch-distance/type overrides, unsupported stack-transfer widths, remaining simulator-owned CALL/RET behavior explicitly assigned to later phases, additional stack-frame behavior beyond accepted automatic `LOCAL` frames, Irvine32 routines beyond virtual `exit`, virtual `Crlf`, and direct `WriteChar`, debugger stepping, breakpoints, scaled-index addressing, carry rotates, selected macro-compatibility conveniences explicitly assigned to later accepted milestones, and fuller MASM expression compatibility beyond the currently implemented subset.
