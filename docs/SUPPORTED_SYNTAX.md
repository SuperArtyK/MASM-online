...
Accepted nonzero-argument `INVOKE` requires metadata proving exact callee cleanup with `ret imm16` equal to four bytes per accepted argument. A plain `ret`, inconsistent `ret imm16` metadata, or mismatched cleanup byte count reports `invoke-cleanup-mismatch` before execution. A `PROTO` argument-count mismatch reports `invoke-argument-count-mismatch`. Full 32-bit register arguments are accepted; 8-bit and 16-bit register aliases report `invoke-argument-width-unsupported`. Unsupported argument shapes report `unsupported-invoke-argument` or a more specific ADDR diagnostic such as `invalid-addr-target`. External/API targets report `unsupported-external-invoke`, and Irvine32 routine targets remain rejected with `unsupported-irvine-invoke`. Source-level executable `ADDR` remains unsupported as a standalone instruction or general operand outside accepted Phase 84 `INVOKE` argument forms. `PROTO` metadata does not make named runtime parameters, external calls, WinAPI calls, Irvine32 routine calls, C runtime calls, MASM32 runtime calls, import libraries, linking, or full calling conventions executable.
...
Consult the current instruction-support table in this document for implemented branch, control-flow, stack-transfer, LOCAL operand, limited `PROTO` metadata, zero-argument `INVOKE` forms, and Phase 84 limited INVOKE DWORD arguments and Phase 83 helper-level `ADDR symbol` record preparation. Direct near `call ProcedureName`, zero-argument `INVOKE ProcedureName` to same-file user procedures, plain near `ret`/`RET` with no operands, near `ret imm16`/`RET imm16`, the documented 32-bit source-level `push`/`pop` subset, `leave`/`LEAVE`, automatic runtime LOCAL frames for procedures with parsed `LOCAL` declarations, supported Phase 80 LOCAL operands, Phase 81 limited same-file `PROTO` metadata, and helper-level ADDR record construction are implemented. Do not infer unsupported CALL forms, unsupported source-level `INVOKE` argument forms beyond Phase 84, `OFFSET local`, executable source-level `ADDR` outside accepted INVOKE arguments, pointer or unnamed prototype parameters, scaled-index LOCAL addressing, far returns, Irvine32 routine dispatch through INVOKE, named runtime parameter metadata, external calls, import/linker behavior, or calling-convention behavior from future roadmap examples.

Phase 84 has completed limited DWORD `INVOKE` argument lowering and cleanup for same-file user procedures. Future phases may extend parameter access, Irvine32 routine dispatch, pointer or VARARG metadata, computed ADDR expressions, or calling-convention behavior only when explicitly assigned. Source-level `ADDR` outside accepted INVOKE argument forms remains unsupported.

The following stack/procedure features remain future work only if a later accepted milestone explicitly implements them:

- executable `INVOKE` arguments beyond the accepted zero-argument and Phase 84 limited DWORD same-file user-procedure forms;
- unsupported source-level `INVOKE` argument forms, including 8-bit/16-bit registers, QWORD/SQWORD values, computed ADDR expressions, memory expressions not accepted by Phase 84, pointer metadata, VARARG, external/API targets, and Irvine32 routine dispatch;
...
Phase 84 `INVOKE` and ADDR diagnostics: accepted `INVOKE Helper` / `invoke Helper` to same-file zero-argument user procedures lowers to direct-CALL behavior. Accepted nonzero-argument `INVOKE` forms to same-file user procedures lower DWORD arguments to checked stack slots and require exact `ret imm16` cleanup metadata. Wrong-kind, unknown, malformed, data-symbol, label, equate, register, memory, immediate, or other non-procedure INVOKE targets report `invalid-invoke-target`. External/API targets such as `ExitProcess`, C runtime targets such as `crt_printf`, and MASM32 runtime targets such as `StdOut` report `unsupported-external-invoke`. Irvine32 routine targets such as `WriteString` report `unsupported-irvine-invoke`. Argument-count mismatches report `invoke-argument-count-mismatch`. Unsupported argument forms report `unsupported-invoke-argument`; 8-bit and 16-bit register aliases report `invoke-argument-width-unsupported`; missing or mismatched callee cleanup reports `invoke-cleanup-mismatch`. Standalone/general source-level `ADDR` reports `addr-outside-invoke`. Wrong-kind ADDR targets such as registers, immediates, numeric equates, memory expressions, and unsupported computed expressions report `invalid-addr-target`, `unknown-addr-symbol`, or `unsupported-addr-expression` as appropriate. These diagnostics are emitted through Simulator Messages, refuse execution, and do not write to Program Console.
...
- The virtual Irvine32 registry recognizes known routine names for classification and diagnostics, including CALL-target classification. Only the zero-operand virtual `exit` terminator is executable as an Irvine32 behavior in the current subset; direct user-procedure `CALL` is executable only for user `PROC` entries. Calls to Irvine32 routines remain deferred according to the future-feature sections in this document. Additional procedure-frame and calling-convention behavior beyond accepted automatic LOCAL frames, supported LOCAL operands, `PROC USES` preservation, direct user-procedure `CALL`/`RET`, Phase 82 zero-argument INVOKE, Phase 83 helper-level ADDR preparation, and Phase 84 limited INVOKE DWORD arguments also remains deferred unless a later accepted phase explicitly owns it. Near `RET imm16` is implemented only as explicit helper return cleanup; it does not imply Irvine32 routine calls or procedure metadata. Windows/API execution remains outside the simulator boundary as a permanent non-goal unless the canonical specification and guide are deliberately revised.
