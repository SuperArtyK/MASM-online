...
Phase 57L implements the v1 policy that source-level memory reads or writes whose final byte range overlaps `.code` are unsupported. Wholly contained `.code` accesses report `unsupported-code-memory-access`; cross-region accesses whose final range intersects `.code` report `region-boundary-crossing` with `.code` protected-region context from runtime layout metadata. Phase 57K also defines MASM/object/linker segment and group names such as `_TEXT`, `_DATA`, `_BSS`, `CONST`, `STACK`, `DGROUP`, and `FLAT` as unsupported addressable symbols for later Phase 57M targeted diagnostics. Phase 57J configurable `const-uninitialized-storage` declaration diagnostics, Phase 57I accepted `.CONST ?` / `.CONST DUP(?)` storage, Phase 57H final-register `[unchanged]` display markers, Phase 57G seeded uninitialized-storage visible-byte settings, Phase 57F seeded register/flag startup, Phase 57E startup-state notices, Phase 57D diagnostic-policy migration, Phase 57C diagnostic-policy registry design, Phase 57B documentation extraction, Phase 57A README cleanup, Phase 57-CORR2 compact negative register-indirect displacement parsing, and Phase 57-CORR1 protected-region diagnostic clarification remain accepted behavior.

This reference describes the implemented MASM source syntax and instruction subset through Phase 57 - Signed IDIV. The current runtime/source-run MASM behavior phase is Phase 57L - .CODE Memory Access Diagnostics because Phase 57L changes user-visible runtime behavior for memory ranges that overlap `.code` while leaving the instruction subset anchored by Phase 57 - Signed IDIV. This reference also includes Phase 57K segment-symbol policy documentation, Phase 57H final-register `[unchanged]` display markers, Phase 57G seeded uninitialized-storage settings, Phase 57F seeded register/flag startup, Milestone 52A display formatting, Milestone 53A memory-validation policy clarification, Milestone 53B opt-in section-capacity and section-image validation modes, Milestone 53C default teaching diagnostics, and Milestone 53D compatibility notices for accepted no-op, metadata-only, virtual-only, and limited-behavior MASM constructs. This document is intentionally not a full MASM reference. Unsupported constructs listed here should produce stable diagnostics instead of vague parser errors.
...
### Phase 57L `.code` memory diagnostics and Phase 57K MASM segment/group symbol policy
...
Runtime/source-run MASM behavior phase Phase 57L preserves the independent source-run/test-facing startup settings added by Phase 57F and Phase 57G:
