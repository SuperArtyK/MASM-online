...
`PROC` starts with limited structural and entry-boundary behavior. Direct user-procedure `CALL` is added by Phase 69. Later completed procedure phases add `RET`, root-return behavior, any additional simulator-owned CALL target forms only where explicitly assigned, `USES`, `LOCAL` declaration metadata, automatic Phase 79 LOCAL frame behavior, Phase 80 local operand behavior, Phase 81 limited `PROTO` metadata, Phase 82 zero-argument `INVOKE` to same-file user procedures, Phase 83 helper-level `ADDR symbol` preparation, and Phase 84 limited same-file user-procedure `INVOKE` DWORD argument lowering with exact cleanup validation. Named runtime parameters, unsupported executable `INVOKE` argument forms, source-level executable `ADDR` outside accepted INVOKE arguments, general `ADDR` operands, additional stack-frame behavior, pointer or unnamed prototype parameters, executable prototype behavior beyond Phase 84 validation, and calling-convention metadata remain assigned only to future accepted phases.
...
None of these rules implement native x86 byte execution, PE loading, Windows process teardown, WinAPI calls, stack-frame features beyond accepted simulator-owned procedure phases, unsupported LOCAL operand forms, executable `PROTO` behavior beyond Phase 84 validation, unsupported `INVOKE` argument forms, source-level executable `ADDR` outside accepted INVOKE arguments, general `ADDR` operands, or additional Irvine32 routine dispatch unless a separate accepted phase owns those features. Phase 78 owns LOCAL declaration metadata, Phase 78A owns limited `OPTION NOKEYWORD` parser keyword opt-out for `LOOP` and `OFFSET`, Phase 79 owns automatic LOCAL frame allocation/lifetime for selected-entry and direct-CALL procedure paths, Phase 80 owns supported LOCAL operand resolution/addressing, Phase 81 owns limited parser-only `PROTO` metadata, Phase 82 owns zero-argument `INVOKE` to same-file user procedures, Phase 83 owns helper-level `ADDR symbol` preparation, and Phase 84 owns limited same-file user-procedure `INVOKE` DWORD argument lowering with exact cleanup validation.
...
A reserved-word declaration must not be inserted into any user-symbol table. This rule applies to data symbols, numeric equates, code labels, procedure names, and procedure-local symbols unless an owning phase explicitly documents a narrow compatibility exception. Phase 78 contains one such exception: the accepted-syntax sample `LOCAL ch:BYTE` is accepted as LOCAL declaration metadata even though `CH` is otherwise a recognized register alias; other register names remain reserved as local symbol names. If parser recovery continues, later references to that spelling may produce follow-on diagnostics such as `unknown-symbol`, `invalid-branch-target`, or parser-specific keyword diagnostics. The declaration-site `reserved-word-symbol` diagnostic is the primary diagnostic and should be present whenever the parser can identify the declaration.
...
Recognized Irvine32 routine and terminator names are not user procedure symbols. Before the owning Irvine32 routine-dispatch phase implements a specific routine, a source form such as `call DumpRegs` must be classified as a recognized-but-deferred or recognized-but-unsupported Irvine32 routine call, not as a user-procedure call. Implemented direct forms such as `call WriteString`, `call WriteDec`, `call WriteInt`, `call WriteHex`, and `call WriteBin` must route through their simulator-defined Irvine32 intrinsic paths rather than user-procedure lookup.
...
- executable procedure-frame conveniences beyond implemented LOCAL, limited `PROTO` metadata, Phase 82 zero-argument same-file user-procedure `INVOKE`, Phase 83 helper-level `ADDR symbol` preparation, and Phase 84 limited same-file user-procedure `INVOKE` DWORD argument lowering, including unsupported `INVOKE` argument forms, source-level executable `ADDR` outside accepted INVOKE arguments, and general `ADDR` operands
...
Current `LEAVE` is implemented as validation-first frame teardown shorthand. It reads saved `EBP` from DWORD `[EBP]` through the central checked memory helpers, then commits `ESP = old EBP + 4` and `EBP = saved EBP` only after that read succeeds. The read is internal VM stack/frame state and must not create a public `memoryChanges` row. A failed `[EBP]` read must report the existing checked-memory diagnostic on the `LEAVE` source span before mutating `ESP`, `EBP`, flags, memory, Program Console output, terminal state, or public memory-change rows. In Phase 79, `LEAVE` also releases the active automatic LOCAL frame for the current procedure when one exists, and the following `RET` must acknowledge that released frame without double release. `LEAVE` does not itself create automatic frames and does not imply `ENTER`, new `PROC USES` entry forms, argument metadata, calling-convention inference, or Irvine32 callable routine dispatch.
...
### Root RET in MASM32 Educational Mode

The simulator distinguishes ordinary helper-procedure RET from root-procedure RET.
...
- a user data-memory address;
- a public source-run JSON `memoryChanges` row;
- source-level PUSH/POP behavior;
- stack-frame support;
- unsupported LOCAL operand addressing, executable PROTO behavior beyond Phase 84 validation, unsupported INVOKE argument forms, source-level executable ADDR outside accepted INVOKE arguments, general ADDR operands, or calling-convention support;
...
Phase 79 uses this metadata to allocate automatic runtime LOCAL frames for selected-entry startup and direct-CALL helper entry, save and restore `EBP`, reserve rounded LOCAL bytes, initialize visible LOCAL bytes to deterministic zero while preserving uninitialized-origin metadata, and create runtime local-object descriptors. Phase 80 resolves supported source-level local names as active-frame LOCAL operands for accepted instruction forms. Supported examples include `mov temp, eax`, `mov eax, temp`, `mov BYTE PTR buf[0], 'A'`, `mov al, BYTE PTR buf[1]`, `lea eax, temp`, and `lea eax, buf`. LOCAL operands resolve through the currently active procedure frame, use the parser-owned negative `EBP`-relative metadata, and route reads and writes through the central checked memory helpers. `LEA` computes the local runtime address without reading local bytes.

Phase 81 accepts limited parser-owned `PROTO` metadata for same-file educational prototypes using zero parameters or named `DWORD`/`SDWORD` parameters. The parser preserves prototype name spelling, parameter names, parameter type identities, declaration order, parameter count, and source locations/spans. Phase 81 `PROTO` declarations emit no executable IR and do not change runtime call behavior by themselves. Phase 81 `PROTO` metadata must not be interpreted as executable prototype behavior. Phase 82 may use compatible zero-argument `PROTO` metadata only to permit zero-argument same-file user-procedure `INVOKE`. `PROTO` metadata must not imply `ADDR`, stack argument passing, caller or callee cleanup, PROC parameter access, external linkage, import metadata, WinAPI behavior, or calling-convention inference.

Phase 81 compatibility rule: a zero-argument `PROTO` may link to a same-name bare `PROC` because no runtime parameter metadata is required. A parameterized `PROTO`, such as `MyProc PROTO arg1:DWORD`, must not be treated as compatible with a same-name bare `PROC` while accepted PROC parameter metadata, runtime parameters, and calling conventions remain unimplemented. That mismatch must report `proto-proc-mismatch` with the current conflicting declaration and the earlier declaration as a related location when available. A later phase that explicitly owns PROC parameters and calling conventions may revise this rule only by updating this spec, the implementation guide, supported syntax, structured diagnostics, rendered Simulator Messages tests, and source-run acceptance tests.

The corrected `INVOKE`/`ADDR` roadmap is intentionally staged. Phase 82 is the first executable `INVOKE` phase and supports only zero-argument `INVOKE` calls to same-file user procedures. Phase 82 does not implement argument pushes, argument expressions, `ADDR`, `OFFSET` as an INVOKE argument, PROC parameter access, external calls, Irvine32 routine calls through `INVOKE`, Windows APIs, import metadata, object linking, PE loading, native x86 execution, or calling-convention behavior. Phase 83 prepares a limited helper-level `ADDR` representation for later `INVOKE` arguments, but it does not make `ADDR` a general source-level operand. Phase 84 implements a limited DWORD `INVOKE` argument subset after zero-argument `INVOKE` exists and `ADDR` argument preparation is specified. Full MASM `INVOKE` behavior, register calling conventions, varargs, structures, floating point, 64-bit argument execution, language attributes, external procedure invocation, WinAPI calls, C runtime calls, MASM32 runtime calls, host callbacks, import/linker behavior, and PE loading remain non-goals unless a later accepted milestone explicitly names and limits a specific behavior.

`OFFSET local`, source-level executable `ADDR` outside accepted INVOKE arguments, executable `PROTO` behavior beyond Phase 84 validation, pointer or unnamed prototype parameters, `VARARG`, named runtime parameters, calling conventions, computed ADDR expressions, scaled-index LOCAL addressing, QWORD/SQWORD executable LOCAL memory access, and expanded LOCAL grammar remain unsupported unless a later accepted phase explicitly implements the specific form. Targeted LOCAL diagnostics include `local-outside-procedure`, `local-after-instruction`, `unsupported-local-type`, `invalid-local-declaration`, `duplicate-local-symbol`, and `invalid-local-count`; targeted PROTO diagnostics include `invalid-proto-declaration`, `unsupported-proto-type`, `unsupported-external-proto`, `duplicate-proto`, and `proto-proc-mismatch`.
...
Phase 83 defines helper-level metadata for `ADDR dataSymbol`, `ADDR dataQuestionSymbol`, `ADDR constSymbol`, and `ADDR localSymbol` as later `INVOKE` arguments. Phase 84 uses that preparation for the accepted limited INVOKE argument subset. The helper-level ADDR preparation does not make `ADDR` a general source-level operand and does not legalize unsupported INVOKE argument forms.

Phase 84 supports a limited educational DWORD stack-argument subset for same-file user-procedure INVOKE. It defines right-to-left push order, accepted argument forms, argument-count checks against PROTO metadata, exact `ret imm16` stack cleanup validation, source-span diagnostics, validation-before-mutation behavior, and rendered Simulator Messages. Full MASM calling-convention inference, external procedure invocation, Windows ABI behavior, WinAPI calls, import libraries, PE loading, object linking, C runtime calls, host callbacks, and native x86 execution remain outside the simulator boundary.

### INVOKE and ADDR phase-boundary invariant

No milestone may claim source-level `INVOKE target, ADDR symbol` support unless all of the following are true in the same or earlier accepted milestones:

1. zero-argument `INVOKE` to same-file user procedures exists and is tested as executable behavior;
2. `INVOKE` argument parsing is explicitly owned by the milestone;
3. `ADDR` argument handling is explicitly owned by the milestone or by an earlier helper/preparation milestone;
4. argument lowering, argument-count validation, cleanup policy, source-span diagnostics, and rendered Simulator Messages are specified and tested;
5. the supported syntax reference documents the accepted form as current behavior, not as future work;
6. the default browser editor program, if it uses the form, is covered by a source-run smoke test extracted from `web/index.html`.

After Phase 84, only the accepted limited DWORD same-file user-procedure `INVOKE` argument forms are supported. Other `INVOKE` arguments remain unsupported. `ADDR` remains unsupported as a standalone source-level executable operand or general operand and must not be accepted merely because parser helper code can represent it internally.

### PROTO metadata non-execution invariant

`PROTO` metadata is not executable behavior. A `PROTO` declaration may support diagnostics, metadata inspection, Phase 84 argument-count validation, and same-name metadata consistency checks. It must not by itself allocate runtime parameter slots, make named parameters readable in a `PROC`, select a calling convention, import an external symbol, link a library routine, or make unsupported `INVOKE` forms executable. Any future phase that converts PROTO metadata into broader executable call behavior must explicitly define the accepted source forms, runtime stack layout, cleanup policy, diagnostic precedence, rendered Simulator Messages, and non-goal boundaries.
...
- `call WriteString`, `call WriteDec`, `call WriteInt`, `call WriteHex`, and `call WriteBin` must classify through the Irvine32 registry, not through the user procedure table.
- Before the owning Irvine32 dispatch phase implements a recognized routine, that direct call must produce a recognized-deferred or recognized-unsupported Irvine32 diagnostic; for example, `call DumpRegs` remains deferred until its owning phase.
- After Phase 89, direct `call WriteString` routes to simulator-defined Irvine32 behavior, after Phase 90 direct `call WriteDec` does likewise, after Phase 91 direct `call WriteInt` does likewise, after Phase 92 direct `call WriteHex` does likewise, and after Phase 93 direct `call WriteBin` does likewise; none becomes an ordinary user procedure symbol.
...
WriteHex:
  Implemented by Phase 92 for the direct virtual Irvine32 call form defined here.
  Accepted executable source form after INCLUDE Irvine32.inc:
    call WriteHex
  Matching must be case-insensitive for the Irvine32 routine name even when OPTION CASEMAP:NONE is active.
  CALL WriteHex without INCLUDE Irvine32.inc must be an assembly error.
  Bare WriteHex must be an assembly error.
  ...
  Write no simulated memory.
  Write no Simulator Messages on success except non-program-output diagnostics required by active policy.
  Implementing WriteHex must not alter existing WriteDec or WriteInt behavior and must not implement WriteBin, INVOKE WriteHex, later Irvine32 input/debug/random/file routine families, WinAPI behavior, PE/linking behavior, external libraries, host callbacks, host filesystem behavior, native process behavior, or full x86 emulation.

WriteBin:
  Implemented by Phase 93 for the direct virtual Irvine32 call form defined here.
  Accepted executable source form after INCLUDE Irvine32.inc:
    call WriteBin
  Matching must be case-insensitive for the Irvine32 routine name even when OPTION CASEMAP:NONE is active.
  CALL WriteBin without INCLUDE Irvine32.inc must be an assembly error.
  Bare WriteBin must be an assembly error.
  ...
  Write no simulated memory.
  Write no Simulator Messages on success except non-program-output diagnostics required by active policy.
  Implementing WriteBin must not alter existing WriteDec, WriteInt, or WriteHex behavior and must not implement later Irvine32 input/debug/random/file routine families, WinAPI behavior, PE/linking behavior, external libraries, host callbacks, host filesystem behavior, native process behavior, or full x86 emulation.
```
...
```text
DumpRegs is a recognized Irvine32 routine, but executable register-dump behavior is deferred to its owning later phase.
```
...
This rule preserves the Phase 69 public output behavior that remains part of the Phase 70 source-run output contract: successful direct user-procedure `CALL` performs an internal checked pseudo-EIP return-token stack write, updates `ESP`, and transfers to the target procedure entry, but that implicit stack write does not appear as a public source-run `memoryChanges` row.
...
Near `RET imm16` is implemented for helper returns as explicit caller-cleanup behavior. The operand is an unsigned 16-bit immediate byte count. The executor reads and validates the pseudo-EIP return token exactly like plain helper `RET`, applies the immediate cleanup only after the token is valid, validates that the resulting `ESP` remains inside the active stack region or exactly at the documented empty-stack boundary, preserves modeled flags, does not read or clear skipped argument bytes, and reports a runtime error on the `RET imm16` source span before changing `ESP` or transferring control when cleanup would leave the valid stack boundary. Selected-entry root-code-stream `RET` compatibility behavior remains governed by root RET mode and does not infer helper return cleanup semantics from the immediate operand.
...
- `RETF`, stack-frame creation, and Irvine32 callable routine dispatch until their owning phases implement those features;
...
Current direct user-procedure `CALL`, current plain near helper `RET`, current near helper `RET imm16`, current selected-entry root `RET`, current source-level `PUSH`/`POP`, and current `LEAVE` behavior must not be removed or described as unsupported merely because Irvine32 routine dispatch, stack-frame creation, far returns, and other CALL/RET forms remain future-owned.
