...
Before those conditions are met, `INVOKE` with any argument remains unsupported. `ADDR` remains unsupported as a source-level executable operand and must not be accepted merely because parser helper code can represent it internally.

### PROTO metadata non-execution invariant

`PROTO` metadata is not executable behavior. A `PROTO` declaration may support diagnostics, metadata inspection, future argument-count validation, and same-name metadata consistency checks. It must not by itself allocate runtime parameter slots, make named parameters readable in a `PROC`, select a calling convention, import an external symbol, link a library routine, or make `INVOKE` executable. Any future phase that converts PROTO metadata into executable call behavior must explicitly define the accepted source forms, runtime stack layout, cleanup policy, diagnostic precedence, rendered Simulator Messages, and non-goal boundaries.
