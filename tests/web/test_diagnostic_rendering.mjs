...
test("Current protocol renders stale output-contract warning exactly", () => {
  ...
  assert.equal(
    rendered,
    "[internal-simulator-error] stale-wasm-output-contract: The loaded Wasm artifact does not report the current source-run output-contract identifier required by the current UI/source files. Rebuild web/dist with the Emscripten build script."
  );
});

test("Phase 70A renders stale runtime artifact warning exactly", () => {
  const rendered = formatSimulatorMessages([
    {
      kind: "internal-simulator-error",
      code: "stale-wasm-artifact",
      message: "The loaded Wasm artifact reports runtime/source-run MASM behavior Phase 71, but the UI/source files expect Phase 93 - Irvine32 WriteBin. Rebuild web/dist with the Emscripten build script."
    }
  ]);

  assert.equal(
    rendered,
    "[internal-simulator-error] stale-wasm-artifact: The loaded Wasm artifact reports runtime/source-run MASM behavior Phase 71, but the UI/source files expect Phase 93 - Irvine32 WriteBin. Rebuild web/dist with the Emscripten build script."
  );
});

test("Phase 86 renders Program Console output-limit diagnostic exactly", () => {
  const rendered = formatSimulatorMessages([
    {
  ...
  assert.equal(
    rendered,
    "[resource-limit-error] console-output-limit-exceeded: Program Console output would exceed the configured byte or line limit. Execution stopped before appending partial output."
  );
});
...
/**
 * Asserts that a source-run result carries the current empty Program Console payload.
 *
 * @param {object} json Parsed source-run JSON.
 * @returns {void}
 */
function assertEmptyProgramConsole(json) {
  assert.deepEqual(json.programConsole, {
    text: "",
    truncated: false,
    byteCount: 0,
    lineCount: 0,
...
END main
`,
    reason: "Phase 71 selected-entry root RET completion fixture."
  },
...
END main
`,
    reason: "Current runtime metadata with Phase 63 CMP register/immediate success fixture."
  },
...
`;
  ...
  assert.equal(json.phaseName, "Phase 93 - Irvine32 WriteBin");
...
test("renders Phase 58 duplicate and conflicting code-label diagnostics exactly", () => {
  ...
  assert.equal(duplicateResult.json.phase, 93);
...
test("renders known Irvine32 routine diagnostic exactly", () => {
  ...
  assertNoExecutionComplete(json.simulatorMessages);
  assertRenderedEquals(name, source, rawJson, rendered, "[unsupported-feature] unsupported-irvine32-routine line 4, column 5, byte offset 41, span length 8: Recognized Irvine32 routine, but executable behavior for this routine is deferred to the routine-specific Irvine32 phases.");
});
...
test("renders Phase 84 INVOKE PROTO argument-count diagnostic exactly", () => {
  const name = "phase84InvokeProtoArgumentCount";
  const source = "MyProc PROTO arg1:DWORD\n.code\nmain PROC\n    INVOKE MyProc\nmain ENDP\nMyProc PROC\n    ret\nMyProc ENDP\nEND main\n";
  const { json, rawJson, rendered } = runFixture(name, source);
  assertRunStatus(json, false, "parse-error");
  assertMessageEquals(json.simulatorMessages[0], {
    kind: "assembly-error",
    code: "invoke-argument-count-mismatch",
    message: "INVOKE argument count mismatch: target prototype expects 1 DWORD argument(s), but this INVOKE supplies 0.",
    line: 4,
    column: 12,
    byteOffset: 51,
    spanLength: 6
  ...
  assertNoExecutionComplete(json.simulatorMessages);
  assertRenderedEquals(name, source, rawJson, rendered, "[assembly-error] invoke-argument-count-mismatch line 4, column 12, byte offset 51, span length 6: INVOKE argument count mismatch: target prototype expects 1 DWORD argument(s), but this INVOKE supplies 0.");
});
...
test("renders Phase 84 INVOKE register-width diagnostic exactly", () => {
  ...
  assertMessageEquals(json.simulatorMessages[0], {
    kind: "assembly-error",
    code: "invoke-argument-width-unsupported",
    message: "Phase 84 INVOKE accepts only full 32-bit register arguments; use explicit extension before INVOKE for 8-bit or 16-bit aliases.",
    line: 3,
    column: 22,
    byteOffset: 37,
    spanLength: 2
  ...
  assertNoExecutionComplete(json.simulatorMessages);
  assertRenderedEquals(name, source, rawJson, rendered, "[assembly-error] invoke-argument-width-unsupported line 3, column 22, byte offset 37, span length 2: Phase 84 INVOKE accepts only full 32-bit register arguments; use explicit extension before INVOKE for 8-bit or 16-bit aliases.");
});

test("Phase 84 renders unsupported INVOKE argument diagnostic exactly", () => {
  const name = "phase84-invoke-unsupported-argument";
  const source = `.code
main PROC
    INVOKE Helper, [esp]
main ENDP
...
`;
  ...
  assert.deepEqual(json.simulatorMessages, [
    {
      kind: "assembly-error",
      code: "unsupported-invoke-argument",
      message: "Unsupported INVOKE argument form. The accepted DWORD INVOKE subset accepts only 32-bit immediates, full 32-bit registers, DWORD/SDWORD data symbols, OFFSET symbol, ADDR symbol, and ADDR active LOCAL.",
      line: 3,
      column: 20,
      byteOffset: 35,
      spanLength: 1
  ...
  assertNoExecutionComplete(json.simulatorMessages);
  assertRenderedEquals(name, source, rawJson, rendered, "[assembly-error] unsupported-invoke-argument line 3, column 20, byte offset 35, span length 1: Unsupported INVOKE argument form. The accepted DWORD INVOKE subset accepts only 32-bit immediates, full 32-bit registers, DWORD/SDWORD data symbols, OFFSET symbol, ADDR symbol, and ADDR active LOCAL.");
});

test("Phase 84 renders invalid INVOKE ADDR target diagnostic exactly", () => {
  const name = "phase84-invoke-invalid-addr-target";
  const source = `.code
main PROC
...
`;
  ...
  assert.deepEqual(json.simulatorMessages, [
    {
      kind: "assembly-error",
      code: "invalid-addr-target",
      message: "ADDR target must be an addressable data, .CONST, .DATA?, or active-frame LOCAL symbol.",
      line: 3,
      column: 25,
      byteOffset: 40,
      spanLength: 3
  ...
  assertNoExecutionComplete(json.simulatorMessages);
  assertRenderedEquals(name, source, rawJson, rendered, "[assembly-error] invalid-addr-target line 3, column 25, byte offset 40, span length 3: ADDR target must be an addressable data, .CONST, .DATA?, or active-frame LOCAL symbol.");
});

test("Phase 84 renders INVOKE ADDR cleanup mismatch exactly", () => {
  const name = "phase84-invoke-addr-cleanup-mismatch";
  const source = `.data
msg BYTE "Hi", 0
.code
main PROC
...
`;
  ...
  assert.deepEqual(json.simulatorMessages, [
    {
      kind: "assembly-error",
      code: "invoke-cleanup-mismatch",
      message: "INVOKE cleanup mismatch: Phase 84 DWORD INVOKE to `Helper` requires callee cleanup `ret 4` for 4 argument byte(s).",
      line: 5,
      column: 12,
      byteOffset: 50,
      spanLength: 6
  ...
  assertRenderedEquals(name, source, rawJson, rendered, "[assembly-error] invoke-cleanup-mismatch line 5, column 12, byte offset 50, span length 6: INVOKE cleanup mismatch: Phase 84 DWORD INVOKE to `Helper` requires callee cleanup `ret 4` for 4 argument byte(s).");
});

test("Phase 84 preserves standalone ADDR outside INVOKE diagnostic exactly", () => {
...
`;
  ...
  assert.deepEqual(json.simulatorMessages, [
    {
      ...
      message: "ADDR is reserved for accepted INVOKE argument handling and is not a standalone source-level instruction or general operand.",
      line: 5,
      column: 5,
      byteOffset: 38,
      spanLength: 4
  ...
  assertNoExecutionComplete(json.simulatorMessages);
  assertRenderedEquals(name, source, rawJson, rendered, "[assembly-error] addr-outside-invoke line 5, column 5, byte offset 38, span length 4: ADDR is reserved for accepted INVOKE argument handling and is not a standalone source-level instruction or general operand.");
});
...
test("Phase 87 renders Crlf CALL without Irvine32 include diagnostic exactly", () => {
  const name = "phase87-crlf-call-without-include";
  const source = `.code
main PROC
    call Crlf
main ENDP
...
`;
  ...
  assert.deepEqual(json.simulatorMessages, [
    {
      ...
      message: "CALL Crlf requires INCLUDE Irvine32.inc before Crlf can be used as a virtual Irvine32 routine.",
      line: 3,
      column: 10,
      byteOffset: 25,
      spanLength: 4
  ...
  assertNoExecutionComplete(json.simulatorMessages);
  assertRenderedEquals(name, source, rawJson, rendered, "[assembly-error] missing-irvine32-include line 3, column 10, byte offset 25, span length 4: CALL Crlf requires INCLUDE Irvine32.inc before Crlf can be used as a virtual Irvine32 routine.");
});

test("Phase 87 renders Crlf INVOKE without Irvine32 include diagnostic exactly", () => {
  const name = "phase87-crlf-invoke-without-include";
  const source = `.code
main PROC
    invoke Crlf
main ENDP
...
`;
  ...
  assert.deepEqual(json.simulatorMessages, [
    {
      kind: "assembly-error",
      code: "missing-irvine32-include",
      message: "INVOKE Crlf requires INCLUDE Irvine32.inc before Crlf can be used as a virtual Irvine32 routine.",
      line: 3,
      column: 12,
      byteOffset: 27,
      spanLength: 4
  ...
  assertNoExecutionComplete(json.simulatorMessages);
  assertRenderedEquals(name, source, rawJson, rendered, "[assembly-error] missing-irvine32-include line 3, column 12, byte offset 27, span length 4: INVOKE Crlf requires INCLUDE Irvine32.inc before Crlf can be used as a virtual Irvine32 routine.");
});

test("Phase 87 renders argument-bearing INVOKE Crlf diagnostic exactly", () => {
  const name = "phase87-crlf-invoke-argument";
  const source = `INCLUDE Irvine32.inc
.code
main PROC
    invoke Crlf, eax
...
`;
  ...
  assert.deepEqual(json.simulatorMessages, [
    {
      kind: "assembly-error",
      code: "invalid-irvine32-argument-count",
      message: "INVOKE Crlf takes no arguments. Remove the argument list or use CALL Crlf.",
      line: 4,
      column: 18,
      byteOffset: 54,
      spanLength: 3
  ...
  assertNoExecutionComplete(json.simulatorMessages);
  assertRenderedEquals(name, source, rawJson, rendered, "[assembly-error] invalid-irvine32-argument-count line 4, column 18, byte offset 54, span length 3: INVOKE Crlf takes no arguments. Remove the argument list or use CALL Crlf.");
});

test("Phase 87 renders bare Crlf diagnostic exactly", () => {
  const name = "phase87-bare-crlf";
  const source = `INCLUDE Irvine32.inc
.code
main PROC
    Crlf
...
`;
  ...
  assert.deepEqual(json.simulatorMessages, [
    {
      ...
      message: "Crlf is a virtual Irvine32 routine and must be called with CALL Crlf or zero-argument INVOKE Crlf.",
      line: 4,
      column: 5,
      byteOffset: 41,
      spanLength: 4
  ...
  assertNoExecutionComplete(json.simulatorMessages);
  assertRenderedEquals(name, source, rawJson, rendered, "[assembly-error] invalid-irvine32-call-form line 4, column 5, byte offset 41, span length 4: Crlf is a virtual Irvine32 routine and must be called with CALL Crlf or zero-argument INVOKE Crlf.");
});

test("Phase 89 renders WriteChar CALL without Irvine32 include diagnostic exactly", () => {
  const name = "phase88-writechar-call-without-include";
  const source = `.code
main PROC
    call WriteChar
main ENDP
...
`;
  ...
  assert.deepEqual(json.simulatorMessages, [
    {
      kind: "assembly-error",
      code: "missing-irvine32-include",
      message: "CALL WriteChar requires INCLUDE Irvine32.inc before WriteChar can be used as a virtual Irvine32 routine.",
      line: 3,
      column: 10,
      byteOffset: 25,
      spanLength: 9
  ...
  assertNoExecutionComplete(json.simulatorMessages);
  assertRenderedEquals(name, source, rawJson, rendered, "[assembly-error] missing-irvine32-include line 3, column 10, byte offset 25, span length 9: CALL WriteChar requires INCLUDE Irvine32.inc before WriteChar can be used as a virtual Irvine32 routine.");
});

test("Phase 89 renders bare WriteChar diagnostic exactly", () => {
  const name = "phase88-bare-writechar";
  const source = `INCLUDE Irvine32.inc
.code
main PROC
    WriteChar
...
`;
  ...
  assert.deepEqual(json.simulatorMessages, [
    {
      kind: "assembly-error",
      code: "invalid-irvine32-call-form",
      message: "WriteChar is a virtual Irvine32 routine and must be called with CALL WriteChar.",
      line: 4,
      column: 5,
      byteOffset: 41,
      spanLength: 9
  ...
  assertNoExecutionComplete(json.simulatorMessages);
  assertRenderedEquals(name, source, rawJson, rendered, "[assembly-error] invalid-irvine32-call-form line 4, column 5, byte offset 41, span length 9: WriteChar is a virtual Irvine32 routine and must be called with CALL WriteChar.");
});

test("Phase 89 renders WriteString CALL without Irvine32 include diagnostic exactly", () => {
  const name = "phase89-writestring-call-without-include";
  const source = `.code
main PROC
    call WriteString
main ENDP
...
`;
  ...
  assert.deepEqual(json.simulatorMessages, [
    {
      kind: "assembly-error",
      code: "missing-irvine32-include",
      message: "CALL WriteString requires INCLUDE Irvine32.inc before WriteString can be used as a virtual Irvine32 routine.",
      line: 3,
      column: 10,
      byteOffset: 25,
      spanLength: 11
  ...
  assertNoExecutionComplete(json.simulatorMessages);
  assertRenderedEquals(name, source, rawJson, rendered, "[assembly-error] missing-irvine32-include line 3, column 10, byte offset 25, span length 11: CALL WriteString requires INCLUDE Irvine32.inc before WriteString can be used as a virtual Irvine32 routine.");
});

test("Phase 89 renders bare WriteString diagnostic exactly", () => {
  const name = "phase89-bare-writestring";
  const source = `INCLUDE Irvine32.inc
.code
main PROC
    WriteString
  ...
  assert.deepEqual(json.simulatorMessages, [
    {
      kind: "assembly-error",
      code: "invalid-irvine32-call-form",
      message: "WriteString is a virtual Irvine32 routine and must be called with CALL WriteString.",
      line: 4,
      column: 5,
      byteOffset: 41,
      spanLength: 11
...
test("Phase 91 keeps successful WriteInt program text in Program Console only", () => {
  const name = "phase91-writeint-success-program-console-only";
  const source = `INCLUDE Irvine32.inc
.code
main PROC
    mov eax, 80000000h
...
`;
  const { json, rawJson, rendered } = runFixture(name, source);
  assertRunStatus(json, true, "ok");
  assert.equal(json.programConsole.text, "-2147483648");
  assert.equal(json.programConsole.byteCount, 11);
  assert.equal(json.programConsole.lineCount, 0);
  assert.deepEqual(json.simulatorMessages, [
    {
      kind: "info",
...
test("Phase 91 renders WriteInt CALL without Irvine32 include diagnostic exactly", () => {
  const name = "phase91-writeint-call-without-include";
  const source = `.code
main PROC
    call WriteInt
main ENDP
...
`;
  ...
  assert.deepEqual(json.simulatorMessages, [
    {
      kind: "assembly-error",
      code: "missing-irvine32-include",
      message: "CALL WriteInt requires INCLUDE Irvine32.inc before WriteInt can be used as a virtual Irvine32 routine.",
      line: 3,
      column: 10,
      byteOffset: 25,
      spanLength: 8
  ...
  assertNoExecutionComplete(json.simulatorMessages);
  assertRenderedEquals(name, source, rawJson, rendered, "[assembly-error] missing-irvine32-include line 3, column 10, byte offset 25, span length 8: CALL WriteInt requires INCLUDE Irvine32.inc before WriteInt can be used as a virtual Irvine32 routine.");
});

test("Phase 91 renders bare WriteInt diagnostic exactly", () => {
  const name = "phase91-bare-writeint";
  const source = `INCLUDE Irvine32.inc
.code
main PROC
    WriteInt
...
`;
  ...
  assert.deepEqual(json.simulatorMessages, [
    {
      kind: "assembly-error",
      code: "invalid-irvine32-call-form",
      message: "WriteInt is a virtual Irvine32 routine and must be called with CALL WriteInt.",
      line: 4,
      column: 5,
      byteOffset: 41,
      spanLength: 8
  ...
  assertNoExecutionComplete(json.simulatorMessages);
  assertRenderedEquals(name, source, rawJson, rendered, "[assembly-error] invalid-irvine32-call-form line 4, column 5, byte offset 41, span length 8: WriteInt is a virtual Irvine32 routine and must be called with CALL WriteInt.");
});

test("Phase 91 renders INVOKE WriteInt deferred diagnostic exactly", () => {
  const name = "phase91-invoke-writeint-deferred";
  const source = `INCLUDE Irvine32.inc
.code
main PROC
    INVOKE WriteInt
...
test("Phase 91 renders WriteInt Program Console byte-limit diagnostic exactly", () => {
  const name = "phase91-writeint-output-limit";
  const source = `INCLUDE Irvine32.inc
.code
main PROC
    mov eax, 80000000h
...
`;
  const { json, rawJson, rendered } = runFixture(name, source, {
    MASM32_DIAGNOSTIC_PROGRAM_CONSOLE_MAX_BYTES: "10",
    MASM32_DIAGNOSTIC_PROGRAM_CONSOLE_MAX_LINES: "1"
  });
  assertRunStatus(json, false, "resource-limit-exceeded");
  assert.equal(json.programConsole.text, "");
  assert.equal(json.programConsole.byteCount, 0);
  assert.equal(json.programConsole.lineCount, 0);
  assert.equal(json.programConsole.maxBytes, 10);
  assert.equal(json.programConsole.limitExceeded, true);
  assert.equal(json.programConsole.limitKind, "byte");
  assert.deepEqual(json.simulatorMessages, [
    {
  ...
  assertNoExecutionComplete(json.simulatorMessages);
  assertRenderedEquals(name, source, rawJson, rendered, `${STARTUP_STATE_NOTICE_RENDERED}\n\n[resource-limit-error] console-output-limit-exceeded line 5, column 5, byte offset 64, span length 13: Program Console output would exceed the configured byte or line limit. Execution stopped before appending partial output.`);
});

test("Phase 92 keeps successful WriteHex program text in Program Console only", () => {
  const name = "phase92-writehex-success-program-console-only";
  const source = `INCLUDE Irvine32.inc
.code
main PROC
    mov eax, 0Ah
...
`;
  ...
  assert.equal(json.phaseSuffix, "");
  assert.equal(json.phaseName, "Phase 93 - Irvine32 WriteBin");
  assert.equal(json.sourceRunOutputContract, "phase-93-irvine32-writebin-contract-v1");
  assert.equal(json.programConsole.text, "0000000A");
  assert.equal(json.programConsole.byteCount, 8);
  assert.equal(json.programConsole.lineCount, 0);
  assert.deepEqual(json.simulatorMessages, [
    {
      kind: "info",
...
test("Phase 92 renders WriteHex CALL without Irvine32 include diagnostic exactly", () => {
  const name = "phase92-writehex-call-without-include";
  const source = `.code
main PROC
    call WriteHex
main ENDP
...
`;
  ...
  assert.deepEqual(json.simulatorMessages, [
    {
      kind: "assembly-error",
      code: "missing-irvine32-include",
      message: "CALL WriteHex requires INCLUDE Irvine32.inc before WriteHex can be used as a virtual Irvine32 routine.",
      line: 3,
      column: 10,
      byteOffset: 25,
      spanLength: 8
  ...
  assertNoExecutionComplete(json.simulatorMessages);
  assertRenderedEquals(name, source, rawJson, rendered, "[assembly-error] missing-irvine32-include line 3, column 10, byte offset 25, span length 8: CALL WriteHex requires INCLUDE Irvine32.inc before WriteHex can be used as a virtual Irvine32 routine.");
});

test("Phase 92 renders bare WriteHex diagnostic exactly", () => {
  const name = "phase92-bare-writehex";
  const source = `INCLUDE Irvine32.inc
.code
main PROC
    WriteHex
...
`;
  ...
  assert.deepEqual(json.simulatorMessages, [
    {
      kind: "assembly-error",
      code: "invalid-irvine32-call-form",
      message: "WriteHex is a virtual Irvine32 routine and must be called with CALL WriteHex.",
      line: 4,
      column: 5,
      byteOffset: 41,
      spanLength: 8
  ...
  assertNoExecutionComplete(json.simulatorMessages);
  assertRenderedEquals(name, source, rawJson, rendered, "[assembly-error] invalid-irvine32-call-form line 4, column 5, byte offset 41, span length 8: WriteHex is a virtual Irvine32 routine and must be called with CALL WriteHex.");
});

test("Phase 92 renders INVOKE WriteHex deferred diagnostic exactly", () => {
  const name = "phase92-invoke-writehex-deferred";
  const source = `INCLUDE Irvine32.inc
.code
main PROC
    INVOKE WriteHex
...
test("Phase 92 renders WriteHex Program Console byte-limit diagnostic exactly", () => {
  const name = "phase92-writehex-output-limit";
  const source = `INCLUDE Irvine32.inc
.code
main PROC
    mov eax, 89ABCDEFh
...
`;
  const { json, rawJson, rendered } = runFixture(name, source, {
    MASM32_DIAGNOSTIC_PROGRAM_CONSOLE_MAX_BYTES: "7",
    MASM32_DIAGNOSTIC_PROGRAM_CONSOLE_MAX_LINES: "1"
  });
  assertRunStatus(json, false, "resource-limit-exceeded");
  assert.equal(json.programConsole.text, "");
  assert.equal(json.programConsole.byteCount, 0);
  assert.equal(json.programConsole.lineCount, 0);
  assert.equal(json.programConsole.maxBytes, 7);
  assert.equal(json.programConsole.limitExceeded, true);
  assert.equal(json.programConsole.limitKind, "byte");
  assert.deepEqual(json.simulatorMessages, [
    {
...
test("Phase 93 keeps successful WriteBin program text in Program Console only", () => {
  const name = "phase93-writebin-success-program-console-only";
  const source = `INCLUDE Irvine32.inc
.code
main PROC
    mov eax, 0Ah
...
`;
  ...
  assert.equal(json.phaseName, "Phase 93 - Irvine32 WriteBin");
  assert.equal(json.sourceRunOutputContract, "phase-93-irvine32-writebin-contract-v1");
  assert.equal(json.programConsole.text, "00000000000000000000000000001010");
  assert.equal(json.programConsole.byteCount, 32);
  assert.equal(json.programConsole.lineCount, 0);
  assert.deepEqual(json.simulatorMessages, [
    {
      kind: "info",
...
test("Phase 93 renders WriteBin CALL without Irvine32 include diagnostic exactly", () => {
  const name = "phase93-writebin-call-without-include";
  const source = `.code
main PROC
    call WriteBin
main ENDP
...
`;
  ...
  assert.deepEqual(json.simulatorMessages, [
    {
      kind: "assembly-error",
      code: "missing-irvine32-include",
      message: "CALL WriteBin requires INCLUDE Irvine32.inc before WriteBin can be used as a virtual Irvine32 routine.",
      line: 3,
      column: 10,
      byteOffset: 25,
      spanLength: 8
  ...
  assertNoExecutionComplete(json.simulatorMessages);
  assertRenderedEquals(name, source, rawJson, rendered, "[assembly-error] missing-irvine32-include line 3, column 10, byte offset 25, span length 8: CALL WriteBin requires INCLUDE Irvine32.inc before WriteBin can be used as a virtual Irvine32 routine.");
});

test("Phase 93 renders bare WriteBin diagnostic exactly", () => {
  const name = "phase93-bare-writebin";
  const source = `INCLUDE Irvine32.inc
.code
main PROC
    WriteBin
...
`;
  ...
  assert.deepEqual(json.simulatorMessages, [
    {
      kind: "assembly-error",
      code: "invalid-irvine32-call-form",
      message: "WriteBin is a virtual Irvine32 routine and must be called with CALL WriteBin.",
      line: 4,
      column: 5,
      byteOffset: 41,
      spanLength: 8
  ...
  assertNoExecutionComplete(json.simulatorMessages);
  assertRenderedEquals(name, source, rawJson, rendered, "[assembly-error] invalid-irvine32-call-form line 4, column 5, byte offset 41, span length 8: WriteBin is a virtual Irvine32 routine and must be called with CALL WriteBin.");
});

test("Phase 93 renders INVOKE WriteBin deferred diagnostic exactly", () => {
  const name = "phase93-invoke-writebin-deferred";
  const source = `INCLUDE Irvine32.inc
.code
main PROC
    INVOKE WriteBin
...
test("Phase 93 renders WriteBin Program Console byte-limit diagnostic exactly", () => {
  const name = "phase93-writebin-output-limit";
  const source = `INCLUDE Irvine32.inc
.code
main PROC
    mov eax, 89ABCDEFh
...
`;
  const { json, rawJson, rendered } = runFixture(name, source, {
    MASM32_DIAGNOSTIC_PROGRAM_CONSOLE_MAX_BYTES: "31",
    MASM32_DIAGNOSTIC_PROGRAM_CONSOLE_MAX_LINES: "1"
  });
  assertRunStatus(json, false, "resource-limit-exceeded");
  assert.equal(json.programConsole.text, "");
  assert.equal(json.programConsole.byteCount, 0);
  assert.equal(json.programConsole.lineCount, 0);
  assert.equal(json.programConsole.maxBytes, 31);
  assert.equal(json.programConsole.limitExceeded, true);
  assert.equal(json.programConsole.limitKind, "byte");
  assert.deepEqual(json.simulatorMessages, [
    {
...
`;
  ...
  assert.deepEqual(json.simulatorMessages, [
    {
      ...
      message: "Invalid memory read at 00000000h for 1 byte. The address is outside the simulator's configured memory regions.",
      line: 5,
      column: 5,
      byteOffset: 56,
      spanLength: 16
  ...
  assertNoExecutionComplete(json.simulatorMessages);
  assertRenderedEquals(name, source, rawJson, rendered, "[runtime-error] invalid-memory-read line 5, column 5, byte offset 56, span length 16: Invalid memory read at 00000000h for 1 byte. The address is outside the simulator's configured memory regions.");
});

test("Phase 89 renders WriteString uninitialized-read warning exactly", () => {
  const name = "phase89-writestring-uninitialized-warning";
  const source = `INCLUDE Irvine32.inc
.DATA?
msg BYTE ?
.code
...
`;
  ...
  assert.deepEqual(json.simulatorMessages, [
    {
      ...
      message: "Memory read range 00500000h..00500000h reads 1 byte from msg + 0; 1 of those byte still originated from uninitialized storage.",
      line: 7,
      column: 5,
      byteOffset: 83,
      spanLength: 16,
      ...
      accessEndAddress: "00500000h",
      accessSizeBytes: 1,
      uninitializedByteCount: 1,
  ...
  assertRenderedEquals(name, source, rawJson, rendered, "[simulator-warning] uninitialized-read line 7, column 5, byte offset 83, span length 16: Memory read range 00500000h..00500000h reads 1 byte from msg + 0; 1 of those byte still originated from uninitialized storage.\n\n[info] execution-complete: Execution completed successfully.");
});

test("Phase 89 renders WriteString strict uninitialized-read failure exactly", () => {
  const name = "phase89-writestring-uninitialized-strict";
  const source = `INCLUDE Irvine32.inc
.DATA?
msg BYTE ?
.code
...
`;
  ...
  assertNoExecutionComplete(json.simulatorMessages);
  assertRenderedEquals(name, source, rawJson, rendered, "[runtime-error] uninitialized-read line 7, column 5, byte offset 83, span length 16: Memory read range 00500000h..00500000h reads 1 byte from msg + 0; 1 of those byte still originated from uninitialized storage.");
});

test("Phase 89 renders WriteString scan-limit failure exactly", () => {
  const name = "phase89-writestring-scan-limit";
  const source = `INCLUDE Irvine32.inc
.data
msg BYTE "abc",0
.code
...
`;
  const { json, rawJson, rendered } = runFixture(name, source, {
    MASM32_DIAGNOSTIC_WRITESTRING_SCAN_LIMIT: "3"
  });
  assertRunStatus(json, false, "execution-error");
  assert.equal(json.programConsole.text, "");
  assert.deepEqual(json.simulatorMessages, [
    ...
    {
      kind: "runtime-error",
      code: "string-scan-limit-exceeded",
      message: "WriteString scanned the configured byte limit without reading a null terminator. Execution stopped before appending partial output.",
      line: 7,
      column: 5,
      byteOffset: 88,
      spanLength: 16
  ...
  assertNoExecutionComplete(json.simulatorMessages);
  assertRenderedEquals(name, source, rawJson, rendered, `${STARTUP_STATE_NOTICE_RENDERED}\n\n[runtime-error] string-scan-limit-exceeded line 7, column 5, byte offset 88, span length 16: WriteString scanned the configured byte limit without reading a null terminator. Execution stopped before appending partial output.`);
});
...
test("renders Phase 71E default selected-entry code-stream fallthrough error exactly", () => {
...
test("renders Phase 71E stop-at-entry-end selected-entry completion without procedure fallthrough", () => {
  const name = "phase71eStopAtEntryEndSelectedEntryCompletion";
  const source = `.code
main PROC
    mov eax, 1
main ENDP
...
`;
  ...
  assert.equal(defaultResult.json.phase, 93);
  assert.equal(defaultResult.json.phaseSuffix, "");
  assert.equal(defaultResult.json.phaseName, "Phase 93 - Irvine32 WriteBin");
  ...
  assert.equal(stopResult.json.phase, 93);
  assert.equal(stopResult.json.phaseSuffix, "");
  assert.equal(stopResult.json.phaseName, "Phase 93 - Irvine32 WriteBin");
...
test("renders Phase 71E default front notice after code-stream falloff error exactly", () => {
...
test("renders Phase 71E default front notice only for exact case-insensitive procedure names", () => {
...
test("renders Phase 80 LOCAL object-bounds runtime error exactly", () => {
  const name = "phase80LocalObjectBounds";
  const source = `.code
main PROC
    LOCAL buf[2]:BYTE
...
`;
  ...
  assert.deepEqual(json.simulatorMessages, [
    {
      ...
      message: "Memory write escapes LOCAL main.buf at byte offset 1 for 4 bytes; LOCAL size is 2 bytes.",
      line: 4,
      column: 19,
      byteOffset: 56,
      spanLength: 3
  ...
  assertRenderedEquals(name, source, rawJson, rendered, "[runtime-error] object-bounds-violation line 4, column 19, byte offset 56, span length 3: Memory write escapes LOCAL main.buf at byte offset 1 for 4 bytes; LOCAL size is 2 bytes.");
});

test("renders Phase 80 LOCAL uninitialized-read warning exactly", () => {
  const name = "phase80LocalUninitializedWarning";
  const source = `.code
main PROC
    LOCAL temp:DWORD
    mov eax, temp
...
`;
  ...
  assert.deepEqual(json.simulatorMessages, [
    {
      kind: "simulator-warning",
      code: "uninitialized-read",
      message: "Memory read uses 4 bytes from LOCAL temp + 0; 4 of those bytes still originated from uninitialized storage.",
      line: 4,
      column: 14,
      byteOffset: 50,
      spanLength: 4,
      ...
      symbolName: "temp",
      accessStartAddress: "00000000h",
      accessEndAddress: "00000003h",
      accessSizeBytes: 4,
      uninitializedByteCount: 4,
      initializedByteCount: 0,
      accessByteOffset: 0
  ...
  assertRenderedEquals(name, source, rawJson, rendered, "[simulator-warning] uninitialized-read line 4, column 14, byte offset 50, span length 4: Memory read uses 4 bytes from LOCAL temp + 0; 4 of those bytes still originated from uninitialized storage.\n\n[info] execution-complete: Execution completed successfully.");
});

test("renders Phase 80 out-of-scope LOCAL token as unknown symbol exactly", () => {
  const name = "phase80OutOfScopeLocalToken";
  const source = `.code
main PROC
    LOCAL temp:DWORD
    ret
...
`;
  ...
  assertRenderedEquals(name, source, rawJson, rendered, "[assembly-error] unknown-symbol line 7, column 14, byte offset 79, span length 4: Unknown data symbol.");
});
...
[assembly-error] invoke-argument-width-unsupported line 10, column 22, byte offset 99, span length 2: Phase 84 INVOKE accepts only full 32-bit register arguments; use explicit extension before INVOKE for 8-bit or 16-bit aliases.
[unsupported-feature] unsupported-high-level-if line 11, column 5, byte offset 106, span length 3: .IF high-level MASM flow is not implemented; the simulator does not lower high-level conditions into labels or branches.
[unsupported-feature] unsupported-high-level-endif line 13, column 5, byte offset 142, span length 6: .ENDIF closes unsupported high-level MASM flow; the simulator does not lower high-level conditions into labels or branches.`);
});
...
test("Phase 57T renders realistic playground diagnostics exactly", () => {
  ...
  assertRunStatus(json, false, "parse-error");
  assertNoExecutionComplete(json.simulatorMessages);
  assertEmptyProgramConsole(json);
...
test("Phase 77 preserves invalid PROC USES register diagnostic", () => {
  const name = "phase76InvalidProcUsesRegister";
  const source = `.code
...
`;
  ...
  assertMessageEquals(json.simulatorMessages[0], {
    kind: "assembly-error",
    code: "invalid-proc-uses-register",
    message: "Invalid PROC USES register `esp`; accepted PROC USES registers are EAX, EBX, ECX, EDX, ESI, and EDI. ESP, EBP, 8-bit aliases, 16-bit aliases, punctuation, and unknown names are not accepted in PROC USES metadata.",
    line: 2,
    column: 18,
    byteOffset: 23,
    spanLength: 3
...
test("Phase 77 renders PROC USES save overflow diagnostic", () => {
  const name = "phase77ProcUsesSaveOverflow";
  const source = `INCLUDE Irvine32.inc
.code
main PROC
    mov esp, 008F0008h
...
`;
  ...
  assertMessageEquals(json.simulatorMessages[0], {
    kind: "runtime-error",
    code: "stack-overflow",
    message: "Automatic PROC USES register save could not reserve or write the required stack slots. Execution stopped before entering the procedure.",
    line: 5,
    column: 5,
    byteOffset: 64,
    spanLength: 11,
...
test("Phase 77 renders PROC USES restore underflow diagnostic", () => {
  const name = "phase77ProcUsesRestoreUnderflow";
  const source = `INCLUDE Irvine32.inc
.code
main PROC
    call Helper
...
`;
  ...
  assertMessageEquals(json.simulatorMessages[0], {
    ...
    message: "Automatic PROC USES register restore could not read the saved register slots. Execution stopped before restoring registers or popping the return token.",
    line: 10,
    column: 5,
    byteOffset: 139,
    spanLength: 3,
  ...
  assertRenderedEquals(name, source, rawJson, rendered, `[runtime-error] stack-underflow line 10, column 5, byte offset 139, span length 3, procedure Helper: Automatic PROC USES register restore could not read the saved register slots. Execution stopped before restoring registers or popping the return token.`);
});

test("Phase 74 renders RET register operand diagnostic without milestone-relative wording", () => {
  const name = "phase74-ret-register-operand";
  const source = `.code
main PROC
    ret eax
main ENDP
...
`;
  ...
  assertMessageEquals(json.simulatorMessages[0], {
    ...
    message: "RET accepts only no operand or an unsigned 16-bit immediate byte count; register, memory, and far-return operands are unsupported.",
    line: 3,
    column: 9,
    byteOffset: 24,
    spanLength: 3
  ...
  assertNoExecutionComplete(json.simulatorMessages);
  assertRenderedEquals(name, source, rawJson, rendered, "[assembly-error] unsupported-instruction-form line 3, column 9, byte offset 24, span length 3: RET accepts only no operand or an unsigned 16-bit immediate byte count; register, memory, and far-return operands are unsupported.");
  assertNoPhase71bForbiddenDiagnosticWording(name, rawJson, rendered);
});

test("Phase 74 renders RET imm16 invalid immediate diagnostic", () => {
  const name = "phase74-ret-imm16-invalid-immediate";
  const source = `.code
main PROC
    ret 10000h
main ENDP
...
`;
  ...
  assertMessageEquals(json.simulatorMessages[0], {
    ...
    message: "RET immediate cleanup must be an unsigned 16-bit byte count in the range 0..65535.",
    line: 3,
    column: 9,
    byteOffset: 24,
    spanLength: 6
  ...
  assertNoExecutionComplete(json.simulatorMessages);
  assertRenderedEquals(name, source, rawJson, rendered, "[assembly-error] number-out-of-range line 3, column 9, byte offset 24, span length 6: RET immediate cleanup must be an unsigned 16-bit byte count in the range 0..65535.");
});
...
`;
  ...
  assertMessageEquals(json.simulatorMessages[0], {
    kind: "assembly-error",
    code: "unsupported-instruction-form",
    message: "Far RET forms are not implemented. This simulator accepts only near RET with no operand or an unsigned 16-bit immediate byte count.",
    line: 3,
    column: 5,
    byteOffset: 20,
    spanLength: 4
  ...
  assertNoExecutionComplete(json.simulatorMessages);
  assertRenderedEquals(name, source, rawJson, rendered, "[assembly-error] unsupported-instruction-form line 3, column 5, byte offset 20, span length 4: Far RET forms are not implemented. This simulator accepts only near RET with no operand or an unsigned 16-bit immediate byte count.");
  assertNoPhase71bForbiddenDiagnosticWording(name, rawJson, rendered);
});
...
`;
  ...
  assert.deepEqual(json.simulatorMessages, [
    {
      ...
      message: "Procedure `helper` conflicts with prior procedure `Helper` because user-defined symbols are case-insensitive under the active CASEMAP policy; first defined at line 2, column 1.",
      line: 4,
      column: 1,
      byteOffset: 30,
      spanLength: 6
  ...
  assertNoExecutionComplete(json.simulatorMessages);
  assertRenderedEquals(name, source, rawJson, rendered, "[assembly-error] duplicate-procedure line 4, column 1, byte offset 30, span length 6: Procedure `helper` conflicts with prior procedure `Helper` because user-defined symbols are case-insensitive under the active CASEMAP policy; first defined at line 2, column 1.");
});
...
test("Phase 71A renders root RET completion, strict-mode rejection, helper fallthrough, loop, and WinAPI diagnostics exactly", () => {
  const retName = "phase71RootRetCompletes";
  const retSource = fixtureSource(retName);
  const retResult = runFixture(retName, retSource);
  assertRunStatus(retResult.json, true, "ok");
  assert.deepEqual(retResult.json.memoryChanges, []);
  assert.equal(retResult.json.simulatorMessages.some((message) => message.code === "invalid-address"), false);
  assert.equal(retResult.json.simulatorMessages.some((message) => message.code === "invalid-return-address"), false);
  assertRenderedEquals(retName, retSource, retResult.rawJson, retResult.rendered, "[info] execution-complete: Execution completed successfully.");

  const strictRootName = "phase71aStrictRootRetDiagnostic";
  const strictRootSource = `.code
main PROC
    ret
main ENDP
END main
`;
  const strictRootResult = runFixture(strictRootName, strictRootSource, { MASM32_DIAGNOSTIC_ROOT_RET_MODE: "strict-call-frame" });
  assertRunStatus(strictRootResult.json, false, "execution-error");
  assert.equal(strictRootResult.json.rootRetMode, "strict-call-frame");
  assert.deepEqual(strictRootResult.json.memoryChanges, []);
...
test("Phase 74 renders RET imm16 runtime cleanup diagnostic", () => {
  const name = "phase74-ret-imm16-cleanup-runtime";
  const source = `INCLUDE Irvine32.inc
.code
main PROC
    push 1234h
...
`;
  ...
  assertRunStatus(json, false, "execution-error");
  assertNoExecutionComplete(json.simulatorMessages);
  assertRenderedEquals(name, source, rawJson, rendered, `[runtime-error] ret-stack-cleanup-out-of-range line 9, column 5, byte offset 103, span length 9: RET imm16 cleanup would leave ESP outside the active stack region or empty-stack boundary. Execution stopped before changing ESP or transferring control.`);
});
...
`;
  ...
  assert.equal(rawJson.includes("\"programConsole\":{\"text\":\"\",\"truncated\":false,\"byteCount\":0,\"lineCount\":0,\"maxBytes\":1048576,\"maxLines\":10000,\"limitExceeded\":false,\"limitKind\":null}"), true, rawJson);
...
test("Phase 73 renders LEAVE diagnostics", () => {
  const malformedName = "phase73-leave-malformed-diagnostic";
  const malformedSource = `.code
main PROC
    leave eax
main ENDP
END main
`;
  const malformedFixture = runFixture(malformedName, malformedSource);
  assertRunStatus(malformedFixture.json, false, "parse-error");
  assertMessageEquals(malformedFixture.json.simulatorMessages[0], {
    kind: "assembly-error",
    code: "invalid-instruction-operands",
    message: "LEAVE does not take operands.",
    line: 3,
    column: 11,
    byteOffset: 26,
    spanLength: 3
  ...
  assertRenderedEquals(
    malformedName,
    malformedSource,
    malformedFixture.rawJson,
    malformedFixture.rendered,
    "[assembly-error] invalid-instruction-operands line 3, column 11, byte offset 26, span length 3: LEAVE does not take operands."
  );

  const invalidReadName = "phase73-leave-invalid-ebp-diagnostic";
  const invalidReadSource = `INCLUDE Irvine32.inc
.code
main PROC
    mov ebp, 0
    leave
...
END main
`;
  const invalidReadFixture = runFixture(invalidReadName, invalidReadSource);
  assertRunStatus(invalidReadFixture.json, false, "execution-error");
  assertMessageEquals(invalidReadFixture.json.simulatorMessages[0], {
    kind: "runtime-error",
    code: "invalid-address",
    message: "Invalid memory read at 00000000h for 4 bytes. The address is outside the simulator's configured memory regions.",
    line: 5,
  ...
  assertNoExecutionComplete(invalidReadFixture.json.simulatorMessages);
  assert.equal(invalidReadFixture.rawJson.includes("\"memoryChanges\":[]"), true);
  assertRenderedEquals(
    invalidReadName,
    invalidReadSource,
    invalidReadFixture.rawJson,
    invalidReadFixture.rendered,
    "[runtime-error] invalid-address line 5, column 5, byte offset 56, span length 5: Invalid memory read at 00000000h for 4 bytes. The address is outside the simulator's configured memory regions."
  );
});
