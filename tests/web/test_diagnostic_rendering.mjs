  ...
  const rendered = formatSimulatorMessages([
    {
      ...
      message: "The loaded Wasm artifact reports runtime/source-run MASM behavior Phase 71, but the UI/source files expect Phase 87 - Irvine32 Crlf. Rebuild web/dist with the Emscripten build script."
    }
  ]);

  assert.equal(
    rendered,
    "[internal-simulator-error] stale-wasm-artifact: The loaded Wasm artifact reports runtime/source-run MASM behavior Phase 71, but the UI/source files expect Phase 87 - Irvine32 Crlf. Rebuild web/dist with the Emscripten build script."
  );
});
...
`;
  ...
  assert.equal(json.phaseName, "Phase 87 - Irvine32 Crlf");
...
test("renders Phase 58 duplicate and conflicting code-label diagnostics exactly", () => {
  ...
  assert.equal(duplicateResult.json.phase, 87);
...
`;
  ...
  assert.equal(defaultResult.json.phase, 87);
  assert.equal(defaultResult.json.phaseSuffix, "");
  assert.equal(defaultResult.json.phaseName, "Phase 87 - Irvine32 Crlf");
  ...
  assert.equal(stopResult.json.phase, 87);
  assert.equal(stopResult.json.phaseSuffix, "");
  assert.equal(stopResult.json.phaseName, "Phase 87 - Irvine32 Crlf");
