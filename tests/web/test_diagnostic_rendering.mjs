  ...
  const rendered = formatSimulatorMessages([
    {
      ...
      message: "The loaded Wasm artifact reports runtime/source-run MASM behavior Phase 71, but the UI/source files expect Phase 76 - PROC USES Parsing and Metadata. Rebuild web/dist with the Emscripten build script."
    }
  ]);

  assert.equal(
    rendered,
    "[internal-simulator-error] stale-wasm-artifact: The loaded Wasm artifact reports runtime/source-run MASM behavior Phase 71, but the UI/source files expect Phase 76 - PROC USES Parsing and Metadata. Rebuild web/dist with the Emscripten build script."
  );
});
...
`;
  ...
  assert.equal(json.phaseName, "Phase 76 - PROC USES Parsing and Metadata");
...
test("renders Phase 58 duplicate and conflicting code-label diagnostics exactly", () => {
  ...
  assert.equal(duplicateResult.json.phase, 76);
...
`;
  ...
  assert.equal(defaultResult.json.phase, 76);
  assert.equal(defaultResult.json.phaseSuffix, "");
  assert.equal(defaultResult.json.phaseName, "Phase 76 - PROC USES Parsing and Metadata");
  ...
  assert.equal(stopResult.json.phase, 76);
  assert.equal(stopResult.json.phaseSuffix, "");
  assert.equal(stopResult.json.phaseName, "Phase 76 - PROC USES Parsing and Metadata");
...
test("Phase 76 renders invalid PROC USES register diagnostic", () => {
...
test("Phase 76 renders called PROC USES runtime deferral diagnostic", () => {
  const name = "phase76CalledProcUsesRuntimeDeferral";
  const source = `.code
main PROC
    call Helper
    ret
...
`;
  ...
  assertMessageEquals(json.simulatorMessages[0], {
    kind: "runtime-error",
    code: "unsupported-proc-uses-runtime",
    message: "PROC USES metadata parsed successfully, but runtime register save/restore begins in Phase 77. Execution stopped before entering the procedure so the simulator does not silently ignore USES.",
    line: 3,
    column: 5,
    byteOffset: 20,
    spanLength: 11
  });
  assertRenderedEquals(name, source, rawJson, rendered, "[runtime-error] unsupported-proc-uses-runtime line 3, column 5, byte offset 20, span length 11: PROC USES metadata parsed successfully, but runtime register save/restore begins in Phase 77. Execution stopped before entering the procedure so the simulator does not silently ignore USES.");
});
