...
static int test_signed_ptr_width_aliases_source_run_programs(void) {
    ...
    failures += expect_json_contains(read_copy, "\"phase\":88", "signed PTR read response should identify current runtime phase");
...
static int test_additional_data_sections_layout_and_const_protection(void) {
    ...
    failures += expect_json_contains(acceptance_json, "\"phase\":88", "additional data source-run should identify current runtime phase");
