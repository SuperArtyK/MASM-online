...
static int test_signed_ptr_width_aliases_source_run_programs(void) {
    ...
    failures += expect_json_contains(read_copy, "\"phase\":68", "signed PTR read response should identify Phase 68");
...
static int test_additional_data_sections_layout_and_const_protection(void) {
    ...
    failures += expect_json_contains(acceptance_json, "\"phase\":68", "additional data source-run should identify Phase 68");
    ...
    failures += expect_json_contains(const_uninitialized_json, "\"phaseSuffix\":\"B\"", ".CONST uninitialized source-run should report the current Phase 68B runtime phase suffix");
