...
static int test_signed_ptr_width_aliases_source_run_programs(void) {
    ...
    failures += expect_json_contains(read_copy, "\"phase\":65", "signed PTR read response should identify Phase 65");
...
static int test_additional_data_sections_layout_and_const_protection(void) {
    ...
    failures += expect_json_contains(acceptance_json, "\"phase\":65", "additional data source-run should identify Phase 65");
    ...
    failures += expect_json_contains(const_uninitialized_json, "\"phaseSuffix\":\"\"", ".CONST uninitialized source-run should report the current Phase 65 runtime phase suffix");
