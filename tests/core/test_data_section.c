...
static int test_signed_ptr_width_aliases_source_run_programs(void) {
    ...
    failures += expect_json_contains(read_copy, "\"phase\":71", "signed PTR read response should identify current runtime phase");
...
static int test_additional_data_sections_layout_and_const_protection(void) {
    ...
    failures += expect_json_contains(acceptance_json, "\"phase\":71", "additional data source-run should identify current runtime phase");
    ...
    failures += expect_json_contains(const_uninitialized_json, "\"phaseSuffix\":\"E\"", ".CONST uninitialized source-run should report the current runtime phase suffix");
