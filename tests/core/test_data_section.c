...
static int test_signed_ptr_width_aliases_source_run_programs(void) {
    ...
    failures += expect_json_contains(read_copy, "\"phase\":64", "signed PTR read response should identify Phase 64");
...
static int test_additional_data_sections_layout_and_const_protection(void) {
    ...
    failures += expect_json_contains(acceptance_json, "\"phase\":64", "additional data source-run should identify Phase 64");
    ...
    failures += expect_json_contains(const_uninitialized_json, "\"phaseSuffix\":\"A\"", ".CONST uninitialized source-run should report the current Phase 64A runtime phase suffix");
