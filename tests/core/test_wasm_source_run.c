 ...
 * @brief Tests for the Wasm-facing source execution API through Phase 93 Irvine32 WriteBin.
 *
...
#define TEST_SOURCE_RUN_OUTPUT_CONTRACT_FRAGMENT "\"sourceRunOutputContract\":\"phase-93-irvine32-writebin-contract-v1\""

/// Exact empty Program Console object expected for source-run paths that emit no Program Console output.
#define TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT \
    "\"programConsole\":{\"text\":\"\",\"truncated\":false,\"byteCount\":0,\"lineCount\":0," \
    "\"maxBytes\":1048576,\"maxLines\":10000,\"limitExceeded\":false,\"limitKind\":null}"
...
static int test_minimal_source_runs_to_eax_42(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "response should identify current runtime phase");
    ...
    failures += expect_json_not_contains(json, "\"message\":\"Program Console", "source-run JSON should not merge Program Console text into simulator messages");
...
static int test_phase24_eax_base_acceptance_program(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "Phase 26 EAX-base response should identify current runtime phase");
...
static int test_all_gpr_register_indirect_source_run_succeeds(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "all-GPR response should identify current runtime phase");
...
static int test_phase22_source_run_acceptance_program(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "Phase 22 response should identify current runtime phase");
...
static int test_textbook_unsupported_features_return_unsupported_feature_messages(void) {
    ...
    failures += expect_json_contains(invoke_json, "\"kind\":\"unsupported-feature\"", "INVOKE ExitProcess should be classified as unsupported feature");
    failures += expect_json_contains(invoke_json, "unsupported-external-invoke", "INVOKE ExitProcess should expose the Phase 82 external INVOKE code");
    failures += expect_json_contains(invoke_json, "Windows/API", "INVOKE ExitProcess message should describe external/API behavior");
...
static int test_multi_diagnostic_unsupported_feature_source_run_reports_all(void) {
    ...
    failures += expect_json_contains(json, "invoke-argument-width-unsupported", "source-run should include INVOKE argument width diagnostic");
    failures += expect_json_contains(json, "only full 32-bit register arguments", "source-run should include INVOKE argument diagnostic text");
    ...
    failures += expect_json_contains(json, "\"byteOffset\":99", "INVOKE argument diagnostic byte offset should be surfaced");
    failures += expect_json_contains(json, "\"byteOffset\":106", ".IF diagnostic byte offset should be surfaced");
    failures += expect_json_contains(json, "\"spanLength\":6", "six-byte recovered diagnostic spans should be surfaced");
    failures += expect_json_contains(json, "\"spanLength\":2", "INVOKE alias span should be surfaced");
...
static int test_phase57s_high_level_flow_source_run_diagnostics(void) {
    ...
    failures += expect_json_contains(json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "high-level flow diagnostics should not produce Program Console output");
...
static int test_signed_ptr_alias_source_run_acceptance_program(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "signed PTR alias response should identify current runtime phase");
...
static int test_phase20_source_run_acceptance_program(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "source-run response should identify current runtime phase");
...
static int test_phase57n_nop_source_run_hardening(void) {
    ...
    failures += expect_json_contains(between_copy, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "NOP source-run program should not produce Program Console output");
    ...
    failures += expect_json_contains(only_copy, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "NOP-only program should not produce Program Console output");
...
static int test_phase57o_nop_encoding_operand_source_run(void) {
    ...
    failures += expect_json_contains(invalid_address_copy, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "Phase 57O NOP encoding operand should not produce Program Console output");
...
static int test_phase57o_nop_source_run_rejections(void) {
    ...
    for (index = 0U; index < sizeof(invalid_cases) / sizeof(invalid_cases[0]); index += 1U) {
        ...
        failures += expect_json_contains(json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "Phase 57O invalid NOP form should not produce Program Console output");
    }
...
static int test_phase25_register_supplied_memory_width_source_run_program(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "Phase 26 source-run response should identify current runtime phase");
...
static int test_phase25_register_supplied_source_memory_width_source_run_program(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "Phase 26 source-memory response should identify current runtime phase");
...
static int test_phase25_explicit_ptr_symbol_register_override_source_run_program(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "Explicit PTR override source-run response should identify current runtime phase");
...
static int test_phase26_header_source_run_acceptance_program(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "Phase 26 header response should identify current runtime phase");
...
static int test_phase26_header_source_run_error_paths(void) {
    ...
    failures += expect_json_contains(option_copy, "nokeyword-protected-keyword", "Protected OPTION NOKEYWORD word should expose targeted code");
...
static int test_phase57q_includelib_source_run_diagnostics(void) {
    ...
    failures += expect_json_contains(masm32_copy, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "Phase 57Q INCLUDELIB should not produce Program Console output");
...
static int test_phase28_additional_data_sections_source_run_programs(void) {
    ...
    failures += expect_json_contains(acceptance_json, "\"phase\":93", "Phase 30 response should identify current runtime phase");
...
static int test_phase30_dup_initializer_list_source_run_program(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "DUP initializer list source should report current runtime/source-run MASM behavior phase metadata");
...
static int test_phase61d_token_capacity_diagnostic_source_run_program(void) {
    ...
    failures += expect_json_contains(json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "token-capacity source should not produce Program Console output");
...
static int test_phase61d_source_text_capacity_diagnostic_source_run_program(void) {
    ...
    failures += expect_json_contains(json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "source-text capacity source should not produce Program Console output");
...
static int test_phase61d_code_label_capacity_diagnostic_source_run_program(void) {
    ...
    failures += expect_json_contains(json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "code-label capacity source should not produce Program Console output");
...
static int test_phase41_irvine32_virtual_include_metadata_source_run(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "Irvine32 virtual include response should report current runtime/source-run MASM behavior phase metadata");
    ...
    failures += expect_json_not_contains(json, "\"message\":\"Program Console", "Phase 41 must not add Program Console routine behavior");
...
static int test_phase42_irvine32_exit_terminator_source_run(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "Irvine32 exit response should report current runtime/source-run MASM behavior phase metadata");
    ...
    failures += expect_json_contains(json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "exit must not create Program Console output");
...
static int test_phase43_inc_dec_register_source_run_program(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "Phase 43 INC/DEC response should report current runtime metadata");
...
static int test_phase44_logical_binary_source_run_program(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "Phase 44 logical response should report current runtime metadata");
...
static int test_phase45_not_source_run_program(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "Milestone 45 NOT regression response should report current runtime/source-run MASM behavior phase metadata");
...
static int test_phase46_shift_left_source_run_program(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "Phase 46 SHL/SAL response should report current runtime metadata");
...
static int test_phase47_shr_source_run_program(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "Phase 47 SHR regression should report current runtime/source-run MASM behavior phase metadata");
...
static int test_phase48_sar_source_run_program(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "Phase 48 SAR response should report current runtime metadata");
...
static int test_phase49_rol_source_run_program(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "Phase 49 ROL response should report current runtime metadata");
...
static int test_phase50_ror_source_run_program(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "Phase 50 ROR response should report current runtime metadata");
...
static int test_phase52_lea_source_run_program(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "Phase 52 LEA response should report current runtime metadata");
...
static int test_phase57corr2_compact_negative_displacement_write_source_run(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "Phase 57-CORR2 write response should report current numeric runtime metadata");
...
static int test_phase57corr2_compact_negative_displacement_read_source_run(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "Phase 57-CORR2 read response should report current numeric runtime metadata");
...
static int test_phase57corr2_compact_negative_displacement_lea_source_run(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "Phase 57-CORR2 LEA response should report current numeric runtime metadata");
...
static int test_phase57corr2_compact_negative_displacement_advanced_rejection_source_run(void) {
    ...
    failures += expect_json_contains(json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "advanced compact displacement expression should not produce Program Console output");
...
static int test_phase53_mul_source_run_program(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "Phase 53 MUL response should report current runtime metadata");
...
static int test_phase56_div_source_run_programs(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "Phase 56 DIV response should report current runtime metadata");
...
static int test_phase57_idiv_source_run_programs(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "Phase 57 IDIV response should report runtime metadata");
...
static int test_phase54_imul_source_run_program(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "Phase 54 IMUL response should report current runtime metadata");
...
static int test_phase55_imul_source_run_programs(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "Phase 55 IMUL response should report current runtime metadata");
...
static int test_phase57e_default_startup_state_notice_source_run(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "Startup-state notice regression should report current numeric phase metadata");
    ...
    failures += expect_json_not_contains(json, "\"message\":\"Program Console", "Startup-state notice must not be Program Console output");
...
static int test_phase57h_register_write_metadata_no_register_writes(void) {
    ...
    failures += expect_json_contains(json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "Phase 57H metadata should not add Program Console output");
...
static int test_phase57h_register_write_metadata_same_value_parent_write(void) {
    ...
    failures += expect_json_contains(json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "Same-value write metadata should not add Program Console output");
...
/// Verifies Phase 82 INVOKE argument and external routine diagnostics through source-run JSON.
///
/// @return Number of failures.
static int test_phase82_invoke_addr_external_routine_source_run_diagnostics(void) {
    ...
    failures += expect_json_contains(stdout_copy, "unsupported-external-invoke", "StdOut source should expose unsupported-external-invoke");
    failures += expect_json_contains(stdout_copy, "unsupported-external-invoke", "StdOut source should not report legacy ADDR diagnostic independently");
    failures += expect_json_contains(stdout_copy, "unsupported-external-invoke", "StdOut source should expose external INVOKE code");
    ...
    failures += expect_json_contains(stdout_copy, "\"column\":12", "StdOut source should point at external target token");
    failures += expect_json_not_contains(stdout_copy, "execution-complete", "StdOut INVOKE should not execute");
    failures += expect_json_contains(stdout_copy, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "StdOut INVOKE should not produce Program Console output");

    failures += expect_json_contains(crt_copy, "unsupported-external-invoke", "CRT source should expose external INVOKE code");
    failures += expect_json_contains(crt_copy, "C runtime", "CRT source should describe C runtime boundary");
    failures += expect_json_contains(crt_copy, "unsupported-external-invoke", "CRT source should not report legacy ADDR diagnostic independently");
    failures += expect_json_not_contains(crt_copy, "unknown-symbol", "CRT INVOKE should not cascade into unknown symbols");

    failures += expect_json_contains(exitprocess_copy, "unsupported-external-invoke", "ExitProcess INVOKE should expose external INVOKE code");
    failures += expect_json_contains(exitprocess_copy, "Windows/API", "ExitProcess message should describe external/API behavior");
    ...
    failures += expect_json_contains(multi_copy, "unsupported-external-invoke", "multi INVOKE should include external INVOKE diagnostics");
    failures += expect_json_contains(multi_copy, "unsupported-external-invoke", "multi INVOKE should include CRT as external INVOKE diagnostic");
    failures += expect_json_contains(multi_copy, "unsupported-external-invoke", "multi INVOKE should include WinAPI as external INVOKE diagnostic");
    ...
    failures += expect_json_not_contains(virtual_exit_copy, "unsupported-external-invoke", "Virtual exit should not be classified as external INVOKE");
...
static int test_phase57t_playground_diagnostic_recovery_smoke_fixtures(void) {
    ...
    failures += expect_json_contains(playground_copy, "unsupported-external-invoke", "Phase 57T playground fixture should classify external INVOKE");
    failures += expect_json_contains(playground_copy, "unsupported-external-invoke", "Phase 57T playground fixture should not rely on legacy ADDR diagnostics for INVOKE arguments");
    failures += expect_json_contains(playground_copy, "unsupported-external-invoke", "Phase 57T playground fixture should classify StdOut as external INVOKE");
    failures += expect_json_contains(playground_copy, "unsupported-external-invoke", "Phase 57T playground fixture should classify crt_printf as external INVOKE");
    ...
    failures += expect_json_contains(playground_copy, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "Phase 57T playground fixture should not produce Program Console output");
    ...
    failures += expect_json_contains(call_copy, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "Phase 57T CALL fixture should not produce Program Console output");
    ...
    failures += expect_json_contains(ret_copy, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "Phase 71 root RET fixture should not produce Program Console output");
    ...
    failures += expect_json_contains(cmp_copy, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "Phase 62 CMP register/immediate fixture should not produce Program Console output");
    ...
    failures += expect_json_contains(conditional_jump_copy, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "Phase 57T loop fixture should not produce Program Console output");

    failures += expect_json_contains(exitprocess_copy, "unsupported-external-invoke", "Phase 57T WinAPI fixture should classify ExitProcess as external INVOKE boundary");
    failures += expect_json_contains(exitprocess_copy, "Windows/API", "Phase 57T WinAPI fixture should describe external/API behavior");
    failures += expect_json_not_contains(exitprocess_copy, "execution-complete", "Phase 57T WinAPI fixture should not execute");
    failures += expect_json_contains(exitprocess_copy, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "Phase 57T WinAPI fixture should not produce Program Console output");
...
static int test_phase58_label_source_run_behavior(void) {
    ...
    failures += expect_json_contains(labeled_copy, "\"phase\":93", "Labeled source should report numeric current runtime phase metadata");
    failures += expect_json_contains(labeled_copy, "\"phaseName\":\"Phase 93 - Irvine32 WriteBin\"", "Labeled source should report current runtime phase name");
    ...
    failures += expect_json_contains(labeled_copy, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "labels should not create Program Console output");
    ...
    failures += expect_json_contains(duplicate_copy, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "duplicate label source should not create Program Console output");
...
static int test_phase64_equality_jumps_source_run_programs(void) {
    ...
    failures += expect_json_contains(acceptance_json, "\"phase\":93", "Equality-jump regression should report current numeric phase metadata");
    failures += expect_json_contains(acceptance_json, "\"phaseName\":\"Phase 93 - Irvine32 WriteBin\"", "Equality-jump regression should report current runtime phase name");
...
static int test_phase65_signed_relational_jumps_source_run_programs(void) {
    ...
    failures += expect_json_contains(acceptance_json, "\"phase\":93", "current runtime acceptance program should report numeric metadata");
    failures += expect_json_contains(acceptance_json, "\"phaseName\":\"Phase 93 - Irvine32 WriteBin\"", "current runtime acceptance program should report runtime phase name");
...
static int test_phase69_direct_call_source_run_behavior(void) {
    ...
    failures += expect_json_contains(valid_copy, "\"phase\":93", "Phase 69 CALL regression fixture should report current runtime metadata");
    ...
    failures += expect_json_contains(invalid_stack_copy, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "failed CALL should not produce Program Console output");
    ...
    failures += expect_json_contains(invalid_target_copy, "\"ok\":false", "still-deferred CALL Irvine32 routine target should remain deferred");
...
static int test_phase71_call_ret_source_run_behavior(void) {
    ...
    failures += expect_json_contains(success_copy, "\"phase\":93", "Phase 71 CALL/RET acceptance program should report numeric phase metadata");
    failures += expect_json_contains(success_copy, "\"phaseName\":\"Phase 93 - Irvine32 WriteBin\"", "Phase 71 CALL/RET acceptance program should report runtime phase name");
    ...
    failures += expect_json_contains(success_copy, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "CALL/RET acceptance program should not produce Program Console output");
    ...
    failures += expect_json_contains(invalid_token_copy, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "invalid-return-address should not produce Program Console output");
...
/// Verifies Phase 82 zero-argument INVOKE source-run execution and diagnostics.
///
/// @return Number of failures.
static int test_phase82_zero_argument_invoke_source_run_semantics(void) {
    char acceptance_json[TEST_JSON_COPY_CAPACITY];
    char lowercase_json[TEST_JSON_COPY_CAPACITY];
    char argument_json[TEST_JSON_COPY_CAPACITY];
    char stack_args_json[TEST_JSON_COPY_CAPACITY];
    char cleanup_mismatch_json[TEST_JSON_COPY_CAPACITY];
    char addr_argument_json[TEST_JSON_COPY_CAPACITY];
    char addr_const_write_json[TEST_JSON_COPY_CAPACITY];
    char offset_argument_json[TEST_JSON_COPY_CAPACITY];
    char addr_local_argument_json[TEST_JSON_COPY_CAPACITY];
    char unknown_json[TEST_JSON_COPY_CAPACITY];
    char needs_arg_json[TEST_JSON_COPY_CAPACITY];
    char uses_json[TEST_JSON_COPY_CAPACITY];
    char local_json[TEST_JSON_COPY_CAPACITY];
    char irvine_json[TEST_JSON_COPY_CAPACITY];
    char data_json[TEST_JSON_COPY_CAPACITY];
    int failures = 0;
    ...
    failures += expect_json_contains(acceptance_json, "\"ok\":true", "Phase 84 accepted zero-argument INVOKE source should execute successfully");
    failures += expect_json_contains(acceptance_json, "\"phase\":93", "Current metadata should accompany accepted zero-argument INVOKE source");
    failures += expect_json_contains(acceptance_json, TEST_SOURCE_RUN_OUTPUT_CONTRACT_FRAGMENT, "Phase 84 accepted zero-argument INVOKE source should report current output contract");
    failures += expect_json_contains(acceptance_json, "\"EAX\":{\"hex\":\"00000037h\",\"unsigned\":55}", "Phase 82 accepted INVOKE source should execute Helper and leave EAX = 55");
    failures += expect_json_contains(acceptance_json, "execution-complete", "Phase 82 accepted INVOKE source should complete execution");
    failures += expect_json_not_contains(acceptance_json, "unsupported-invoke", "Phase 82 accepted INVOKE source should not report legacy unsupported-invoke");

    copy_source_run_json(lowercase_json, sizeof(lowercase_json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n"
        ".code\n"
        "main PROC\n"
        "    invoke Helper\n"
    ...
    failures += expect_json_contains(lowercase_json, "\"ok\":true", "lowercase invoke should execute successfully");
    failures += expect_json_contains(lowercase_json, "\"EAX\":{\"hex\":\"00000042h\",\"unsigned\":66}", "lowercase invoke should execute the helper body");

    copy_source_run_json(argument_json, sizeof(argument_json), masm32_sim_wasm_run_source_json(
        ".code\n"
        "main PROC\n"
        "    INVOKE Helper, 1\n"
        "    ret\n"
    ...
    failures += expect_json_contains(argument_json, "\"ok\":true", "Phase 84 INVOKE with a DWORD immediate argument should execute when cleanup matches");
    failures += expect_json_contains(argument_json, "\"EAX\":{\"hex\":\"0000004Dh\",\"unsigned\":77}", "accepted INVOKE argument source should execute Helper");
    failures += expect_json_contains(argument_json, "execution-complete", "accepted INVOKE argument source should complete execution");

    copy_source_run_json(stack_args_json, sizeof(stack_args_json), masm32_sim_wasm_run_source_json(
        ".data\n"
        "value DWORD 7\n"
        ".code\n"
        "main PROC\n"
        "    mov eax, 5\n"
        "    INVOKE Helper, 1, eax, value\n"
        "    ret\n"
        "main ENDP\n"
        "Helper PROC\n"
        "    mov ebx, [esp + 4]\n"
    ...
    failures += expect_json_contains(stack_args_json, "\"ok\":true", "Phase 84 multi-argument INVOKE should execute when cleanup matches");
    failures += expect_json_contains(stack_args_json, "\"EBX\":{\"hex\":\"00000001h\",\"unsigned\":1}", "first source argument should be at [esp + 4]");
    failures += expect_json_contains(stack_args_json, "\"ECX\":{\"hex\":\"00000005h\",\"unsigned\":5}", "register argument should be captured before stack mutation");
    failures += expect_json_contains(stack_args_json, "\"EDX\":{\"hex\":\"00000007h\",\"unsigned\":7}", "DWORD data-symbol argument should load the 32-bit value");

    copy_source_run_json(cleanup_mismatch_json, sizeof(cleanup_mismatch_json), masm32_sim_wasm_run_source_json(
        ".code\n"
        "main PROC\n"
        "    INVOKE Helper, 1\n"
        "    ret\n"
    ...
    failures += expect_json_contains(cleanup_mismatch_json, "\"ok\":false", "INVOKE with an argument and plain RET should reject before execution");
    failures += expect_json_contains(cleanup_mismatch_json, "invoke-cleanup-mismatch", "INVOKE cleanup mismatch should report the Phase 84 cleanup diagnostic");
    failures += expect_json_not_contains(cleanup_mismatch_json, "\"EAX\":{\"hex\":\"0000004Dh\"", "cleanup mismatch source should not execute Helper");
    failures += expect_json_not_contains(cleanup_mismatch_json, "execution-complete", "cleanup mismatch source should not complete execution");

    copy_source_run_json(addr_argument_json, sizeof(addr_argument_json), masm32_sim_wasm_run_source_json(
        ".data\n"
        "msg BYTE \"Hi\", 0\n"
        ".code\n"
        "main PROC\n"
    ...
    failures += expect_json_contains(addr_argument_json, "\"ok\":true", "Phase 84 INVOKE with ADDR data argument should execute when cleanup matches");
    failures += expect_json_contains(addr_argument_json, "\"EAX\":{\"hex\":\"0000004Dh\",\"unsigned\":77}", "accepted INVOKE ADDR source should execute Helper");
    failures += expect_json_not_contains(addr_argument_json, "invalid-addr-target", "addressable data ADDR argument should not route through helper diagnostics");

    copy_source_run_json(addr_const_write_json, sizeof(addr_const_write_json), masm32_sim_wasm_run_source_json(
        ".CONST\n"
        "msg DWORD 77\n"
        ".code\n"
        "main PROC\n"
    ...
    failures += expect_json_contains(addr_const_write_json, "\"ok\":false", "Phase 84 ADDR .CONST argument write-through fixture should fail execution");
    failures += expect_json_contains(addr_const_write_json, "permission-denied", "ADDR .CONST argument must not bypass .CONST write protection");
    failures += expect_json_contains(addr_const_write_json, "\"memoryChanges\":[]", "failed ADDR .CONST write-through should not commit public memory changes");
    failures += expect_json_not_contains(addr_const_write_json, "execution-complete", "ADDR .CONST write-through failure should not complete execution");

    copy_source_run_json(offset_argument_json, sizeof(offset_argument_json), masm32_sim_wasm_run_source_json(
        ".data\n"
        "msg BYTE \"Hi\", 0\n"
        ".code\n"
        "main PROC\n"
    ...
    failures += expect_json_contains(offset_argument_json, "\"ok\":true", "Phase 84 INVOKE with OFFSET data argument should execute when cleanup matches");
    failures += expect_json_contains(offset_argument_json, "\"EAX\":{\"hex\":\"0000004Dh\",\"unsigned\":77}", "accepted INVOKE OFFSET source should execute Helper");

    copy_source_run_json(addr_local_argument_json, sizeof(addr_local_argument_json), masm32_sim_wasm_run_source_json(
        ".code\n"
        "main PROC\n"
        "    LOCAL temp:DWORD\n"
        "    INVOKE Helper, ADDR temp\n"
        "    ret\n"
        "main ENDP\n"
        "Helper PROC\n"
        "    mov eax, 88\n"
    ...
    failures += expect_json_contains(addr_local_argument_json, "\"ok\":true", "Phase 84 INVOKE with ADDR local argument should execute when cleanup matches");
    failures += expect_json_contains(addr_local_argument_json, "\"EAX\":{\"hex\":\"00000058h", "accepted INVOKE ADDR local source should execute Helper");
    failures += expect_json_contains(addr_local_argument_json, "execution-complete", "accepted INVOKE ADDR local source should complete execution");
    ...
    failures += expect_json_contains(unknown_json, "invalid-invoke-target", "unknown INVOKE target should report target diagnostic");
    failures += expect_json_contains(unknown_json, "not a known same-file user procedure", "unknown INVOKE target should explain same-file user-procedure requirement");
    failures += expect_json_not_contains(unknown_json, "execution-complete", "unknown INVOKE target should not execute");

    copy_source_run_json(needs_arg_json, sizeof(needs_arg_json), masm32_sim_wasm_run_source_json(
        "NeedsArg PROTO x:DWORD\n"
        ".code\n"
        "main PROC\n"
        "    INVOKE NeedsArg\n"
        "    ret\n"
    ...
    failures += expect_json_contains(needs_arg_json, "\"ok\":false", "INVOKE to Phase 81-incompatible parameterized PROTO/bare PROC should reject");
    failures += expect_json_contains(needs_arg_json, "invoke-argument-count-mismatch", "Phase 84 argument-count validation should own this PROTO/INVOKE regression");
    failures += expect_json_not_contains(needs_arg_json, "proto-proc-mismatch", "Phase 84 should not use the old PROTO/PROC mismatch once parameter-count metadata is linkable");
    failures += expect_json_not_contains(needs_arg_json, "\"EAX\":{\"hex\":\"00000063h", "rejected NeedsArg INVOKE source should not execute helper body");
    failures += expect_json_not_contains(needs_arg_json, "execution-complete", "rejected NeedsArg INVOKE source should not complete execution");

    copy_source_run_json(uses_json, sizeof(uses_json), masm32_sim_wasm_run_source_json(
        ".code\n"
        "main PROC\n"
        "    mov eax, 11\n"
        "    INVOKE Helper\n"
    ...
    failures += expect_json_contains(uses_json, "\"ok\":true", "INVOKE into PROC USES helper should execute successfully");
    failures += expect_json_contains(uses_json, "\"EAX\":{\"hex\":\"0000000Bh\",\"unsigned\":11}", "INVOKE should reuse CALL PROC USES save/restore behavior");
    failures += expect_json_contains(uses_json, "execution-complete", "INVOKE PROC USES fixture should complete execution");

    copy_source_run_json(local_json, sizeof(local_json), masm32_sim_wasm_run_source_json(
        ".code\n"
        "main PROC\n"
        "    INVOKE Helper\n"
        "    ret\n"
    ...
    failures += expect_json_contains(local_json, "\"ok\":true", "INVOKE into LOCAL helper should execute successfully");
    failures += expect_json_contains(local_json, "\"EAX\":{\"hex\":\"00000058h\",\"unsigned\":88}", "INVOKE should reuse CALL LOCAL frame setup and operand behavior");
    failures += expect_json_contains(local_json, "execution-complete", "INVOKE LOCAL fixture should complete execution");

    copy_source_run_json(irvine_json, sizeof(irvine_json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n"
        ".code\n"
        "main PROC\n"
        "    INVOKE WriteString\n"
    ...
    failures += expect_json_contains(irvine_json, "unsupported-irvine-invoke", "Irvine32 INVOKE target should report the Phase 82 Irvine32 diagnostic");
    failures += expect_json_not_contains(irvine_json, "execution-complete", "unsupported Irvine32 INVOKE source should not execute");
    ...
    failures += expect_json_contains(data_json, "invalid-invoke-target", "data-symbol INVOKE target should report the Phase 82 target diagnostic");
    failures += expect_json_contains(data_json, "data symbol", "data-symbol INVOKE target should explain the wrong-kind target");

    return failures;
}

/// Verifies current runtime/source-run status metadata is emitted by source-run JSON.
///
/// @return Number of failures.
static int test_current_source_run_phase_metadata(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "Current runtime metadata should report numeric Phase 93 metadata");
    failures += expect_json_contains(json, "\"phaseSuffix\":\"\"", "Current runtime metadata should report empty suffix metadata");
    failures += expect_json_contains(json, "\"phaseName\":\"Phase 93 - Irvine32 WriteBin\"", "Current runtime metadata should report the Phase 93 runtime phase name");
    failures += expect_json_contains(json, TEST_SOURCE_RUN_OUTPUT_CONTRACT_FRAGMENT, "current source-run should report separate output-contract metadata");
...
/// Verifies Phase 81 accepted PROTO metadata is source-run non-executable.
///
/// @return Number of failures.
static int test_phase81_proto_source_run_non_effect(void) {
    char accepted_json[TEST_JSON_COPY_CAPACITY];
    char invoke_json[TEST_JSON_COPY_CAPACITY];
    char pointer_json[TEST_JSON_COPY_CAPACITY];
    int failures = 0;

    copy_source_run_json(accepted_json, sizeof(accepted_json), masm32_sim_wasm_run_source_json(
    ...
    failures += expect_json_contains(accepted_json, "\"ok\":true", "accepted PROTO metadata should not block source-run execution");
    failures += expect_json_contains(accepted_json, "\"phase\":93", "accepted PROTO source-run should report current numeric phase");
    failures += expect_json_contains(accepted_json, TEST_SOURCE_RUN_OUTPUT_CONTRACT_FRAGMENT, "accepted PROTO source-run should report current output contract");
    failures += expect_json_contains(accepted_json, "\"EAX\":{\"hex\":\"0000002Ah\"", "accepted PROTO should not change runtime register behavior");
    failures += expect_json_not_contains(accepted_json, "unsupported-feature", "accepted PROTO should no longer use the generic unsupported-feature diagnostic");

    copy_source_run_json(invoke_json, sizeof(invoke_json), masm32_sim_wasm_run_source_json(
        "MyProc PROTO\n"
        ".code\n"
        "main PROC\n"
        "    INVOKE MyProc\n"
    ...
    failures += expect_json_contains(invoke_json, "\"ok\":true", "zero-argument PROTO plus same-file PROC should legalize INVOKE source-run behavior in Phase 82");
    failures += expect_json_contains(invoke_json, "\"EAX\":{\"hex\":\"00000037h\"", "PROTO plus INVOKE should execute the helper body");
    failures += expect_json_contains(invoke_json, "execution-complete", "accepted INVOKE should execute after PROTO metadata");

    copy_source_run_json(pointer_json, sizeof(pointer_json), masm32_sim_wasm_run_source_json(
    ...
    failures += expect_json_contains(pointer_json, "\"ok\":false", "unsupported pointer PROTO should block source-run execution");
    failures += expect_json_contains(pointer_json, "\"code\":\"unsupported-proto-type\"", "pointer PROTO should use unsupported-proto-type in source-run JSON");
    failures += expect_json_contains(pointer_json, "\"line\":1", "pointer PROTO diagnostic should preserve source line");
    failures += expect_json_contains(pointer_json, "\"column\":16", "pointer PROTO diagnostic should preserve PTR column");
    failures += expect_json_contains(pointer_json, "\"spanLength\":3", "pointer PROTO diagnostic should preserve PTR span");
...
static int test_phase67a_entry_procedure_runtime_boundary_source_run(void) {
    ...
    failures += expect_json_contains(before_copy, "\"phase\":93", "Phase 67A helper-before-entry source should report numeric phase metadata");
...
static int test_phase59_instruction_limit_source_run(void) {
    ...
    failures += expect_json_contains(invalid_json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "Invalid instructionLimit response should include the current empty Program Console limit fields");
...
static int test_phase57f_seeded_startup_notice_source_run(void) {
    ...
    failures += expect_json_not_contains(json, "\"message\":\"Program Console", "Seeded startup notice should not write Program Console output");
...
static int test_phase57g_seeded_uninitialized_storage_notice_source_run(void) {
    ...
    failures += expect_json_not_contains(json, "\"message\":\"Program Console", "Seeded uninitialized storage notice should not write Program Console output");
...
static int test_phase50b_undefined_flag_use_warn_policy_source_run(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "Phase 50B source-run response should report current numeric runtime metadata");
...
static int test_phase57m_segment_symbol_source_run(void) {
    ...
    failures += expect_json_contains(offset_text_json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "OFFSET _TEXT should not create Program Console output");
    ...
    failures += expect_json_contains(segment_def_json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "_TEXT SEGMENT should not create Program Console output");
    ...
    failures += expect_json_contains(group_def_json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "DGROUP GROUP should not create Program Console output");
...
static int test_phase57corr1_const_cross_region_write_diagnostic_context(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "Phase 57-CORR1 regression should report current runtime/source-run phase metadata");
...
static int test_phase61_jmp_executes_after_prior_instruction(void) {
    ...
    failures += expect_json_contains(json_copy, "\"phase\":93", "JMP regression response should report current Phase 78 numeric phase metadata");
...
static int test_phase61a_jmp_skips_memory_write_and_keeps_console_empty(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "current runtime should report numeric phase metadata");
    failures += expect_json_contains(json, "\"phaseName\":\"Phase 93 - Irvine32 WriteBin\"", "Current runtime metadata should report the Phase 93 runtime phase name");
    ...
    failures += expect_json_contains(json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "direct JMP should not create Program Console output");
...
/// Verifies Phase 86 Program Console stream separation and limits in source-run JSON.
///
/// @return Zero on success, otherwise a positive failure count.
static int test_phase86_program_console_source_run_separation_and_limits(void) {
    int failures = 0;
    char success_json[TEST_JSON_COPY_CAPACITY];
    char parse_error_json[TEST_JSON_COPY_CAPACITY];
    char runtime_error_json[TEST_JSON_COPY_CAPACITY];

    copy_source_run_json(success_json, sizeof(success_json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n"
        ".code\n"
        "main PROC\n"
        "    mov eax, 42\n"
    ...
    failures += expect_json_contains(success_json, "\"ok\":true", "Phase 86 no-output program should execute successfully");
    failures += expect_json_contains(success_json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "Phase 86 no-output program should return an empty Program Console object");
    failures += expect_json_contains(success_json, "\"simulatorMessages\":[", "Phase 86 no-output program should keep Simulator Messages separate");
    failures += expect_json_contains(success_json, "execution-complete", "Phase 86 no-output program should still report execution-complete as a Simulator Message");
    failures += expect_json_fragment_order(success_json, "\"programConsole\"", "\"simulatorMessages\"", "Program Console should serialize before Simulator Messages as a separate field");
    failures += expect_json_not_contains(success_json, "\"message\":\"Program Console", "Simulator Messages should not contain Program Console display text");

    copy_source_run_json(parse_error_json, sizeof(parse_error_json), masm32_sim_wasm_run_source_json(
        ".code\n"
        "main PROC\n"
        "    mov eax, missingSymbol\n"
        "main ENDP\n"
        "END main\n"
    ));
    failures += expect_json_contains(parse_error_json, "\"ok\":false", "Phase 86 assembly-error source should fail");
    failures += expect_json_contains(parse_error_json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "Phase 86 assembly errors should return an empty Program Console object");
    failures += expect_json_contains(parse_error_json, "\"simulatorMessages\":[", "Phase 86 assembly errors should keep diagnostics in Simulator Messages");
    failures += expect_json_contains(parse_error_json, "unknown-symbol", "Phase 86 assembly diagnostic code should remain in Simulator Messages");
    failures += expect_json_fragment_order(parse_error_json, "\"programConsole\"", "\"simulatorMessages\"", "Assembly-error Program Console should remain separate from diagnostics");

    copy_source_run_json(runtime_error_json, sizeof(runtime_error_json), masm32_sim_wasm_run_source_json(
        ".code\n"
        "main PROC\n"
        "    mov eax, 1\n"
        "    mov ebx, 0\n"
    ...
    failures += expect_json_contains(runtime_error_json, "\"ok\":false", "Phase 86 runtime-error source should fail");
    failures += expect_json_contains(runtime_error_json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "Phase 86 runtime errors should return an empty Program Console object");
    failures += expect_json_contains(runtime_error_json, "divide-by-zero", "Phase 86 runtime diagnostic code should remain in Simulator Messages");
    failures += expect_json_fragment_order(runtime_error_json, "\"programConsole\"", "\"simulatorMessages\"", "Runtime-error Program Console should remain separate from diagnostics");

    return failures;
}

/// Verifies Phase 86 synthetic Program Console output-limit source-run diagnostics.
///
/// @return Zero on success, otherwise a positive failure count.
static int test_phase86_synthetic_console_output_limit_source_run(void) {
    int failures = 0;
    char byte_json[TEST_JSON_COPY_CAPACITY];
    char line_json[TEST_JSON_COPY_CAPACITY];
    const char *source =
        "INCLUDE Irvine32.inc\n"
        ".code\n"
        "main PROC\n"
    ...
    copy_source_run_json(
        byte_json,
        sizeof(byte_json),
        masm32_sim_wasm_run_source_json_with_synthetic_console_output(source, "abc", "d", 3U, 10U)
    );
    failures += expect_json_contains(byte_json, "\"ok\":false", "Phase 86 synthetic byte-limit fixture should fail");
    failures += expect_json_contains(byte_json, "\"status\":\"resource-limit-exceeded\"", "Phase 86 byte-limit failure should use resource-limit status");
    failures += expect_json_contains(byte_json, "\"programConsole\":{\"text\":\"abc\",\"truncated\":false,\"byteCount\":3,\"lineCount\":0,\"maxBytes\":3,\"maxLines\":10,\"limitExceeded\":true,\"limitKind\":\"byte\"}", "Phase 86 byte-limit failure should serialize committed output and limit status");
    failures += expect_json_contains(byte_json, "\"kind\":\"resource-limit-error\"", "Phase 86 byte-limit diagnostic should use resource-limit-error kind");
    failures += expect_json_contains(byte_json, "\"code\":\"console-output-limit-exceeded\"", "Phase 86 byte-limit diagnostic should use console-output-limit-exceeded code");
    failures += expect_json_not_contains(byte_json, "abcd", "Phase 86 byte-limit failure must not partially append rejected output");
    failures += expect_json_not_contains(byte_json, "execution-complete", "Phase 86 byte-limit failure should not report execution-complete");
    failures += expect_json_fragment_order(byte_json, "\"programConsole\"", "\"simulatorMessages\"", "Phase 86 byte-limit result should keep Program Console before Simulator Messages");

    copy_source_run_json(
        line_json,
        sizeof(line_json),
        masm32_sim_wasm_run_source_json_with_synthetic_console_output(source, "ok\n", "blocked\n", 64U, 1U)
    );
    failures += expect_json_contains(line_json, "\"ok\":false", "Phase 86 synthetic line-limit fixture should fail");
    failures += expect_json_contains(line_json, "\"status\":\"resource-limit-exceeded\"", "Phase 86 line-limit failure should use resource-limit status");
    failures += expect_json_contains(line_json, "\"programConsole\":{\"text\":\"ok\\n\",\"truncated\":false,\"byteCount\":3,\"lineCount\":1,\"maxBytes\":64,\"maxLines\":1,\"limitExceeded\":true,\"limitKind\":\"line\"}", "Phase 86 line-limit failure should serialize committed output and limit status");
    failures += expect_json_contains(line_json, "\"kind\":\"resource-limit-error\"", "Phase 86 line-limit diagnostic should use resource-limit-error kind");
    failures += expect_json_contains(line_json, "\"code\":\"console-output-limit-exceeded\"", "Phase 86 line-limit diagnostic should use console-output-limit-exceeded code");
    failures += expect_json_not_contains(line_json, "blocked", "Phase 86 line-limit failure must not partially append rejected output");
    failures += expect_json_fragment_order(line_json, "\"programConsole\"", "\"simulatorMessages\"", "Phase 86 line-limit result should keep Program Console before Simulator Messages");

    return failures;
}


/// Verifies Phase 87 public Irvine32 Crlf source-run Program Console behavior.
///
/// @return Zero on success, otherwise a positive failure count.
static int test_phase87_irvine32_crlf_source_run_program_console(void) {
    int failures = 0;
    char crlf_json[TEST_JSON_COPY_CAPACITY];
    char missing_call_json[TEST_JSON_COPY_CAPACITY];
    char missing_invoke_json[TEST_JSON_COPY_CAPACITY];
    char invalid_argument_json[TEST_JSON_COPY_CAPACITY];
    char invalid_form_json[TEST_JSON_COPY_CAPACITY];
    char writestring_json[TEST_JSON_COPY_CAPACITY];
    char writestring_empty_json[TEST_JSON_COPY_CAPACITY];
    char writestring_const_json[TEST_JSON_COPY_CAPACITY];
    char writestring_embedded_json[TEST_JSON_COPY_CAPACITY];
    char writestring_scan_success_json[TEST_JSON_COPY_CAPACITY];
    char writestring_scan_fail_n_json[TEST_JSON_COPY_CAPACITY];
    char writestring_missing_json[TEST_JSON_COPY_CAPACITY];
    char writestring_bare_json[TEST_JSON_COPY_CAPACITY];
    char writestring_invoke_json[TEST_JSON_COPY_CAPACITY];
    char writestring_invalid_json[TEST_JSON_COPY_CAPACITY];
    char writestring_scan_json[TEST_JSON_COPY_CAPACITY];
    char writestring_strict_json[TEST_JSON_COPY_CAPACITY];
    char writestring_warning_json[TEST_JSON_COPY_CAPACITY];
    char writestring_overwritten_json[TEST_JSON_COPY_CAPACITY];
    char writestring_uninit_off_json[TEST_JSON_COPY_CAPACITY];
    char writestring_prior_warning_invalid_json[TEST_JSON_COPY_CAPACITY];
    char writechar_json[TEST_JSON_COPY_CAPACITY];
    char writechar_high_json[TEST_JSON_COPY_CAPACITY];
    char writechar_missing_json[TEST_JSON_COPY_CAPACITY];
    char writechar_invoke_json[TEST_JSON_COPY_CAPACITY];

    copy_source_run_json(crlf_json, sizeof(crlf_json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n"
        "OPTION CASEMAP:NONE\n"
        ".code\n"
        "main PROC\n"
    ...
    failures += expect_json_contains(crlf_json, "\"ok\":true", "Phase 87 Crlf source should execute successfully");
    failures += expect_json_contains(crlf_json, "\"programConsole\":{\"text\":\"\\n\\n\",\"truncated\":false,\"byteCount\":2,\"lineCount\":2,\"maxBytes\":1048576,\"maxLines\":10000,\"limitExceeded\":false,\"limitKind\":null}", "Phase 87 Crlf should serialize exactly two LF bytes in Program Console");
    failures += expect_json_contains(crlf_json, "execution-complete", "Phase 87 Crlf success should still report execution-complete");
    failures += expect_json_fragment_order(crlf_json, "\"programConsole\"", "\"simulatorMessages\"", "Phase 87 Program Console should remain separate from Simulator Messages");
    failures += expect_json_contains(crlf_json, "\"registerWrites\":{\"EAX\":false,\"EBX\":false,\"ECX\":false,\"EDX\":false", "Phase 87 Crlf should not write general-purpose registers");
    failures += expect_json_contains(crlf_json, "\"memoryChanges\":[]", "Phase 87 Crlf should not mutate memory");
    failures += expect_json_not_contains(crlf_json, "unsupported-irvine32-routine", "Phase 87 Crlf should not use the deferred Irvine32 diagnostic");

    copy_source_run_json(missing_call_json, sizeof(missing_call_json), masm32_sim_wasm_run_source_json(
        ".code\n"
        "main PROC\n"
        "    call Crlf\n"
        "main ENDP\n"
    ...
    failures += expect_json_contains(missing_call_json, "\"kind\":\"assembly-error\"", "CALL Crlf without include should be an assembly error");
    failures += expect_json_contains(missing_call_json, "\"code\":\"missing-irvine32-include\"", "CALL Crlf without include should use missing-irvine32-include");
    failures += expect_json_contains(missing_call_json, "CALL Crlf requires INCLUDE Irvine32.inc before Crlf can be used as a virtual Irvine32 routine.", "CALL Crlf without include should explain the required virtual include");
    failures += expect_json_not_contains(missing_call_json, "unsupported-irvine32-routine", "CALL Crlf without include should not imply Crlf is unsupported");
    failures += expect_json_contains(missing_call_json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "CALL Crlf without include should not write Program Console output");

    copy_source_run_json(missing_invoke_json, sizeof(missing_invoke_json), masm32_sim_wasm_run_source_json(
        ".code\n"
        "main PROC\n"
        "    invoke Crlf\n"
        "main ENDP\n"
    ...
    failures += expect_json_contains(missing_invoke_json, "\"kind\":\"assembly-error\"", "INVOKE Crlf without include should be an assembly error");
    failures += expect_json_contains(missing_invoke_json, "\"code\":\"missing-irvine32-include\"", "INVOKE Crlf without include should use missing-irvine32-include");
    failures += expect_json_contains(missing_invoke_json, "INVOKE Crlf requires INCLUDE Irvine32.inc before Crlf can be used as a virtual Irvine32 routine.", "INVOKE Crlf without include should explain the required virtual include");
    failures += expect_json_not_contains(missing_invoke_json, "unsupported-irvine-invoke", "INVOKE Crlf without include should not imply INVOKE Crlf is unsupported");
    failures += expect_json_contains(missing_invoke_json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "INVOKE Crlf without include should not write Program Console output");

    copy_source_run_json(invalid_argument_json, sizeof(invalid_argument_json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n"
        ".code\n"
        "main PROC\n"
        "    invoke Crlf, eax\n"
    ...
    failures += expect_json_contains(invalid_argument_json, "\"kind\":\"assembly-error\"", "INVOKE Crlf with an argument should be an assembly error");
    failures += expect_json_contains(invalid_argument_json, "\"code\":\"invalid-irvine32-argument-count\"", "INVOKE Crlf with an argument should use invalid-irvine32-argument-count");
    failures += expect_json_contains(invalid_argument_json, "INVOKE Crlf takes no arguments. Remove the argument list or use CALL Crlf.", "INVOKE Crlf argument diagnostic should give exact fix guidance");
    failures += expect_json_not_contains(invalid_argument_json, "unsupported-invoke-argument", "INVOKE Crlf with an argument should not imply a deferred argument form");
    failures += expect_json_contains(invalid_argument_json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "INVOKE Crlf with an argument should not write Program Console output");

    copy_source_run_json(invalid_form_json, sizeof(invalid_form_json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n"
        ".code\n"
        "main PROC\n"
        "    Crlf\n"
    ...
    failures += expect_json_contains(invalid_form_json, "\"kind\":\"assembly-error\"", "Bare Crlf should be an assembly error");
    failures += expect_json_contains(invalid_form_json, "\"code\":\"invalid-irvine32-call-form\"", "Bare Crlf should use invalid-irvine32-call-form");
    failures += expect_json_contains(invalid_form_json, "Crlf is a virtual Irvine32 routine and must be called with CALL Crlf or zero-argument INVOKE Crlf.", "Bare Crlf diagnostic should list supported source forms");
    failures += expect_json_not_contains(invalid_form_json, "unsupported-irvine32-routine", "Bare Crlf should not imply Crlf is unsupported");
    failures += expect_json_contains(invalid_form_json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "Bare Crlf should not write Program Console output");

    copy_source_run_json(writestring_json, sizeof(writestring_json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n"
        ".data\n"
        "msg BYTE \"Hello\",0\n"
        ".code\n"
    ...
    failures += expect_json_contains(writestring_json, "\"ok\":true", "WriteString source should execute successfully after Phase 89");
    failures += expect_json_contains(writestring_json, "\"programConsole\":{\"text\":\"Hello\",\"truncated\":false,\"byteCount\":5,\"lineCount\":0", "WriteString should serialize string bytes before the null terminator");
    failures += expect_json_contains(writestring_json, "\"EDX\":{\"hex\":\"00500000h\"", "WriteString should preserve EDX after output");
    failures += expect_json_contains(writestring_json, "\"memoryChanges\":[]", "WriteString should not mutate simulated memory");
    failures += expect_json_not_contains(writestring_json, "unsupported-irvine32-routine", "Phase 89 WriteString should not use the deferred Irvine32 diagnostic");

    copy_source_run_json(writestring_empty_json, sizeof(writestring_empty_json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n"
        ".data\n"
        "msg BYTE 0\n"
        ".code\n"
    ...
    failures += expect_json_contains(writestring_empty_json, "\"ok\":true", "Empty WriteString source should complete");
    failures += expect_json_contains(writestring_empty_json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "Empty WriteString should not append output bytes");

    copy_source_run_json(writestring_const_json, sizeof(writestring_const_json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n"
        ".CONST\n"
        "msg BYTE \"RO\",0\n"
        ".code\n"
    ...
    failures += expect_json_contains(writestring_const_json, "\"ok\":true", "WriteString should read initialized .CONST strings");
    failures += expect_json_contains(writestring_const_json, "\"programConsole\":{\"text\":\"RO\"", "WriteString should append bytes read from .CONST");
    failures += expect_json_not_contains(writestring_const_json, "uninitialized-read", "Initialized .CONST WriteString should not warn about uninitialized reads");

    copy_source_run_json(writestring_embedded_json, sizeof(writestring_embedded_json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n"
        ".data\n"
        "msg BYTE \"Hi\",0,\"tail\",0\n"
        ".code\n"
        "main PROC\n"
        "    mov edx, OFFSET msg\n"
        "    call WriteString\n"
    ...
    failures += expect_json_contains(writestring_embedded_json, "\"ok\":true", "WriteString should complete with an embedded null terminator");
    failures += expect_json_contains(writestring_embedded_json, "\"programConsole\":{\"text\":\"Hi\"", "WriteString should stop at the first null terminator");
    failures += expect_json_not_contains(writestring_embedded_json, "tail", "WriteString should not append bytes after the first terminator");

    copy_source_run_json(writestring_scan_success_json, sizeof(writestring_scan_success_json), masm32_sim_wasm_run_source_json_with_writestring_scan_limit(
        "INCLUDE Irvine32.inc\n"
        ".data\n"
        "msg BYTE \"ab\",0\n"
        ".code\n"
    ...
    failures += expect_json_contains(writestring_scan_success_json, "\"ok\":true", "WriteString scan limit should allow a terminator at index N minus one");
    failures += expect_json_contains(writestring_scan_success_json, "\"programConsole\":{\"text\":\"ab\"", "WriteString should append bytes before a terminator at index N minus one");

    copy_source_run_json(writestring_scan_fail_n_json, sizeof(writestring_scan_fail_n_json), masm32_sim_wasm_run_source_json_with_writestring_scan_limit(
        "INCLUDE Irvine32.inc\n"
        ".data\n"
        "msg BYTE \"abc\"\n"
        ".code\n"
    ...
    failures += expect_json_contains(writestring_scan_fail_n_json, "\"ok\":false", "WriteString should fail when bytes 0 through N minus one are nonzero");
    failures += expect_json_contains(writestring_scan_fail_n_json, "string-scan-limit-exceeded", "WriteString should diagnose all-nonzero bytes through the configured scan limit");
    failures += expect_json_contains(writestring_scan_fail_n_json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "All-nonzero scan-limit failure should append no candidate output");

    copy_source_run_json(writestring_missing_json, sizeof(writestring_missing_json), masm32_sim_wasm_run_source_json(
        ".data\n"
        "msg BYTE \"Hello\",0\n"
        ".code\n"
        "main PROC\n"
    ...
    failures += expect_json_contains(writestring_missing_json, "\"kind\":\"assembly-error\"", "CALL WriteString without include should be an assembly error");
    failures += expect_json_contains(writestring_missing_json, "\"code\":\"missing-irvine32-include\"", "CALL WriteString without include should use missing-irvine32-include");
    failures += expect_json_contains(writestring_missing_json, "CALL WriteString requires INCLUDE Irvine32.inc before WriteString can be used as a virtual Irvine32 routine.", "CALL WriteString without include should explain the required virtual include");
    failures += expect_json_contains(writestring_missing_json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "CALL WriteString without include should not write Program Console output");

    copy_source_run_json(writestring_bare_json, sizeof(writestring_bare_json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n"
        ".code\n"
        "main PROC\n"
        "    WriteString\n"
    ...
    failures += expect_json_contains(writestring_bare_json, "\"code\":\"invalid-irvine32-call-form\"", "Bare WriteString should use invalid-irvine32-call-form");
    failures += expect_json_contains(writestring_bare_json, "WriteString is a virtual Irvine32 routine and must be called with CALL WriteString.", "Bare WriteString should report the exact supported form");
    failures += expect_json_contains(writestring_bare_json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "Bare WriteString should not write Program Console output");

    copy_source_run_json(writestring_invoke_json, sizeof(writestring_invoke_json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n"
        ".code\n"
        "main PROC\n"
        "    INVOKE WriteString\n"
    ...
    failures += expect_json_contains(writestring_invoke_json, "\"code\":\"unsupported-irvine-invoke\"", "INVOKE WriteString should remain deferred after Phase 89");
    failures += expect_json_contains(writestring_invoke_json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "Deferred INVOKE WriteString should not write Program Console output");

    copy_source_run_json(writestring_invalid_json, sizeof(writestring_invalid_json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n"
        ".data\n"
        "prefix BYTE \"ok\",0\n"
        ".code\n"
    ...
    failures += expect_json_contains(writestring_invalid_json, "\"ok\":false", "Invalid-address WriteString should fail source-run execution");
    failures += expect_json_contains(writestring_invalid_json, "\"code\":\"invalid-memory-read\"", "Invalid-address WriteString should use the Phase 89 invalid-memory-read diagnostic code");
    failures += expect_json_contains(writestring_invalid_json, "\"programConsole\":{\"text\":\"Z\"", "Failing WriteString should preserve previously committed output");
    failures += expect_json_not_contains(writestring_invalid_json, "\"text\":\"Zok", "Failing WriteString should not append candidate bytes from the failing call");

    copy_source_run_json(writestring_scan_json, sizeof(writestring_scan_json), masm32_sim_wasm_run_source_json_with_writestring_scan_limit(
        "INCLUDE Irvine32.inc\n"
        ".data\n"
        "msg BYTE \"abc\",0\n"
        ".code\n"
    ...
    failures += expect_json_contains(writestring_scan_json, "\"ok\":false", "WriteString scan-limit fixture should fail before terminator at index N");
    failures += expect_json_contains(writestring_scan_json, "string-scan-limit-exceeded", "WriteString scan-limit fixture should use the Phase 89 diagnostic code");
    failures += expect_json_contains(writestring_scan_json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "Scan-limit failure should not append partial candidate bytes");

    copy_source_run_json(writestring_strict_json, sizeof(writestring_strict_json), masm32_sim_wasm_run_source_json_with_memory_validation_mode(
        "INCLUDE Irvine32.inc\n"
        ".data?\n"
        "msg BYTE ?\n"
        ".code\n"
    ...
    failures += expect_json_contains(writestring_strict_json, "\"ok\":false", "Strict uninitialized WriteString should fail before execution");
    failures += expect_json_contains(writestring_strict_json, "uninitialized-read", "Strict uninitialized WriteString should report uninitialized-read");
    failures += expect_json_contains(writestring_strict_json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "Strict uninitialized WriteString should not append output");

    copy_source_run_json(writestring_warning_json, sizeof(writestring_warning_json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n"
        ".data?\n"
        "msg BYTE ?\n"
        ".code\n"
    ...
    failures += expect_json_contains(writestring_warning_json, "\"ok\":true", "Default warning-mode uninitialized zero string should complete");
    failures += expect_json_contains(writestring_warning_json, "uninitialized-read", "Default warning-mode uninitialized zero string should emit a non-fatal warning");
    failures += expect_json_contains(writestring_warning_json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "Uninitialized zero terminator should not append output bytes");

    copy_source_run_json(writestring_overwritten_json, sizeof(writestring_overwritten_json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n"
        ".DATA?\n"
        "msg BYTE 3 DUP(?)\n"
        ".code\n"
    ...
    failures += expect_json_contains(writestring_overwritten_json, "\"ok\":true", "WriteString should complete after prior writes initialize .DATA? bytes");
    failures += expect_json_contains(writestring_overwritten_json, "\"programConsole\":{\"text\":\"OK\"", "WriteString should print overwritten .DATA? bytes");
    failures += expect_json_not_contains(writestring_overwritten_json, "uninitialized-read", "Overwritten .DATA? bytes should not warn during WriteString");

    copy_source_run_json(writestring_uninit_off_json, sizeof(writestring_uninit_off_json), masm32_sim_wasm_run_source_json_with_memory_validation_mode(
        "INCLUDE Irvine32.inc\n"
        ".DATA?\n"
        "msg BYTE ?\n"
        ".code\n"
    ...
    failures += expect_json_contains(writestring_uninit_off_json, "\"ok\":true", "Explicit uninitialized-read off should still let WriteString complete");
    failures += expect_json_not_contains(writestring_uninit_off_json, "uninitialized-read", "Explicit uninitialized-read off should suppress only the warning");
    failures += expect_json_contains(writestring_uninit_off_json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "Uninitialized zero terminator with warning off should not append output bytes");

    copy_source_run_json(writestring_prior_warning_invalid_json, sizeof(writestring_prior_warning_invalid_json), masm32_sim_wasm_run_source_json_with_memory_validation_mode(
        "INCLUDE Irvine32.inc\n"
        ".DATA?\n"
        "warn DWORD ?\n"
        ".code\n"
    ...
    failures += expect_json_contains(writestring_prior_warning_invalid_json, "\"ok\":false", "Fatal WriteString invalid-memory failure should still fail the run");
    failures += expect_json_contains(writestring_prior_warning_invalid_json, "uninitialized-read", "Warnings emitted before a failing WriteString call should be preserved");
    failures += expect_json_contains(writestring_prior_warning_invalid_json, "\"code\":\"invalid-memory-read\"", "Fatal WriteString memory failure should remain the terminal diagnostic");
    failures += expect_json_not_contains(writestring_prior_warning_invalid_json, "execution-complete", "Fatal WriteString invalid-memory failure should not complete execution");

    copy_source_run_json(writechar_json, sizeof(writechar_json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n"
        ".code\n"
        "main PROC\n"
        "    mov eax, 'A'\n"
    ...
    failures += expect_json_contains(writechar_json, "\"ok\":true", "WriteChar should execute successfully after Phase 89");
    failures += expect_json_contains(writechar_json, "\"programConsole\":{\"text\":\"A\",\"truncated\":false,\"byteCount\":1,\"lineCount\":0", "WriteChar should serialize one Program Console byte");
    failures += expect_json_not_contains(writechar_json, "unsupported-irvine32-routine", "Phase 89 WriteChar should not use the deferred Irvine32 diagnostic");

    copy_source_run_json(writechar_high_json, sizeof(writechar_high_json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n"
        ".code\n"
        "main PROC\n"
        "    mov eax, 12345641h\n"
    ...
    failures += expect_json_contains(writechar_high_json, "\"ok\":true", "WriteChar high-byte fixture should execute successfully");
    failures += expect_json_contains(writechar_high_json, "\"programConsole\":{\"text\":\"A\",\"truncated\":false,\"byteCount\":1,\"lineCount\":0", "WriteChar should ignore high EAX bytes");

    copy_source_run_json(writechar_missing_json, sizeof(writechar_missing_json), masm32_sim_wasm_run_source_json(
        ".code\n"
        "main PROC\n"
        "    call WriteChar\n"
        "main ENDP\n"
    ...
    failures += expect_json_contains(writechar_missing_json, "\"kind\":\"assembly-error\"", "CALL WriteChar without include should be an assembly error");
    failures += expect_json_contains(writechar_missing_json, "\"code\":\"missing-irvine32-include\"", "CALL WriteChar without include should use missing-irvine32-include");
    failures += expect_json_contains(writechar_missing_json, "CALL WriteChar requires INCLUDE Irvine32.inc before WriteChar can be used as a virtual Irvine32 routine.", "CALL WriteChar without include should explain the required virtual include");
    failures += expect_json_contains(writechar_missing_json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "CALL WriteChar without include should not write Program Console output");

    copy_source_run_json(writechar_invoke_json, sizeof(writechar_invoke_json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n"
        ".code\n"
        "main PROC\n"
        "    invoke WriteChar\n"
    ...
    failures += expect_json_contains(writechar_invoke_json, "\"ok\":false", "INVOKE WriteChar should remain deferred after Phase 89");
    failures += expect_json_contains(writechar_invoke_json, "unsupported-irvine-invoke", "INVOKE WriteChar should keep the Irvine32 INVOKE deferred diagnostic");
    failures += expect_json_contains(writechar_invoke_json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "Deferred INVOKE WriteChar should not write Program Console output");

    return failures;
}

/// Verifies Phase 90 WriteDec source-run output, diagnostics, limits, and deferral boundaries.
///
/// @return Zero on success, otherwise a positive failure count.
static int test_phase90_irvine32_writedec_source_run(void) {
    int failures = 0;
    char json[TEST_JSON_COPY_CAPACITY];
    const char *max_source =
        "INCLUDE Irvine32.inc\n"
        ".code\n"
        "main PROC\n"
        "    mov eax, 4294967295\n"
    ...
    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(max_source));
    failures += expect_json_contains(json, "\"ok\":true", "Phase 90 WriteDec maximum-value source should execute successfully");
    failures += expect_json_contains(json, "\"phase\":93", "Phase 90 WriteDec regression should report current numeric Phase 93 metadata");
    failures += expect_json_contains(json, "\"phaseName\":\"Phase 93 - Irvine32 WriteBin\"", "Phase 90 WriteDec source-run should report the full phase name");
    failures += expect_json_contains(json, "\"sourceRunOutputContract\":\"phase-93-irvine32-writebin-contract-v1\"", "Phase 90 WriteDec source-run should report the exact output contract");
    failures += expect_json_contains(json, "\"programConsole\":{\"text\":\"4294967295\",\"truncated\":false,\"byteCount\":10,\"lineCount\":0", "WriteDec should append the exact unsigned maximum without a newline");
    failures += expect_json_contains(json, "\"EAX\":{\"hex\":\"FFFFFFFFh\",\"unsigned\":4294967295", "WriteDec should preserve EAX after output");
    failures += expect_json_contains(json, "\"memoryChanges\":[]", "WriteDec should not create memory-change rows");
    failures += expect_json_not_contains(json, "unsupported-irvine32-routine", "Direct WriteDec should no longer use the deferred routine diagnostic");

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n.code\nmain PROC\n    mov eax, 0\n    call WriteDec\n    exit\nmain ENDP\nEND main\n"
    ));
    failures += expect_json_contains(json, "\"programConsole\":{\"text\":\"0\",", "WriteDec zero should emit one zero digit");

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n.code\nmain PROC\n    mov eax, 1\n    call WriteDec\n    exit\nmain ENDP\nEND main\n"
    ));
    failures += expect_json_contains(json, "\"programConsole\":{\"text\":\"1\",", "WriteDec one should emit one digit");

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\nOPTION CASEMAP:NONE\n.code\nmain PROC\n    mov eax, 10\n    CALL wRiTeDeC\n    exit\nmain ENDP\nEND main\n"
    ));
    failures += expect_json_contains(json, "\"programConsole\":{\"text\":\"10\",", "Mixed-case WriteDec should emit 10 under CASEMAP:NONE");

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(
        ".code\nmain PROC\n    call WriteDec\nmain ENDP\nEND main\n"
    ));
    failures += expect_json_contains(json, "\"kind\":\"assembly-error\"", "WriteDec without include should be an assembly error");
    failures += expect_json_contains(json, "\"code\":\"missing-irvine32-include\"", "WriteDec without include should use missing-irvine32-include");
    failures += expect_json_contains(json, "CALL WriteDec requires INCLUDE Irvine32.inc before WriteDec can be used as a virtual Irvine32 routine.", "WriteDec missing-include diagnostic should use the exact message");
    failures += expect_json_contains(json, "\"line\":3,\"column\":10", "WriteDec missing-include diagnostic should point at the target token");
    failures += expect_json_contains(json, "\"spanLength\":8", "WriteDec missing-include diagnostic should span only WriteDec");

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n.code\nmain PROC\n    WriteDec\nmain ENDP\nEND main\n"
    ));
    failures += expect_json_contains(json, "\"kind\":\"assembly-error\"", "Bare WriteDec should be an assembly error");
    failures += expect_json_contains(json, "\"code\":\"invalid-irvine32-call-form\"", "Bare WriteDec should use invalid-irvine32-call-form");
    failures += expect_json_contains(json, "WriteDec is a virtual Irvine32 routine and must be called with CALL WriteDec.", "Bare WriteDec should use the exact source-form message");
    failures += expect_json_contains(json, "\"line\":4,\"column\":5", "Bare WriteDec diagnostic should point at the routine token");
    failures += expect_json_contains(json, "\"spanLength\":8", "Bare WriteDec diagnostic should span only WriteDec");

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n.code\nmain PROC\n    INVOKE WriteDec\nmain ENDP\nEND main\n"
    ));
    failures += expect_json_contains(json, "\"kind\":\"unsupported-feature\"", "INVOKE WriteDec should remain unsupported-feature");
    failures += expect_json_contains(json, "\"code\":\"unsupported-irvine-invoke\"", "INVOKE WriteDec should remain future-owned");

    {
        static const char *const deferred_routines[] = {"DumpRegs", "DumpMem"};
        size_t routine_index = 0U;
        char deferred_source[256];

        for (routine_index = 0U; routine_index < sizeof(deferred_routines) / sizeof(deferred_routines[0]); routine_index += 1U) {
            int source_length = snprintf(
                deferred_source,
                sizeof(deferred_source),
                "INCLUDE Irvine32.inc\n.code\nmain PROC\n    call %s\nmain ENDP\nEND main\n",
                deferred_routines[routine_index]
            );
            if (source_length < 0 || (size_t)source_length >= sizeof(deferred_source)) {
                failures += record_failure("Phase 90 deferred Irvine32 sibling fixture should fit in its source buffer");
                continue;
            }
            copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(deferred_source));
            failures += expect_json_contains(json, "\"code\":\"unsupported-irvine32-routine\"", "Phase 90 sibling Irvine32 routines should remain deferred");
            failures += expect_json_not_contains(json, "execution-complete", "Deferred Phase 90 sibling Irvine32 routines should not execute");
        }
    }

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json_with_program_console_limits(max_source, 10U, 1U));
    failures += expect_json_contains(json, "\"ok\":true", "WriteDec should succeed with exact ten-byte capacity");
    failures += expect_json_contains(json, "\"programConsole\":{\"text\":\"4294967295\",\"truncated\":false,\"byteCount\":10,\"lineCount\":0,\"maxBytes\":10", "WriteDec exact-fit run should commit all digits");

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json_with_program_console_limits(max_source, 9U, 1U));
    failures += expect_json_contains(json, "\"ok\":false", "WriteDec should fail when byte capacity is one short");
    failures += expect_json_contains(json, "\"kind\":\"resource-limit-error\"", "WriteDec output-limit failure should use resource-limit-error");
    failures += expect_json_contains(json, "\"code\":\"console-output-limit-exceeded\"", "WriteDec output-limit failure should use the shared diagnostic code");
    failures += expect_json_contains(json, "\"programConsole\":{\"text\":\"\",\"truncated\":false,\"byteCount\":0,\"lineCount\":0,\"maxBytes\":9", "WriteDec output-limit failure should append no partial digits");
    failures += expect_json_contains(json, "\"line\":5,\"column\":5,\"byteOffset\":65,\"spanLength\":13", "WriteDec runtime diagnostic should span the complete call instruction");
    failures += expect_json_not_contains(json, "execution-complete", "Fatal WriteDec output-limit failure should omit execution-complete");

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json_with_program_console_limits(
        "INCLUDE Irvine32.inc\n"
        ".code\n"
        "main PROC\n"
        "    mov al, 'X'\n"
    ...
    failures += expect_json_contains(json, "\"ok\":false", "WriteDec should fail after preserving earlier Program Console output when no byte capacity remains");
    failures += expect_json_contains(json, "\"code\":\"console-output-limit-exceeded\"", "WriteDec failure after prior output should use the shared output-limit diagnostic");
    failures += expect_json_contains(json, "\"programConsole\":{\"text\":\"X\",\"truncated\":false,\"byteCount\":1,\"lineCount\":0,\"maxBytes\":1", "WriteDec failure should preserve previously committed Program Console output without appending digits");
    failures += expect_json_not_contains(json, "execution-complete", "WriteDec failure after prior output should omit execution-complete");

    return failures;
}

/// Verifies Phase 91 WriteInt source-run output, diagnostics, limits, and deferral boundaries.
///
/// @return Zero on success, otherwise a positive failure count.
static int test_phase91_irvine32_writeint_source_run(void) {
    int failures = 0;
    char json[TEST_JSON_COPY_CAPACITY];
    const char *int_min_source =
        "INCLUDE Irvine32.inc\n"
        ".code\n"
        "main PROC\n"
        "    mov eax, 80000000h\n"
    ...
    static const struct {
        const char *literal;
        const char *expected_text_fragment;
    } cases[] = {
        {"00000000h", "\"programConsole\":{\"text\":\"0\","},
        {"00000001h", "\"programConsole\":{\"text\":\"1\","},
        {"0FFFFFFFFh", "\"programConsole\":{\"text\":\"-1\","},
        {"7FFFFFFFh", "\"programConsole\":{\"text\":\"2147483647\","},
        {"80000000h", "\"programConsole\":{\"text\":\"-2147483648\","}
    };
    ...
    for (case_index = 0U; case_index < sizeof(cases) / sizeof(cases[0]); case_index += 1U) {
        ...
        int source_length = snprintf(
            ...
            sizeof(source),
            "INCLUDE Irvine32.inc\n.code\nmain PROC\n    mov eax, %s\n    call WriteInt\n    exit\nmain ENDP\nEND main\n",
            cases[case_index].literal
        );
        if (source_length < 0 || (size_t)source_length >= sizeof(source)) {
            failures += record_failure("Phase 91 WriteInt value fixture should fit in its source buffer");
            continue;
        }
        copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(source));
        failures += expect_json_contains(json, "\"ok\":true", "Phase 91 WriteInt representative value should execute successfully");
        failures += expect_json_contains(json, cases[case_index].expected_text_fragment, "WriteInt should emit the required signed decimal text without a newline");
        failures += expect_json_contains(json, "\"lineCount\":0", "WriteInt representative values should not increase Program Console line count");
        failures += expect_json_contains(json, "\"memoryChanges\":[]", "WriteInt should not create memory-change rows");
    }

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(int_min_source));
    failures += expect_json_contains(json, "\"phase\":93", "Phase 91 WriteInt source-run should report current numeric Phase 93 metadata");
    failures += expect_json_contains(json, "\"phaseName\":\"Phase 93 - Irvine32 WriteBin\"", "Phase 91 WriteInt source-run should report the current full Phase 93 name");
    failures += expect_json_contains(json, "\"sourceRunOutputContract\":\"phase-93-irvine32-writebin-contract-v1\"", "Phase 91 WriteInt source-run should report the current Phase 93 output contract");
    failures += expect_json_contains(json, "\"EAX\":{\"hex\":\"80000000h\"", "WriteInt should preserve the INT32_MIN EAX bit pattern");
    failures += expect_json_not_contains(json, "unsupported-irvine32-routine", "Direct WriteInt should no longer use the deferred routine diagnostic");

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\nOPTION CASEMAP:NONE\n.code\nmain PROC\n    mov eax, 0FFFFFFFFh\n    CALL wRiTeInT\n    exit\nmain ENDP\nEND main\n"
    ));
    failures += expect_json_contains(json, "\"programConsole\":{\"text\":\"-1\",", "Mixed-case WriteInt should emit -1 under CASEMAP:NONE");

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(
        ".code\nmain PROC\n    call WriteInt\nmain ENDP\nEND main\n"
    ));
    failures += expect_json_contains(json, "\"kind\":\"assembly-error\"", "WriteInt without include should be an assembly error");
    failures += expect_json_contains(json, "\"code\":\"missing-irvine32-include\"", "WriteInt without include should use missing-irvine32-include");
    failures += expect_json_contains(json, "CALL WriteInt requires INCLUDE Irvine32.inc before WriteInt can be used as a virtual Irvine32 routine.", "WriteInt missing-include diagnostic should use the exact message");
    failures += expect_json_contains(json, "\"line\":3,\"column\":10,\"byteOffset\":25,\"spanLength\":8", "WriteInt missing-include diagnostic should preserve exact target location");

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n.code\nmain PROC\n    WriteInt\nmain ENDP\nEND main\n"
    ));
    failures += expect_json_contains(json, "\"kind\":\"assembly-error\"", "Bare WriteInt should be an assembly error");
    failures += expect_json_contains(json, "\"code\":\"invalid-irvine32-call-form\"", "Bare WriteInt should use invalid-irvine32-call-form");
    failures += expect_json_contains(json, "WriteInt is a virtual Irvine32 routine and must be called with CALL WriteInt.", "Bare WriteInt should use the exact source-form message");
    failures += expect_json_contains(json, "\"line\":4,\"column\":5,\"byteOffset\":41,\"spanLength\":8", "Bare WriteInt diagnostic should preserve exact routine location");

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n.code\nmain PROC\n    INVOKE WriteInt\nmain ENDP\nEND main\n"
    ));
    failures += expect_json_contains(json, "\"kind\":\"unsupported-feature\"", "INVOKE WriteInt should remain unsupported-feature");
    failures += expect_json_contains(json, "\"code\":\"unsupported-irvine-invoke\"", "INVOKE WriteInt should remain future-owned");
    failures += expect_json_contains(json, "\"line\":4,\"column\":12,\"byteOffset\":48,\"spanLength\":8", "INVOKE WriteInt should preserve exact target location");

    {
        ...
        for (routine_index = 0U; routine_index < sizeof(deferred_routines) / sizeof(deferred_routines[0]); routine_index += 1U) {
            ...
            if (source_length < 0 || (size_t)source_length >= sizeof(deferred_source)) {
                failures += record_failure("Phase 91 deferred Irvine32 sibling fixture should fit in its source buffer");
                continue;
            }
            copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(deferred_source));
            failures += expect_json_contains(json, "\"code\":\"unsupported-irvine32-routine\"", "Phase 91 sibling Irvine32 routines should remain deferred");
            failures += expect_json_not_contains(json, "execution-complete", "Deferred Phase 91 sibling Irvine32 routines should not execute");
        }
    }

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json_with_program_console_limits(int_min_source, 11U, 1U));
    failures += expect_json_contains(json, "\"ok\":true", "WriteInt should succeed with exact eleven-byte capacity");
    failures += expect_json_contains(json, "\"programConsole\":{\"text\":\"-2147483648\",\"truncated\":false,\"byteCount\":11,\"lineCount\":0,\"maxBytes\":11", "WriteInt exact-fit run should commit sign and all digits");

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json_with_program_console_limits(int_min_source, 10U, 1U));
    failures += expect_json_contains(json, "\"ok\":false", "WriteInt should fail when capacity can hold the sign but not the complete value");
    failures += expect_json_contains(json, "\"kind\":\"resource-limit-error\"", "WriteInt output-limit failure should use resource-limit-error");
    failures += expect_json_contains(json, "\"code\":\"console-output-limit-exceeded\"", "WriteInt output-limit failure should use the shared diagnostic code");
    failures += expect_json_contains(json, "\"programConsole\":{\"text\":\"\",\"truncated\":false,\"byteCount\":0,\"lineCount\":0,\"maxBytes\":10", "WriteInt output-limit failure should append neither sign nor partial digits");
    failures += expect_json_contains(json, "\"line\":5,\"column\":5,\"byteOffset\":64,\"spanLength\":13", "WriteInt runtime diagnostic should span the complete call instruction");
    failures += expect_json_not_contains(json, "execution-complete", "Fatal WriteInt output-limit failure should omit execution-complete");
    ...
    failures += expect_json_contains(json, "\"ok\":false", "WriteInt should fail atomically after preserving earlier Program Console output");
    failures += expect_json_contains(json, "\"programConsole\":{\"text\":\"X\",\"truncated\":false,\"byteCount\":1", "WriteInt failure should preserve earlier Program Console output without appending a sign");
    failures += expect_json_not_contains(json, "execution-complete", "WriteInt failure after prior output should omit execution-complete");
    ...
    failures += expect_json_contains(json, "\"ok\":true", "WriteInt should not fail solely because Program Console line count is already at its limit");
    failures += expect_json_contains(json, "\"programConsole\":{\"text\":\"\\n-1\",", "WriteInt should append after an existing newline without adding another line");
    failures += expect_json_contains(json, "\"lineCount\":1", "WriteInt should preserve the already-full Program Console line count");

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n.code\nmain PROC\n    mov eax, 0FFFFFFFFh\n    call WriteDec\n    call WriteInt\n    exit\nmain ENDP\nEND main\n"
    ));
    failures += expect_json_contains(json, "\"programConsole\":{\"text\":\"4294967295-1\",", "WriteDec should remain unsigned while WriteInt interprets the same EAX bits as signed");

    return failures;
}

/// Verifies Phase 92 WriteHex source-run output, diagnostics, limits, and deferral boundaries.
///
/// @return Zero on success, otherwise a positive failure count.
static int test_phase92_irvine32_writehex_source_run(void) {
    int failures = 0;
    char json[TEST_JSON_COPY_CAPACITY];
    const char *hex_source =
        "INCLUDE Irvine32.inc\n"
        ".code\n"
        "main PROC\n"
        "    mov eax, 89ABCDEFh\n"
    ...
    static const struct {
        ...
        {"00000000h", "\"programConsole\":{\"text\":\"00000000\","},
        {"00000001h", "\"programConsole\":{\"text\":\"00000001\","},
        {"0000000Ah", "\"programConsole\":{\"text\":\"0000000A\","},
        {"7FFFFFFFh", "\"programConsole\":{\"text\":\"7FFFFFFF\","},
        {"0FFFFFFFFh", "\"programConsole\":{\"text\":\"FFFFFFFF\","}
    };
    size_t case_index = 0U;
    ...
    for (case_index = 0U; case_index < sizeof(cases) / sizeof(cases[0]); case_index += 1U) {
        ...
        if (source_length < 0 || (size_t)source_length >= sizeof(source)) {
            failures += record_failure("Phase 92 WriteHex value fixture should fit in its source buffer");
            continue;
        }
        copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(source));
        failures += expect_json_contains(json, "\"ok\":true", "Phase 92 WriteHex representative value should execute successfully");
        failures += expect_json_contains(json, cases[case_index].expected_text_fragment, "WriteHex should emit the required fixed-width uppercase hexadecimal text without a newline");
        failures += expect_json_contains(json, "\"byteCount\":8", "WriteHex representative values should append exactly eight bytes");
        failures += expect_json_contains(json, "\"lineCount\":0", "WriteHex representative values should not increase Program Console line count");
        failures += expect_json_contains(json, "\"memoryChanges\":[]", "WriteHex should not create memory-change rows");
    }

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(hex_source));
    failures += expect_json_contains(json, "\"phase\":93", "Phase 92 WriteHex regression should report current numeric Phase 93 metadata");
    failures += expect_json_contains(json, "\"phaseName\":\"Phase 93 - Irvine32 WriteBin\"", "Phase 92 WriteHex source-run should report the full phase name");
    failures += expect_json_contains(json, "\"sourceRunOutputContract\":\"phase-93-irvine32-writebin-contract-v1\"", "Phase 92 WriteHex source-run should report the exact output contract");
    failures += expect_json_contains(json, "\"EAX\":{\"hex\":\"89ABCDEFh\"", "WriteHex should preserve the EAX bit pattern");
    failures += expect_json_not_contains(json, "unsupported-irvine32-routine", "Direct WriteHex should no longer use the deferred routine diagnostic");

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\nOPTION CASEMAP:NONE\n.code\nmain PROC\n    mov eax, 0000000Ah\n    CALL wRiTeHeX\n    exit\nmain ENDP\nEND main\n"
    ));
    failures += expect_json_contains(json, "\"programConsole\":{\"text\":\"0000000A\",", "Mixed-case WriteHex should emit uppercase fixed-width hexadecimal under CASEMAP:NONE");

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(
        ".code\nmain PROC\n    call WriteHex\nmain ENDP\nEND main\n"
    ));
    failures += expect_json_contains(json, "\"kind\":\"assembly-error\"", "WriteHex without include should be an assembly error");
    failures += expect_json_contains(json, "\"code\":\"missing-irvine32-include\"", "WriteHex without include should use missing-irvine32-include");
    failures += expect_json_contains(json, "CALL WriteHex requires INCLUDE Irvine32.inc before WriteHex can be used as a virtual Irvine32 routine.", "WriteHex missing-include diagnostic should use the exact message");
    failures += expect_json_contains(json, "\"line\":3,\"column\":10,\"byteOffset\":25,\"spanLength\":8", "WriteHex missing-include diagnostic should preserve exact target location");

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n.code\nmain PROC\n    WriteHex\nmain ENDP\nEND main\n"
    ));
    failures += expect_json_contains(json, "\"kind\":\"assembly-error\"", "Bare WriteHex should be an assembly error");
    failures += expect_json_contains(json, "\"code\":\"invalid-irvine32-call-form\"", "Bare WriteHex should use invalid-irvine32-call-form");
    failures += expect_json_contains(json, "WriteHex is a virtual Irvine32 routine and must be called with CALL WriteHex.", "Bare WriteHex should use the exact source-form message");
    failures += expect_json_contains(json, "\"line\":4,\"column\":5,\"byteOffset\":41,\"spanLength\":8", "Bare WriteHex diagnostic should preserve exact routine location");

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n.code\nmain PROC\n    INVOKE WriteHex\nmain ENDP\nEND main\n"
    ));
    failures += expect_json_contains(json, "\"kind\":\"unsupported-feature\"", "INVOKE WriteHex should remain unsupported-feature");
    failures += expect_json_contains(json, "\"code\":\"unsupported-irvine-invoke\"", "INVOKE WriteHex should remain future-owned");
    failures += expect_json_contains(json, "\"line\":4,\"column\":12,\"byteOffset\":48,\"spanLength\":8", "INVOKE WriteHex should preserve exact target location");

    {
        ...
        for (routine_index = 0U; routine_index < sizeof(deferred_routines) / sizeof(deferred_routines[0]); routine_index += 1U) {
            ...
            if (source_length < 0 || (size_t)source_length >= sizeof(deferred_source)) {
                failures += record_failure("Phase 92 deferred Irvine32 sibling fixture should fit in its source buffer");
                continue;
            }
            copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(deferred_source));
            failures += expect_json_contains(json, "\"code\":\"unsupported-irvine32-routine\"", "Phase 92 sibling Irvine32 routines should remain deferred");
            failures += expect_json_not_contains(json, "execution-complete", "Deferred Phase 92 sibling Irvine32 routines should not execute");
        }
    }

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json_with_program_console_limits(hex_source, 8U, 1U));
    failures += expect_json_contains(json, "\"ok\":true", "WriteHex should succeed with exact eight-byte capacity");
    failures += expect_json_contains(json, "\"programConsole\":{\"text\":\"89ABCDEF\",\"truncated\":false,\"byteCount\":8,\"lineCount\":0,\"maxBytes\":8", "WriteHex exact-fit run should commit all eight digits");

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json_with_program_console_limits(hex_source, 7U, 1U));
    failures += expect_json_contains(json, "\"ok\":false", "WriteHex should fail when only seven bytes are available");
    failures += expect_json_contains(json, "\"kind\":\"resource-limit-error\"", "WriteHex output-limit failure should use resource-limit-error");
    failures += expect_json_contains(json, "\"code\":\"console-output-limit-exceeded\"", "WriteHex output-limit failure should use the shared diagnostic code");
    failures += expect_json_contains(json, "\"programConsole\":{\"text\":\"\",\"truncated\":false,\"byteCount\":0,\"lineCount\":0,\"maxBytes\":7", "WriteHex output-limit failure should append no partial digits");
    failures += expect_json_contains(json, "\"line\":5,\"column\":5,\"byteOffset\":64,\"spanLength\":13", "WriteHex runtime diagnostic should span the complete call instruction");
    failures += expect_json_not_contains(json, "execution-complete", "Fatal WriteHex output-limit failure should omit execution-complete");
    ...
    failures += expect_json_contains(json, "\"ok\":false", "WriteHex should fail atomically after preserving earlier Program Console output");
    failures += expect_json_contains(json, "\"programConsole\":{\"text\":\"X\",\"truncated\":false,\"byteCount\":1", "WriteHex failure should preserve earlier Program Console output without appending partial digits");
    failures += expect_json_not_contains(json, "execution-complete", "WriteHex failure after prior output should omit execution-complete");
    ...
    failures += expect_json_contains(json, "\"ok\":true", "WriteHex should not fail solely because Program Console line count is already at its limit");
    failures += expect_json_contains(json, "\"programConsole\":{\"text\":\"\\n0000000A\",", "WriteHex should append after an existing newline without adding another line");
    failures += expect_json_contains(json, "\"lineCount\":1", "WriteHex should preserve the already-full Program Console line count");

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n.code\nmain PROC\n    mov eax, 0FFFFFFFFh\n    call WriteDec\n    call WriteInt\n    call WriteHex\n    exit\nmain ENDP\nEND main\n"
    ));
    failures += expect_json_contains(json, "\"programConsole\":{\"text\":\"4294967295-1FFFFFFFF\",", "WriteDec, WriteInt, and WriteHex should preserve their distinct interpretations of the same EAX bits");

    return failures;
}

/// Verifies Phase 93 WriteBin source-run output, diagnostics, limits, and deferral boundaries.
///
/// @return Zero on success, otherwise a positive failure count.
static int test_phase93_irvine32_writebin_source_run(void) {
    int failures = 0;
    char json[TEST_JSON_COPY_CAPACITY];
    const char *bin_source =
        "INCLUDE Irvine32.inc\n"
        ".code\n"
        "main PROC\n"
        "    mov eax, 89ABCDEFh\n"
    ...
    static const struct {
        ...
        {"00000000h", "\"programConsole\":{\"text\":\"00000000000000000000000000000000\","},
        {"0FFFFFFFFh", "\"programConsole\":{\"text\":\"11111111111111111111111111111111\","},
        {"00000001h", "\"programConsole\":{\"text\":\"00000000000000000000000000000001\","},
        {"80000000h", "\"programConsole\":{\"text\":\"10000000000000000000000000000000\","},
        {"0AAAAAAAAh", "\"programConsole\":{\"text\":\"10101010101010101010101010101010\","},
        {"55555555h", "\"programConsole\":{\"text\":\"01010101010101010101010101010101\","}
    };
    size_t case_index = 0U;
    ...
    for (case_index = 0U; case_index < sizeof(cases) / sizeof(cases[0]); case_index += 1U) {
        ...
        if (source_length < 0 || (size_t)source_length >= sizeof(source)) {
            failures += record_failure("Phase 93 WriteBin value fixture should fit in its source buffer");
            continue;
        }
        copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(source));
        failures += expect_json_contains(json, "\"ok\":true", "Phase 93 WriteBin representative value should execute successfully");
        failures += expect_json_contains(json, cases[case_index].expected_text_fragment, "WriteBin should emit the required fixed-width most-significant-bit-first binary text without a newline");
        failures += expect_json_contains(json, "\"byteCount\":32", "WriteBin representative values should append exactly 32 bytes");
        failures += expect_json_contains(json, "\"lineCount\":0", "WriteBin representative values should not increase Program Console line count");
        failures += expect_json_contains(json, "\"memoryChanges\":[]", "WriteBin should not create memory-change rows");
    }

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(bin_source));
    failures += expect_json_contains(json, "\"phase\":93", "Phase 93 WriteBin source-run should report numeric Phase 93 metadata");
    failures += expect_json_contains(json, "\"phaseName\":\"Phase 93 - Irvine32 WriteBin\"", "Phase 93 WriteBin source-run should report the full phase name");
    failures += expect_json_contains(json, "\"sourceRunOutputContract\":\"phase-93-irvine32-writebin-contract-v1\"", "Phase 93 WriteBin source-run should report the exact output contract");
    failures += expect_json_contains(json, "\"EAX\":{\"hex\":\"89ABCDEFh\"", "WriteBin should preserve the EAX bit pattern");
    failures += expect_json_not_contains(json, "unsupported-irvine32-routine", "Direct WriteBin should no longer use the deferred routine diagnostic");

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\nOPTION CASEMAP:NONE\n.code\nmain PROC\n    mov eax, 0000000Ah\n    CALL wRiTeBiN\n    exit\nmain ENDP\nEND main\n"
    ));
    failures += expect_json_contains(json, "\"programConsole\":{\"text\":\"00000000000000000000000000001010\",", "Mixed-case WriteBin should emit fixed-width binary under CASEMAP:NONE");

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(
        ".code\nmain PROC\n    call WriteBin\nmain ENDP\nEND main\n"
    ));
    failures += expect_json_contains(json, "\"kind\":\"assembly-error\"", "WriteBin without include should be an assembly error");
    failures += expect_json_contains(json, "\"code\":\"missing-irvine32-include\"", "WriteBin without include should use missing-irvine32-include");
    failures += expect_json_contains(json, "CALL WriteBin requires INCLUDE Irvine32.inc before WriteBin can be used as a virtual Irvine32 routine.", "WriteBin missing-include diagnostic should use the exact message");
    failures += expect_json_contains(json, "\"line\":3,\"column\":10,\"byteOffset\":25,\"spanLength\":8", "WriteBin missing-include diagnostic should preserve exact target location");

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n.code\nmain PROC\n    WriteBin\nmain ENDP\nEND main\n"
    ));
    failures += expect_json_contains(json, "\"kind\":\"assembly-error\"", "Bare WriteBin should be an assembly error");
    failures += expect_json_contains(json, "\"code\":\"invalid-irvine32-call-form\"", "Bare WriteBin should use invalid-irvine32-call-form");
    failures += expect_json_contains(json, "WriteBin is a virtual Irvine32 routine and must be called with CALL WriteBin.", "Bare WriteBin should use the exact source-form message");
    failures += expect_json_contains(json, "\"line\":4,\"column\":5,\"byteOffset\":41,\"spanLength\":8", "Bare WriteBin diagnostic should preserve exact routine location");

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n.code\nmain PROC\n    INVOKE WriteBin\nmain ENDP\nEND main\n"
    ));
    failures += expect_json_contains(json, "\"kind\":\"unsupported-feature\"", "INVOKE WriteBin should remain unsupported-feature");
    failures += expect_json_contains(json, "\"code\":\"unsupported-irvine-invoke\"", "INVOKE WriteBin should remain future-owned");
    failures += expect_json_contains(json, "\"line\":4,\"column\":12,\"byteOffset\":48,\"spanLength\":8", "INVOKE WriteBin should preserve exact target location");

    {
        ...
        for (routine_index = 0U; routine_index < sizeof(deferred_routines) / sizeof(deferred_routines[0]); routine_index += 1U) {
            ...
            if (source_length < 0 || (size_t)source_length >= sizeof(deferred_source)) {
                failures += record_failure("Phase 93 deferred Irvine32 sibling fixture should fit in its source buffer");
                continue;
            }
            copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(deferred_source));
            failures += expect_json_contains(json, "\"code\":\"unsupported-irvine32-routine\"", "Phase 93 sibling Irvine32 routines should remain deferred");
            failures += expect_json_not_contains(json, "execution-complete", "Deferred Phase 93 sibling Irvine32 routines should not execute");
        }
    }

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json_with_program_console_limits(bin_source, 32U, 1U));
    failures += expect_json_contains(json, "\"ok\":true", "WriteBin should succeed with exact 32-byte capacity");
    failures += expect_json_contains(json, "\"programConsole\":{\"text\":\"10001001101010111100110111101111\",\"truncated\":false,\"byteCount\":32,\"lineCount\":0,\"maxBytes\":32", "WriteBin exact-fit run should commit all 32 digits");

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json_with_program_console_limits(bin_source, 31U, 1U));
    failures += expect_json_contains(json, "\"ok\":false", "WriteBin should fail when only 31 bytes are available");
    failures += expect_json_contains(json, "\"kind\":\"resource-limit-error\"", "WriteBin output-limit failure should use resource-limit-error");
    failures += expect_json_contains(json, "\"code\":\"console-output-limit-exceeded\"", "WriteBin output-limit failure should use the shared diagnostic code");
    failures += expect_json_contains(json, "\"programConsole\":{\"text\":\"\",\"truncated\":false,\"byteCount\":0,\"lineCount\":0,\"maxBytes\":31", "WriteBin output-limit failure should append no partial digits");
    failures += expect_json_contains(json, "\"line\":5,\"column\":5,\"byteOffset\":64,\"spanLength\":13", "WriteBin runtime diagnostic should span the complete call instruction");
    failures += expect_json_not_contains(json, "execution-complete", "Fatal WriteBin output-limit failure should omit execution-complete");
    ...
    failures += expect_json_contains(json, "\"ok\":false", "WriteBin should fail atomically after preserving earlier Program Console output");
    failures += expect_json_contains(json, "\"programConsole\":{\"text\":\"X\",\"truncated\":false,\"byteCount\":1", "WriteBin failure should preserve earlier Program Console output without appending partial digits");
    failures += expect_json_not_contains(json, "execution-complete", "WriteBin failure after prior output should omit execution-complete");
    ...
    failures += expect_json_contains(json, "\"ok\":true", "WriteBin should not fail solely because Program Console line count is already at its limit");
    failures += expect_json_contains(json, "\"programConsole\":{\"text\":\"\\n00000000000000000000000000001010\",", "WriteBin should append after an existing newline without adding another line");
    failures += expect_json_contains(json, "\"lineCount\":1", "WriteBin should preserve the already-full Program Console line count");

    copy_source_run_json(json, sizeof(json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n.code\nmain PROC\n    mov eax, 0FFFFFFFFh\n    call WriteDec\n    call WriteInt\n    call WriteBin\n    exit\nmain ENDP\nEND main\n"
    ));
    failures += expect_json_contains(json, "\"programConsole\":{\"text\":\"4294967295-111111111111111111111111111111111\",", "WriteDec, WriteInt, and WriteBin should preserve their distinct output contracts for the same EAX bits");
...
static int test_phase73_leave_source_run_semantics_and_diagnostics(void) {
    ...
    failures += expect_json_contains(direct_json, "\"phase\":93", "Current metadata should report numeric Phase 93");
    failures += expect_json_contains(direct_json, "\"phaseSuffix\":\"\"", "Current metadata should report empty suffix metadata");
    failures += expect_json_contains(direct_json, TEST_SOURCE_RUN_OUTPUT_CONTRACT_FRAGMENT, "Current metadata should report the Phase 93 output contract");
...
/// Verifies current source-run metadata and the default callDepthLimit.
///
/// @return Zero on success, otherwise a positive failure count.
static int test_phase73_source_run_default_metadata_and_call_depth(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":93", "Current default source-run metadata should report numeric phase");
    failures += expect_json_contains(json, "\"phaseSuffix\":\"\"", "Current default source-run metadata should report suffix metadata");
    failures += expect_json_contains(json, "\"phaseName\":\"Phase 93 - Irvine32 WriteBin\"", "Current default source-run metadata should report full Phase 93 name");
    failures += expect_json_contains(json, TEST_SOURCE_RUN_OUTPUT_CONTRACT_FRAGMENT, "Current default source-run metadata should report current output contract");
...
static int test_phase74_ret_imm16_source_run_semantics_and_diagnostics(void) {
    ...
    failures += expect_json_contains(acceptance_json, "\"phase\":93", "Phase 74 RET imm16 regression should report current numeric phase");
...
static int test_phase72_invalid_call_depth_limit_source_run(void) {
    ...
    failures += expect_json_contains(low_copy, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "Invalid callDepthLimit response should include the current empty Program Console limit fields");
    failures += expect_json_contains(high_json, TEST_EMPTY_PROGRAM_CONSOLE_FRAGMENT, "Oversized callDepthLimit response should include the current empty Program Console limit fields");
...
/// Verifies Phase 78A limited OPTION NOKEYWORD source-run behavior.
///
/// @return Zero on success, otherwise a positive failure count.
static int test_phase78a_nokeyword_source_run(void) {
    int failures = 0;
    char success_json[TEST_JSON_COPY_CAPACITY];
    char atomic_json[TEST_JSON_COPY_CAPACITY];
    char unknown_json[TEST_JSON_COPY_CAPACITY];
    char disabled_loop_json[TEST_JSON_COPY_CAPACITY];
    char disabled_offset_json[TEST_JSON_COPY_CAPACITY];
    ...
    failures += expect_json_contains(success_json, "\"ok\":true", "Phase 78A accepted OPTION NOKEYWORD source should run");
    failures += expect_json_contains(success_json, "\"phase\":93", "Current metadata should report numeric Phase 93");
    failures += expect_json_contains(success_json, "\"phaseSuffix\":\"\"", "Current metadata should report an empty suffix");
    failures += expect_json_contains(success_json, TEST_SOURCE_RUN_OUTPUT_CONTRACT_FRAGMENT, "Current metadata should report the Phase 93 output contract");
    failures += expect_json_contains(success_json, "\"EAX\":{\"hex\":\"00001234h\"", "disabled OFFSET data symbol should load normally");
    failures += expect_json_contains(success_json, "\"EBX\":{\"hex\":\"00000005h\"", "disabled LOOP data symbol should load normally");
    failures += expect_json_not_contains(success_json, "unsupported-option", "accepted Phase 78A OPTION NOKEYWORD source should not use unsupported-option");

    copy_source_run_json(atomic_json, sizeof(atomic_json), masm32_sim_wasm_run_source_json(
    ...
    failures += expect_json_contains(atomic_json, "\"ok\":false", "invalid mixed NOKEYWORD list should fail source-run");
    failures += expect_json_contains(atomic_json, "nokeyword-protected-keyword", "protected NOKEYWORD word should use targeted diagnostic");
    failures += expect_json_contains(atomic_json, "reserved-word-symbol", "atomic NOKEYWORD failure should not disable earlier valid words");
    failures += expect_json_not_contains(atomic_json, "execution-complete", "NOKEYWORD parse failures should not execute");
    ...
    failures += expect_json_contains(unknown_json, "\"ok\":false", "unknown NOKEYWORD word should fail source-run");
    failures += expect_json_contains(unknown_json, "nokeyword-unknown-keyword", "unknown NOKEYWORD word should use targeted diagnostic");

    copy_source_run_json(disabled_loop_json, sizeof(disabled_loop_json), masm32_sim_wasm_run_source_json(
    ...
    failures += expect_json_contains(disabled_loop_json, "\"ok\":false", "disabled LOOP old keyword role should fail source-run");
    failures += expect_json_contains(disabled_loop_json, "disabled-keyword-used-as-keyword", "disabled LOOP old keyword role should use targeted diagnostic");

    copy_source_run_json(disabled_offset_json, sizeof(disabled_offset_json), masm32_sim_wasm_run_source_json(
    ...
    failures += expect_json_contains(disabled_offset_json, "\"ok\":false", "disabled OFFSET old operator role should fail source-run");
    failures += expect_json_contains(disabled_offset_json, "disabled-keyword-used-as-keyword", "disabled OFFSET old operator role should use targeted diagnostic");

    return failures;
}

/// Verifies Phase 79 automatic LOCAL frames and Phase 80 LOCAL operand access.
///
/// @return Zero on success, otherwise a positive failure count.
static int test_phase79_local_frame_source_run(void) {
    int failures = 0;
    char success_json[TEST_JSON_COPY_CAPACITY];
    char local_operand_json[TEST_JSON_COPY_CAPACITY];
    char local_byte_json[TEST_JSON_COPY_CAPACITY];
    char local_lea_json[TEST_JSON_COPY_CAPACITY];
    char local_same_name_json[TEST_JSON_COPY_CAPACITY];
    char local_recursive_operand_json[TEST_JSON_COPY_CAPACITY];
    char local_object_bounds_json[TEST_JSON_COPY_CAPACITY];
    char local_padding_bounds_json[TEST_JSON_COPY_CAPACITY];
    char local_uninitialized_warning_json[TEST_JSON_COPY_CAPACITY];
    char local_uninitialized_strict_json[TEST_JSON_COPY_CAPACITY];
    char local_partial_uninitialized_json[TEST_JSON_COPY_CAPACITY];
    char direct_call_json[TEST_JSON_COPY_CAPACITY];
    char uses_ret_imm_json[TEST_JSON_COPY_CAPACITY];
    char leave_ret_json[TEST_JSON_COPY_CAPACITY];
    char local_stack_overflow_json[TEST_JSON_COPY_CAPACITY];
    char selected_entry_stack_overflow_json[TEST_JSON_COPY_CAPACITY];
    char nested_call_json[TEST_JSON_COPY_CAPACITY];
    char recursive_call_json[TEST_JSON_COPY_CAPACITY];
    char fallthrough_entry_json[TEST_JSON_COPY_CAPACITY];
    char branch_entry_json[TEST_JSON_COPY_CAPACITY];
    char include_branch_entry_json[TEST_JSON_COPY_CAPACITY];
    char invalid_type_json[TEST_JSON_COPY_CAPACITY];
    char invalid_count_json[TEST_JSON_COPY_CAPACITY];
    ...
    failures += expect_json_contains(success_json, "\"ok\":true", "Phase 79 LOCAL frame source should execute supported non-local instructions");
    failures += expect_json_contains(success_json, "\"phase\":93", "Phase 79 LOCAL frame source should report current numeric phase");
    failures += expect_json_contains(success_json, TEST_SOURCE_RUN_OUTPUT_CONTRACT_FRAGMENT, "Phase 79 LOCAL frame source should report current output contract");
    failures += expect_json_contains(success_json, "\"EAX\":{\"hex\":\"00001234h\",\"unsigned\":4660}", "Phase 79 LOCAL frame source should preserve ordinary execution");
    failures += expect_json_not_contains(success_json, "unsupported-feature", "valid Phase 79 LOCAL declarations should not be generic unsupported features");
    failures += expect_json_not_contains(success_json, "stack-overflow", "Phase 79 LOCAL frame allocation should fit the default stack");
    failures += expect_json_not_contains(success_json, "stack-underflow", "Phase 79 LOCAL frame release should not underflow the default stack");
    failures += expect_json_contains(success_json, "\"EBP\":{\"hex\":\"00000000h\",\"unsigned\":0}", "Phase 79 LOCAL frame release should restore EBP before exit");
    failures += expect_json_contains(success_json, "\"memoryChanges\":[]", "Phase 79 automatic LOCAL stack housekeeping should remain hidden from public memoryChanges");


    copy_source_run_json(direct_call_json, sizeof(direct_call_json), masm32_sim_wasm_run_source_json(
        ".code\n"
        "main PROC\n"
        "    call Helper\n"
        "    mov eax, esp\n"
    ...
    failures += expect_json_contains(direct_call_json, "\"ok\":true", "Phase 79 direct-CALL LOCAL helper should run successfully");
    failures += expect_json_contains(direct_call_json, "\"EBX\":{\"hex\":\"00000005h\",\"unsigned\":5}", "Phase 79 direct-CALL LOCAL helper should execute body instructions");
    failures += expect_json_contains(direct_call_json, "\"EAX\":{\"hex\":\"00900000h\",\"unsigned\":9437184}", "Phase 79 direct-CALL LOCAL helper should restore ESP before caller continues");
    failures += expect_json_contains(direct_call_json, "\"memoryChanges\":[]", "Phase 79 automatic direct-CALL LOCAL stack housekeeping should remain hidden");

    copy_source_run_json(uses_ret_imm_json, sizeof(uses_ret_imm_json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n"
        ".code\n"
        "main PROC\n"
        "    mov eax, 1234h\n"
    ...
    failures += expect_json_contains(uses_ret_imm_json, "\"ok\":true", "Phase 79 LOCAL helper with PROC USES and RET imm16 should run successfully");
    failures += expect_json_contains(uses_ret_imm_json, "\"EAX\":{\"hex\":\"00000063h\",\"unsigned\":99}", "Phase 79 LOCAL helper should preserve unlisted EAX return value through USES restore");
    failures += expect_json_contains(uses_ret_imm_json, "\"EBX\":{\"hex\":\"00001111h\",\"unsigned\":4369}", "Phase 79 LOCAL helper should restore listed EBX before return");
    failures += expect_json_contains(uses_ret_imm_json, "\"ESI\":{\"hex\":\"00002222h\",\"unsigned\":8738}", "Phase 79 LOCAL helper should restore listed ESI before return");
    failures += expect_json_contains(uses_ret_imm_json, "\"EDX\":{\"hex\":\"00900000h\",\"unsigned\":9437184}", "Phase 79 RET imm16 cleanup should run only after LOCAL and USES cleanup");

    copy_source_run_json(leave_ret_json, sizeof(leave_ret_json), masm32_sim_wasm_run_source_json(
        ".code\n"
        "main PROC\n"
        "    call Helper\n"
        "    mov eax, esp\n"
    ...
    failures += expect_json_contains(leave_ret_json, "\"ok\":true", "Phase 79 LEAVE followed by RET should release a LOCAL frame exactly once");
    failures += expect_json_contains(leave_ret_json, "\"EBX\":{\"hex\":\"00000006h\",\"unsigned\":6}", "Phase 79 LEAVE+RET helper should execute body before release");
    failures += expect_json_contains(leave_ret_json, "\"EAX\":{\"hex\":\"00900000h\",\"unsigned\":9437184}", "Phase 79 LEAVE+RET helper should restore ESP before caller continues");
    failures += expect_json_not_contains(leave_ret_json, "invalid-frame-state", "Phase 79 LEAVE+RET should not double-release the automatic frame");

    copy_source_run_json(local_stack_overflow_json, sizeof(local_stack_overflow_json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n"
        ".code\n"
        "main PROC\n"
        "    mov esp, 008F0008h\n"
    ...
    failures += expect_json_contains(local_stack_overflow_json, "\"ok\":false", "Phase 79 combined direct-CALL LOCAL footprint overflow should fail source-run");
    failures += expect_json_contains(local_stack_overflow_json, "\"code\":\"stack-overflow\"", "Phase 79 combined direct-CALL LOCAL footprint overflow should use stack-overflow");
    failures += expect_json_contains(local_stack_overflow_json, "\"procedure\":\"Helper\"", "Phase 79 combined direct-CALL LOCAL overflow should identify the target procedure");
    failures += expect_json_contains(local_stack_overflow_json, "direct-call-local-combined-frame-setup", "Phase 79 combined direct-CALL LOCAL overflow should report the operation stage");
    failures += expect_json_contains(local_stack_overflow_json, "\"EBX\":{\"hex\":\"00000000h\"", "Phase 79 combined direct-CALL LOCAL overflow should stop before helper or following caller mutation");
    failures += expect_json_contains(local_stack_overflow_json, "\"ESP\":{\"hex\":\"008F0008h\"", "Phase 79 combined direct-CALL LOCAL overflow should preserve caller ESP");
    failures += expect_json_contains(local_stack_overflow_json, "\"memoryChanges\":[]", "Phase 79 failed automatic LOCAL setup should not expose memoryChanges");

    copy_source_run_json(selected_entry_stack_overflow_json, sizeof(selected_entry_stack_overflow_json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n"
        ".code\n"
        "main PROC\n"
        "    LOCAL huge[2000000]:BYTE\n"
        "    mov eax, 1\n"
        "    exit\n"
        "main ENDP\n"
        "END main\n"
    ));
    failures += expect_json_contains(selected_entry_stack_overflow_json, "\"ok\":false", "Phase 79 selected-entry oversized LOCAL frame should fail before body execution");
    failures += expect_json_contains(selected_entry_stack_overflow_json, "\"code\":\"stack-overflow\"", "Phase 79 selected-entry oversized LOCAL frame should use stack-overflow");
    failures += expect_json_contains(selected_entry_stack_overflow_json, "selected-entry-local-frame-setup", "Phase 79 selected-entry oversized LOCAL frame should report setup stage");
    failures += expect_json_contains(selected_entry_stack_overflow_json, "\"EAX\":{\"hex\":\"00000000h\"", "Phase 79 selected-entry oversized LOCAL frame should stop before body mutation");
    failures += expect_json_contains(selected_entry_stack_overflow_json, "\"ESP\":{\"hex\":\"00900000h\"", "Phase 79 selected-entry oversized LOCAL frame should preserve initial ESP");

    copy_source_run_json(nested_call_json, sizeof(nested_call_json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n"
        ".code\n"
        "main PROC\n"
        "    call First\n"
    ...
    failures += expect_json_contains(nested_call_json, "\"ok\":true", "Phase 79 nested LOCAL helper frames should run successfully");
    failures += expect_json_contains(nested_call_json, "\"EBX\":{\"hex\":\"00000001h\",\"unsigned\":1}", "Phase 79 outer LOCAL helper should resume after nested return");
    failures += expect_json_contains(nested_call_json, "\"ECX\":{\"hex\":\"00000002h\",\"unsigned\":2}", "Phase 79 nested LOCAL helper should execute body");
    failures += expect_json_contains(nested_call_json, "\"ESP\":{\"hex\":\"00900000h\"", "Phase 79 nested LOCAL helper frames should restore ESP");
    failures += expect_json_contains(nested_call_json, "\"EBP\":{\"hex\":\"00000000h\"", "Phase 79 nested LOCAL helper frames should restore EBP");

    copy_source_run_json(recursive_call_json, sizeof(recursive_call_json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n"
        ".code\n"
        "main PROC\n"
        "    call Recur\n"
    ...
    failures += expect_json_contains(recursive_call_json, "\"ok\":true", "Phase 79 recursive direct-CALL LOCAL frames should run successfully");
    failures += expect_json_contains(recursive_call_json, "\"ECX\":{\"hex\":\"00000003h\",\"unsigned\":3}", "Phase 79 recursive LOCAL frames should keep separate frame instances until the base case");
    failures += expect_json_contains(recursive_call_json, "\"ESP\":{\"hex\":\"00900000h\"", "Phase 79 recursive LOCAL frames should restore ESP after unwinding");
    failures += expect_json_contains(recursive_call_json, "\"EBP\":{\"hex\":\"00000000h\"", "Phase 79 recursive LOCAL frames should restore EBP after unwinding");

    copy_source_run_json(fallthrough_entry_json, sizeof(fallthrough_entry_json), masm32_sim_wasm_run_source_json(
        ".code\n"
        "main PROC\n"
        "    mov eax, 1\n"
        "main ENDP\n"
    ...
    failures += expect_json_contains(fallthrough_entry_json, "\"ok\":false", "Phase 79 fallthrough entry into LOCAL helper should fail");
    failures += expect_json_contains(fallthrough_entry_json, "local-frame-entry-unsupported", "Phase 79 fallthrough entry into LOCAL helper should use targeted diagnostic");
    failures += expect_json_contains(fallthrough_entry_json, "\"procedure\":\"Helper\"", "Phase 79 fallthrough LOCAL frame-entry diagnostic should identify the destination procedure");
    failures += expect_json_not_contains(fallthrough_entry_json, "\"EBX\":{\"hex\":\"00000005h", "Phase 79 fallthrough rejection should stop before destination body mutation");

    copy_source_run_json(branch_entry_json, sizeof(branch_entry_json), masm32_sim_wasm_run_source_json(
        ".code\n"
        "main PROC\n"
        "    jmp Helper\n"
        "    ret\n"
    ...
    failures += expect_json_contains(branch_entry_json, "\"ok\":false", "Phase 79 branch entry into LOCAL helper should fail");
    failures += expect_json_contains(branch_entry_json, "local-frame-entry-unsupported", "Phase 79 branch entry into LOCAL helper should use targeted diagnostic");
    failures += expect_json_contains(branch_entry_json, "\"procedure\":\"Helper\"", "Phase 79 LOCAL frame-entry diagnostic should identify the target procedure");

    copy_source_run_json(include_branch_entry_json, sizeof(include_branch_entry_json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n"
        ".code\n"
        "main PROC\n"
        "    jmp Helper\n"
    ...
    failures += expect_json_contains(include_branch_entry_json, "\"ok\":false", "Phase 79 INCLUDE/exit branch entry into LOCAL helper should fail");
    failures += expect_json_contains(include_branch_entry_json, "local-frame-entry-unsupported", "Phase 79 INCLUDE/exit branch entry should use targeted LOCAL frame diagnostic");
    failures += expect_json_contains(include_branch_entry_json, "\"EBX\":{\"hex\":\"00000000h\"", "Phase 79 INCLUDE/exit branch entry should stop before destination body mutation");
    failures += expect_json_not_contains(include_branch_entry_json, "invalid-address", "Phase 79 INCLUDE/exit branch entry should not fall through to RET stack-token failure");

    copy_source_run_json(local_operand_json, sizeof(local_operand_json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n"
        ".code\n"
        "main PROC\n"
        "    LOCAL temp:DWORD\n"
    ...
    failures += expect_json_contains(local_operand_json, "\"ok\":true", "Phase 80 should run DWORD LOCAL read/write operands");
    failures += expect_json_contains(local_operand_json, "\"EAX\":{\"hex\":\"0000002Ah\"", "Phase 80 LOCAL read should load the written DWORD value");
    failures += expect_json_not_contains(local_operand_json, "unknown-symbol", "Phase 80 LOCAL operands should no longer fail parser symbol resolution");

    copy_source_run_json(local_byte_json, sizeof(local_byte_json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n"
        ".code\n"
        "main PROC\n"
        "    LOCAL buf[2]:BYTE\n"
    ...
    failures += expect_json_contains(local_byte_json, "\"ok\":true", "Phase 80 should run BYTE LOCAL array operands");
    failures += expect_json_contains(local_byte_json, "\"EAX\":{\"hex\":\"00000041h\"", "Phase 80 BYTE LOCAL array read should load ASCII A into AL");

    copy_source_run_json(local_lea_json, sizeof(local_lea_json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n"
        ".code\n"
        "main PROC\n"
        "    LOCAL temp:DWORD\n"
    ...
    failures += expect_json_contains(local_lea_json, "\"ok\":true", "Phase 80 should run LEA over LOCAL operands");
    failures += expect_json_contains(local_lea_json, "\"EAX\":{\"hex\":\"008FFFF8h\"", "Phase 80 LEA LOCAL should return EBP-relative runtime address");

    copy_source_run_json(local_same_name_json, sizeof(local_same_name_json), masm32_sim_wasm_run_source_json(
        ".code\n"
        "main PROC\n"
        "    LOCAL temp:DWORD\n"
        "    mov temp, 11\n"
    ...
    failures += expect_json_contains(local_same_name_json, "\"ok\":true", "Phase 80 same-named LOCALs in separate procedures should run");
    failures += expect_json_contains(local_same_name_json, "\"EAX\":{\"hex\":\"0000002Ah\"", "Phase 80 helper LOCAL should resolve to helper frame");
    failures += expect_json_contains(local_same_name_json, "\"EBX\":{\"hex\":\"0000000Bh\"", "Phase 80 caller LOCAL should remain distinct after helper returns");

    copy_source_run_json(local_recursive_operand_json, sizeof(local_recursive_operand_json), masm32_sim_wasm_run_source_json(
        "INCLUDE Irvine32.inc\n"
        ".code\n"
        "main PROC\n"
        "    call Recur\n"
    ...
    failures += expect_json_contains(local_recursive_operand_json, "\"ok\":true", "Phase 80 recursive LOCAL operands should run");
    failures += expect_json_contains(local_recursive_operand_json, "\"EAX\":{\"hex\":\"00000003h\"", "Phase 80 deepest recursive frame should read its own LOCAL value");
    failures += expect_json_contains(local_recursive_operand_json, "\"EBX\":{\"hex\":\"00000001h\"", "Phase 80 unwound recursive frame should read the current frame LOCAL value");

    copy_source_run_json(local_object_bounds_json, sizeof(local_object_bounds_json), masm32_sim_wasm_run_source_json_with_memory_validation_mode(
        ".code\n"
        "main PROC\n"
        "    LOCAL buf[2]:BYTE\n"
        "    mov DWORD PTR buf[1], 1234h\n"
        "    ret\n"
        "main ENDP\n"
        "END main\n",
        MASM32_SIM_WASM_MEMORY_VALIDATION_ALLOCATED_OBJECT_STRICT
    ));
    failures += expect_json_contains(local_object_bounds_json, "\"ok\":false", "Phase 80 strict LOCAL object bounds should stop execution");
    failures += expect_json_contains(local_object_bounds_json, "object-bounds-violation", "Phase 80 strict LOCAL object bounds should use object-bounds-violation");
    failures += expect_json_contains(local_object_bounds_json, "LOCAL main.buf at byte offset 1 for 4 bytes", "Phase 80 LOCAL object-bounds diagnostic should name procedure, local, offset, and width");
    failures += expect_json_contains(local_object_bounds_json, "\"line\":4", "Phase 80 LOCAL object-bounds diagnostic should preserve line");
    failures += expect_json_contains(local_object_bounds_json, "\"column\":19", "Phase 80 LOCAL object-bounds diagnostic should preserve operand column");
    failures += expect_json_contains(local_object_bounds_json, "\"spanLength\":3", "Phase 80 LOCAL object-bounds diagnostic should span the local token");
    failures += expect_json_contains(local_object_bounds_json, "\"EAX\":{\"hex\":\"00000000h\"", "Phase 80 LOCAL bounds strict error should stop before register mutation");

    copy_source_run_json(local_padding_bounds_json, sizeof(local_padding_bounds_json), masm32_sim_wasm_run_source_json_with_memory_validation_mode(
        ".code\n"
        "main PROC\n"
        "    LOCAL temp:DWORD, ch:BYTE, signedVal:SDWORD\n"
        "    mov BYTE PTR signedVal[4], 1\n"
        "    ret\n"
        "main ENDP\n"
        "END main\n",
        MASM32_SIM_WASM_MEMORY_VALIDATION_ALLOCATED_OBJECT_STRICT
    ));
    failures += expect_json_contains(local_padding_bounds_json, "\"ok\":false", "Phase 80 LOCAL access into frame padding should stop in strict object mode");
    failures += expect_json_contains(local_padding_bounds_json, "object-bounds-violation", "Phase 80 LOCAL padding access should use object-bounds-violation");
    failures += expect_json_contains(local_padding_bounds_json, "LOCAL main.signedVal at byte offset 4 for 1 byte", "Phase 80 LOCAL padding diagnostic should identify the escaped local object range");
    failures += expect_json_not_contains(local_padding_bounds_json, "execution-complete", "Phase 80 LOCAL padding access should not complete execution in strict object mode");

    copy_source_run_json(local_uninitialized_warning_json, sizeof(local_uninitialized_warning_json), masm32_sim_wasm_run_source_json_with_memory_validation_mode(
        ".code\n"
        "main PROC\n"
        "    LOCAL temp:DWORD\n"
        "    mov eax, temp\n"
    ...
    failures += expect_json_contains(local_uninitialized_warning_json, "\"ok\":true", "Phase 80 warning-mode LOCAL uninitialized read should continue");
    failures += expect_json_contains(local_uninitialized_warning_json, "\"kind\":\"simulator-warning\",\"code\":\"uninitialized-read\"", "Phase 80 LOCAL uninitialized warning should be a simulator warning");
    failures += expect_json_contains(local_uninitialized_warning_json, "LOCAL temp + 0", "Phase 80 LOCAL uninitialized warning should name the local and byte offset");
    failures += expect_json_contains(local_uninitialized_warning_json, "\"EAX\":{\"hex\":\"00000000h\"", "Phase 80 warning-mode LOCAL uninitialized read should use deterministic visible zero");
    failures += expect_json_contains(local_uninitialized_warning_json, "execution-complete", "Phase 80 warning-mode LOCAL uninitialized read should still complete");

    copy_source_run_json(local_uninitialized_strict_json, sizeof(local_uninitialized_strict_json), masm32_sim_wasm_run_source_json_with_memory_validation_mode(
        ".code\n"
        "main PROC\n"
        "    LOCAL temp:DWORD\n"
        "    mov eax, temp\n"
    ...
    failures += expect_json_contains(local_uninitialized_strict_json, "\"ok\":false", "Phase 80 strict LOCAL uninitialized read should stop execution");
    failures += expect_json_contains(local_uninitialized_strict_json, "\"kind\":\"runtime-error\",\"code\":\"uninitialized-read\"", "Phase 80 strict LOCAL uninitialized read should be a runtime error");
    failures += expect_json_contains(local_uninitialized_strict_json, "LOCAL temp + 0", "Phase 80 strict LOCAL uninitialized read should name the local and byte offset");
    failures += expect_json_contains(local_uninitialized_strict_json, "\"registerWrites\":{\"EAX\":false", "Phase 80 strict LOCAL uninitialized read should stop before register mutation");

    copy_source_run_json(local_partial_uninitialized_json, sizeof(local_partial_uninitialized_json), masm32_sim_wasm_run_source_json_with_memory_validation_mode(
        ".code\n"
        "main PROC\n"
        "    LOCAL buf[4]:BYTE\n"
        "    mov BYTE PTR buf[0], 1\n"
        "    mov eax, DWORD PTR buf[0]\n"
        "    ret\n"
        "main ENDP\n"
        "END main\n",
        MASM32_SIM_WASM_MEMORY_VALIDATION_UNINITIALIZED_READ_STRICT
    ));
    failures += expect_json_contains(local_partial_uninitialized_json, "\"ok\":false", "Phase 80 partial LOCAL initialization should stop in strict mode");
    failures += expect_json_contains(local_partial_uninitialized_json, "\"uninitializedByteCount\":3", "Phase 80 partial LOCAL initialization should report only remaining uninitialized bytes");
    failures += expect_json_contains(local_partial_uninitialized_json, "from buf + 0", "Phase 80 partial LOCAL uninitialized read should identify the local byte offset");

    copy_source_run_json(invalid_type_json, sizeof(invalid_type_json), masm32_sim_wasm_run_source_json(
        ".code\n"
        "main PROC\n"
        "LOCAL q:QWORD\n"
        "main ENDP\n"
    ...
    failures += expect_json_contains(invalid_type_json, "\"ok\":false", "unsupported LOCAL type should fail source-run parse");
    failures += expect_json_contains(invalid_type_json, "\"code\":\"unsupported-local-type\"", "unsupported LOCAL type should use Phase 78 diagnostic code");
    failures += expect_json_contains(invalid_type_json, "\"line\":3", "unsupported LOCAL type diagnostic should preserve source line");
    failures += expect_json_contains(invalid_type_json, "\"column\":9", "unsupported LOCAL type diagnostic should preserve source column");
    failures += expect_json_contains(invalid_type_json, "\"spanLength\":5", "unsupported LOCAL type diagnostic should span the type token");

    copy_source_run_json(invalid_count_json, sizeof(invalid_count_json), masm32_sim_wasm_run_source_json(
        ".code\n"
        "main PROC\n"
        "LOCAL buf[0]:BYTE\n"
        "main ENDP\n"
    ...
    failures += expect_json_contains(invalid_count_json, "\"ok\":false", "invalid LOCAL array count should fail source-run parse");
    failures += expect_json_contains(invalid_count_json, "\"code\":\"invalid-local-count\"", "invalid LOCAL array count should use Phase 78 diagnostic code");
    failures += expect_json_contains(invalid_count_json, "\"line\":3", "invalid LOCAL count diagnostic should preserve source line");
    failures += expect_json_contains(invalid_count_json, "\"column\":11", "invalid LOCAL count diagnostic should preserve source column");
    failures += expect_json_contains(invalid_count_json, "\"spanLength\":1", "invalid LOCAL count diagnostic should span the count token");
...
static const SourceRunTestCase SOURCE_RUN_TEST_CASES[] = {
    ...
    {"test_phase78a_nokeyword_source_run", SOURCE_RUN_TEST_DIAGNOSTICS, test_phase78a_nokeyword_source_run},
    {"test_phase79_local_frame_source_run", SOURCE_RUN_TEST_DIAGNOSTICS, test_phase79_local_frame_source_run},
    ...
    {"test_phase82_invoke_addr_external_routine_source_run_diagnostics", SOURCE_RUN_TEST_DIAGNOSTICS, test_phase82_invoke_addr_external_routine_source_run_diagnostics},
    ...
    {"test_phase82_zero_argument_invoke_source_run_semantics", SOURCE_RUN_TEST_CORE, test_phase82_zero_argument_invoke_source_run_semantics},
    {"test_phase86_program_console_source_run_separation_and_limits", SOURCE_RUN_TEST_CORE, test_phase86_program_console_source_run_separation_and_limits},
    {"test_phase86_synthetic_console_output_limit_source_run", SOURCE_RUN_TEST_CORE, test_phase86_synthetic_console_output_limit_source_run},
    {"test_phase87_irvine32_crlf_source_run_program_console", SOURCE_RUN_TEST_CORE, test_phase87_irvine32_crlf_source_run_program_console},
    {"test_phase90_irvine32_writedec_source_run", SOURCE_RUN_TEST_CORE, test_phase90_irvine32_writedec_source_run},
    {"test_phase91_irvine32_writeint_source_run", SOURCE_RUN_TEST_CORE, test_phase91_irvine32_writeint_source_run},
    {"test_phase92_irvine32_writehex_source_run", SOURCE_RUN_TEST_CORE, test_phase92_irvine32_writehex_source_run},
    {"test_phase93_irvine32_writebin_source_run", SOURCE_RUN_TEST_CORE, test_phase93_irvine32_writebin_source_run},
    {"test_current_source_run_phase_metadata", SOURCE_RUN_TEST_MEMORY_LAYOUT, test_current_source_run_phase_metadata},
    {"test_phase81_proto_source_run_non_effect", SOURCE_RUN_TEST_CORE, test_phase81_proto_source_run_non_effect},
...
int main(int argc, char **argv) {
    ...
    if (has_filter) {
        ...
        puts("Source execution tests through Phase 93 Irvine32 WriteBin passed.");
    }
    return 0;
}
