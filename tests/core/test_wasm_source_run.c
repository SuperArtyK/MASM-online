 ...
 * @brief Tests for the Milestone 23 Wasm-facing source execution API.
 *
...
/// Verifies the Milestone 23 register-indirect source-run acceptance program.
///
/// @return Number of failures.
...
/// Verifies the Milestone 23 TYPE acceptance program through the source-run API.
///
/// @return Number of failures.
...
/// Verifies the Milestone 23 LENGTHOF acceptance program through the source-run API.
///
/// @return Number of failures.
...
/// Verifies the Milestone 23 SIZEOF and character-literal acceptance program through the source-run API.
///
/// @return Number of failures.
...
/// Verifies the Milestone 23 constant symbol-offset acceptance program.
///
/// @return Number of failures.
...
static int test_phase22_source_run_acceptance_program(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":23", "Phase 22 response should identify Milestone 23");
...
static int test_phase22_source_run_error_paths(void) {
    ...
    unsupported_base_json = masm32_sim_wasm_run_source_json(
        ".code\n"
        "main PROC\n"
        "    mov eax, 0\n"
        "    test [eax], eax\n"
    ...
    if (unsupported_base_json == NULL) {
        return record_failure("TEST unsupported memory base result should not be NULL");
    }
    (void)snprintf(unsupported_base_copy, sizeof(unsupported_base_copy), "%s", unsupported_base_json);
    ...
    failures += expect_json_contains(unsupported_base_copy, "\"kind\":\"unsupported-feature\"", "TEST [eax], eax should classify unsupported memory bases as unsupported feature");
    failures += expect_json_contains(unsupported_base_copy, "unsupported-register-indirect-base", "TEST [eax], eax should expose stable unsupported base diagnostic code");
    failures += expect_json_contains(unsupported_base_copy, "bracketed memory operands", "TEST unsupported base diagnostic should clarify memory base context");
    failures += expect_json_contains(unsupported_base_copy, "remove the brackets", "TEST unsupported base diagnostic should mention register operand spelling");
...
/// Verifies Milestone 23 source-run JSON reports multiple recovered unsupported features.
///
/// @return Number of failures.
...
/// Verifies the Milestone 23 signed declaration acceptance program.
///
/// @return Number of failures.
...
static int test_signed_ptr_alias_source_run_acceptance_program(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":23", "signed PTR alias response should identify Milestone 23");
...
/// Verifies the Milestone 23 sign/zero-extension acceptance program.
///
/// @return Number of failures.
...
static int test_phase20_source_run_acceptance_program(void) {
    ...
    failures += expect_json_contains(json, "\"phase\":23", "source-run response should identify Milestone 23");
...
int main(void) {
    ...
    puts("Milestone 23 source execution tests passed.");
    return 0;
}
