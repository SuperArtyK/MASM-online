/*
 * @file test_parser.c
 * @brief Unit and integration tests for parser behavior through Phase 90 Irvine32 WriteDec coverage.
 *
...
static int test_phase87_irvine32_crlf_parser_paths(void) {
    ...
    failures += expect_parser_status(parse_for_test(deferred_write_int, &buffers, &result), VM_PARSER_STATUS_OK_WITH_DIAGNOSTICS, "WriteInt CALL should remain deferred after Phase 90");
    failures += expect_parser_diagnostic_code(buffers.diagnostics[0].code, VM_PARSER_DIAGNOSTIC_UNSUPPORTED_IRVINE32_ROUTINE, "WriteInt CALL should keep Irvine32 deferred diagnostic");
    failures += expect_string_contains(buffers.diagnostics[0].message, "deferred", "WriteInt diagnostic should still say deferred");
...
static int test_phase90_irvine32_writedec_parser_paths(void) {
    ...
    failures += expect_u32(vm_parser_classify_irvine32_symbol("WriteInt", 8U), VM_IRVINE32_SYMBOL_CLASS_PLANNED_ROUTINE, "WriteInt should remain planned after Phase 90");
    ...
    failures += expect_parser_status(parse_for_test(deferred_writeint, &buffers, &result), VM_PARSER_STATUS_OK_WITH_DIAGNOSTICS, "WriteInt CALL should remain deferred after Phase 90");
    failures += expect_parser_diagnostic_code(buffers.diagnostics[0].code, VM_PARSER_DIAGNOSTIC_UNSUPPORTED_IRVINE32_ROUTINE, "WriteInt CALL should retain the deferred Irvine32 diagnostic");
...
int main(void) {
    ...
    printf("Parser tests through Phase 90 Irvine32 WriteDec coverage passed.\n");
    return 0;
}
