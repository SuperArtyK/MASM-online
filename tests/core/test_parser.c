/*
 * @file test_parser.c
 * @brief Unit and integration tests for parser behavior through Phase 93 Irvine32 WriteBin coverage.
 *
...
static int test_phase68_call_target_classifier_metadata(void) {
    ...
    classification = vm_parser_classify_call_target_name(&context, "WriteChar", strlen("WriteChar"));
    failures += expect_call_target_class(classification.target_class, VM_PARSER_CALL_TARGET_IRVINE32_SUPPORTED, "WriteChar should classify as supported Irvine32 routine in Phase 88");
...
static int test_phase69_direct_call_target_rejections(void) {
    ...
    failures += expect_parser_status(parse_for_test(
        ...
        &buffers,
        &result
    ), VM_PARSER_STATUS_OK, "CALL WriteChar target should parse after Phase 88");
    failures += expect_size(result.diagnostic_count, 0U, "CALL WriteChar target should not diagnose after Phase 88");
    if (result.instruction_count >= 1U) {
        failures += expect_u32((uint32_t)buffers.instructions[0].opcode, (uint32_t)VM_IR_OPCODE_IRVINE32_WRITECHAR, "CALL WriteChar should lower to the WriteChar IR opcode");
    }
...
static int test_multi_diagnostic_unsupported_feature_recovery(void) {
    ...
    failures += expect_u32(buffers.diagnostics[1].location.column, 22U, "INVOKE argument diagnostic column should be preserved");
...
static int test_phase41_virtual_irvine32_include_records_registry(void) {
    ...
    failures += expect_u32(vm_parser_classify_irvine32_symbol("Crlf", 4U), VM_IRVINE32_SYMBOL_CLASS_SUPPORTED_ROUTINE, "Crlf should be a supported Irvine32 routine in Phase 87");
    ...
    failures += expect_u32(vm_parser_classify_irvine32_symbol("wRiTeDeC", 8U), VM_IRVINE32_SYMBOL_CLASS_SUPPORTED_ROUTINE, "WriteDec lookup should remain case-insensitive");
...
/// Verifies Phase 82 classifies INVOKE arguments and external routine targets.
///
/// @return Zero on success, otherwise a positive failure count.
static int test_phase82_invoke_addr_external_routine_diagnostics(void) {
    ParserTestBuffers buffers;
    VmParserResult result;
    int failures = 0;
    ...
    failures += expect_parser_status(parse_for_test(stdout_source, &buffers, &result), VM_PARSER_STATUS_OK_WITH_DIAGNOSTICS, "Phase 82 StdOut INVOKE should produce an external INVOKE diagnostic");
    failures += expect_size(result.diagnostic_count, 1U, "StdOut INVOKE should report the external target rather than legacy blanket diagnostics");
    failures += expect_parser_diagnostic_code(buffers.diagnostics[0].code, VM_PARSER_DIAGNOSTIC_UNSUPPORTED_EXTERNAL_INVOKE, "StdOut INVOKE should report unsupported external INVOKE");
    failures += expect_size(buffers.diagnostics[0].location.line, 5U, "StdOut diagnostic should preserve line");
    failures += expect_size(buffers.diagnostics[0].location.column, 12U, "StdOut diagnostic should point at routine token");
    failures += expect_size(buffers.diagnostics[0].lexeme_length, 6U, "StdOut diagnostic span should be six bytes");
    failures += expect_string_contains(buffers.diagnostics[0].message, "MASM32 runtime", "StdOut message should describe MASM32 runtime boundary");
    failures += expect_size(result.instruction_count, 0U, "unsupported external INVOKE with arguments should not emit instructions");

    failures += expect_parser_status(parse_for_test(crt_source, &buffers, &result), VM_PARSER_STATUS_OK_WITH_DIAGNOSTICS, "Phase 82 CRT INVOKE should produce diagnostics");
    failures += expect_size(result.diagnostic_count, 1U, "CRT INVOKE should report one external INVOKE diagnostic");
    failures += expect_parser_diagnostic_code(buffers.diagnostics[0].code, VM_PARSER_DIAGNOSTIC_UNSUPPORTED_EXTERNAL_INVOKE, "CRT source should report unsupported external INVOKE");
    failures += expect_string_contains(buffers.diagnostics[0].message, "C runtime", "CRT diagnostic should describe C runtime output");

    failures += expect_parser_status(parse_for_test(exitprocess_source, &buffers, &result), VM_PARSER_STATUS_OK_WITH_DIAGNOSTICS, "Phase 82 ExitProcess INVOKE should produce diagnostics");
    failures += expect_size(result.diagnostic_count, 1U, "ExitProcess INVOKE should report one external/API diagnostic");
    failures += expect_parser_diagnostic_code(buffers.diagnostics[0].code, VM_PARSER_DIAGNOSTIC_UNSUPPORTED_EXTERNAL_INVOKE, "ExitProcess source should report unsupported external INVOKE");
    failures += expect_string_contains(buffers.diagnostics[0].message, "Windows/API", "ExitProcess message should describe WinAPI/external behavior");
    failures += expect_size(buffers.diagnostics[0].location.line, 4U, "ExitProcess diagnostic should preserve line");
    failures += expect_size(buffers.diagnostics[0].location.column, 12U, "ExitProcess diagnostic should point at routine name");
    failures += expect_size(buffers.diagnostics[0].lexeme_length, 11U, "ExitProcess diagnostic span should match routine name");

    failures += expect_parser_status(parse_for_test(registry_external_non_target_source, &buffers, &result), VM_PARSER_STATUS_OK_WITH_DIAGNOSTICS, "Phase 82 should classify registry external INVOKE targets explicitly");
    failures += expect_size(result.diagnostic_count, 1U, "registry external INVOKE should report one unsupported external INVOKE diagnostic");
    failures += expect_parser_diagnostic_code(buffers.diagnostics[0].code, VM_PARSER_DIAGNOSTIC_UNSUPPORTED_EXTERNAL_INVOKE, "registry external should receive external INVOKE diagnostic");

    failures += expect_parser_status(parse_for_test(multiple_source, &buffers, &result), VM_PARSER_STATUS_OK_WITH_DIAGNOSTICS, "Phase 82 multiple external INVOKE lines should recover");
    failures += expect_size(result.diagnostic_count, 3U, "multiple INVOKE lines should report one target diagnostic per external line");
    failures += expect_parser_diagnostic_code(buffers.diagnostics[0].code, VM_PARSER_DIAGNOSTIC_UNSUPPORTED_EXTERNAL_INVOKE, "first line should report unsupported external INVOKE");
    ...
    failures += expect_parser_diagnostic_code(buffers.diagnostics[0].code, VM_PARSER_DIAGNOSTIC_INVALID_INVOKE_TARGET, "INVOKE unknown target should report target diagnostic");
    ...
    failures += expect_size(result.instruction_count, 4U, "accepted zero-argument INVOKE should emit the same executable shape as direct CALL");
    failures += expect_u32((uint32_t)buffers.instructions[0].opcode, (uint32_t)VM_IR_OPCODE_CALL, "accepted INVOKE should lower to CALL opcode");

    failures += expect_string(vm_parser_diagnostic_code_name(VM_PARSER_DIAGNOSTIC_UNSUPPORTED_INVOKE), "unsupported-invoke", "legacy unsupported-invoke code name should remain stable for historical diagnostics");
    failures += expect_string(vm_parser_diagnostic_code_name(VM_PARSER_DIAGNOSTIC_UNSUPPORTED_ADDR), "unsupported-addr", "unsupported-addr code name should remain stable");
...
/// Verifies Phase 87 accepts virtual Irvine32 Crlf through CALL and zero-argument INVOKE only.
///
/// @return Zero on success, otherwise a positive failure count.
static int test_phase87_irvine32_crlf_parser_paths(void) {
    int failures = 0;
    ParserTestBuffers buffers;
    VmParserResult result;
    const char *accepted_source =
    ...
    const char *without_include =
        ...
        "main ENDP\n"
        "END main\n";
    const char *invoke_without_include =
        ".code\n"
        "main PROC\n"
        "    invoke Crlf\n"
        "main ENDP\n"
        "END main\n";
    const char *invoke_arguments =
        "INCLUDE Irvine32.inc\n"
        ".code\n"
        "main PROC\n"
        "    invoke Crlf, eax\n"
    ...
    failures += expect_parser_status(parse_for_test(accepted_source, &buffers, &result), VM_PARSER_STATUS_OK, "Phase 87 Crlf CALL and zero-argument INVOKE forms should parse");
    failures += expect_size(result.diagnostic_count, 0U, "accepted Crlf forms should not emit diagnostics");
    failures += expect_size(result.instruction_count, 5U, "accepted source should emit four Crlf instructions plus exit");
    if (result.instruction_count >= 5U) {
        failures += expect_u32((uint32_t)buffers.instructions[0].opcode, (uint32_t)VM_IR_OPCODE_IRVINE32_CRLF, "call Crlf should lower to the Crlf IR opcode");
        failures += expect_u32((uint32_t)buffers.instructions[1].opcode, (uint32_t)VM_IR_OPCODE_IRVINE32_CRLF, "mixed-case CALL Crlf should lower to the Crlf IR opcode under CASEMAP:NONE");
        failures += expect_u32((uint32_t)buffers.instructions[2].opcode, (uint32_t)VM_IR_OPCODE_IRVINE32_CRLF, "invoke Crlf should lower to the Crlf IR opcode");
        failures += expect_u32((uint32_t)buffers.instructions[3].opcode, (uint32_t)VM_IR_OPCODE_IRVINE32_CRLF, "uppercase INVOKE CRLF should lower to the Crlf IR opcode");
        failures += expect_u32((uint32_t)buffers.instructions[4].opcode, (uint32_t)VM_IR_OPCODE_EXIT, "exit should remain the final instruction");
    }

    failures += expect_parser_status(parse_for_test(without_include, &buffers, &result), VM_PARSER_STATUS_OK_WITH_DIAGNOSTICS, "Crlf CALL without Irvine32 include should diagnose");
    failures += expect_parser_diagnostic_code(buffers.diagnostics[0].code, VM_PARSER_DIAGNOSTIC_MISSING_IRVINE32_INCLUDE, "CALL Crlf without include should use the missing Irvine32 include diagnostic");
    failures += expect_string(buffers.diagnostics[0].message, "CALL Crlf requires INCLUDE Irvine32.inc before Crlf can be used as a virtual Irvine32 routine.", "CALL Crlf without include diagnostic should give the exact required fix");

    failures += expect_parser_status(parse_for_test(invoke_without_include, &buffers, &result), VM_PARSER_STATUS_OK_WITH_DIAGNOSTICS, "INVOKE Crlf without Irvine32 include should diagnose");
    failures += expect_parser_diagnostic_code(buffers.diagnostics[0].code, VM_PARSER_DIAGNOSTIC_MISSING_IRVINE32_INCLUDE, "INVOKE Crlf without include should use the missing Irvine32 include diagnostic");
    failures += expect_string(buffers.diagnostics[0].message, "INVOKE Crlf requires INCLUDE Irvine32.inc before Crlf can be used as a virtual Irvine32 routine.", "INVOKE Crlf without include diagnostic should give the exact required fix");

    failures += expect_parser_status(parse_for_test(invoke_arguments, &buffers, &result), VM_PARSER_STATUS_OK_WITH_DIAGNOSTICS, "INVOKE Crlf with arguments should diagnose");
    failures += expect_parser_diagnostic_code(buffers.diagnostics[0].code, VM_PARSER_DIAGNOSTIC_INVALID_IRVINE32_ARGUMENT_COUNT, "INVOKE Crlf with arguments should use the Crlf argument-count diagnostic");
    failures += expect_string(buffers.diagnostics[0].message, "INVOKE Crlf takes no arguments. Remove the argument list or use CALL Crlf.", "INVOKE Crlf diagnostic should preserve zero-argument contract and fix guidance");

    failures += expect_parser_status(parse_for_test(bare_crlf, &buffers, &result), VM_PARSER_STATUS_OK_WITH_DIAGNOSTICS, "Bare Crlf form should remain invalid after Phase 87");
    ...
    failures += expect_parser_status(parse_for_test(deferred_dumpregs, &buffers, &result), VM_PARSER_STATUS_OK_WITH_DIAGNOSTICS, "DumpRegs CALL should remain deferred");
    failures += expect_parser_diagnostic_code(buffers.diagnostics[0].code, VM_PARSER_DIAGNOSTIC_UNSUPPORTED_IRVINE32_ROUTINE, "DumpRegs CALL should keep Irvine32 deferred diagnostic");
    failures += expect_string_contains(buffers.diagnostics[0].message, "deferred", "DumpRegs diagnostic should still say deferred");

    return failures;
}

/// Verifies Phase 88 accepts direct virtual Irvine32 WriteChar through CALL only.
///
/// @return Zero on success, otherwise a positive failure count.
static int test_phase88_irvine32_writechar_parser_paths(void) {
    int failures = 0;
    VmParserResult result;
    ParserTestBuffers buffers;
    const char *accepted_source =
    ...
    failures += expect_parser_status(parse_for_test(accepted_source, &buffers, &result), VM_PARSER_STATUS_OK, "Phase 88 WriteChar CALL forms should parse");
    failures += expect_size(result.diagnostic_count, 0U, "accepted WriteChar forms should not emit diagnostics");
    failures += expect_size(result.instruction_count, 3U, "accepted WriteChar source should emit two WriteChar instructions plus exit");
    if (result.instruction_count >= 3U) {
        failures += expect_u32((uint32_t)buffers.instructions[0].opcode, (uint32_t)VM_IR_OPCODE_IRVINE32_WRITECHAR, "call WriteChar should lower to the WriteChar IR opcode");
        failures += expect_u32((uint32_t)buffers.instructions[1].opcode, (uint32_t)VM_IR_OPCODE_IRVINE32_WRITECHAR, "mixed-case CALL WriteChar should lower under CASEMAP:NONE");
        failures += expect_u32((uint32_t)buffers.instructions[2].opcode, (uint32_t)VM_IR_OPCODE_EXIT, "exit should remain the final instruction");
    }

    failures += expect_parser_status(parse_for_test(without_include, &buffers, &result), VM_PARSER_STATUS_OK_WITH_DIAGNOSTICS, "CALL WriteChar without Irvine32 include should diagnose");
    failures += expect_parser_diagnostic_code(buffers.diagnostics[0].code, VM_PARSER_DIAGNOSTIC_MISSING_IRVINE32_INCLUDE, "CALL WriteChar without include should use the missing Irvine32 include diagnostic");
    failures += expect_string(buffers.diagnostics[0].message, "CALL WriteChar requires INCLUDE Irvine32.inc before WriteChar can be used as a virtual Irvine32 routine.", "CALL WriteChar without include diagnostic should give the exact required fix");

    failures += expect_parser_status(parse_for_test(invoke_writechar, &buffers, &result), VM_PARSER_STATUS_OK_WITH_DIAGNOSTICS, "INVOKE WriteChar should remain deferred after Phase 88");
    failures += expect_parser_diagnostic_code(buffers.diagnostics[0].code, VM_PARSER_DIAGNOSTIC_UNSUPPORTED_IRVINE_INVOKE, "INVOKE WriteChar should keep Irvine32 INVOKE deferred diagnostic");
    failures += expect_string_contains(buffers.diagnostics[0].message, "deferred", "INVOKE WriteChar diagnostic should still say deferred");

    failures += expect_parser_status(parse_for_test(bare_writechar, &buffers, &result), VM_PARSER_STATUS_OK_WITH_DIAGNOSTICS, "Bare WriteChar form should remain invalid after Phase 88");
...
/// Verifies Phase 90 accepts direct virtual Irvine32 WriteDec through CALL only.
///
/// @return Zero on success, otherwise a positive failure count.
static int test_phase90_irvine32_writedec_parser_paths(void) {
    int failures = 0;
    VmParserResult result;
    ParserTestBuffers buffers;
    const char *accepted_source =
    ...
    failures += expect_u32(vm_parser_classify_irvine32_symbol("DumpRegs", 8U), VM_IRVINE32_SYMBOL_CLASS_PLANNED_ROUTINE, "DumpRegs should remain planned after Phase 90");
    ...
    failures += expect_size(result.instruction_count, 3U, "accepted WriteDec source should emit two WriteDec instructions plus exit");
    if (result.instruction_count >= 3U) {
        failures += expect_u32((uint32_t)buffers.instructions[0].opcode, (uint32_t)VM_IR_OPCODE_IRVINE32_WRITEDEC, "call WriteDec should lower to the WriteDec IR opcode");
        failures += expect_u32((uint32_t)buffers.instructions[1].opcode, (uint32_t)VM_IR_OPCODE_IRVINE32_WRITEDEC, "mixed-case CALL WriteDec should lower under CASEMAP:NONE");
        failures += expect_u32((uint32_t)buffers.instructions[2].opcode, (uint32_t)VM_IR_OPCODE_EXIT, "exit should remain the final instruction");
    }

    failures += expect_parser_status(parse_for_test(without_include, &buffers, &result), VM_PARSER_STATUS_OK_WITH_DIAGNOSTICS, "CALL WriteDec without Irvine32 include should diagnose");
    failures += expect_parser_diagnostic_code(buffers.diagnostics[0].code, VM_PARSER_DIAGNOSTIC_MISSING_IRVINE32_INCLUDE, "CALL WriteDec without include should use missing-irvine32-include");
    failures += expect_string(buffers.diagnostics[0].message, "CALL WriteDec requires INCLUDE Irvine32.inc before WriteDec can be used as a virtual Irvine32 routine.", "CALL WriteDec without include should use the exact required message");
    failures += expect_size(buffers.diagnostics[0].location.line, 3U, "CALL WriteDec missing-include diagnostic should preserve target line");
    failures += expect_size(buffers.diagnostics[0].location.column, 10U, "CALL WriteDec missing-include diagnostic should point at the target token");
    failures += expect_size(buffers.diagnostics[0].lexeme_length, 8U, "CALL WriteDec missing-include diagnostic should span WriteDec");

    failures += expect_parser_status(parse_for_test(bare_writedec, &buffers, &result), VM_PARSER_STATUS_OK_WITH_DIAGNOSTICS, "Bare WriteDec should diagnose after Phase 90");
    failures += expect_parser_diagnostic_code(buffers.diagnostics[0].code, VM_PARSER_DIAGNOSTIC_INVALID_IRVINE32_CALL_FORM, "Bare WriteDec should use invalid-irvine32-call-form");
    failures += expect_string(buffers.diagnostics[0].message, "WriteDec is a virtual Irvine32 routine and must be called with CALL WriteDec.", "Bare WriteDec should use the exact supported-form message");
    failures += expect_size(buffers.diagnostics[0].location.line, 4U, "Bare WriteDec diagnostic should preserve routine line");
    failures += expect_size(buffers.diagnostics[0].location.column, 5U, "Bare WriteDec diagnostic should point at routine token");
    failures += expect_size(buffers.diagnostics[0].lexeme_length, 8U, "Bare WriteDec diagnostic should span WriteDec");

    failures += expect_parser_status(parse_for_test(invoke_writedec, &buffers, &result), VM_PARSER_STATUS_OK_WITH_DIAGNOSTICS, "INVOKE WriteDec should remain deferred after Phase 90");
    ...
    failures += expect_parser_status(parse_for_test(deferred_dumpregs, &buffers, &result), VM_PARSER_STATUS_OK_WITH_DIAGNOSTICS, "DumpRegs CALL should remain deferred");
    failures += expect_parser_diagnostic_code(buffers.diagnostics[0].code, VM_PARSER_DIAGNOSTIC_UNSUPPORTED_IRVINE32_ROUTINE, "DumpRegs CALL should retain the deferred Irvine32 diagnostic");

    return failures;
}

/// Verifies Phase 91 accepts direct virtual Irvine32 WriteInt through CALL only.
///
/// @return Zero on success, otherwise a positive failure count.
static int test_phase91_irvine32_writeint_parser_paths(void) {
    int failures = 0;
    VmParserResult result;
    ParserTestBuffers buffers;
    const char *accepted_source =
    ...
    failures += expect_u32(vm_parser_classify_irvine32_symbol("DumpRegs", 8U), VM_IRVINE32_SYMBOL_CLASS_PLANNED_ROUTINE, "DumpRegs should remain planned after Phase 91");
    ...
    failures += expect_size(result.instruction_count, 3U, "accepted WriteInt source should emit two WriteInt instructions plus exit");
    if (result.instruction_count >= 3U) {
        failures += expect_u32((uint32_t)buffers.instructions[0].opcode, (uint32_t)VM_IR_OPCODE_IRVINE32_WRITEINT, "call WriteInt should lower to the WriteInt IR opcode");
        failures += expect_u32((uint32_t)buffers.instructions[1].opcode, (uint32_t)VM_IR_OPCODE_IRVINE32_WRITEINT, "mixed-case CALL WriteInt should lower under CASEMAP:NONE");
        failures += expect_u32((uint32_t)buffers.instructions[2].opcode, (uint32_t)VM_IR_OPCODE_EXIT, "exit should remain the final instruction after WriteInt calls");
    }

    failures += expect_parser_status(parse_for_test(without_include, &buffers, &result), VM_PARSER_STATUS_OK_WITH_DIAGNOSTICS, "CALL WriteInt without Irvine32 include should diagnose");
    failures += expect_parser_diagnostic_code(buffers.diagnostics[0].code, VM_PARSER_DIAGNOSTIC_MISSING_IRVINE32_INCLUDE, "CALL WriteInt without include should use missing-irvine32-include");
    failures += expect_string(buffers.diagnostics[0].message, "CALL WriteInt requires INCLUDE Irvine32.inc before WriteInt can be used as a virtual Irvine32 routine.", "CALL WriteInt without include should use the exact required message");
    failures += expect_size(buffers.diagnostics[0].location.line, 3U, "CALL WriteInt missing-include diagnostic should preserve target line");
    failures += expect_size(buffers.diagnostics[0].location.column, 10U, "CALL WriteInt missing-include diagnostic should point at the target token");
    failures += expect_size(buffers.diagnostics[0].location.offset, 25U, "CALL WriteInt missing-include diagnostic should preserve target byte offset");
    failures += expect_size(buffers.diagnostics[0].lexeme_length, 8U, "CALL WriteInt missing-include diagnostic should span WriteInt");

    failures += expect_parser_status(parse_for_test(bare_writeint, &buffers, &result), VM_PARSER_STATUS_OK_WITH_DIAGNOSTICS, "Bare WriteInt should diagnose after Phase 91");
    failures += expect_parser_diagnostic_code(buffers.diagnostics[0].code, VM_PARSER_DIAGNOSTIC_INVALID_IRVINE32_CALL_FORM, "Bare WriteInt should use invalid-irvine32-call-form");
    failures += expect_string(buffers.diagnostics[0].message, "WriteInt is a virtual Irvine32 routine and must be called with CALL WriteInt.", "Bare WriteInt should use the exact supported-form message");
    failures += expect_size(buffers.diagnostics[0].location.line, 4U, "Bare WriteInt diagnostic should preserve routine line");
    failures += expect_size(buffers.diagnostics[0].location.column, 5U, "Bare WriteInt diagnostic should point at routine token");
    failures += expect_size(buffers.diagnostics[0].location.offset, 41U, "Bare WriteInt diagnostic should preserve routine byte offset");
    failures += expect_size(buffers.diagnostics[0].lexeme_length, 8U, "Bare WriteInt diagnostic should span WriteInt");

    failures += expect_parser_status(parse_for_test(invoke_writeint, &buffers, &result), VM_PARSER_STATUS_OK_WITH_DIAGNOSTICS, "INVOKE WriteInt should remain deferred after Phase 91");
    failures += expect_parser_diagnostic_code(buffers.diagnostics[0].code, VM_PARSER_DIAGNOSTIC_UNSUPPORTED_IRVINE_INVOKE, "INVOKE WriteInt should keep unsupported-irvine-invoke");
    failures += expect_size(buffers.diagnostics[0].location.line, 4U, "INVOKE WriteInt diagnostic should preserve target line");
    failures += expect_size(buffers.diagnostics[0].location.column, 12U, "INVOKE WriteInt diagnostic should point at target token");
    ...
    failures += expect_parser_status(parse_for_test(deferred_dumpregs, &buffers, &result), VM_PARSER_STATUS_OK_WITH_DIAGNOSTICS, "DumpRegs CALL should remain deferred after Phase 91");
    failures += expect_parser_diagnostic_code(buffers.diagnostics[0].code, VM_PARSER_DIAGNOSTIC_UNSUPPORTED_IRVINE32_ROUTINE, "DumpRegs CALL should retain the deferred Irvine32 diagnostic");
...
static int test_phase92_irvine32_writehex_parser_paths(void) {
    ...
    failures += expect_u32(vm_parser_classify_irvine32_symbol("DumpRegs", 8U), VM_IRVINE32_SYMBOL_CLASS_PLANNED_ROUTINE, "DumpRegs should remain planned after Phase 92");
    ...
    failures += expect_parser_status(parse_for_test(deferred_dumpregs, &buffers, &result), VM_PARSER_STATUS_OK_WITH_DIAGNOSTICS, "DumpRegs CALL should remain deferred after Phase 92");
    failures += expect_parser_diagnostic_code(buffers.diagnostics[0].code, VM_PARSER_DIAGNOSTIC_UNSUPPORTED_IRVINE32_ROUTINE, "DumpRegs CALL should retain the deferred Irvine32 diagnostic");

    return failures;
}

/// Verifies Phase 93 accepts direct virtual Irvine32 WriteBin through CALL only.
///
/// @return Zero on success, otherwise a positive failure count.
static int test_phase93_irvine32_writebin_parser_paths(void) {
    int failures = 0;
    VmParserResult result;
    ParserTestBuffers buffers;
    const char *accepted_source =
    ...
    failures += expect_u32(vm_parser_classify_irvine32_symbol("WriteBin", 8U), VM_IRVINE32_SYMBOL_CLASS_SUPPORTED_ROUTINE, "WriteBin should be a supported Irvine32 routine in Phase 93");
    failures += expect_u32(vm_parser_classify_irvine32_symbol("wRiTeBiN", 8U), VM_IRVINE32_SYMBOL_CLASS_SUPPORTED_ROUTINE, "WriteBin lookup should remain case-insensitive under CASEMAP:NONE");
    failures += expect_u32(vm_parser_classify_irvine32_symbol("DumpRegs", 8U), VM_IRVINE32_SYMBOL_CLASS_PLANNED_ROUTINE, "DumpRegs should remain planned after Phase 93");

    failures += expect_parser_status(parse_for_test(accepted_source, &buffers, &result), VM_PARSER_STATUS_OK, "Phase 93 WriteBin CALL forms should parse");
    failures += expect_size(result.diagnostic_count, 0U, "accepted WriteBin forms should not emit diagnostics");
    failures += expect_size(result.instruction_count, 3U, "accepted WriteBin source should emit two WriteBin instructions plus exit");
    if (result.instruction_count >= 3U) {
        failures += expect_u32((uint32_t)buffers.instructions[0].opcode, (uint32_t)VM_IR_OPCODE_IRVINE32_WRITEBIN, "call WriteBin should lower to the WriteBin IR opcode");
        failures += expect_u32((uint32_t)buffers.instructions[1].opcode, (uint32_t)VM_IR_OPCODE_IRVINE32_WRITEBIN, "mixed-case CALL WriteBin should lower under CASEMAP:NONE");
        failures += expect_u32((uint32_t)buffers.instructions[2].opcode, (uint32_t)VM_IR_OPCODE_EXIT, "exit should remain the final instruction after WriteBin calls");
    }

    failures += expect_parser_status(parse_for_test(without_include, &buffers, &result), VM_PARSER_STATUS_OK_WITH_DIAGNOSTICS, "CALL WriteBin without Irvine32 include should diagnose");
    failures += expect_parser_diagnostic_code(buffers.diagnostics[0].code, VM_PARSER_DIAGNOSTIC_MISSING_IRVINE32_INCLUDE, "CALL WriteBin without include should use missing-irvine32-include");
    failures += expect_string(buffers.diagnostics[0].message, "CALL WriteBin requires INCLUDE Irvine32.inc before WriteBin can be used as a virtual Irvine32 routine.", "CALL WriteBin without include should use the exact required message");
    failures += expect_size(buffers.diagnostics[0].location.line, 3U, "CALL WriteBin missing-include diagnostic should preserve target line");
    failures += expect_size(buffers.diagnostics[0].location.column, 10U, "CALL WriteBin missing-include diagnostic should point at the target token");
    failures += expect_size(buffers.diagnostics[0].location.offset, 25U, "CALL WriteBin missing-include diagnostic should preserve target byte offset");
    failures += expect_size(buffers.diagnostics[0].lexeme_length, 8U, "CALL WriteBin missing-include diagnostic should span WriteBin");

    failures += expect_parser_status(parse_for_test(bare_writebin, &buffers, &result), VM_PARSER_STATUS_OK_WITH_DIAGNOSTICS, "Bare WriteBin should diagnose after Phase 93");
    failures += expect_parser_diagnostic_code(buffers.diagnostics[0].code, VM_PARSER_DIAGNOSTIC_INVALID_IRVINE32_CALL_FORM, "Bare WriteBin should use invalid-irvine32-call-form");
    failures += expect_string(buffers.diagnostics[0].message, "WriteBin is a virtual Irvine32 routine and must be called with CALL WriteBin.", "Bare WriteBin should use the exact supported-form message");
    failures += expect_size(buffers.diagnostics[0].location.line, 4U, "Bare WriteBin diagnostic should preserve routine line");
    failures += expect_size(buffers.diagnostics[0].location.column, 5U, "Bare WriteBin diagnostic should point at routine token");
    failures += expect_size(buffers.diagnostics[0].location.offset, 41U, "Bare WriteBin diagnostic should preserve routine byte offset");
    failures += expect_size(buffers.diagnostics[0].lexeme_length, 8U, "Bare WriteBin diagnostic should span WriteBin");

    failures += expect_parser_status(parse_for_test(invoke_writebin, &buffers, &result), VM_PARSER_STATUS_OK_WITH_DIAGNOSTICS, "INVOKE WriteBin should remain deferred after Phase 93");
    failures += expect_parser_diagnostic_code(buffers.diagnostics[0].code, VM_PARSER_DIAGNOSTIC_UNSUPPORTED_IRVINE_INVOKE, "INVOKE WriteBin should keep unsupported-irvine-invoke");
    failures += expect_size(buffers.diagnostics[0].location.line, 4U, "INVOKE WriteBin diagnostic should preserve target line");
    failures += expect_size(buffers.diagnostics[0].location.column, 12U, "INVOKE WriteBin diagnostic should point at target token");
    failures += expect_size(buffers.diagnostics[0].location.offset, 48U, "INVOKE WriteBin diagnostic should preserve target byte offset");
    failures += expect_size(buffers.diagnostics[0].lexeme_length, 8U, "INVOKE WriteBin diagnostic should span WriteBin");

    failures += expect_parser_status(parse_for_test(deferred_dumpregs, &buffers, &result), VM_PARSER_STATUS_OK_WITH_DIAGNOSTICS, "DumpRegs CALL should remain deferred after Phase 93");
    failures += expect_parser_diagnostic_code(buffers.diagnostics[0].code, VM_PARSER_DIAGNOSTIC_UNSUPPORTED_IRVINE32_ROUTINE, "DumpRegs CALL should retain the deferred Irvine32 diagnostic");
...
static int test_metadata_helpers(void) {
    ...
    if (strcmp(vm_parser_diagnostic_code_name(VM_PARSER_DIAGNOSTIC_MISSING_IRVINE32_INCLUDE), "missing-irvine32-include") != 0) {
        failures += record_failure("parser diagnostic helper should name missing Irvine32 include diagnostics");
    }
    if (strcmp(vm_parser_diagnostic_code_name(VM_PARSER_DIAGNOSTIC_INVALID_IRVINE32_ARGUMENT_COUNT), "invalid-irvine32-argument-count") != 0) {
        failures += record_failure("parser diagnostic helper should name invalid Irvine32 argument-count diagnostics");
    }
    if (strcmp(vm_parser_diagnostic_code_name(VM_PARSER_DIAGNOSTIC_INVALID_IRVINE32_CALL_FORM), "invalid-irvine32-call-form") != 0) {
        failures += record_failure("parser diagnostic helper should name invalid Irvine32 call-form diagnostics");
    }
    ...
    if (strcmp(vm_parser_irvine32_symbol_class_name(VM_IRVINE32_SYMBOL_CLASS_SUPPORTED_ROUTINE), "supported-routine") != 0) {
        failures += record_failure("Irvine32 symbol-class helper should name supported routines");
    }
...
int main(void) {
    ...
    failures += test_phase87_irvine32_crlf_parser_paths();
    ...
    failures += test_phase93_irvine32_writebin_parser_paths();
    ...
    printf("Parser tests through Phase 93 Irvine32 WriteBin coverage passed.\n");
    return 0;
}
