...
/// Verifies Phase 92 WriteHex formats fixed-width uppercase EAX and preserves VM state.
///
/// @return Zero on success, otherwise a positive failure count.
static int test_phase92_irvine32_writehex_formats_and_preserves_state(void) {
    int failures = 0;
    static const struct {
        uint32_t value;
        const char *expected;
        ...
        {0x00000001U, "00000001"},
        {0x0000000AU, "0000000A"},
        {0x0000000FU, "0000000F"},
        {0x00000010U, "00000010"},
        {0x01234567U, "01234567"},
        {0x89ABCDEFU, "89ABCDEF"},
        {0x7FFFFFFFU, "7FFFFFFF"},
    ...
    for (case_index = 0U; case_index < sizeof(cases) / sizeof(cases[0]); case_index += 1U) {
        ...
        const VmExecDelta *delta = NULL;
        const VmIrInstruction program[] = {
            {VM_IR_OPCODE_IRVINE32_WRITEHEX, {VM_IR_OPERAND_NONE, 0U, 0U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_NONE, 0U, 0U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 24U, "call WriteHex", 0U}
        };

        failures += expect_status(vm_init(&vm, NULL), VM_EXEC_STATUS_OK, "vm init should succeed for WriteHex formatting test");
        failures += expect_status(vm_load_program(&vm, program, 1U), VM_EXEC_STATUS_OK, "WriteHex formatting program should load");
        failures += vm_cpu_write_register(&vm.cpu, VM_REGISTER_EAX, cases[case_index].value) ? 0 : record_failure("seed EAX should succeed before WriteHex");
        failures += vm_cpu_write_register(&vm.cpu, VM_REGISTER_EBX, 0x44444444U) ? 0 : record_failure("seed EBX should succeed before WriteHex");
        failures += vm_cpu_set_flag(&vm.cpu, VM_FLAG_CF) ? 0 : record_failure("seed CF should succeed before WriteHex");
        failures += vm_cpu_mark_flag_undefined(&vm.cpu, VM_FLAG_OF, "seed-invalid", "seed", "seed.asm", 23U, 1U, 230U, 4U, "seed", 0U) ? 0 : record_failure("seed OF invalidity before WriteHex should succeed");

        failures += expect_status(vm_step(&vm), VM_EXEC_STATUS_OK, "WriteHex should execute successfully");
        failures += expect_string(vm_console_text(vm_program_console(&vm)), cases[case_index].expected, "WriteHex should emit exactly eight uppercase hexadecimal digits");
        failures += expect_size(vm_console_byte_count(vm_program_console(&vm)), 8U, "WriteHex should commit exactly eight Program Console bytes");
        failures += expect_size(vm_console_line_count(vm_program_console(&vm)), 0U, "WriteHex should not change Program Console line count");
        failures += vm_cpu_read_register(&vm.cpu, VM_REGISTER_EAX, &value) ? 0 : record_failure("EAX read after WriteHex should succeed");
        failures += expect_u32(value, cases[case_index].value, "WriteHex should preserve EAX bit pattern");
        failures += vm_cpu_read_register(&vm.cpu, VM_REGISTER_EBX, &value) ? 0 : record_failure("EBX read after WriteHex should succeed");
        failures += expect_u32(value, 0x44444444U, "WriteHex should preserve unrelated registers");
        failures += expect_flag(&vm.cpu, VM_FLAG_CF, true, "WriteHex should preserve CF");
        failures += expect_flag_validity(&vm.cpu, VM_FLAG_OF, false, "seed-invalid", "seed", 23U, "WriteHex should preserve OF validity metadata");
        delta = vm_last_delta(&vm);
        failures += expect_size(delta->register_change_count, 0U, "WriteHex should report no register changes");
        failures += expect_size(delta->flag_change_count, 0U, "WriteHex should report no flag changes");
        failures += expect_size(delta->memory_change_count, 0U, "WriteHex should report no memory changes");
        failures += expect_size(delta->memory_access_count, 0U, "WriteHex should report no memory accesses");
...
/// Verifies Phase 92 WriteHex exact-fit success and one-byte-short atomic failure.
///
/// @return Zero on success, otherwise a positive failure count.
static int test_phase92_irvine32_writehex_output_limits_are_atomic(void) {
    int failures = 0;
    Vm vm;
    uint32_t value = 0U;
    const VmExecDiagnostic *diagnostic = NULL;
    const VmExecDelta *delta = NULL;
    const VmIrInstruction program[] = {
        {VM_IR_OPCODE_IRVINE32_WRITEHEX, {VM_IR_OPERAND_NONE, 0U, 0U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_NONE, 0U, 0U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 25U, "call WriteHex", 0U}
    };

    failures += expect_status(vm_init(&vm, NULL), VM_EXEC_STATUS_OK, "vm init should succeed for WriteHex exact-fit test");
    failures += expect_status(vm_load_program(&vm, program, 1U), VM_EXEC_STATUS_OK, "WriteHex exact-fit program should load");
    failures += expect_status(vm_append_program_console_output(&vm, "pre", 3U, NULL), VM_EXEC_STATUS_OK, "WriteHex exact-fit prefix should commit");
    failures += expect_status(vm_console_configure_limits(&vm.program_console, 11U, 1U) == VM_CONSOLE_STATUS_OK ? VM_EXEC_STATUS_OK : VM_EXEC_STATUS_INVALID_ARGUMENT, VM_EXEC_STATUS_OK, "WriteHex exact-fit byte limit should configure");
    failures += vm_cpu_write_register(&vm.cpu, VM_REGISTER_EAX, 0x89ABCDEFU) ? 0 : record_failure("seed EAX before exact-fit WriteHex should succeed");
    failures += expect_status(vm_step(&vm), VM_EXEC_STATUS_OK, "WriteHex should succeed with exactly eight remaining bytes");
    failures += expect_string(vm_console_text(vm_program_console(&vm)), "pre89ABCDEF", "WriteHex exact-fit should append the complete candidate");
    failures += expect_size(vm_console_line_count(vm_program_console(&vm)), 0U, "WriteHex exact-fit should not change line count");
    vm_deinit(&vm);

    failures += expect_status(vm_init(&vm, NULL), VM_EXEC_STATUS_OK, "vm init should succeed for WriteHex line-capacity test");
    failures += expect_status(vm_load_program(&vm, program, 1U), VM_EXEC_STATUS_OK, "WriteHex line-capacity program should load");
    failures += expect_status(vm_console_configure_limits(&vm.program_console, 64U, 1U) == VM_CONSOLE_STATUS_OK ? VM_EXEC_STATUS_OK : VM_EXEC_STATUS_INVALID_ARGUMENT, VM_EXEC_STATUS_OK, "WriteHex line-capacity limit should configure");
    failures += expect_status(vm_append_program_console_output(&vm, "ok\n", 3U, NULL), VM_EXEC_STATUS_OK, "WriteHex line-capacity prefix should fill the permitted line count");
    failures += vm_cpu_write_register(&vm.cpu, VM_REGISTER_EAX, 0x0000000AU) ? 0 : record_failure("seed EAX before WriteHex line-capacity test should succeed");
    failures += expect_status(vm_step(&vm), VM_EXEC_STATUS_OK, "WriteHex should succeed when existing line count is already at its limit");
    failures += expect_string(vm_console_text(vm_program_console(&vm)), "ok\n0000000A", "WriteHex should append digits without consuming another Program Console line");
    failures += expect_size(vm_console_line_count(vm_program_console(&vm)), 1U, "WriteHex should preserve an already-full Program Console line count");
    vm_deinit(&vm);

    failures += expect_status(vm_init(&vm, NULL), VM_EXEC_STATUS_OK, "vm init should succeed for WriteHex one-byte-short test");
    failures += expect_status(vm_load_program(&vm, program, 1U), VM_EXEC_STATUS_OK, "WriteHex one-byte-short program should load");
    failures += expect_status(vm_append_program_console_output(&vm, "pre", 3U, NULL), VM_EXEC_STATUS_OK, "WriteHex one-byte-short prefix should commit");
    failures += expect_status(vm_console_configure_limits(&vm.program_console, 10U, 1U) == VM_CONSOLE_STATUS_OK ? VM_EXEC_STATUS_OK : VM_EXEC_STATUS_INVALID_ARGUMENT, VM_EXEC_STATUS_OK, "WriteHex one-byte-short limit should configure");
    failures += vm_cpu_write_register(&vm.cpu, VM_REGISTER_EAX, 0x89ABCDEFU) ? 0 : record_failure("seed EAX before failing WriteHex should succeed");
    failures += vm_cpu_write_register(&vm.cpu, VM_REGISTER_EBX, 0xABCDEF01U) ? 0 : record_failure("seed EBX before failing WriteHex should succeed");
    failures += vm_cpu_set_flag(&vm.cpu, VM_FLAG_ZF) ? 0 : record_failure("seed ZF before failing WriteHex should succeed");
    failures += vm_cpu_mark_flag_undefined(&vm.cpu, VM_FLAG_OF, "seed-invalid", "seed", "seed.asm", 24U, 1U, 240U, 4U, "seed", 0U) ? 0 : record_failure("seed OF invalidity before failing WriteHex should succeed");
    failures += expect_status(vm_step(&vm), VM_EXEC_STATUS_CONSOLE_OUTPUT_LIMIT_EXCEEDED, "WriteHex should fail when only seven bytes remain");
    failures += expect_string(vm_console_text(vm_program_console(&vm)), "pre", "WriteHex output-limit failure should append no partial digits");
    failures += expect_size(vm_console_line_count(vm_program_console(&vm)), 0U, "WriteHex output-limit failure should preserve line count");
    failures += vm_cpu_read_register(&vm.cpu, VM_REGISTER_EAX, &value) ? 0 : record_failure("EAX read after failing WriteHex should succeed");
    failures += expect_u32(value, 0x89ABCDEFU, "WriteHex failure should preserve EAX bit pattern");
    failures += vm_cpu_read_register(&vm.cpu, VM_REGISTER_EBX, &value) ? 0 : record_failure("EBX read after failing WriteHex should succeed");
    failures += expect_u32(value, 0xABCDEF01U, "WriteHex failure should preserve unrelated registers");
    failures += expect_flag(&vm.cpu, VM_FLAG_ZF, true, "WriteHex failure should preserve flags");
    failures += expect_flag_validity(&vm.cpu, VM_FLAG_OF, false, "seed-invalid", "seed", 24U, "WriteHex failure should preserve flag-validity metadata");
    delta = vm_last_delta(&vm);
    failures += expect_size(delta->register_change_count, 0U, "WriteHex failure should report no register changes");
    failures += expect_size(delta->flag_change_count, 0U, "WriteHex failure should report no flag changes");
    failures += expect_size(delta->memory_change_count, 0U, "WriteHex failure should report no memory changes");
    failures += expect_size(delta->memory_access_count, 0U, "WriteHex failure should report no memory accesses");
    diagnostic = vm_last_diagnostic(&vm);
    if (diagnostic == NULL || diagnostic->status != VM_EXEC_STATUS_CONSOLE_OUTPUT_LIMIT_EXCEEDED) {
        failures += record_failure("WriteHex one-byte-short failure should record the shared output-limit diagnostic");
    } else {
...
/// Verifies Phase 55 explicit-destination signed IMUL semantics.
///
/// @return Zero on success, otherwise a positive failure count.
static int test_phase55_explicit_imul_semantics(void) {
    int failures = 0;
    Vm vm;
    uint32_t eax = 0U;
    uint32_t ebx = 0U;
    ...
    failures += expect_status(vm_init(&vm, NULL), VM_EXEC_STATUS_OK, "vm init should succeed for Phase 55 IMUL test");

    {
        const VmIrInstruction program[] = {
            {VM_IR_OPCODE_IMUL, {VM_IR_OPERAND_REGISTER, 0U, 0U, VM_REGISTER_EAX, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_REGISTER, 0U, 0U, VM_REGISTER_EBX, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 3U, "imul eax, ebx", 0U}
        };
        failures += expect_status(vm_load_program(&vm, program, sizeof(program) / sizeof(program[0])), VM_EXEC_STATUS_OK, "two-operand IMUL program load should succeed");
    }
    failures += (vm_cpu_write_register(&vm.cpu, VM_REGISTER_EAX, 3U) ? 0 : record_failure("EAX setup should succeed"));
    failures += (vm_cpu_write_register(&vm.cpu, VM_REGISTER_EBX, 4U) ? 0 : record_failure("EBX setup should succeed"));
    failures += (vm_cpu_write_flag(&vm.cpu, VM_FLAG_ZF, true) ? 0 : record_failure("ZF setup should succeed"));
    failures += (vm_cpu_write_flag(&vm.cpu, VM_FLAG_SF, true) ? 0 : record_failure("SF setup should succeed"));
    failures += expect_status(vm_step(&vm), VM_EXEC_STATUS_OK, "two-operand IMUL should execute");
    failures += (vm_cpu_read_register(&vm.cpu, VM_REGISTER_EAX, &eax) ? 0 : record_failure("EAX read should succeed"));
    failures += (vm_cpu_read_register(&vm.cpu, VM_REGISTER_EBX, &ebx) ? 0 : record_failure("EBX read should succeed"));
    failures += expect_u32(eax, 12U, "two-operand IMUL should write low product to EAX");
    failures += expect_u32(ebx, 4U, "two-operand IMUL should preserve source register");
    failures += expect_flag(&vm.cpu, VM_FLAG_CF, false, "fitting two-operand IMUL should clear CF");
    failures += expect_flag(&vm.cpu, VM_FLAG_OF, false, "fitting two-operand IMUL should clear OF");
    failures += expect_flag(&vm.cpu, VM_FLAG_ZF, true, "two-operand IMUL should preserve ZF");
    failures += expect_flag(&vm.cpu, VM_FLAG_SF, true, "two-operand IMUL should preserve SF");

    {
        const VmIrInstruction program[] = {
            {VM_IR_OPCODE_IMUL_IMMEDIATE, {VM_IR_OPERAND_REGISTER, 0U, 0xFFFFFFFBU, VM_REGISTER_EAX, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_REGISTER, 0U, 0U, VM_REGISTER_EBX, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 3U, "imul eax, ebx, -5", 0U}
        };
        failures += expect_status(vm_load_program(&vm, program, sizeof(program) / sizeof(program[0])), VM_EXEC_STATUS_OK, "three-operand IMUL program load should succeed");
    }
    failures += (vm_cpu_write_register(&vm.cpu, VM_REGISTER_EBX, 4U) ? 0 : record_failure("EBX immediate setup should succeed"));
    failures += expect_status(vm_step(&vm), VM_EXEC_STATUS_OK, "three-operand IMUL should execute");
    failures += (vm_cpu_read_register(&vm.cpu, VM_REGISTER_EAX, &eax) ? 0 : record_failure("EAX read should succeed after immediate IMUL"));
    failures += expect_u32(eax, 0xFFFFFFECU, "three-operand IMUL should multiply source by signed immediate");

    {
        const VmIrInstruction program[] = {
            {VM_IR_OPCODE_IMUL, {VM_IR_OPERAND_REGISTER, 0U, 0U, VM_REGISTER_AX, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_REGISTER, 0U, 0U, VM_REGISTER_BX, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 3U, "imul ax, bx", 0U}
        };
        failures += expect_status(vm_load_program(&vm, program, sizeof(program) / sizeof(program[0])), VM_EXEC_STATUS_OK, "16-bit two-operand IMUL program load should succeed");
    }
    failures += (vm_cpu_write_register(&vm.cpu, VM_REGISTER_EAX, 0x12340002U) ? 0 : record_failure("EAX 16-bit setup should succeed"));
    failures += (vm_cpu_write_register(&vm.cpu, VM_REGISTER_EBX, 0xAAAAFFFEU) ? 0 : record_failure("EBX 16-bit setup should succeed"));
    failures += expect_status(vm_step(&vm), VM_EXEC_STATUS_OK, "16-bit two-operand IMUL should execute");
    failures += (vm_cpu_read_register(&vm.cpu, VM_REGISTER_EAX, &eax) ? 0 : record_failure("EAX read should succeed after 16-bit explicit IMUL"));
    failures += expect_u32(eax, 0x1234FFFCU, "16-bit explicit IMUL should write only AX and preserve upper EAX");
    failures += expect_flag(&vm.cpu, VM_FLAG_CF, false, "16-bit fitting explicit IMUL should clear CF");
    failures += expect_flag(&vm.cpu, VM_FLAG_OF, false, "16-bit fitting explicit IMUL should clear OF");

    failures += (vm_memory_write_u32(&vm.memory, VM_MEMORY_DEFAULT_DATA_BASE, 0xFFFFFFFDU, NULL) == VM_MEMORY_STATUS_OK ? 0 : record_failure("Phase 55 IMUL memory fixture write should succeed"));
    vm_memory_clear_changes(&vm.memory);
    {
        const VmIrInstruction program[] = {
            {VM_IR_OPCODE_IMUL, {VM_IR_OPERAND_REGISTER, 0U, 0U, VM_REGISTER_EAX, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_MEMORY_ADDRESS, 32U, 0U, VM_REGISTER_COUNT, VM_MEMORY_DEFAULT_DATA_BASE, VM_IR_RELOCATION_NONE}, "main.asm", 5U, "imul eax, factor", 0U}
        };
        failures += expect_status(vm_load_program(&vm, program, sizeof(program) / sizeof(program[0])), VM_EXEC_STATUS_OK, "memory-source explicit IMUL program load should succeed");
    }
    failures += (vm_cpu_write_register(&vm.cpu, VM_REGISTER_EAX, 7U) ? 0 : record_failure("EAX memory-source setup should succeed"));
    failures += expect_status(vm_step(&vm), VM_EXEC_STATUS_OK, "memory-source explicit IMUL should execute");
    failures += (vm_cpu_read_register(&vm.cpu, VM_REGISTER_EAX, &eax) ? 0 : record_failure("EAX read should succeed after memory-source explicit IMUL"));
    failures += expect_u32(eax, 0xFFFFFFEBU, "memory-source explicit IMUL should write signed product to destination");
    delta = vm_last_delta(&vm);
    failures += expect_size(delta != NULL ? delta->memory_access_count : 0U, 1U, "explicit IMUL memory source should record one checked read");
    failures += expect_size(delta != NULL ? delta->memory_change_count : 1U, 0U, "explicit IMUL memory source should not create memory-change rows");

    {
        const VmIrInstruction program[] = {
            {VM_IR_OPCODE_IMUL, {VM_IR_OPERAND_REGISTER, 0U, 0U, VM_REGISTER_EAX, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_REGISTER, 0U, 0U, VM_REGISTER_EBX, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 3U, "imul eax, ebx", 0U}
        };
        failures += expect_status(vm_load_program(&vm, program, sizeof(program) / sizeof(program[0])), VM_EXEC_STATUS_OK, "overflow explicit IMUL program load should succeed");
    }
    failures += (vm_cpu_write_register(&vm.cpu, VM_REGISTER_EAX, 0x80000000U) ? 0 : record_failure("EAX overflow setup should succeed"));
    failures += (vm_cpu_write_register(&vm.cpu, VM_REGISTER_EBX, 0xFFFFFFFFU) ? 0 : record_failure("EBX overflow setup should succeed"));
    failures += expect_status(vm_step(&vm), VM_EXEC_STATUS_OK, "overflow explicit IMUL should execute");
    failures += (vm_cpu_read_register(&vm.cpu, VM_REGISTER_EAX, &eax) ? 0 : record_failure("EAX read should succeed after overflow explicit IMUL"));
    failures += expect_u32(eax, 0x80000000U, "overflow explicit IMUL should store low destination-width result");
    failures += expect_flag(&vm.cpu, VM_FLAG_CF, true, "overflow explicit IMUL should set CF");
    failures += expect_flag(&vm.cpu, VM_FLAG_OF, true, "overflow explicit IMUL should set OF");

    {
        const VmIrInstruction bad_program[] = {
            {VM_IR_OPCODE_IMUL, {VM_IR_OPERAND_REGISTER, 0U, 0U, VM_REGISTER_EBX, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_MEMORY_REGISTER, 32U, 0U, VM_REGISTER_EAX, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 4U, "imul ebx, DWORD PTR [eax]", 0U}
        };
        failures += expect_status(vm_load_program(&vm, bad_program, sizeof(bad_program) / sizeof(bad_program[0])), VM_EXEC_STATUS_OK, "invalid-address explicit IMUL program load should succeed");
    }
    failures += (vm_cpu_write_register(&vm.cpu, VM_REGISTER_EAX, 0U) ? 0 : record_failure("EAX invalid-address setup should succeed"));
    failures += (vm_cpu_write_register(&vm.cpu, VM_REGISTER_EBX, 0x12345678U) ? 0 : record_failure("EBX invalid-address setup should succeed"));
    failures += (vm_cpu_write_flag(&vm.cpu, VM_FLAG_CF, true) ? 0 : record_failure("CF invalid-address setup should succeed"));
    failures += expect_status(vm_step(&vm), VM_EXEC_STATUS_MEMORY_ERROR, "invalid-address explicit IMUL memory source should fail");
    failures += (vm_cpu_read_register(&vm.cpu, VM_REGISTER_EBX, &ebx) ? 0 : record_failure("EBX read should succeed after invalid-address IMUL"));
    failures += expect_u32(ebx, 0x12345678U, "invalid-address explicit IMUL should not mutate destination");
    failures += expect_flag(&vm.cpu, VM_FLAG_CF, true, "invalid-address explicit IMUL should preserve flags");
    diagnostic = vm_last_diagnostic(&vm);
    if (diagnostic == NULL || diagnostic->status != VM_EXEC_STATUS_MEMORY_ERROR) {
        failures += record_failure("invalid-address explicit IMUL should record memory diagnostic");
    }
...
/// Verifies Phase 66 unsigned relational conditional jumps branch and preserve state.
///
/// @return Zero on success, otherwise a positive failure count.
static int test_phase66_unsigned_relational_conditional_jump_runtime(void) {
    int failures = 0;
    Vm vm;
    const VmExecDelta *delta = NULL;
    uint32_t eax = 0U;
    uint32_t ebx = 0U;
    const VmIrInstruction program[] = {
        {VM_IR_OPCODE_JA, {VM_IR_OPERAND_BRANCH_TARGET, 0U, 2U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_NONE, 0U, 0U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 1U, "ja above", 0U},
        {VM_IR_OPCODE_MOV, {VM_IR_OPERAND_REGISTER, 0U, 0U, VM_REGISTER_EAX, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_IMMEDIATE, 32U, 1U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 2U, "mov eax, 1", 1U},
        {VM_IR_OPCODE_JBE, {VM_IR_OPERAND_BRANCH_TARGET, 0U, 4U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_NONE, 0U, 0U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 3U, "jbe below_or_equal", 2U},
        {VM_IR_OPCODE_MOV, {VM_IR_OPERAND_REGISTER, 0U, 0U, VM_REGISTER_EBX, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_IMMEDIATE, 32U, 2U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 4U, "mov ebx, 2", 3U},
        {VM_IR_OPCODE_MOV, {VM_IR_OPERAND_REGISTER, 0U, 0U, VM_REGISTER_EBX, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_IMMEDIATE, 32U, 3U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 5U, "mov ebx, 3", 4U}
    };

    failures += expect_status(vm_init(&vm, NULL), VM_EXEC_STATUS_OK, "vm init should succeed for Phase 66 unsigned relational test");
    failures += expect_status(vm_load_program(&vm, program, sizeof(program) / sizeof(program[0])), VM_EXEC_STATUS_OK, "unsigned relational program should load");
    failures += vm_cpu_write_flag(&vm.cpu, VM_FLAG_CF, false) ? 0 : record_failure("seed CF before JA should succeed");
    failures += vm_cpu_write_flag(&vm.cpu, VM_FLAG_ZF, false) ? 0 : record_failure("seed ZF before JA should succeed");
    failures += vm_cpu_write_flag(&vm.cpu, VM_FLAG_SF, true) ? 0 : record_failure("seed SF before JA should succeed");
    failures += vm_cpu_write_flag(&vm.cpu, VM_FLAG_OF, true) ? 0 : record_failure("seed OF before JA should succeed");

    failures += expect_status(vm_step(&vm), VM_EXEC_STATUS_OK, "JA should execute when CF and ZF are clear");
    failures += expect_size(vm.instruction_pointer, 2U, "JA should branch to the target instruction when unsigned above holds");
    failures += expect_u64(vm.instruction_count, 1U, "JA should count as one executed instruction");
    delta = vm_last_delta(&vm);
    failures += expect_size(delta != NULL ? delta->register_change_count : 1U, 0U, "JA should not produce register changes");
    failures += expect_size(delta != NULL ? delta->flag_change_count : 1U, 0U, "JA should not produce flag changes");
    failures += expect_size(delta != NULL ? delta->memory_change_count : 1U, 0U, "JA should not produce memory changes");
    failures += expect_flag(&vm.cpu, VM_FLAG_CF, false, "JA should preserve CF");
    failures += expect_flag(&vm.cpu, VM_FLAG_ZF, false, "JA should preserve ZF");
    failures += expect_flag(&vm.cpu, VM_FLAG_SF, true, "JA should preserve SF");
    failures += expect_flag(&vm.cpu, VM_FLAG_OF, true, "JA should preserve OF");

    failures += expect_status(vm_step(&vm), VM_EXEC_STATUS_OK, "JBE should execute when reached");
    failures += expect_size(vm.instruction_pointer, 3U, "JBE should fall through when CF and ZF are clear");
    failures += expect_u64(vm.instruction_count, 2U, "not-taken JBE should count as one executed instruction");
    failures += expect_status(vm_step(&vm), VM_EXEC_STATUS_OK, "fall-through MOV after JBE should execute");
    failures += vm_cpu_read_register(&vm.cpu, VM_REGISTER_EAX, &eax) ? 0 : record_failure("EAX read should succeed after unsigned relational jumps");
    failures += vm_cpu_read_register(&vm.cpu, VM_REGISTER_EBX, &ebx) ? 0 : record_failure("EBX read should succeed after unsigned relational jumps");
    failures += expect_u32(eax, 0U, "taken JA should skip EAX mutation");
    failures += expect_u32(ebx, 2U, "not-taken JBE should execute fall-through MOV");

    vm_deinit(&vm);
    return failures;
}

/// Verifies Phase 66 unsigned relational aliases and CF-only conditions.
///
/// @return Zero on success, otherwise a positive failure count.
static int test_phase66_unsigned_relational_conditional_jump_aliases(void) {
    int failures = 0;
    Vm vm;
    const VmIrInstruction program[] = {
        {VM_IR_OPCODE_JNBE, {VM_IR_OPERAND_BRANCH_TARGET, 0U, 2U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_NONE, 0U, 0U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 1U, "jnbe above", 0U},
        {VM_IR_OPCODE_MOV, {VM_IR_OPERAND_REGISTER, 0U, 0U, VM_REGISTER_EAX, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_IMMEDIATE, 32U, 1U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 2U, "mov eax, 1", 1U},
        {VM_IR_OPCODE_JNB, {VM_IR_OPERAND_BRANCH_TARGET, 0U, 4U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_NONE, 0U, 0U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 3U, "jnb above_or_equal", 2U},
        {VM_IR_OPCODE_MOV, {VM_IR_OPERAND_REGISTER, 0U, 0U, VM_REGISTER_EBX, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_IMMEDIATE, 32U, 2U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 4U, "mov ebx, 2", 3U},
        {VM_IR_OPCODE_JNAE, {VM_IR_OPERAND_BRANCH_TARGET, 0U, 6U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_NONE, 0U, 0U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 5U, "jnae below", 4U},
        {VM_IR_OPCODE_JNA, {VM_IR_OPERAND_BRANCH_TARGET, 0U, 7U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_NONE, 0U, 0U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 6U, "jna below_or_equal", 5U},
        {VM_IR_OPCODE_MOV, {VM_IR_OPERAND_REGISTER, 0U, 0U, VM_REGISTER_ECX, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_IMMEDIATE, 32U, 6U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 7U, "mov ecx, 6", 6U},
        {VM_IR_OPCODE_MOV, {VM_IR_OPERAND_REGISTER, 0U, 0U, VM_REGISTER_EDX, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_IMMEDIATE, 32U, 7U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 8U, "mov edx, 7", 7U}
    };

    failures += expect_status(vm_init(&vm, NULL), VM_EXEC_STATUS_OK, "vm init should succeed for Phase 66 alias test");
    failures += expect_status(vm_load_program(&vm, program, sizeof(program) / sizeof(program[0])), VM_EXEC_STATUS_OK, "unsigned relational alias program should load");
    failures += vm_cpu_write_flag(&vm.cpu, VM_FLAG_CF, false) ? 0 : record_failure("seed CF before aliases should succeed");
    failures += vm_cpu_write_flag(&vm.cpu, VM_FLAG_ZF, true) ? 0 : record_failure("seed ZF before aliases should succeed");

    failures += expect_status(vm_step(&vm), VM_EXEC_STATUS_OK, "JNBE should execute with ZF set");
    failures += expect_size(vm.instruction_pointer, 1U, "JNBE should fall through when equality makes above false");
    failures += expect_status(vm_step(&vm), VM_EXEC_STATUS_OK, "fall-through MOV after JNBE should execute");
    failures += expect_status(vm_step(&vm), VM_EXEC_STATUS_OK, "JNB should execute with CF clear");
    failures += expect_size(vm.instruction_pointer, 4U, "JNB should branch when unsigned above-or-equal is true");
    failures += expect_status(vm_step(&vm), VM_EXEC_STATUS_OK, "JNAE should execute with CF clear");
    failures += expect_size(vm.instruction_pointer, 5U, "JNAE should fall through when below is false");
    failures += expect_status(vm_step(&vm), VM_EXEC_STATUS_OK, "JNA should execute with ZF set");
    failures += expect_size(vm.instruction_pointer, 7U, "JNA should branch when equality makes below-or-equal true");

    vm_deinit(&vm);
    return failures;
}

/// Verifies native Phase 50B helper consumption sets for unsigned relational jumps.
///
/// @return Zero on success, otherwise a positive failure count.
static int test_phase66_unsigned_relational_flag_consumption_sets(void) {
    int failures = 0;
    VmCpu cpu;
    VmFlagUseDiagnostic diagnostic;
    const VmIrInstruction jae = {VM_IR_OPCODE_JAE, {VM_IR_OPERAND_BRANCH_TARGET, 0U, 0U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_NONE, 0U, 0U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 3U, "jae target", 0U};
    const VmIrInstruction ja = {VM_IR_OPCODE_JA, {VM_IR_OPERAND_BRANCH_TARGET, 0U, 0U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_NONE, 0U, 0U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 4U, "ja target", 0U};
    const VmFlag jae_flags[] = {VM_FLAG_CF};
    const VmFlag ja_flags[] = {VM_FLAG_CF, VM_FLAG_ZF};

    vm_cpu_init(&cpu);
    failures += vm_cpu_mark_flag_undefined(&cpu, VM_FLAG_ZF, "seed-zf", "seed", "main.asm", 1U, 1U, 1U, 1U, "seed zf", 0U) ? 0 : record_failure("mark ZF invalid should succeed");
    failures += vm_cpu_mark_flag_undefined(&cpu, VM_FLAG_SF, "seed-sf", "seed", "main.asm", 1U, 1U, 1U, 1U, "seed sf", 0U) ? 0 : record_failure("mark SF invalid should succeed");
    failures += vm_cpu_mark_flag_undefined(&cpu, VM_FLAG_OF, "seed-of", "seed", "main.asm", 1U, 1U, 1U, 1U, "seed of", 0U) ? 0 : record_failure("mark OF invalid should succeed");
    failures += expect_status(vm_check_flag_consumption(&cpu, &jae, jae_flags, sizeof(jae_flags) / sizeof(jae_flags[0]), VM_UNDEFINED_FLAG_USE_POLICY_WARN, &diagnostic), VM_EXEC_STATUS_OK, "JAE consumption check should succeed in warning mode");
    failures += expect_size(diagnostic.invalid_flag_count, 0U, "JAE should not consume invalid ZF, SF, or OF when CF is valid");

    failures += vm_cpu_mark_flag_undefined(&cpu, VM_FLAG_CF, "seed-cf", "seed", "main.asm", 2U, 1U, 2U, 1U, "seed cf", 0U) ? 0 : record_failure("mark CF invalid should succeed");
    failures += expect_status(vm_check_flag_consumption(&cpu, &ja, ja_flags, sizeof(ja_flags) / sizeof(ja_flags[0]), VM_UNDEFINED_FLAG_USE_POLICY_WARN, &diagnostic), VM_EXEC_STATUS_OK, "JA consumption check should succeed in warning mode");
    failures += expect_size(diagnostic.invalid_flag_count, 2U, "JA should consume invalid CF and ZF but not SF or OF");
...
/// Verifies Phase 77 saves and restores PROC USES registers around a direct CALL.
///
/// @return Zero on success, otherwise a positive failure count.
static int test_phase77_proc_uses_runtime_save_restore(void) {
    int failures = 0;
    Vm vm;
    uint32_t eax = 0U;
    uint32_t ebx = 0U;
    ...
    const VmIrInstruction program[] = {
        ...
        {VM_IR_OPCODE_MOV, {VM_IR_OPERAND_REGISTER, 32U, 0U, VM_REGISTER_EBX, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_IMMEDIATE, 32U, 0x2222U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 8U, "mov ebx, 2222h", 2U},
        {VM_IR_OPCODE_MOV, {VM_IR_OPERAND_REGISTER, 32U, 0U, VM_REGISTER_ESI, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_IMMEDIATE, 32U, 0x3333U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 9U, "mov esi, 3333h", 3U},
        {VM_IR_OPCODE_MOV, {VM_IR_OPERAND_REGISTER, 32U, 0U, VM_REGISTER_EAX, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_IMMEDIATE, 32U, 0x9999U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 10U, "mov eax, 9999h", 4U},
    ...
    failures += expect_status(vm_load_program(&vm, program, sizeof(program) / sizeof(program[0])), VM_EXEC_STATUS_OK, "Phase 77 USES save/restore program should load");
    failures += expect_status(vm_configure_procedure_boundaries(&vm, boundaries, sizeof(boundaries) / sizeof(boundaries[0])), VM_EXEC_STATUS_OK, "Phase 77 USES boundaries should configure");
    failures += vm_cpu_write_register(&vm.cpu, VM_REGISTER_EAX, 0x5555U) ? 0 : record_failure("EAX setup for USES save/restore should succeed");
    failures += vm_cpu_write_register(&vm.cpu, VM_REGISTER_EBX, 0x1111U) ? 0 : record_failure("EBX setup for USES save/restore should succeed");
    failures += vm_cpu_write_register(&vm.cpu, VM_REGISTER_ESI, 0x2222U) ? 0 : record_failure("ESI setup for USES save/restore should succeed");
    failures += vm_cpu_write_flag(&vm.cpu, VM_FLAG_CF, true) ? 0 : record_failure("CF setup for USES save/restore should succeed");

    failures += expect_status(vm_step(&vm), VM_EXEC_STATUS_OK, "CALL into PROC USES should save listed registers and transfer");
    failures += (vm_cpu_read_register(&vm.cpu, VM_REGISTER_ESP, &esp) ? 0 : record_failure("ESP read after USES CALL should succeed"));
    failures += expect_u32(esp, VM_MEMORY_DEFAULT_STACK_TOP - 12U, "USES CALL should push return token and two saved registers");
    failures += expect_size(vm.instruction_pointer, 2U, "USES CALL should transfer to helper start");
    failures += expect_u64(vm.instruction_count, 1ULL, "USES CALL should count as one committed instruction");
    failures += expect_flag(&vm.cpu, VM_FLAG_CF, true, "USES save should preserve flags");
    delta = vm_last_delta(&vm);
    if (delta == NULL) {
        failures += record_failure("USES CALL should expose a delta object");
    } else {
        failures += expect_size(delta->memory_access_count, 3U, "USES CALL should record return-token and register-save writes");
        failures += expect_size(delta->flag_change_count, 0U, "USES CALL should not report flag changes");
    }

    failures += expect_status(vm_step(&vm), VM_EXEC_STATUS_OK, "helper EBX mutation should execute");
    failures += expect_status(vm_step(&vm), VM_EXEC_STATUS_OK, "helper ESI mutation should execute");
    failures += expect_status(vm_step(&vm), VM_EXEC_STATUS_OK, "helper EAX return mutation should execute");
    failures += expect_status(vm_step(&vm), VM_EXEC_STATUS_OK, "RET from PROC USES should restore listed registers and return");
    failures += (vm_cpu_read_register(&vm.cpu, VM_REGISTER_EAX, &eax) ? 0 : record_failure("EAX read after USES RET should succeed"));
    failures += (vm_cpu_read_register(&vm.cpu, VM_REGISTER_EBX, &ebx) ? 0 : record_failure("EBX read after USES RET should succeed"));
    failures += (vm_cpu_read_register(&vm.cpu, VM_REGISTER_ESI, &esi) ? 0 : record_failure("ESI read after USES RET should succeed"));
    failures += (vm_cpu_read_register(&vm.cpu, VM_REGISTER_ESP, &esp) ? 0 : record_failure("ESP read after USES RET should succeed"));
    failures += expect_u32(eax, 0x9999U, "unlisted EAX should remain available as helper return register");
    failures += expect_u32(ebx, 0x1111U, "listed EBX should be restored by PROC USES");
    failures += expect_u32(esi, 0x2222U, "listed ESI should be restored by PROC USES");
    failures += expect_u32(esp, VM_MEMORY_DEFAULT_STACK_TOP, "USES RET should restore ESP to pre-call value");
    failures += expect_size(vm.instruction_pointer, 1U, "USES RET should return to CALL successor");
    failures += expect_flag(&vm.cpu, VM_FLAG_CF, true, "USES restore should preserve flags");
    failures += expect_status(vm_step(&vm), VM_EXEC_STATUS_OK, "EXIT after USES return should terminate");

    vm_deinit(&vm);
    return failures;
}

/// Verifies Phase 77 preserves EAX when EAX is listed in PROC USES.
///
/// @return Zero on success, otherwise a positive failure count.
static int test_phase77_proc_uses_eax_listed_preserves_return_register(void) {
    int failures = 0;
    Vm vm;
    uint32_t eax = 0U;
    const VmIrInstruction program[] = {
        {VM_IR_OPCODE_CALL, {VM_IR_OPERAND_BRANCH_TARGET, 0U, 2U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_NONE, 0U, 0U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 3U, "call Helper", 0U},
        {VM_IR_OPCODE_EXIT, {VM_IR_OPERAND_NONE, 0U, 0U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_NONE, 0U, 0U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 4U, "exit", 1U},
        {VM_IR_OPCODE_MOV, {VM_IR_OPERAND_REGISTER, 32U, 0U, VM_REGISTER_EAX, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_IMMEDIATE, 32U, 0x9999U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 8U, "mov eax, 9999h", 2U},
    ...
    failures += expect_status(vm_load_program(&vm, program, sizeof(program) / sizeof(program[0])), VM_EXEC_STATUS_OK, "EAX USES program should load");
    failures += expect_status(vm_configure_procedure_boundaries(&vm, boundaries, sizeof(boundaries) / sizeof(boundaries[0])), VM_EXEC_STATUS_OK, "EAX USES boundaries should configure");
    failures += vm_cpu_write_register(&vm.cpu, VM_REGISTER_EAX, 0x1234U) ? 0 : record_failure("EAX setup for listed USES should succeed");
    failures += expect_status(vm_step(&vm), VM_EXEC_STATUS_OK, "CALL into EAX USES helper should save EAX");
    failures += expect_status(vm_step(&vm), VM_EXEC_STATUS_OK, "helper should overwrite EAX");
    failures += expect_status(vm_step(&vm), VM_EXEC_STATUS_OK, "RET should restore listed EAX");
    failures += (vm_cpu_read_register(&vm.cpu, VM_REGISTER_EAX, &eax) ? 0 : record_failure("EAX read after listed USES RET should succeed"));
    failures += expect_u32(eax, 0x1234U, "listed EAX should be restored instead of acting as a return register");

    vm_deinit(&vm);
    return failures;
}

/// Verifies Phase 77 PROC USES save/restore error paths and unsupported non-CALL entry.
///
/// @return Zero on success, otherwise a positive failure count.
static int test_phase77_proc_uses_error_paths(void) {
    int failures = 0;
    Vm overflow_vm;
    Vm underflow_vm;
    Vm entry_vm;
    ...
    uint32_t eax = 0U;
    const VmExecDiagnostic *diagnostic = NULL;
    const VmIrInstruction overflow_program[] = {
        {VM_IR_OPCODE_CALL, {VM_IR_OPERAND_BRANCH_TARGET, 0U, 1U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_NONE, 0U, 0U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 3U, "call Helper", 0U},
    ...
    const VmIrInstruction underflow_program[] = {
        ...
        {VM_IR_OPCODE_EXIT, {VM_IR_OPERAND_NONE, 0U, 0U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_NONE, 0U, 0U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 4U, "exit", 1U},
        {VM_IR_OPCODE_MOV, {VM_IR_OPERAND_REGISTER, 32U, 0U, VM_REGISTER_EBX, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_IMMEDIATE, 32U, 0x2222U, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 8U, "mov ebx, 2222h", 2U},
        {VM_IR_OPCODE_MOV, {VM_IR_OPERAND_REGISTER, 32U, 0U, VM_REGISTER_ESP, 0U, VM_IR_RELOCATION_NONE}, {VM_IR_OPERAND_IMMEDIATE, 32U, VM_MEMORY_DEFAULT_STACK_TOP, VM_REGISTER_COUNT, 0U, VM_IR_RELOCATION_NONE}, "main.asm", 9U, "mov esp, 00900000h", 3U},
    ...
    failures += expect_status(vm_load_program(&overflow_vm, overflow_program, sizeof(overflow_program) / sizeof(overflow_program[0])), VM_EXEC_STATUS_OK, "USES overflow program should load");
    failures += expect_status(vm_configure_procedure_boundaries(&overflow_vm, overflow_boundaries, sizeof(overflow_boundaries) / sizeof(overflow_boundaries[0])), VM_EXEC_STATUS_OK, "USES overflow boundaries should configure");
    failures += vm_cpu_write_register(&overflow_vm.cpu, VM_REGISTER_ESP, VM_MEMORY_DEFAULT_STACK_TOP - VM_MEMORY_DEFAULT_STACK_SIZE + 8U) ? 0 : record_failure("ESP setup for USES overflow should succeed");
    failures += expect_status(vm_step(&overflow_vm), VM_EXEC_STATUS_STACK_OVERFLOW, "USES save should fail before partial stack mutation when capacity is insufficient");
    failures += (vm_cpu_read_register(&overflow_vm.cpu, VM_REGISTER_ESP, &esp) ? 0 : record_failure("ESP read after USES overflow should succeed"));
    failures += expect_u32(esp, VM_MEMORY_DEFAULT_STACK_TOP - VM_MEMORY_DEFAULT_STACK_SIZE + 8U, "USES save overflow should preserve ESP");
    failures += expect_size(overflow_vm.instruction_pointer, 0U, "USES save overflow should not transfer to helper");
    failures += expect_u64(overflow_vm.instruction_count, 0ULL, "USES save overflow should not count a committed instruction");
    diagnostic = vm_last_diagnostic(&overflow_vm);
    if (diagnostic == NULL) {
        failures += record_failure("USES save overflow should populate diagnostic metadata");
    } else {
        failures += expect_status(diagnostic->status, VM_EXEC_STATUS_STACK_OVERFLOW, "USES save overflow diagnostic should report stack-overflow");
        failures += expect_u32(diagnostic->instruction_index, 0U, "USES save overflow diagnostic should point at CALL instruction");
    }

    failures += expect_status(vm_init(&underflow_vm, NULL), VM_EXEC_STATUS_OK, "vm init should succeed for USES underflow test");
    failures += expect_status(vm_load_program(&underflow_vm, underflow_program, sizeof(underflow_program) / sizeof(underflow_program[0])), VM_EXEC_STATUS_OK, "USES underflow program should load");
    failures += expect_status(vm_configure_procedure_boundaries(&underflow_vm, underflow_boundaries, sizeof(underflow_boundaries) / sizeof(underflow_boundaries[0])), VM_EXEC_STATUS_OK, "USES underflow boundaries should configure");
    failures += vm_cpu_write_register(&underflow_vm.cpu, VM_REGISTER_EBX, 0x1111U) ? 0 : record_failure("EBX setup for USES underflow should succeed");
    failures += expect_status(vm_step(&underflow_vm), VM_EXEC_STATUS_OK, "USES underflow CALL setup should succeed");
    failures += expect_status(vm_step(&underflow_vm), VM_EXEC_STATUS_OK, "USES underflow helper mutation should execute");
    failures += expect_status(vm_step(&underflow_vm), VM_EXEC_STATUS_OK, "USES underflow ESP corruption should execute");
    failures += expect_status(vm_step(&underflow_vm), VM_EXEC_STATUS_STACK_UNDERFLOW, "USES restore should fail when saved slots are unavailable");
    failures += (vm_cpu_read_register(&underflow_vm.cpu, VM_REGISTER_EBX, &ebx) ? 0 : record_failure("EBX read after USES underflow should succeed"));
    failures += expect_u32(ebx, 0x2222U, "USES restore underflow should not partially restore EBX");
    failures += expect_size(underflow_vm.instruction_pointer, 4U, "USES restore underflow should keep IP on RET");
    diagnostic = vm_last_diagnostic(&underflow_vm);
    if (diagnostic == NULL) {
        failures += record_failure("USES restore underflow should populate diagnostic metadata");
    } else {
        failures += expect_status(diagnostic->status, VM_EXEC_STATUS_STACK_UNDERFLOW, "USES restore underflow diagnostic should report stack-underflow");
        failures += expect_u32(diagnostic->instruction_index, 4U, "USES restore underflow diagnostic should point at RET instruction");
    }

    failures += expect_status(vm_init(&entry_vm, NULL), VM_EXEC_STATUS_OK, "vm init should succeed for Phase 77 selected-entry guard test");
    failures += expect_status(vm_load_program(&entry_vm, entry_program, sizeof(entry_program) / sizeof(entry_program[0])), VM_EXEC_STATUS_OK, "Phase 77 selected-entry guard program should load");
    failures += expect_status(vm_configure_procedure_boundaries(&entry_vm, entry_boundaries, sizeof(entry_boundaries) / sizeof(entry_boundaries[0])), VM_EXEC_STATUS_OK, "Phase 77 selected-entry guard boundaries should configure");
    failures += expect_status(vm_step(&entry_vm), VM_EXEC_STATUS_UNSUPPORTED_PROC_USES_RUNTIME, "selected-entry PROC USES body should still require direct CALL entry");
    failures += (vm_cpu_read_register(&entry_vm.cpu, VM_REGISTER_EAX, &eax) ? 0 : record_failure("EAX read after selected-entry PROC USES guard should succeed"));
    failures += expect_u32(eax, 0U, "selected-entry PROC USES guard should stop before executing body");
    ...
    failures += expect_u32(strcmp(vm_exec_status_name(VM_EXEC_STATUS_STACK_OVERFLOW), "stack-overflow") == 0 ? 1U : 0U, 1U, "executor status helper should name stack-overflow");
    failures += expect_u32(strcmp(vm_exec_status_name(VM_EXEC_STATUS_STACK_UNDERFLOW), "stack-underflow") == 0 ? 1U : 0U, 1U, "executor status helper should name stack-underflow");
...
static int test_metadata_helpers(void) {
    ...
    if (strcmp(vm_ir_opcode_name(VM_IR_OPCODE_IRVINE32_WRITEHEX), "writehex") != 0) {
        failures += record_failure("WriteHex opcode name should be writehex");
    }
