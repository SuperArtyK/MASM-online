...
/// Returns the requested Phase 71E entry procedure end mode.
///
/// @param out_mode Receives the selected entry procedure end mode.
/// @return Nonzero when MASM32_DIAGNOSTIC_ENTRY_PROCEDURE_END_MODE selects a setting.
static int diagnostic_json_producer_get_entry_procedure_end_mode(Masm32SimWasmEntryProcedureEndMode *out_mode) {
    const char *mode = getenv("MASM32_DIAGNOSTIC_ENTRY_PROCEDURE_END_MODE");

    if (out_mode == NULL) {
        return 0;
    }

    *out_mode = MASM32_SIM_WASM_ENTRY_PROCEDURE_END_CODE_STREAM;
    if (mode == NULL) {
        return 0;
    }
    if (strcmp(mode, "code-stream") == 0) {
        *out_mode = MASM32_SIM_WASM_ENTRY_PROCEDURE_END_CODE_STREAM;
        return 1;
    }
    if (strcmp(mode, "stop-at-entry-end") == 0) {
        *out_mode = MASM32_SIM_WASM_ENTRY_PROCEDURE_END_STOP_AT_ENTRY_END;
        return 1;
    }
...
static int diagnostic_json_producer_emit_json(const char *source) {
    ...
    has_entry_procedure_end_mode = diagnostic_json_producer_get_entry_procedure_end_mode(&entry_procedure_end_mode);
