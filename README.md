...
- Milestone 38 allocated-object strict mode: tests/configuration can stop execution with `object-bounds-violation` for those same valid-region/object-boundary escapes. Default region-only execution remains unchanged; provenance diagnostics and UI controls remain deferred, while uninitialized-read diagnostics are covered by Milestone 40 opt-in validation modes.
