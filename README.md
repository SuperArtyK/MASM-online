...
Phase 57L implements the v1 policy that source-level memory reads or writes whose final byte range overlaps `.code` are unsupported. Such accesses now stop with `unsupported-code-memory-access` or, for cross-region overlaps, `region-boundary-crossing`. The diagnostic explains that the simulator does not expose `.CODE/_TEXT` as an accessible memory region and stops before access. This is diagnostic wording only: `_TEXT` remains an unsupported MASM/object segment symbol and is not an addressable alias for the simulator's internal `.code` region. Phase 57K - .CODE and MASM Segment Symbol Access Policy remains documented for Phase 57M implementation; Phase 57J - .CONST Uninitialized Storage Diagnostics and Policy remains the owner of configurable `.CONST ?` / `.CONST DUP(?)` declaration diagnostics; names such as `_TEXT`, `_DATA`, `_BSS`, `CONST`, `STACK`, `DGROUP`, and `FLAT` are still not addressable simulator symbols. Existing `.CONST` declaration diagnostics, `.CONST` write protection, read-time `uninitialized-read` diagnostics, Phase 57H final-register `[unchanged]` display markers, Phase 57G - Seeded Random Uninitialized Storage Mode seeded uninitialized-storage visible-byte settings, and Phase 57F seeded register/flag startup remain available. The instruction subset remains anchored by Phase 57 - Signed IDIV.

## Current simulator scope

The current runtime supports the MASM32 Educational Mode subset documented in [`docs/SUPPORTED_SYNTAX.md`](docs/SUPPORTED_SYNTAX.md). In broad terms, that includes the C99 core VM/parser/executor, checked simulated memory, `.data`, `.DATA?`, `.CONST`, selected MASM compatibility directives, selected register and memory operands, selected arithmetic/logic/shift/rotate/multiply/divide instructions through signed `idiv`, diagnostic settings, the default `startup-state-notice`, opt-in seeded register/flag startup and opt-in seeded uninitialized-storage visible bytes for tests/configuration, final-register unchanged display markers, accepted read-only `.CONST ?` / `.CONST DUP(?)` storage with configurable declaration diagnostics, Phase 57L `.code` memory-access diagnostics and Phase 57K segment-symbol policy documentation, and the virtual Irvine32 `exit` terminator.
