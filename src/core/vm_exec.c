 ...
 * end-falloff diagnostics, Irvine32 exit, Phase 87 virtual Irvine32 Crlf, Phase 88 virtual Irvine32 WriteChar, Phase 89 virtual Irvine32 WriteString, Phase 90 virtual Irvine32 WriteDec, Phase 91 virtual Irvine32 WriteInt, Phase 92 virtual Irvine32 WriteHex, and Phase 93 virtual Irvine32 WriteBin
...
/// Executes virtual Irvine32 Crlf.
///
/// Crlf appends exactly one internal line-feed byte to Program Console. It
/// does not read operands, registers, flags, or memory, and it mutates no CPU
/// state beyond ordinary instruction-position advancement performed by the
/// stepping layer.
///
/// @param vm VM instance to mutate.
/// @param instruction Crlf instruction descriptor used for limit diagnostics.
/// @return Executor status.
static VmExecStatus vm_exec_execute_irvine32_crlf(Vm *vm, const VmIrInstruction *instruction) {
...
/// Executes virtual Irvine32 WriteDec.
///
/// WriteDec reads EAX as an unsigned 32-bit value, constructs the shortest
/// base-10 representation without separators or a newline, and commits the
/// complete candidate through the shared atomic Program Console limit path.
/// The routine reads no simulated memory and preserves all modeled CPU state.
///
/// @param vm VM instance to mutate.
/// @param instruction WriteDec instruction descriptor used for limit diagnostics.
/// @return Executor status.
static VmExecStatus vm_exec_execute_irvine32_writedec(Vm *vm, const VmIrInstruction *instruction) {
    uint32_t eax = 0U;
    char reversed[10];
    char candidate[10];
    ...
    if (eax == 0U) {
        ...
        while (remaining != 0U) {
            reversed[digit_count] = (char)('0' + (remaining % 10U));
            digit_count += 1U;
            remaining /= 10U;
        }
        for (index = 0U; index < digit_count; index += 1U) {
            candidate[index] = reversed[digit_count - index - 1U];
        }
    }

    return vm_append_program_console_output(vm, candidate, digit_count, instruction);
}


/// Executes virtual Irvine32 WriteInt.
///
/// WriteInt reads the raw 32-bit EAX bit pattern, interprets it as signed
/// two's-complement, constructs the shortest signed decimal representation,
/// and commits the complete sign-plus-digit candidate through the shared
/// atomic Program Console limit path. Negative magnitude conversion uses only
/// defined unsigned arithmetic so 0x80000000 formats as -2147483648 without
/// signed overflow. The routine reads no simulated memory and preserves all
/// modeled CPU state.
///
/// @param vm VM instance to mutate.
/// @param instruction WriteInt instruction descriptor used for limit diagnostics.
/// @return Executor status.
static VmExecStatus vm_exec_execute_irvine32_writeint(Vm *vm, const VmIrInstruction *instruction) {
    uint32_t eax = 0U;
    ...
    negative = (eax & 0x80000000U) != 0U;
    magnitude = negative ? (~eax + 1U) : eax;
    if (negative) {
    ...
    if (magnitude == 0U) {
        ...
        for (index = 0U; index < digit_count; index += 1U) {
            candidate[candidate_length + index] = reversed[digit_count - index - 1U];
        }
        candidate_length += digit_count;
    }

    return vm_append_program_console_output(vm, candidate, candidate_length, instruction);
}
...
/// Executes virtual Irvine32 WriteBin.
///
/// WriteBin reads the raw 32-bit EAX bit pattern as an unsigned value, formats
/// exactly 32 most-significant-bit-first binary digits with leading zeroes, and
/// commits the complete candidate through the shared atomic Program Console
/// limit path. The routine reads no simulated memory and preserves all modeled
/// CPU state.
///
/// @param vm VM instance to mutate.
/// @param instruction WriteBin instruction descriptor used for limit diagnostics.
/// @return Executor status.
static VmExecStatus vm_exec_execute_irvine32_writebin(Vm *vm, const VmIrInstruction *instruction) {
    uint32_t eax = 0U;
    char candidate[32];
    size_t index = 0U;
    ...
    for (index = 0U; index < sizeof(candidate); index += 1U) {
        const uint32_t shift = (uint32_t)(sizeof(candidate) - index - 1U);
        candidate[index] = ((eax >> shift) & 1U) != 0U ? '1' : '0';
    }
...
static VmExecStatus vm_exec_execute_instruction(Vm *vm, const VmIrInstruction *instruction) {
    ...
    switch (instruction->opcode) {
        ...
        case VM_IR_OPCODE_IRVINE32_WRITEBIN:
            return vm_exec_execute_irvine32_writebin(vm, instruction);
        default:
            return VM_EXEC_STATUS_INVALID_INSTRUCTION;
    }
}
