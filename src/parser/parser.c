 ...
 * Phase 87 virtual Irvine32 Crlf CALL/INVOKE lowering, Phase 88 direct WriteChar lowering, Phase 89 direct WriteString lowering, Phase 90 direct WriteDec lowering, and Phase 91 direct WriteInt lowering are supported; the Phase 79 executor uses parser-owned LOCAL metadata to create
 ...
 * Irvine32 routine bodies beyond Crlf, direct WriteChar, direct WriteString, direct WriteDec, and direct WriteInt, and full MASM expression parsing remain later
...
static bool vm_parser_token_is_irvine32_writechar(const VmLexerToken *token);

static bool vm_parser_token_is_irvine32_writestring(const VmLexerToken *token);

static bool vm_parser_token_is_irvine32_writedec(const VmLexerToken *token);

static bool vm_parser_token_is_irvine32_writeint(const VmLexerToken *token);
...
static const VmParserIrvine32RegistryEntry VM_PARSER_IRVINE32_REGISTRY[] = {
    ...
    {"WriteChar", VM_IRVINE32_SYMBOL_CLASS_SUPPORTED_ROUTINE},
    {"WriteString", VM_IRVINE32_SYMBOL_CLASS_SUPPORTED_ROUTINE},
...
static bool vm_parser_parse_direct_call_instruction(VmParserState *state, const VmLexerToken *mnemonic_token) {
    ...
    if (vm_parser_token_is_irvine32_writechar(target_token)) {
        if (!state->result->has_irvine32_virtual_include) {
            vm_parser_add_diagnostic(state, VM_PARSER_DIAGNOSTIC_MISSING_IRVINE32_INCLUDE, target_token, "CALL WriteChar requires INCLUDE Irvine32.inc before WriteChar can be used as a virtual Irvine32 routine.");
            return false;
        }
        vm_parser_advance(state);
        if (!vm_parser_expect_line_end(state)) {
        ...
        return vm_parser_emit_instruction(state, VM_IR_OPCODE_IRVINE32_WRITECHAR, vm_ir_operand_none(), vm_ir_operand_none(), mnemonic_token);
    }

    if (vm_parser_token_is_irvine32_writestring(target_token)) {
        if (!state->result->has_irvine32_virtual_include) {
            vm_parser_add_diagnostic(state, VM_PARSER_DIAGNOSTIC_MISSING_IRVINE32_INCLUDE, target_token, "CALL WriteString requires INCLUDE Irvine32.inc before WriteString can be used as a virtual Irvine32 routine.");
            return false;
        }
        vm_parser_advance(state);
        if (!vm_parser_expect_line_end(state)) {
        ...
        return vm_parser_emit_instruction(state, VM_IR_OPCODE_IRVINE32_WRITESTRING, vm_ir_operand_none(), vm_ir_operand_none(), mnemonic_token);
    }

    if (vm_parser_token_is_irvine32_writedec(target_token)) {
        if (!state->result->has_irvine32_virtual_include) {
            vm_parser_add_diagnostic(state, VM_PARSER_DIAGNOSTIC_MISSING_IRVINE32_INCLUDE, target_token, "CALL WriteDec requires INCLUDE Irvine32.inc before WriteDec can be used as a virtual Irvine32 routine.");
            return false;
        }
        vm_parser_advance(state);
        if (!vm_parser_expect_line_end(state)) {
        ...
        return vm_parser_emit_instruction(state, VM_IR_OPCODE_IRVINE32_WRITEDEC, vm_ir_operand_none(), vm_ir_operand_none(), mnemonic_token);
    }

    if (vm_parser_token_is_irvine32_writeint(target_token)) {
        if (!state->result->has_irvine32_virtual_include) {
            vm_parser_add_diagnostic(state, VM_PARSER_DIAGNOSTIC_MISSING_IRVINE32_INCLUDE, target_token, "CALL WriteInt requires INCLUDE Irvine32.inc before WriteInt can be used as a virtual Irvine32 routine.");
            return false;
        }
        vm_parser_advance(state);
        if (!vm_parser_expect_line_end(state)) {
        ...
        return vm_parser_emit_instruction(state, VM_IR_OPCODE_IRVINE32_WRITEINT, vm_ir_operand_none(), vm_ir_operand_none(), mnemonic_token);
    }
...
/// Returns whether a token names the virtual Irvine32 `WriteChar` routine.
///
/// @param token Token to inspect.
/// @return true when @p token is an identifier spelling `WriteChar` case-insensitively.
static bool vm_parser_token_is_irvine32_writechar(const VmLexerToken *token) {
    return token != NULL &&
           token->kind == VM_LEXER_TOKEN_IDENTIFIER &&
           vm_parser_token_equals(token, "WriteChar");
}

/// Returns whether a token names the virtual Irvine32 `WriteString` routine.
///
/// @param token Token to inspect.
/// @return true when @p token is an identifier spelling `WriteString` case-insensitively.
static bool vm_parser_token_is_irvine32_writestring(const VmLexerToken *token) {
    return token != NULL &&
           token->kind == VM_LEXER_TOKEN_IDENTIFIER &&
           vm_parser_token_equals(token, "WriteString");
}

/// Returns whether a token names the virtual Irvine32 `WriteDec` routine.
///
/// @param token Token to inspect.
/// @return true when @p token is an identifier spelling `WriteDec` case-insensitively.
static bool vm_parser_token_is_irvine32_writedec(const VmLexerToken *token) {
    return token != NULL &&
           token->kind == VM_LEXER_TOKEN_IDENTIFIER &&
           vm_parser_token_equals(token, "WriteDec");
}

/// Returns whether a token names the virtual Irvine32 `WriteInt` routine.
///
/// @param token Token to inspect.
/// @return true when @p token is an identifier spelling `WriteInt` case-insensitively.
static bool vm_parser_token_is_irvine32_writeint(const VmLexerToken *token) {
    return token != NULL &&
           token->kind == VM_LEXER_TOKEN_IDENTIFIER &&
           vm_parser_token_equals(token, "WriteInt");
}
...
static bool vm_parser_diagnose_irvine32_symbol_use_if_known(VmParserState *state, const VmLexerToken *mnemonic_token) {
    ...
    if (symbol_class == VM_IRVINE32_SYMBOL_CLASS_SUPPORTED_ROUTINE) {
        if (vm_parser_token_is_irvine32_crlf(mnemonic_token)) {
        ...
        if (vm_parser_token_is_irvine32_writechar(mnemonic_token)) {
            vm_parser_add_diagnostic(state, VM_PARSER_DIAGNOSTIC_INVALID_IRVINE32_CALL_FORM, mnemonic_token, "WriteChar is a virtual Irvine32 routine and must be called with CALL WriteChar.");
            return true;
        }
        if (vm_parser_token_is_irvine32_writestring(mnemonic_token)) {
            vm_parser_add_diagnostic(state, VM_PARSER_DIAGNOSTIC_INVALID_IRVINE32_CALL_FORM, mnemonic_token, "WriteString is a virtual Irvine32 routine and must be called with CALL WriteString.");
            return true;
        }
        if (vm_parser_token_is_irvine32_writedec(mnemonic_token)) {
            vm_parser_add_diagnostic(state, VM_PARSER_DIAGNOSTIC_INVALID_IRVINE32_CALL_FORM, mnemonic_token, "WriteDec is a virtual Irvine32 routine and must be called with CALL WriteDec.");
            return true;
        }
        if (vm_parser_token_is_irvine32_writeint(mnemonic_token)) {
            vm_parser_add_diagnostic(state, VM_PARSER_DIAGNOSTIC_INVALID_IRVINE32_CALL_FORM, mnemonic_token, "WriteInt is a virtual Irvine32 routine and must be called with CALL WriteInt.");
            return true;
        }
    }
