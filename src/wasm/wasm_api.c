 ...
 * Milestone 12 source execution export parses optional `.data`, initializes
 * simulated memory, runs the currently supported `.code` subset including TYPE,
 * and reports a compact JSON result for the UI.
 */
...
/// Maximum lexer tokens accepted by the Milestone 12 source-run API.
#define MASM32_SIM_WASM_MAX_RUN_TOKENS 512U

/// Maximum lexer diagnostics retained by the Milestone 12 source-run API.
#define MASM32_SIM_WASM_MAX_RUN_LEXER_DIAGNOSTICS 64U

/// Maximum parser diagnostics retained by the Milestone 12 source-run API.
...
/// Bytes available for the returned Milestone 12 JSON response.
...
/// Stores all fixed buffers needed for one Milestone 12 parse-and-run request.
...
/// Appends the canonical 32-bit register object used by the Milestone 12 UI.
///
/// @param writer Writer to mutate.
...
static const char *masm32_sim_wasm_build_run_json(
    ...
    (void)masm32_sim_json_append(&writer, "{\"phase\":12,\"ok\":%s,\"status\":", ok ? "true" : "false");
    ...
    if (writer.overflowed) {
        (void)snprintf(
            ...
            "{\"phase\":12,\"ok\":false,\"status\":\"response-truncated\",\"instructionCount\":0,\"simulatorMessages\":[{\"kind\":\"internal-simulator-error\",\"code\":\"response-truncated\",\"message\":\"The simulator response exceeded its fixed buffer.\"}]}"
        );
    }
