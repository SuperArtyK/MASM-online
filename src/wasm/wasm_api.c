 ...
 * Milestone 20 source execution export parses optional `.data`, initializes
 ...
 * accumulator conversions, exchange/negation/no-op instructions, and recovered
...
/// Maximum lexer tokens accepted by the Milestone 20 source-run API.
#define MASM32_SIM_WASM_MAX_RUN_TOKENS 512U

/// Maximum lexer diagnostics retained by the Milestone 20 source-run API.
#define MASM32_SIM_WASM_MAX_RUN_LEXER_DIAGNOSTICS 64U

/// Maximum parser diagnostics retained by the Milestone 20 source-run API.
...
/// Bytes available for the returned Milestone 20 JSON response.
...
/// Stores all fixed buffers needed for one Milestone 20 parse-and-run request.
...
/// Appends the canonical 32-bit register object used by the Milestone 20 UI.
///
/// @param writer Writer to mutate.
...
static const char *masm32_sim_wasm_build_run_json(
    ...
    (void)masm32_sim_json_append(&writer, "{\"phase\":20,\"ok\":%s,\"status\":", ok ? "true" : "false");
    ...
    if (writer.overflowed) {
        (void)snprintf(
            ...
            "{\"phase\":20,\"ok\":false,\"status\":\"response-truncated\",\"instructionCount\":0,\"simulatorMessages\":[{\"kind\":\"internal-simulator-error\",\"code\":\"response-truncated\",\"message\":\"The simulator response exceeded its fixed buffer.\"}]}"
        );
    }
