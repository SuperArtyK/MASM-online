 ...
 * sends editor source and diagnostic settings, then renders final registers,
 * memory changes, Program Console text, and structured Simulator Messages
 * returned by the worker.
 */

import { initializeCollapsiblePanel } from "./collapsible.js";
import { formatMemoryChanges, formatProgramConsole, formatRegisters, formatSimulatorMessages } from "./formatters.js";
import {
 ...
 * @param {HTMLElement} programConsole Program Console panel.
 * @param {HTMLElement} finalRegisters Final Registers panel.
 * @param {HTMLElement} memoryChanges Memory Changes panel.
 * @returns {void}
 */
function renderRunResult(payload, simulatorMessages, programConsole, finalRegisters, memoryChanges) {
  const messages = Array.isArray(payload.simulatorMessages) ? payload.simulatorMessages : [];
  const consoleStream = payload.programConsole && typeof payload.programConsole === "object" ? payload.programConsole : undefined;
  ...
  setPanelText(programConsole, formatProgramConsole(consoleStream));
...
worker.addEventListener("message", (event) => {
  ...
  if (event.data && event.data.type === "RUN_RESULT") {
    renderRunResult(event.data.payload || {}, simulatorMessages, programConsole, finalRegisters, memoryChanges);
    return;
  }
